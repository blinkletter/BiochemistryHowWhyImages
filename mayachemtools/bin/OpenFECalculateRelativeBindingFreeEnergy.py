#!/usr/bin/env python
#
# File: OpenFECalculateRelativeBindingFreeEnergy.py
...
def main():
    ...
    if Options["--list"]:
        ...
        CalculateRelativeBindingFreeEnergy()

    MiscUtil.PrintInfo("\n%s: Done...\n" % ScriptName)
    MiscUtil.PrintInfo("Total time: %s" % MiscUtil.GetFormattedElapsedTime(WallClockTime, ProcessorTime))


def CalculateRelativeBindingFreeEnergy():
    """Calculate relative binding free energy."""

    # Process input files...
    MacroMol, Mols = ProcessInputFiles()
    ...
    # Initialize RBFE protocols...
    RBFEProtocol, RBFEProtocolChargeCorrection = InitializeRelativeHybridTopologyProtocol()
    ...
    MolAToMolBTransformations = SetupTransformations(
        MolAToMolBMappings, MacroMol, Solvent, RBFEProtocol, RBFEProtocolChargeCorrection
    )
...
def InitializeRelativeHybridTopologyProtocol():
    ...
    RBFESettings = OpenFEUtil.SetupRelativeFreeEnergySettings("-r, --rbfeParams", OptionsInfo["RBFEParams"])
    RBFEProtocol = OpenFEUtil.InitializeRelativeFreeEngeryHybridTopologyProtocol(RBFESettings)

    RBFESettingsChargeCorrection = OpenFEUtil.SetupRelativeFreeEnergySettings(
        "-r, --rbfeParams", OptionsInfo["RBFEParams"]
    )
    OpenFEUtil.UpdateRelativeFreeEnergySettingsForChargeCorrection(
        "--rbfeChargeCorrectionParams", OptionsInfo["RBFEChargeCorrectionParams"], RBFESettingsChargeCorrection
    )
    RBFEProtocolChargeCorrection = OpenFEUtil.InitializeRelativeFreeEngeryHybridTopologyProtocol(
        RBFESettingsChargeCorrection
    )

    return (RBFEProtocol, RBFEProtocolChargeCorrection)
...
def SetupTransformations(MolAToMolBMappings, MacroMol, Solvent, RBFEProtocol, RBFEProtocolChargeCorrection):
    ...
    for MolAToMolBMapping in MolAToMolBMappings:
        ...
        MolAComplexSolventSystem = OpenFEUtil.InitializeChemicalSystem(
            SmallMol=MolA, MacroMol=MacroMol, Solvent=Solvent, Name="%s_Complex_Solvent" % MolA.name
        )
        ...
        MolBComplexSolventSystem = OpenFEUtil.InitializeChemicalSystem(
            SmallMol=MolB, MacroMol=MacroMol, Solvent=Solvent, Name="%s_Complex_Solvent" % MolB.name
        )
        ...
        TransformationProtocol = SetupTransformationProtocol(
            MolAToMolBMapping, RBFEProtocol, RBFEProtocolChargeCorrection
        )

        # Setup MolAComplexSolvent to MolBComplexSolvent transformation...
        TransformationName = "%s_To_%s_Complex_Solvent" % (MolA.name, MolB.name)
        MolAToMolBComplexSolventTransformation = OpenFEUtil.InitializeTransformation(
            StateA=MolAComplexSolventSystem,
            StateB=MolBComplexSolventSystem,
            Mapping=MolAToMolBMapping,
            Protocol=TransformationProtocol,
            Name=TransformationName,
            Validate=False,
        )
        MolAToMolBTransformations.append(MolAToMolBComplexSolventTransformation)
...
def SetupTransformationProtocol(MolAToMolBMapping, RBFEProtocol, RBFEProtocolChargeCorrection):
    """Setup transformation protocol."""
    ...
    if ChargeDifference != 0:
        MolA = MolAToMolBMapping.componentA
        MolB = MolAToMolBMapping.componentB
        if OptionsInfo["RBFEChargeCorrection"]:
            TransformationProtocol = RBFEProtocolChargeCorrection
            MiscUtil.PrintInfo("")
            MiscUtil.PrintWarning(
                'The transformation between molecules %s and %s involves a charge change of %s. The RBFE setting parameters have been automatically updated for "Yes" value of option "--rbfeChargeCorrection" to employ a more expensive set of parameters specified by option "--rbfeChargeCorrectionParams". '
                % (MolA.name, MolB.name, ChargeDifference)
            )
        else:
            TransformationProtocol = RBFEProtocol
            MiscUtil.PrintInfo("")
            MiscUtil.PrintWarning(
...
def ProcessProtocolResults(MolAToMolBTransformations, MolAToMolBProtocolResults):
    ...
    ResultFile = "%s_RBFE_Results.%s" % (OptionsInfo["OutfilePrefix"], ResultFileParams["Ext"])
    ...
    for Index in range(0, len(MolAToMolBProtocolResults), 2):
        MolAToMolBComplexSolventProtocolResult = MolAToMolBProtocolResults[Index]
        MolAToMolBSolventProtocolResult = MolAToMolBProtocolResults[Index + 1]

        # Setup mol names using complex solvent transformation. The solvent transformation
        # also contains the same pair of moleules.
        MolAToMolBComplexSolventTransformation = MolAToMolBTransformations[Index]

        MolA = MolAToMolBComplexSolventTransformation.stateA.components["ligand"]
        MolB = MolAToMolBComplexSolventTransformation.stateB.components["ligand"]
        MolAName = MolA.name
        MolBName = MolB.name

        if MolAToMolBComplexSolventProtocolResult is None or MolAToMolBSolventProtocolResult is None:
            DeltaDeltaGBinding = "NA"
            DeltaDeltaGBindingUncertainty = "NA"
        else:
            # Setup binding value without the units...
            MolAToMolBComplexSolventDeltaG = MolAToMolBComplexSolventProtocolResult.get_estimate()
            MolAToMolBSolventDeltaG = MolAToMolBSolventProtocolResult.get_estimate()

            DeltaDeltaGBinding = MolAToMolBComplexSolventDeltaG.m - MolAToMolBSolventDeltaG.m
            DeltaDeltaGBinding = "%.*f" % (Precision, DeltaDeltaGBinding)

            # Setup uncertainty value without the units...
            MolAToMolBComplexSolventDeltaGUncertainty = MolAToMolBComplexSolventProtocolResult.get_uncertainty()
            MolAToMolBSolventDeltaGUncertainty = MolAToMolBSolventProtocolResult.get_uncertainty()

            DeltaDeltaGBindingUncertainty = np.sqrt(
                np.sum(np.square([MolAToMolBComplexSolventDeltaGUncertainty.m, MolAToMolBSolventDeltaGUncertainty.m]))
            )
            DeltaDeltaGBindingUncertainty = "%.*f" % (Precision, DeltaDeltaGBindingUncertainty)

        ResultData.append([MolAName, MolBName, DeltaDeltaGBinding, DeltaDeltaGBindingUncertainty])

    ResultDF = pd.DataFrame(
        ResultData,
        columns=["MolAName", "MolBName", "DeltaDeltaG (MolA->MolB; RBFE) (kcal/mol)", "Uncertainty (kcal/mol)"],
    )
...
def ProcessListOption():
    ...
    RBFESettings = openfe.protocols.openmm_rfe.RelativeHybridTopologyProtocol.default_settings()

    MiscUtil.PrintInfo("\nListing RBFE settings...")
    OpenFEUtil.ListOpenFESettings(RBFESettings)
...
def ProcessOptions():
    ...
    OptionsInfo["RBFEParams"] = OpenFEUtil.ProcessOptionOpenFERelativeFreeEnergyParameters(
        "--rbfeParams", Options["--rbfeParams"], ParamsDefaultInfo=ParamsDefaultInfoOverride
    )

    OptionsInfo["RBFEChargeCorrection"] = True if re.match("^yes$", Options["--rbfeChargeCorrection"]) else False
    OptionsInfo["RBFEChargeCorrectionParams"] = (
        OpenFEUtil.ProcessOptionOpenFERelativeFreeEnergyChargeCorrectionParameters(
            "--rbfeChargeCorrectionParams", Options["--rbfeChargeCorrectionParams"]
        )
    )
...
def ValidateOptions():
    ...
    MiscUtil.ValidateOptionTextValue("--rbfeChargeCorrection", Options["--rbfeChargeCorrection"], "yes no")


# Setup a usage string for docopt...
_docoptUsage_ = """
OpenFECalculateRelativeBindingFreeEnergy.py - Calculate relative binding free energy

Usage:
    OpenFECalculateRelativeBindingFreeEnergy.py [--executeDAGParams <Name,Value,..>] [--loggingLevel <Info, Warning or Error>]
                                                [--mapper <mapper1, mapper2,...>] [--mapperParams <Name,Value,..>] [--mapperScorer <LOMAP>]
                                                [--mode <MoleculePairs or MoleculeNetwork>] [--missingChargeMode <Calculate or Stop>]
                                                [--moleculePairs <MolName1,MolName2,..>] [--network <text>] [--networkParams <Name,Value,..>]
                                                [--outfilePrefix <text>] [--overwrite] [--rbfeParams <Name,Value,...>] [--rbfeChargeCorrection <yes or no>]
                                                [--rbfeChargeCorrectionParams <Name,Value,...>] [--resultFileParams <Name,Value,..>]
                                                [--solventParams <Name,Value,...>] [--smallMolFileParams <Name,Value,...> ]
                                                [-w <dir>] -i <infile> -s <smallmolfile> -o <outifiledir>
    OpenFECalculateRelativeBindingFreeEnergy.py -l | --list
    OpenFECalculateRelativeBindingFreeEnergy.py -h | --help | -e | --examples

Description:
    Calculate Relative Binding Free Energy (RBFE) for a pair of molecules in a
    small molecule input file. You may calculate RBFEs for specific pairs of
    ...
    The MD simulation workflow, employed for the calculation of RBFEs, involves the
    ...
    pair of transformations, MolAToMolBComplexSolvent and MolAToMolBSolvent,
    and the results are analyzed to estimate RBFEs. The default time and step size
    ...
    of transformations, MolAToMolBComplexSolvent and MolAToMolBSolvent,
    ...
    Possible output files and directories under <OutfileDir>:
        ...
        <OutfilePrefix>_RBFE_Results.<csv or tsv>
        ...
        Transformations/<MolAName>_To_<MolBName>_Complex_Solvent.json
...
Options:
    ...
    --executeDAGParams <Name,Value,..>  [default: auto]
        ...
        protocol DAGs (Directed Acyclic Graph) to run RBFE calculations.
    ...
    -l, --list
        List default RBFE protocol settings provided by OpenFE module
    ...
    -m, --mode <MoleculePairs or MoleculeNetwork>  [default: MoleculePairs]
        Calculate RBFEs for specified pairs of molecules or all molecule pairs
        corresponding to edges in a molecule network.
    --missingChargeMode <Calculate or Stop>  [default: Stop]
        Calculate missing partial charges for molecules before running RBFE
        ...
        RelativeHybridTopologyProtocol during the calculation of RBFE. You
        may control the calculation of partial charges by specifying values for
        partialCharge* parameters using '--rbfeParams' option.
    --moleculePairs <MolName1,MolName2,..>  [default: auto]
        A comma delimited list of molecule name pairs for calculating RBFEs.
    ...
    -n, --network <text>  [default: MinimalSpanning]
        Name of a molecule network to generate for calculating RBFEs. Possible
    ...
    --resultFileParams <Name,Value,..>  [default: auto]
        ...
        calculated RBFEs values to a results file.
    ...
    -r, --rbfeParams <Name,Value,...>  [default: auto]
        A comma delimited list of parameter name and value pairs for RBFE protocol
        settings employed during the calculation of RBFEs.
        ...
        are shown below:
            ...
            forcefieldHydrogenMass, 3.0  [ Units: amu ]y
    ...
    --rbfeChargeCorrection <yes or no>  [default: yes]
        Perform automatic charge correction for charge changing transformations
        during the calculation of RBFEs. The '--rbfeChargeCorrectionParams' are
        used during the automatic charge correction to override the corresponding
        values in '-r, --rbfeParams'.
    --rbfeChargeCorrectionParams <Name,Value,...>  [default: auto]
        A comma delimited list of parameter name and value pairs to use for RBFE
        ...
        corresponding values in '-r, --rbfeParams'.
        ...
        parameters in the section for '-r, --rbfeParams'.
    ...
    --solventParams <Name,Value,...>  [default: auto]
        ...
        parameters available through '--rbfeParams' to perform solvation.
...
Examples:
    ...
    To calculate RBFE for a pair molecules corresponding to the fist and second
    molecules in a SD file, performing 3 independent repeats of the entire MD sampling
    process to calculate RBFE for a pair of molecules, each MD repeat consisting of
    minimization (5,000 steps) followed by NVT and NPT equilibration (1 ns;
    250,000 steps) leading to NPT production (5ns; 1,250,000 steps) using a step size
    ...
    along with appropriate graph and image files under output directory, type:
        ...
        % OpenFECalculateRelativeBindingFreeEnergy.py -i SampleTyk2.pdb
          -s SampleTyk2Ligands.sdf -o SampleTyk2LigandsRBFE

    To run the first example for calculating RBFE for a specific pair molecules
    using CUDA platform on your machine to perform MD simulations and generate
    various output files, type:

        % OpenFECalculateRelativeBindingFreeEnergy.py -i SampleTyk2.pdb
          -s SampleTyk2Ligands.sdf -o SampleTyk2LigandsRBFE -m MoleculePairs
          --moleculePairs "lig_ejm_31, lig_ejm_47"
          --rbfeParams "engineComputePlatform,CUDA"
    ...
    To run the first example for calculating RBFE for all pairs of molecules
    ...
    machine to perform MD simulations and generate various output files, type:
        ...
        % OpenFECalculateRelativeBindingFreeEnergy.py -i SampleTyk2.pdb
          -s SampleTyk2Ligands.sdf -o SampleTyk2LigandsRBFE -m MoleculeNetwork
    ...
    files, type:
        ...
        % OpenFECalculateRelativeBindingFreeEnergy.py -i SampleTyk2.pdb
          -s SampleTyk2LigandsNoCharges.sdf -o SampleTyk2LigandsRBFE
    ...
    and generate various output files, type:
        ...
        % OpenFECalculateRelativeBindingFreeEnergy.py -i SampleTyk2.pdb
          ...
          --mapperParams "lomapTime, 20, lomapThreeD, yes"
           --missingChargeMode Stop --rbfeParams "protocolRepeats,3,
