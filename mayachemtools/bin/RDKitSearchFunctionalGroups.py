...
def ProcessMoleculesUsingSingleProcess(Mols, GroupsPatternMols, Writer, GroupOutfilesWriters):
    ...
    for Mol in Mols:
        ...
        RemainingMolCount += 1
        WriteMolecule(
            Writer, GroupOutfilesWriters, Mol, Compute2DCoords, CombineMatchResults, GroupsPatternMatchStatusList
        )
...
def ProcessMoleculesUsingMultipleProcesses(Mols, GroupsPatternMols, Writer, GroupOutfilesWriters):
    ...
    OptionsInfo["EncodedGroupPatternMols"] = [
        RDKitUtil.MolToBase64EncodedMolString(PatternMol) for PatternMol in GroupsPatternMols
    ]
...
def InitializeWorkerProcess(*EncodedArgs):
    ...
    OptionsInfo["GroupPatternMols"] = [
        RDKitUtil.MolFromBase64EncodedMolString(EncodedMol) for EncodedMol in OptionsInfo["EncodedGroupPatternMols"]
    ]
...
def MatchMolecule(Mol, GroupsPatternMols):
    ...
    # Match pattern mols...
    for GroupIndex in range(0, len(OptionsInfo["SpecifiedFunctionalGroups"])):
        Status = DoesPatternMolMatch(
            GroupsPatternMols[GroupIndex],
            Mol,
            OptionsInfo["UseChirality"],
            OptionsInfo["SpecifiedFunctionalGroupsNegateMatch"][GroupIndex],
        )
    ...
    MolMatched = DoesMolMeetSpecifiedMatchCriteria(
        GroupsPatternMatchStatusList, OptionsInfo["CombineMatchResults"], OptionsInfo["AndCombineOperatorMode"]
    )
    ...
    if CombineMatchResults:
        ...
        if GroupsCount > 4:
            MiscUtil.PrintInfo(
                ...
                % (GroupsCount, OptionsInfo["OutfileBasename"], OptionsInfo["OutfileExt"])
            )
        else:
            Delmiter = ", "
...
def RetrieveFunctionalGroupsInfo():
    ...
    MiscUtil.PrintInfo(
        "Total number of functional groups present functional group hierarchy: %d" % (len(FunctionalGroupsMap["Names"]))
    )
