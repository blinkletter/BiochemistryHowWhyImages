#!/usr/bin/env python
#
# File: OpenMMPerformSimulatedAnnealing.py
...
def PerformSimulatedAnnealing():
    """Perform MD simulation."""

    # Prepare system for simulation...
    System, Barostat, Topology, Positions = PrepareSystem()
    ...
    #  Perform annealing...
    PerformSimulatedAnnealingWorkflow(Simulation, Integrator, Barostat)
...
def PrepareSystem():
    ...
    return (System, Barostat, Topology, Positions)
...
def SetupInitialVelocities(Simulation):
    ...
    # temperature...
    AnnealingParams = OptionsInfo["AnnealingParams"]
    AnnealingParamsInfo = OpenMMUtil.SetupAnnealingParameters(OptionsInfo["AnnealingParams"])

    MiscUtil.PrintInfo("\nSetting initial velocities to temperature (%s K)..." % AnnealingParams["InitialStart"])
    Simulation.context.setVelocitiesToTemperature(AnnealingParamsInfo["InitialStart"])
...
def PerformSimulatedAnnealingWorkflow(Simulation, Integrator, Barostat):
    """Perform simulated annealing workflow."""
    ...
    InitialStart = AnnealingParams["InitialStart"]
    InitialEnd = AnnealingParams["InitialEnd"]
    InitialChange = AnnealingParams["InitialChange"]
    InitialSteps = AnnealingParams["InitialSteps"]
    ...
    InitialEquilibrationSteps = AnnealingParams["InitialEquilibrationSteps"]
    MiscUtil.PrintInfo(
    ...
    Cycles = AnnealingParams["Cycles"]
    CycleStart = AnnealingParams["CycleStart"]
    CycleEnd = AnnealingParams["CycleEnd"]
    CycleChange = AnnealingParams["CycleChange"]
    CycleSteps = AnnealingParams["CycleSteps"]
    CycleEquilibrationSteps = AnnealingParams["CycleEquilibrationSteps"]
    ...
    # Perform final equilibration...
    FinalEquilibrationSteps = AnnealingParams["FinalEquilibrationSteps"]
    MiscUtil.PrintInfo(
        "\nPerforming final equilibration after heating and cooling cycles (Steps: %s; Time: %s)..."
        % (FinalEquilibrationSteps, GetTotalSimulationTime(FinalEquilibrationSteps))
    )
    Simulation.step(FinalEquilibrationSteps)

    TotalSimulationSteps += FinalEquilibrationSteps
    MiscUtil.PrintInfo(
...
def GetTotalSimulationTime(SimulationSteps):
...
def ProcessOutfileNames():
    ...
    for Outfile in [PDBOutfile, ReimagedPDBOutfile, ReimagedTrajOutfile, MinimizedPDBOutfile, FinalPDBOutfile]:
        if not Options["--overwrite"]:
...
def ProcessOptions():
    ...
    if OptionsInfo["NVTMode"]:
        ParamsDefaultInfoOverride = {"DataOutType": "Step Speed PotentialEnergy Temperature Time Volume"}
        ...
        ParamsDefaultInfoOverride = {"DataOutType": "Step Speed PotentialEnergy Temperature Time Density"}
        ParamsDefaultInfoOverride["DataOutTypePlotX"] = "Time"
    ...
    OptionsInfo["AnnealingParams"] = OpenMMUtil.ProcessOptionOpenMMAnnealingParameters(
    ...
    OptionsInfo["IntegratorParams"]["Temperature"] = OptionsInfo["AnnealingParams"]["InitialStart"]
...
Usage:
    OpenMMPerformSimulatedAnnealing.py [--annealingParams <Name,Value,..> ] [--forcefieldParams <Name,Value,..>]
                                       [--freezeAtoms <yes or no>] [--freezeAtomsParams <Name,Value,..>] [--integratorParams <Name,Value,..>]
                                       [--mode <NVT or NPT>] [--outputParams <Name,Value,..>] [--outfileDir <outfiledir>] [--outfilePrefix <text>]
                                       [--outPlotParams <Name,Value,...>] [--overwrite] [--platform <text>] [--platformParams <Name,Value,..>]
                                       [--restraintAtoms <yes or no>] [--restraintAtomsParams <Name,Value,..>] [--restraintSpringConstant <number>]
...
Description:
    Perform simulated annealing using an NPT or NVT statistical ensemble. You may
    ...
    The simulated annealing workflow involves the following steps: Initial heating
    and equilibration after each change in temperature; Heating and cooling cycles
    along with equilibration after each change in temperature; Final equilibration.
    By default, the simulated annealing is performed for a total of 3.35 ns as shown
    below:
...
Options:
    -a, --annealingParams <Name,Value,..>  [default: auto]
        A comma delimited list of parameter name and value pairs for simulated
        annealing.
...
Examples:
    To perform simulated annealing for a macromolecule in a PDB file by using an NPT
    ensemble, applying system constraints for bonds involving hydrogens along with
    hydrogen mass repartioning, using a step size of 4 fs, performing minimization
    until it's converged, performing initial heating along with equilibration for
    305,000 steps (1.22 ns), performing one heating and cooling cycle along with
    equilibration 232,000 steps (928 ps), performing final equilibration for
    100,000 steps (400 ps), writing trajectory file every 10,000 steps (40 ps),
    writing data log file every 1,000 steps (4 ps), generating a checkpoint file
    after the completion of the calculation, and generating a PDB for the
    final system, type:

        % OpenMMPerformSimulatedAnnealing.py -i Sample13.pdb -o Sample13Annealing
          --waterBox yes
...
See also:
    ...
    OpenMMPerformMDSimulation.py, OpenMMPerformMinimization.py
