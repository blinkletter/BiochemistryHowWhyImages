...
def ProcessMoleculesUsingMultipleProcesses(Mols, Writer):
    ...
    for Result in Results:
        MolCount += 1
        MolIndex, EncodedMol, EncodedStandardizedMol, StandardizationStatus = Result
        ...
        StandardizedMol = RDKitUtil.MolFromBase64EncodedMolString(EncodedStandardizedMol)
...
def WorkerProcess(EncodedMolInfo):
    ...
    if EncodedMol is None:
        return [MolIndex, None, None, False]
    ...
    StandardizedMol, StandardizationStatus = PerformStandardization(Mol, (MolIndex + 1))
    EncodedStandardizedMol = RDKitUtil.MolToBase64EncodedMolString(
        StandardizedMol,
    ...
    return [MolIndex, EncodedMol, EncodedStandardizedMol, StandardizationStatus]
...
def PerformStandardization(Mol, MolNum):
    ...
    # Track molname for its restoration after the standardization. RDKit standardization
    # functions might mangle molname for molecules containing disconnected components...
    MolName = Mol.GetProp("_Name") if Mol.HasProp("_Name") else None

    StandardizedMol = Mol
    try:
        # Step 1: Cleanup...
        if OptionsInfo["MethodologyParams"]["Cleanup"]:
            StandardizedMol = CleanupMolecule(StandardizedMol)

        # Step2: Get largest fragment...
        if OptionsInfo["MethodologyParams"]["RemoveFragments"]:
            StandardizedMol = ChooseLargestMoleculeFragment(StandardizedMol)

        # Step3: Neutralize...
        if OptionsInfo["MethodologyParams"]["Neutralize"]:
            StandardizedMol = NeutralizeMolecule(StandardizedMol)

        # Step4: Canonicalize tautomer...
        if OptionsInfo["MethodologyParams"]["CanonicalizeTautomer"]:
            StandardizedMol = CanonicalizeMoleculeTautomer(StandardizedMol)

        Status = True
    except Exception as ErrMsg:
        StandardizedMol = None
        if not OptionsInfo["QuietMode"]:
    ...
    if MolName is not None:
        if StandardizedMol is not None:
            StandardizedMol.SetProp("_Name", MolName)

    return (StandardizedMol, Status)
...
def CleanupMolecule(Mol):
    ...
    if OptionsInfo["StandardizeParams"]["CleanupAssignStereo"]:
        ...
        Chem.AssignStereochemistry(
            Mol,
            force=OptionsInfo["StandardizeParams"]["CleanupAssignStereoForce"],
            cleanIt=OptionsInfo["StandardizeParams"]["CleanupAssignStereoCleanIt"],
        )
...
def SetupStandardize():
    ...
    if OptionsInfo["MethodologyParams"]["RemoveFragments"]:
        OptionsInfo["StandardizeObjects"]["LargestFragmentChooser"] = rdMolStandardize.LargestFragmentChooser(
            OptionsInfo["CleanupParams"]
        )

    if OptionsInfo["MethodologyParams"]["Neutralize"]:
        OptionsInfo["StandardizeObjects"]["Uncharger"] = rdMolStandardize.Uncharger(
            OptionsInfo["CleanupParams"].doCanonical
        )

    if OptionsInfo["MethodologyParams"]["CanonicalizeTautomer"]:
        OptionsInfo["StandardizeObjects"]["TautomerEnumerator"] = rdMolStandardize.TautomerEnumerator(
            OptionsInfo["CleanupParams"]
        )
...
def SetupStandardizeCleanupParameters():
    ...
    CleanupParams.largestFragmentChooserCountHeavyAtomsOnly = StandardizeParams[
...
def ProcessMethodologyParameters():
    ...
    ParamsDefaultInfo = {
        "Cleanup": ["bool", True],
        "RemoveFragments": ["bool", True],
        "Neutralize": ["bool", True],
        "CanonicalizeTautomer": ["bool", True],
    }
    OptionsInfo["MethodologyParams"] = MiscUtil.ProcessOptionNameValuePairParameters(
        "--methodologyParams", Options["--methodologyParams"], ParamsDefaultInfo
    )
...
def ProcessStandardizationParameters():
    ...
    ParamsDefaultInfo = {
        "AcidBaseFile": ["file", None],
        "FragmentFile": ["file", None],
        "NormalizationsFile": ["file", None],
        "TautomerTransformsFile": ["file", None],
        "CleanupRemoveHydrogens": ["bool", True],
        "CleanupDisconnectMetals": ["bool", True],
        "CleanupNormalize": ["bool", True],
        "CleanupNormalizeMaxRestarts": ["int", 200],
        "CleanupReionize": ["bool", True],
        "CleanupAssignStereo": ["bool", True],
        "CleanupAssignStereoCleanIt": ["bool", True],
        "CleanupAssignStereoForce": ["bool", True],
        "DoCanonical": ["bool", True],
        "LargestFragmentChooserUseAtomCount": ["bool", True],
        "LargestFragmentChooserCountHeavyAtomsOnly": ["bool", False],
        "PreferOrganic": ["bool", False],
        "MaxTautomers": ["int", 1000],
        "MaxTransforms": ["int", 1000],
        "TautomerRemoveBondStereo": ["bool", True],
        "TautomerRemoveIsotopicHs": ["bool", True],
        "TautomerRemoveSp3Stereo": ["bool", True],
        "TautomerReassignStereo": ["bool", True],
    }

    OptionsInfo["StandardizeParams"] = MiscUtil.ProcessOptionNameValuePairParameters(
        "--standardizeParams", Options["--standardizeParams"], ParamsDefaultInfo
    )
    ...
    for ParamName in ["CleanupNormalizeMaxRestarts", "MaxTautomers", "MaxTransforms"]:
        ...
        if ParamValue <= 0:
            MiscUtil.PrintError(
                'The parameter value, %s, specified for parameter name, %s, using "-s, --standardizeParams" option is not a valid value. Supported values: > 0'
...
def ProcessOptions():
    ...
    OptionsInfo["InfileParams"] = MiscUtil.ProcessOptionInfileParameters(
        "--infileParams", Options["--infileParams"], Options["--infile"], ParamsDefaultInfo=ParamsDefaultInfoOverride
    )
...
def ValidateOptions():
    ...
    if re.match("^standardize$", Options["--mode"], re.I):
        if not Options["--outfile"]:
            MiscUtil.PrintError(
                'The outfile must be specified using "-o, --outfile" during "standardize" value of "--mode" option'
            )
