...
def ValidateOptions():
    ...
    if not re.match("^none$", Options["--energyRMSDCutoff"], re.I):
        ...
        if not re.match("^yes$", Options["--alignConformers"], re.I):
            MiscUtil.PrintError(
                '"%s" value of "--alignConformers" is not allowed for %s value of "--energyRMSDCutoff" option. It must be set to "yes".'
...
Usage:
    RDKitGenerateConformers.py [--alignConformers <yes or no>] [--addHydrogens <yes or no>]
                               [--conformerGenerator <SDG, ETDG, KDG, ETKDG, ETKDGv2>] [--embedRMSDCutoff <number>]
                               [--energyOut  <yes or no>] [--enforceChirality <yes or no>] [--energyRMSDCalcMode <RMSD or BestRMSD>]
                               [--energyRMSDCutoff <number>] [--energyRMSDCutoffMode <All or Lowest>] [--energyWindow <number>]
                               [--forceField <UFF, MMFF, None>] [--forceFieldMMFFVariant <MMFF94 or MMFF94s>]
...
Options:
    ...
    -c, --conformerGenerator <text>  [default: ETKDGv2]
        Conformation generation methodology for generating initial 3D coordinates. The
        possible values along with a brief description are shown below:
    ...
    --energyRMSDCutoff <number>  [default: none]
        ...
        by the specified RMSD cutoff and are with in the specified energy window are
        kept. The lowest energy conformation is always retained.
    --energyRMSDCutoffMode <All or Lowest>  [default: All]
        RMSD cutoff mode for  retaining conformations after energy minimization. 
        Possible values: All or Lowest. The RMSD values are compared against all
