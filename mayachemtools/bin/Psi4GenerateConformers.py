#!/usr/bin/env python
#
# File: Psi4GenerateConformers.py
...
def ProcessMoleculesUsingSingleProcess(Mols, Writer):
    ...
    for Mol in Mols:
        ...
        ConformerMol, CalcStatus, ConfIDs, ConfEnergies = GenerateMolConformers(Psi4Handle, Mol, MolCount)

        if not CalcStatus:
            if not OptionsInfo["QuietMode"]:
                MiscUtil.PrintWarning(
                    "Failed to calculate energy for molecule %s" % RDKitUtil.GetMolName(Mol, MolCount)
                )
...
def ProcessMoleculesUsingMultipleProcessesAtMoleculesLevel(Mols, Writer):
    """Process molecules to generate conformers using multiprocessing at molecules level."""
...
def WorkerProcess(EncodedMolInfo):
    ...
    Mol, CalcStatus, ConfIDs, ConfEnergies = GenerateMolConformers(OptionsInfo["psi4"], Mol, MolNum)
...
def ProcessMoleculesUsingMultipleProcessesAtConformersLevel(Mols, Writer):
    """Process molecules to generate conformers using multiprocessing at conformers level."""
    ...
    for Mol in Mols:
        ...
        Mol, CalcStatus, ConfIDs, ConfEnergies = ProcessConformersUsingMultipleProcesses(Mol, MolCount)
...
def ProcessConformersUsingMultipleProcesses(Mol, MolNum):
    """Generate coformers and minimize them using multiple processes."""
    ...
    if OptionsInfo["ConfGenerationParams"]["AlignConformers"]:
        AllChem.AlignMolConformers(Mol)

    # Filter conformers...
    SelectedConfIDs, SelectedConfEnergies = FilterMolConformers(Mol, MolNum, ConfIDs, CalcEnergyMap)
...
def ConformerWorkerProcess(EncodedMolInfo):
    ...
    if not OptionsInfo["QuietMode"]:
        MiscUtil.PrintInfo("Processing conformer ID %s for molecule %s..." % (ConfID, MolName))
...
def GenerateMolConformers(Psi4Handle, Mol, MolNum):
...
def FilterMolConformers(Mol, MolNum, ConfIDs, CalcEnergyMap):
    ...
    RefMol, ProbeMol = [None] * 2
    if EnergyRMSDCalcModeBest:
        # Copy molecules for best RMSD calculations to avoid change in the coordinates
        # of the conformations...
        RefMol = AllChem.Mol(Mol)
        ProbeMol = AllChem.Mol(Mol)
    ...
    for ConfID in SortedConfIDs:
        ...
        if ApplyEnergyRMSDCutoff:
            ...
            if EnergyRMSDCutoffLowest:
                ...
                if EnergyRMSDCalcModeBest:
                    CalcRMSD = AllChem.GetBestRMS(ProbeMol, RefMol, prbId=ConfID, refId=MinEnergyConfID)
                else:
                    CalcRMSD = AllChem.GetConformerRMS(Mol, MinEnergyConfID, ConfID, prealigned=PreAligned)
                if CalcRMSD < EnergyRMSDCutoff:
                    IgnoreConf = True
            else:
                for SelectedConfID in SelectedConfIDs:
                    if EnergyRMSDCalcModeBest:
                        CalcRMSD = AllChem.GetBestRMS(ProbeMol, RefMol, prbId=ConfID, refId=SelectedConfID)
                    else:
                        CalcRMSD = AllChem.GetConformerRMS(Mol, SelectedConfID, ConfID, prealigned=PreAligned)
                    if CalcRMSD < EnergyRMSDCutoff:
                        IgnoreConf = True
                        break
            if IgnoreConf:
    ...
    return [SelectedConfIDs, SelectedConfEnergies]
...
def WriteMolConformers(Writer, Mol, MolNum, ConfIDs, ConfEnergies=None):
    ...
    for Index, ConfID in enumerate(ConfIDs):
        ...
        if OptionsInfo["EnergyOut"] and ConfEnergies is not None:
            Mol.SetProp(OptionsInfo["EnergyDataFieldLabel"], ConfEnergies[Index])
...
def ProcessOptions():
    ...
    if OptionsInfo["EnergyRMSDCutoff"] > 0:
        # Make sure that the alignConformers option is being used...
        if not OptionsInfo["ConfGenerationParams"]["AlignConformers"]:
            MiscUtil.PrintError(
                'The value for "alignConformers" specified using "--confParams" must  be set to " yes" for non-zero values of "--energyRMSDCutoff" '
            )
    ...
    OptionsInfo["RecenterAndReorient"] = True if re.match("^yes$", Options["--recenterAndReorient"], re.I) else False

    Symmetrize = Options["--symmetrize"]
    if re.match("^auto$", Symmetrize, re.I):
        Symmetrize = True if OptionsInfo["RecenterAndReorient"] else False
    else:
...
def ValidateOptions():
    ...
    MiscUtil.ValidateOptionTextValue("--recenterAndReorient", Options["--recenterAndReorient"], "yes no")
...
Usage:
    Psi4GenerateConformers.py [--basisSet <text>] [--confParams <Name,Value,...>] [--energyOut <yes or no>]
                              ...
                              [--methodName <text>] [--mp <yes or no>] [--mpLevel <Molecules or Conformers>]
                              [--mpParams <Name, Value,...>] [ --outfileParams <Name,Value,...> ] [--overwrite] [--precision <number>]
...
Options:
    ...
    --confParams <Name,Value,...>  [default: auto]
        ...
        along with basic Knowledge-terms with Distance Geometry (ETKDG or
        ETKDGv2) [Ref 129, 167] .
    ...
    --energyRMSDCutoffMode <All or Lowest>  [default: All]
        ...
        conformations by the specified RMSD cutoff and are with in the specified
...
Examples:
    ...
    combination of ETKDGv2 distance geometry methodology, applying embed RMSD
    cutoff of 0.5 and MMFF forcefield minimization, followed by energy minimization
    using B3LYP/6-31G** or B3LYP/6-31+G** (sulfur containing), selecting a final set
    of minimized conformers for molecules in a SMILES file, applying energy RMSD
    cutoff of 0.5 and energy window value value of 20 kcal/mol, and write out a SD
    ...
    write out a SD file, type
        ...
        % Psi4GenerateConformers.py --confParams "confMethod,ETKDGv2,
