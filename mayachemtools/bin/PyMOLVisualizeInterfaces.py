...
def WriteComplexesAndChainsViews(OutFH):
    ...
    for FileIndex in range(0, len(OptionsInfo["InfilesInfo"]["InfilesNames"])):
        ...
        for ChainID in SpecifiedChainsAndLigandsInfo["ChainIDs"]:
            ...
            for LigandID in SpecifiedChainsAndLigandsInfo["LigandIDs"][ChainID]:
                ...
                GenerateAndWritePMLForGroup(
                    OutFH,
                    PyMOLObjectNamesInfo["Ligands"][ChainID][LigandID]["ChainLigandGroup"],
                    PyMOLObjectNamesInfo["Ligands"][ChainID][LigandID]["ChainLigandGroupMembers"],
                    Enable,
                    Action,
                )
            ...
            GenerateAndWritePMLForGroup(
                OutFH,
                PyMOLObjectNamesInfo["Chains"][ChainID]["ChainGroup"],
                PyMOLObjectNamesInfo["Chains"][ChainID]["ChainGroupMembers"],
                Enable,
                Action,
            )
        ...
        Enable, Action = [True, "close"]
        GenerateAndWritePMLForGroup(
            OutFH, PyMOLObjectNamesInfo["PDBGroup"], PyMOLObjectNamesInfo["PDBGroupMembers"], Enable, Action
        )
...
def WriteInterfaceViews(OutFH):
    ...
    for InterfaceID in InterfaceChainsAndResiduesInfo["InterfaceIDs"]:
        WriteInterfacePolarContactsView(
            OutFH, InterfaceID, InterfaceChainPairsAndResiduesInfo, PyMOLInterfaceObjectNamesInfo
        )
        WriteInterfaceHydrophobicContactsView(
            OutFH, InterfaceID, InterfaceChainPairsAndResiduesInfo, PyMOLInterfaceObjectNamesInfo
        )
        ...
        for InterfaceChainID in InterfaceChainsAndResiduesInfo["InterfaceChainIDs"][InterfaceID]:
            ...
            GenerateAndWritePMLForGroup(
                OutFH,
                PyMOLInterfaceObjectNamesInfo["InterfaceChainIDs"][InterfaceID][InterfaceChainID]["ChainGroup"],
                PyMOLInterfaceObjectNamesInfo["InterfaceChainIDs"][InterfaceID][InterfaceChainID]["ChainGroupMembers"],
                Enable,
                Action,
            )
        ...
        GenerateAndWritePMLForGroup(
            OutFH,
            PyMOLInterfaceObjectNamesInfo["InterfaceIDs"][InterfaceID]["InterfaceIDGroup"],
            PyMOLInterfaceObjectNamesInfo["InterfaceIDs"][InterfaceID]["InterfaceIDGroupMembers"],
            Enable,
            Action,
        )
    ...
    GenerateAndWritePMLForGroup(
        OutFH,
        PyMOLInterfaceObjectNamesInfo["InterfacesGroup"],
        PyMOLInterfaceObjectNamesInfo["InterfacesGroupMembers"],
        Enable,
        Action,
    )
...
def WriteChainAloneViews(OutFH, FileIndex, PyMOLObjectNames, ChainID):
    ...
    InterfaceResNums = OptionsInfo["InfilesInfo"]["SpecifiedChainsAndLigandsInfo"][FileIndex]["InterfaceResNums"][
        ChainID
    ]
    InterfaceResNumsSelection = "+".join(InterfaceResNums)

    Selection = "%s and chain %s and polymer and (not (resi %s))" % (
        ChainComplexName,
        ChainID,
        InterfaceResNumsSelection,
    )
    ...
    if GetChainAloneContainsSurfacesStatus(FileIndex, ChainID):
        ...
        PML = PyMOLUtil.SetupPMLForSurfaceView(
            ...
            NonInterfaceChainName,
            Enable=False,
            Color=OptionsInfo["SurfaceNonInterfaceColor"],
        )
        OutFH.write("\n%s\n" % PML)
        ...
        if GetChainAloneSurfaceChainElectrostaticsStatus(FileIndex, ChainID):
            ...
            WriteSurfaceElectrostaticsView(
                "Chain", OutFH, SelectionObjectName, ElectrostaticsGroupName, ElectrostaticsGroupMembers
            )
    ...
    GenerateAndWritePMLForGroup(
        OutFH,
        PyMOLObjectNames["Chains"][ChainID]["ChainAloneNonInterfaceGroup"],
...
def WriteInterfacePolarContactsView(OutFH, InterfaceID, InterfaceChainPairsAndResiduesInfo, PyMOLObjectNames):
    ...
    WriteInterfaceContactsView(
        "InterfacePolarConatcts", OutFH, InterfaceID, InterfaceChainPairsAndResiduesInfo, PyMOLObjectNames
    )
...
def WriteInterfaceHydrophobicContactsView(OutFH, InterfaceID, InterfaceChainPairsAndResiduesInfo, PyMOLObjectNames):
    ...
    WriteInterfaceContactsView(
        "InterfaceHydrophobicContacts", OutFH, InterfaceID, InterfaceChainPairsAndResiduesInfo, PyMOLObjectNames
    )
...
def WriteInterfaceContactsView(Mode, OutFH, InterfaceID, InterfaceChainPairsAndResiduesInfo, PyMOLObjectNames):
    ...
    if InterfacePolarContacts:
        ...
        PML = PyMOLUtil.SetupPMLForPolarContactsView(
            ContactsName, Selection1, Selection2, Enable=False, Color=ContactsColor, Cutoff=ContactsCutoff
        )
    else:
...
def WriteInterfaceChainView(
    OutFH, FileIndex, InterfaceID, InterfaceChainID, ChainID, ResiduesNums, ComplexName, PyMOLObjectNames
):
    ...
    if GetInterfaceContainsSurfacesStatus(FileIndex, ChainID):
        ...
        PML = PyMOLUtil.SetupPMLForSurfaceView(
            PyMOLObjectNames["InterfaceChainIDs"][InterfaceID][InterfaceChainID]["ChainSurface"],
            ChainName,
            Color=OptionsInfo["SurfaceInterfaceColor"],
            Enable=True,
            DisplayAs="lines",
        )
        OutFH.write("\n%s\n" % PML)
        ...
        if GetInterfaceSurfaceChainStatus(FileIndex, ChainID):
            # Setup surface colored by hydrophobicity...
            PML = PyMOLUtil.SetupPMLForHydrophobicSurfaceView(
                PyMOLObjectNames["InterfaceChainIDs"][InterfaceID][InterfaceChainID]["ChainHydrophobicSurface"],
                ChainName,
                ColorPalette=OptionsInfo["SurfaceColorPalette"],
                Enable=False,
                DisplayAs="lines",
            ...
            # Setup surface colored by hyrdophobicity and charge...
            PML = PyMOLUtil.SetupPMLForHydrophobicAndChargeSurfaceView(
                PyMOLObjectNames["InterfaceChainIDs"][InterfaceID][InterfaceChainID]["ChainHydrophobicChargeSurface"],
                ChainName,
                OptionsInfo["AtomTypesColorNames"]["HydrophobicAtomsColor"],
                OptionsInfo["AtomTypesColorNames"]["NegativelyChargedAtomsColor"],
                OptionsInfo["AtomTypesColorNames"]["PositivelyChargedAtomsColor"],
        ...
        if GetInterfaceSurfaceChainElectrostaticsStatus(FileIndex, ChainID):
            ...
            ElectrostaticsGroupName = PyMOLObjectNames["InterfaceChainIDs"][InterfaceID][InterfaceChainID][
                "ChainElectrostaticsGroup"
            ]
            ElectrostaticsGroupMembers = PyMOLObjectNames["InterfaceChainIDs"][InterfaceID][InterfaceChainID][
                "ChainElectrostaticsGroupMembers"
            ]
            WriteSurfaceElectrostaticsView(
                "InterfaceResidues", OutFH, SelectionObjectName, ElectrostaticsGroupName, ElectrostaticsGroupMembers
            )

        # Setup surface group...
        GenerateAndWritePMLForGroup(
            OutFH,
            PyMOLObjectNames["InterfaceChainIDs"][InterfaceID][InterfaceChainID]["ChainSurfaceGroup"],
    ...
    GenerateAndWritePMLForGroup(
        OutFH,
        PyMOLObjectNames["InterfaceChainIDs"][InterfaceID][InterfaceChainID]["ChainGroup"],
...
def WriteInterfaceChainResidueTypesView(OutFH, FileIndex, InterfaceID, InterfaceChainID, ChainID, PyMOLObjectNames):
    ...
    for SubGroupType in ["Aromatic", "Hydrophobic", "Polar", "Positively_Charged", "Negatively_Charged", "Other"]:
        ...
        ResiduesSurfaceObjectName = PyMOLObjectNames["InterfaceChainIDs"][InterfaceID][InterfaceChainID][
            ResiduesSurfaceObjectID
        ]
        ...
        GenerateAndWritePMLForGroup(
            OutFH,
            PyMOLObjectNames["InterfaceChainIDs"][InterfaceID][InterfaceChainID][ResiduesSubGroupID],
    ...
    GenerateAndWritePMLForGroup(
        OutFH,
        PyMOLObjectNames["InterfaceChainIDs"][InterfaceID][InterfaceChainID]["ChainResiduesGroup"],
        PyMOLObjectNames["InterfaceChainIDs"][InterfaceID][InterfaceChainID]["ChainResiduesGroupMembers"],
        False,
        "close",
    )
...
def WriteSurfaceElectrostaticsView(
    Mode, OutFH, SelectionObjectName, ElectrostaticsGroupName, ElectrostaticsGroupMembers
):
...
def DeleteEmptyPyMOLInterfaceObjects(OutFH):
    ...
    for InterfaceID in InterfaceChainsAndResiduesInfo["InterfaceIDs"]:
        for InterfaceChainID in InterfaceChainsAndResiduesInfo["InterfaceChainIDs"][InterfaceID]:
            ...
            DeleteEmptyInterfaceChainResidueTypesObjects(
                OutFH, FileIndex, InterfaceID, InterfaceChainID, ChainID, PyMOLInterfaceObjectNamesInfo
            )


def DeleteEmptyInterfaceChainResidueTypesObjects(
    OutFH, FileIndex, InterfaceID, InterfaceChainID, ChainID, PyMOLObjectNames
):
...
def RetrieveInfilesInfo():
    ...
    if InfilesCount > 2:
        MiscUtil.PrintError(
            'Number of input files, %s, specified using "-i, --infiles" option is not valid. Number of allowed files: 1 or 2'
            % (InfilesCount)
        )
...
def ValidateInterfaceChainIDs():
    ...
    for Index in range(0, len(InterfaceChainIDsList), 2):
        ...
        for ChainID in ChainIDs1:
            if ChainID not in Infile1ChainIDs:
                MiscUtil.PrintError(
                    'The chain ID, %s, specified using "-c, --chainIDs" for a chain IDs pairs, "%s,%s", must be present in first input file.'
                    % (ChainID, "+".join(ChainIDs1), "+".join(ChainIDs2))
                )
        ...
        for ChainID in ChainIDs2:
            if SingleInfileMode:
                ...
                if ChainID not in Infile2ChainIDs:
                    MiscUtil.PrintError(
                        'The chain ID, %s, specified using "-c, --chainIDs" for a chain IDs pairs, "%s,%s", must be present in second input file.'
                        % (ChainID, "+".join(ChainIDs1), "+".join(ChainIDs2))
                    )
    ...
    for Index in range(0, len(InterfaceChainIDsList), 2):
        ...
        if CanonicalInterfaceID in CanonicalInterfaceIDsMap:
            MiscUtil.PrintError(
                'The chain ID pair, "%s", using "-c, --chainIDs", option has been specified multiple times, "%s".'
                % (CanonicalInterfaceIDsMap[CanonicalInterfaceID], OptionsInfo["InterfaceChainIDs"])
            )
        else:
...
def AutoAssignInterfaceChainIDs():
    ...
    if OptionsInfo["InfilesInfo"]["SingleInfileMode"]:
        ...
        if len(Infile1ChainIDs) < 2:
            MiscUtil.PrintError(
                'Failed to automatically set interface chain IDs. Number of chains, %s, in input file specified using "-i, --infiles" option must be >= 2. '
                % (len(Infile1ChainIDs))
            )
        ...
        if len(Infile1ChainIDs) < 1:
            MiscUtil.PrintError(
                'Failed to automatically set interface chain IDs. Number of chains, %s, in first input file specified using "-i, --infiles" option must be >= 1. '
                % (len(Infile1ChainIDs))
            )
        if len(Infile2ChainIDs) < 1:
            MiscUtil.PrintError(
                'Failed to automatically set interface chain IDs. Number of chains, %s, in second input file specified using "-i, --infiles" option must be >= 1. '
                % (len(Infile1ChainIDs))
            )
        InterfaceChainIDsList.append([Infile1ChainIDs[0]])
    ...
    for FileIndex in range(0, len(OptionsInfo["InfilesInfo"]["InfilesNames"])):
        ...
        if len(InfilesInterfaceChainIDsList):
            ...
            for ChainID in SpecifiedChainsAndLigandsInfo["ChainIDs"]:
                ...
                # Setup ligand IDs...
                SpecifiedChainsAndLigandsInfo["LigandIDs"][ChainID] = SetupSpecifiedLigandIDs(
                    FileIndex, ChainID, ChainsAndLigandsInfo
                )
...
def SetupSpecifiedLigandIDs(FileIndex, ChainID, ChainsAndLigandsInfo):
    ...
    if re.match("^All$", OptionsInfo["LigandIDs"], re.I):
        ...
        LargestLigandID = (
            ChainsAndLigandsInfo["LigandIDs"][ChainID][0] if (len(ChainsAndLigandsInfo["LigandIDs"][ChainID])) else None
        )
    ...
    for LigandID in LigandIDsWords:
        if LigandID not in ValidLigandIDs:
            LigandIDsListNames = ",".join(ValidLigandIDs) if len(ValidLigandIDs) else "None"
            MiscUtil.PrintWarning(
                'The ligand ID, %s, specified using "-l, --ligandiIDs" option is not valid for chain, %s, in input file, %s. It\'ll be ignored. Valid ligand IDs: %s'
                % (LigandID, ChainID, Infile, LigandIDsListNames)
            )
            continue

        if LigandID in LigandIDs:
            MiscUtil.PrintWarning(
                'The ligand ID, %s, has already been specified using "-l, --ligandIDs" option for chain, %s, in input file, %s. It\'ll be ignored.'
                % (LigandID, ChainID, Infile)
            )
            continue
    ...
    if not len(LigandIDs):
        MiscUtil.PrintWarning(
            'No valid ligand IDs "%s" specified using "-l, --ligandIDs" option for chain ID, %s, in input file, %s.'
            % (OptionsInfo["LigandIDs"], ChainID, Infile)
        )
...
def SetupInterfaceChainPairsAndResiduesInfo():
    ...
    for Index in range(0, len(InterfaceChainIDsList), 2):
        ...
        ChainsAndResiduesInfo1, ChainsAndResiduesInfo2 = GetInterfaceChainsAndResiduesInfo(
            MolName1, ChainIDs1, MolName2, ChainIDs2, Method, MethodCutoff
        )
        ListInterfaceChainsAndResidues(
            ListInterfaceID, MolName1, ChainIDs1, ChainsAndResiduesInfo1, MolName2, ChainIDs2, ChainsAndResiduesInfo2
        )

        # Check presence of interface residues...
        if not (len(ChainsAndResiduesInfo1["ChainIDs"]) and len(ChainsAndResiduesInfo2["ChainIDs"])):
            MiscUtil.PrintWarning(
                "Ignoring interface ID %s: Failed to identify any interface residues. PyMOL groups and objects won't be created."
                % InterfaceID
            )
            continue

        # Collect residue numbers for set of interface residues in each chain...
        InterfaceChainIDs1, InterfaceChainResidueNums1 = GetInterfaceChainsAndResidueNumbers(
            ChainIDs1, ChainsAndResiduesInfo1
        )
        InterfaceChainIDs2, InterfaceChainResidueNums2 = GetInterfaceChainsAndResidueNumbers(
            ChainIDs2, ChainsAndResiduesInfo2
        )
        ...
        InterfaceChainPairsAndResiduesInfo["ChainIDsResNumsPairs"][InterfaceID] = [
            InterfaceChainResidueNums1,
            InterfaceChainResidueNums2,
        ]
...
def ProcessInterfaceChainPairsAndResiduesInfo():
    ...
    for InterfaceID in InterfaceChainPairsAndResiduesInfo["InterfaceIDs"]:
        ...
        (
            FlattenedInterfaceChainIDs,
            FlattenedChainIDs,
            FlattenedResNums,
            FlattenedComplexNames,
            FlattenedInfileIndices,
        ) = FlattenIntferfaceChainPairsAndResiduesInfo(InterfaceChainPairsAndResiduesInfo, InterfaceID)

        if InterfaceID not in InterfaceChainsAndResiduesInfo["InterfaceIDs"]:
        ...
        for Index in range(0, len(FlattenedInterfaceChainIDs)):
            ...
            OptionsInfo["InfilesInfo"]["SpecifiedChainsAndLigandsInfo"][FileIndex]["InterfaceResNums"][ChainID].extend(
                ResNums
            )
...
def GetInterfaceChainsAndResiduesInfo(MolName1, ChainIDs1, MolName2, ChainIDs2, Method, Cutoff):
    ...
    if re.match("^BySASAChange$", Method, re.I):
        ...
        InterfaceChainsResiduesInfo1, InterfaceChainsResiduesInfo2 = (
            PyMOLUtil.GetInterfaceChainsResiduesByHeavyAtomsDistance(
                MolName1, ChainNames1, MolName2, ChainNames2, Cutoff
            )
        )
    elif re.match("^ByCAlphaAtomsDistance$", Method, re.I):
        InterfaceChainsResiduesInfo1, InterfaceChainsResiduesInfo2 = (
            PyMOLUtil.GetInterfaceChainsResiduesByCAlphaAtomsDistance(
                MolName1, ChainNames1, MolName2, ChainNames2, Cutoff
            )
        )
    else:
...
def ListInterfaceChainsAndResidues(
    InterfaceID, MolName1, ChainIDs1, ChainsAndResiduesInfo1, MolName2, ChainIDs2, ChainsAndResiduesInfo2
):
...
def PrepareInterfaceChainsAndResiduesInfo(ChainsAndResiduesInfo):
    ...
    for ChainID in ChainsAndResiduesInfo["ChainIDs"]:
        ...
        SortedResNames = sorted(
            ChainsAndResiduesInfo["ResNames"][ChainID],
            key=lambda ResName: ChainsAndResiduesInfo["ResCount"][ChainID][ResName],
            reverse=True,
        )
...
def ListInterfaceChainsAndResiduesInfo(InterfaceID, MolName, InterfaceChainIDs, ChainNames, ResiduesInfo):
    ...
    for ChainID in InterfaceChainIDs:
        if ChainID not in ChainNames:
            MiscUtil.PrintWarning(
                "Interface chain IDs: %s; MoleculeID: %s; ChainID: %s; Number of interface residues: 0. PyMOL groups and objects related to chain won't be created."
                % (InterfaceID, MolName, ChainID)
            )
            continue
        ...
        for Index in range(0, len(ChainNames)):
            ...
            if re.match(ChainID, ChainName, re.I):
                MiscUtil.PrintInfo(
                    ...
                    % (InterfaceID, MolName, ChainName, ChainResiduesInfo[0])
                )
                MiscUtil.PrintInfo(
...
def GetInterfaceChainsAndResidueNumbers(ChainIDs, ChainsAndResiduesInfo):
    ...
    for ChainID in ChainIDs:
        if ChainID not in ChainsAndResiduesInfo["ChainIDs"]:
            continue
...
def SetupPyMOLInterfaceObjectNamesForChain(PyMOLObjectNames, InterfaceID, InterfaceChainID):
    ...
    if GetInterfaceResidueTypesStatus(FileIndex, ChainID):
        ...
        PyMOLObjectNames["InterfaceChainIDs"][InterfaceID][InterfaceChainID]["ChainGroupMembers"].append(
            ResiduesGroupName
        )
        ...
        for SubGroupType in ["Aromatic", "Hydrophobic", "Polar", "Positively_Charged", "Negatively_Charged", "Other"]:
            ...
            PyMOLObjectNames["InterfaceChainIDs"][InterfaceID][InterfaceChainID][ResiduesSubGroupID] = (
                ResiduesSubGroupName
            )
            PyMOLObjectNames["InterfaceChainIDs"][InterfaceID][InterfaceChainID][ResiduesGroupMembersID].append(
                ResiduesSubGroupName
            )
            ...
            for MemberType in ["Residues", "Surface"]:
                ...
                PyMOLObjectNames["InterfaceChainIDs"][InterfaceID][InterfaceChainID][SubGroupMemberID] = (
                    SubGroupMemberName
                )
                PyMOLObjectNames["InterfaceChainIDs"][InterfaceID][InterfaceChainID][ResiduesSubGroupMembersID].append(
                    SubGroupMemberName
                )
    ...
    if GetInterfaceContainsSurfacesStatus(FileIndex, ChainID):
        ...
        if GetInterfaceSurfaceChainStatus(FileIndex, ChainID):
            ...
            PyMOLObjectNames["InterfaceChainIDs"][InterfaceID][InterfaceChainID]["ChainSurfaceGroupMembers"].append(
                Name
            )
...
def ProcessResidueTypesAndSurfaceOptions(FileIndex, SpecifiedChainsAndLigandsInfo):
    ...
    for ChainID in SpecifiedChainsAndLigandsInfo["ChainIDs"]:
        ...
        if re.match("^auto$", OptionsInfo["InterfaceSurfaceElectrostatics"], re.I):
            ...
            InterfaceSurfaceElectrostatics = (
                True if re.match("^yes$", OptionsInfo["InterfaceSurfaceElectrostatics"], re.I) else False
            )
...
def GetInterfaceSurfaceChainElectrostaticsStatus(FileIndex, ChainID):
    ...
    return OptionsInfo["InfilesInfo"]["SpecifiedChainsAndLigandsInfo"][FileIndex]["SurfaceInterfaceElectrostatics"][
        ChainID
    ]
...
def ProcessOptions():
    ...
    OptionsInfo["InterfaceHydrophobicContacts"] = (
        True if re.match("^Yes$", Options["--interfaceHydrophobicContacts"], re.I) else False
    )

    OptionsInfo["InterfacePolarContactsColor"] = Options["--interfacePolarContactsColor"]
    OptionsInfo["InterfacePolarContacts"] = (
        True if re.match("^Yes$", Options["--interfacePolarContacts"], re.I) else False
    )
    ...
    if re.match("^auto$", MethodCutoff, re.I):
        if re.match("BySASAChange", Method, re.I):
            ...
            MiscUtil.PrintError(
                'The method, %s, specified using "-m, --method" option  is  not a valid method.' % (Method)
            )
    else:
    ...
    if len(SurfaceColorsWords) != 2:
        MiscUtil.PrintError(
            'The number of comma delinited color names, %d, specified using "--surfaceColors" option, "%s",  must be a 2.'
            % (len(SurfaceColorsWords), Options["--surfaceColors"])
        )
    ...
    if not re.match("^auto$", InterfaceChainIDs, re.I):
        ...
        if len(InterfaceChainIDsWords) % 2:
            MiscUtil.PrintError(
                'The number of comma delimited chain IDs, %d, specified using "-c, --chainIDs" option, "%s",  must be a multple of 2.'
                % (len(InterfaceChainIDsWords), Options["--chainIDs"])
            )
        for ChainID in InterfaceChainIDsWords:
            if not len(ChainID):
                MiscUtil.PrintError(
                    'A chain ID specified, "%s" using "-c, --chainIDs" option is empty.' % (Options["--chainIDs"])
                )
...
def ValidateOptions():
    ...
    if InfilesCount == 2:
        ...
        if re.match(Infile1Pattern, Infile2Name, re.I):
            MiscUtil.PrintError(
                'The file names specified, "%s", for option "-i, --infiles" must be different.\n'
                % (Options["--infiles"])
            )
    ...
    MiscUtil.ValidateOptionTextValue(
        "--interfaceHydrophobicContacts", Options["--interfaceHydrophobicContacts"], "yes no"
    )
    ...
    MiscUtil.ValidateOptionTextValue(
        "--interfaceSurfaceElectrostatics", Options["--interfaceSurfaceElectrostatics"], "yes no auto"
    )

    MiscUtil.ValidateOptionIntegerValue("--labelFontID", Options["--labelFontID"], {})

    MiscUtil.ValidateOptionTextValue(
        "--method", Options["--method"], "BySASAChange ByHeavyAtomsDistance ByCAlphaAtomsDistance"
    )
...
Usage:
    PyMOLVisualizeInterfaces.py [--allowEmptyObjects <yes or no>] [--chainIDs <ChainID1 or ChainID1,ChainID2>]
                                              ...
                                              [--ignoreHydrogens <yes or no>] [--ligandIDs <Largest, All, None or ID1,ID2...>]
                                              [--method <text>] [--methodCutoff <number>]
