...
def PerformMinimization():
    ...
    MiscUtil.PrintInfo(
        "Number of molecules failed during conformation generation or minimization: %d" % MinimizationFailedCount
    )
    MiscUtil.PrintInfo("Number of molecules failed during writing: %d" % WriteFailedCount)
    MiscUtil.PrintInfo(
        "Number of ignored molecules: %d" % (MolCount - ValidMolCount + MinimizationFailedCount + WriteFailedCount)
    )
...
def ProcessMoleculesUsingSingleProcess(Mols, Writer):
    ...
    if OptionsInfo["Psi4DDXSolvationMode"]:
        Psi4Util.UpdatePsi4OptionsParameters(Psi4Handle, OptionsInfo["Psi4DDXSolvationOptions"])
    ...
    if OptionsInfo["SkipConformerGeneration"]:
        if OptionsInfo["Psi4ConstrainTorsionsMode"]:
    ...
    for Mol in Mols:
        ...
        Mol, CalcStatus, ConfID, Energy, SolvationEnergy = MinimizeMoleculeOrConformers(Psi4Handle, Mol, MolCount)
        ...
        WriteStatus = WriteMolecule(Writer, Mol, MolCount, ConfID, Energy, SolvationEnergy)
...
def ProcessMoleculesUsingMultipleProcessesAtMoleculesLevel(Mols, Writer):
    ...
    for Result in Results:
        MolCount += 1
        MolIndex, EncodedMol, CalcStatus, ConfID, Energy, SolvationEnergy = Result
...
def WorkerProcess(EncodedMolInfo):
    ...
    ConfID = None
    Energy = None
    SolvationEnergy = None

    if EncodedMol is None:
        return [MolIndex, None, CalcStatus, ConfID, Energy, SolvationEnergy]
    ...
    Mol, CalcStatus, ConfID, Energy, SolvationEnergy = MinimizeMoleculeOrConformers(OptionsInfo["psi4"], Mol, MolNum)
...
def ProcessMoleculesUsingMultipleProcessesAtConformersLevel(Mols, Writer):
    ...
    for Mol in Mols:
        ...
        Mol, CalcStatus, ConfID, Energy, SolvationEnergy = ProcessConformersUsingMultipleProcesses(Mol, MolCount)
...
def ProcessConformersUsingMultipleProcesses(Mol, MolNum):
    ...
    for Result in Results:
        ...
        CalcSolvationEnergyMap[ConfID] = SolvationEnergy
    ...
    SolvationEnergy = CalcSolvationEnergyMap[MinEnergyConfID]

    return (Mol, True, MinEnergyConfID, MinEnergy, SolvationEnergy)
...
def ConformerWorkerProcess(EncodedMolInfo):
    ...
    if not Status:
        return [MolIndex, EncodedMol, CalcStatus, ConfID, Energy, SolvationEnergy]
    ...
    CalcStatus, Energy, SolvationEnergy = MinimizeMoleculeUsingPsi4(OptionsInfo["psi4"], Mol, MolNum, ConfID)
    if not CalcStatus:
        if not OptionsInfo["QuietMode"]:
            MiscUtil.PrintWarning("Minimization couldn't be performed for molecule %s\n" % (MolName))
            return [MolIndex, EncodedMol, False, ConfID, Energy, SolvationEnergy]
...
def InitializePsi4ForWorkerProcess():
    ...
    if OptionsInfo["MPLevelConformersMode"] and re.match(
    ...
    if OptionsInfo["Psi4DDXSolvationMode"]:
        Psi4Util.UpdatePsi4OptionsParameters(OptionsInfo["psi4"], OptionsInfo["Psi4DDXSolvationOptions"])
...
def MinimizeMoleculeOrConformers(Psi4Handle, Mol, MolNum):
    ...
    ConfID = None
    if OptionsInfo["SkipConformerGeneration"]:
        Mol, CalcStatus, Energy, SolvationEnergy = MinimizeMolecule(Psi4Handle, Mol, MolNum)
    else:
        Mol, CalcStatus, ConfID, Energy, SolvationEnergy = GenerateAndMinimizeConformers(Psi4Handle, Mol, MolNum)

    return (Mol, CalcStatus, ConfID, Energy, SolvationEnergy)
...
def GenerateAndMinimizeConformers(Psi4Handle, Mol, MolNum=None):
    ...
    for ConfID in ConfIDs:
        ...
        # Perform Psi4 minimization...
        CalcStatus, Energy, SolvationEnergy = MinimizeMoleculeUsingPsi4(Psi4Handle, Mol, MolNum, ConfID)
        if not CalcStatus:
            if not OptionsInfo["QuietMode"]:
                MiscUtil.PrintWarning("Minimization couldn't be performed for molecule %s\n" % (MolName))
            return (Mol, False, None, None, None)
    ...
    return (Mol, True, MinEnergyConfID, Energy, SolvationEnergy)
...
def MinimizeMolecule(Psi4Handle, Mol, MolNum):
    ...
    CalcStatus, Energy, SolvationEnergy = MinimizeMoleculeUsingPsi4(Psi4Handle, Mol, MolNum)

    return (Mol, CalcStatus, Energy, SolvationEnergy)
...
def MinimizeMoleculeUsingPsi4(Psi4Handle, Mol, MolNum, ConfID=-1):
    ...
    # Setup constrained torsions...
    if OptionsInfo["Psi4ConstrainTorsionsMode"]:
        SetupConstrainedTorsionsAroundRotatableBonds(Psi4Handle, Mol)
    ...
    SolvationEnergy = None
    if OptionsInfo["Psi4DDXSolvationMode"]:
        SolvationEnergy = RetrievePsi4DDXSolvationEnergy(WaveFunction)
    ...
    if OptionsInfo["ApplyEnergyConversionFactor"]:
        ...
        if SolvationEnergy is not None:
            SolvationEnergy = SolvationEnergy * OptionsInfo["EnergyConversionFactor"]

    # Clean up
    PerformPsi4Cleanup(Psi4Handle)

    return (True, Energy, SolvationEnergy)


def RetrievePsi4DDXSolvationEnergy(WaveFunction):
    """Retrieve DDX solvation energy."""

    SolvationEnergy = None
    try:
        SolvationEnergy = WaveFunction.variable("DD Solvation Energy")
    except Exception as ErrMsg:
        SolvationEnergy = None
        if not OptionsInfo["QuietMode"]:
            MiscUtil.PrintWarning("Failed to retrieve Psi4 DD Solvation Energy using wave function: %s" % ErrMsg)
...
def EmbedMolecule(Mol, MolNum=None):
    ...
    if not OptionsInfo["QuietMode"]:
        if EmbedRMSDCutoff > 0:
            MiscUtil.PrintInfo(
                "Generating initial conformation ensemble by distance geometry for %s - EmbedRMSDCutoff: %s; Size: %s; Size after RMSD filtering: %s"
                % (RDKitUtil.GetMolName(Mol, MolNum), EmbedRMSDCutoff, MaxConfs, len(ConfIDs))
            )
        else:
            MiscUtil.PrintInfo(
                "Generating initial conformation ensemble by distance geometry for %s - EmbedRMSDCutoff: None; Size: %s"
                % (RDKitUtil.GetMolName(Mol, MolNum), len(ConfIDs))
            )
...
def SetupConstrainedTorsionsAroundRotatableBonds(Psi4Handle, Mol):
    """Setup constrained torsions around rotatable bonds."""

    if not OptionsInfo["Psi4ConstrainTorsionsMode"]:
        return

    # Get torsions around rotatable bonds...
    TorsionAtomsIndicesList = RDKitUtil.GetTorsionsAroundRotatableBonds(
        Mol, GetConstrainTorsionsRotBondsPatternMol(), OptionsInfo["Psi4ConstrainTorsionsParams"]["IgnoreHydrogens"]
    )

    # Freeze dihedrals corresponding to torsions around rotatable bonds...
    FreezeDihedralsString = SetupFreezeDihedralsStringForPsi4(TorsionAtomsIndicesList)
    if FreezeDihedralsString is None:
        # Clear any frozen dihedral values
        Psi4Handle.set_options({"OPTKING__frozen_dihedral": ""})
    else:
        # Setup frozen dihedral values...
        Psi4Handle.set_options({"OPTKING__frozen_dihedral": FreezeDihedralsString})


def SetupFreezeDihedralsStringForPsi4(TorsionAtomsIndicesList):
    """Setup freeze dihedrals string for Psi4."""

    FreezeDihedralsString = None
    if TorsionAtomsIndicesList is None or len(TorsionAtomsIndicesList) == 0:
        return FreezeDihedralsString

    TorsionsList = []
    for TorsionAtomIndices in TorsionAtomsIndicesList:
        AtomIndex1, AtomIndex2, AtomIndex3, AtomIndex4 = TorsionAtomIndices

        # Atom indices start from 1 for Psi4 instead 0 for RDKit...
        TorsionString = "%s %s %s %s" % (AtomIndex1 + 1, AtomIndex2 + 1, AtomIndex3 + 1, AtomIndex4 + 1)
        TorsionsList.append(TorsionString)

    if len(TorsionsList):
        FreezeDihedralsString = "%s" % "\n".join(TorsionsList)

    return FreezeDihedralsString


def GetConstrainTorsionsRotBondsPatternMol():
    """Get constrain torsions rotatable bonds pattern molecule."""

    if "RotBondsSMARTSPatternMol" not in OptionsInfo["Psi4ConstrainTorsionsParams"]:
        OptionsInfo["Psi4ConstrainTorsionsParams"]["RotBondsSMARTSPatternMol"] = Chem.MolFromSmarts(
            OptionsInfo["Psi4ConstrainTorsionsParams"]["RotBondsSMARTSPattern"]
        )

    return OptionsInfo["Psi4ConstrainTorsionsParams"]["RotBondsSMARTSPatternMol"]
...
def WriteMolecule(Writer, Mol, MolNum=None, ConfID=None, Energy=None, SolvationEnergy=None):
    """Write molecule."""
    ...
    if OptionsInfo["EnergyOut"] and Energy is not None:
        ...
        if OptionsInfo["Psi4DDXSolvationMode"] and SolvationEnergy is not None:
            SolvationEnergy = "%.*f" % (OptionsInfo["Precision"], SolvationEnergy)
            Mol.SetProp(OptionsInfo["SolvationEnergyDataFieldLabel"], SolvationEnergy)
...
def CheckAndSetupDDX():
    """Load pyddx and return its handle."""

    try:
        import pyddx
    except ImportError as ErrMsg:
        sys.stderr.write("\nFailed to import module/package pyddx for solvation: %s\n" % ErrMsg)
        sys.stderr.write("Psi4 relies on pyddx module for DDX solvation calculation.\n")
        sys.stderr.write("You may need to install it from pypi or conda-forge.\n")
        sys.stderr.write("Check/update your environment and try again.\n\n")
        sys.exit(1)

    return pyddx


def ProcessDDXListSolventsOption():
    """List solvent information provided by tge DDX module for the calculation
    of solvation energy."""

    PyDDXHandle = CheckAndSetupDDX()

    MiscUtil.PrintInfo("Solvent name and dielectric constant information avaibale in the DDX module:\n")
    MiscUtil.PrintInfo("Solvent\tEpsilon")
    Count = 0
    for Solvent in sorted(PyDDXHandle.data.solvent_epsilon.keys()):
        Count += 1
        Epsilon = PyDDXHandle.data.solvent_epsilon[Solvent]
        MiscUtil.PrintInfo("%s\t%s" % (Solvent, Epsilon))
...
def ProcessOptions():
    ...
    # Psi4 constrain torsions paramaters...
    OptionsInfo["Psi4ConstrainTorsionsMode"] = (
        True if re.match("^yes$", Options["--psi4ConstrainTorsions"], re.I) else False
    )
    ParamsDefaultInfoOverride = {"RotBondsSMARTSMode": "NonStrict"}
    Psi4ConstrainTorsionsParams = Psi4Util.ProcessPsi4ConstrainTorsionsParameters(
        "--psi4ConstrainTorsionsParams", Options["--psi4ConstrainTorsionsParams"], ParamsDefaultInfoOverride
    )
    OptionsInfo["Psi4ConstrainTorsionsParams"] = Psi4ConstrainTorsionsParams

    if OptionsInfo["Psi4ConstrainTorsionsMode"]:
        RotBondsSMARTSPattern = OptionsInfo["Psi4ConstrainTorsionsParams"]["RotBondsSMARTSPattern"]
        RotBondsPatternMol = Chem.MolFromSmarts(RotBondsSMARTSPattern)
        if RotBondsPatternMol is None:
            MiscUtil.PrintError(
                'Failed to create rotatable bonds pattern molecule. The rotatable bonds SMARTS pattern, "%s", specified for parameter name "--rotBondsSMARTSPattern" using "--psi4ConstrainTorsions"option is not valid.'
                % (RotBondsSMARTSPattern)
            )

    # DDX solvation parameters...
    OptionsInfo["Psi4DDXSolvationMode"] = True if re.match("^yes$", Options["--psi4DDXSolvation"], re.I) else False
    Psi4DDXSolvationParams = Psi4Util.ProcessPsi4DDXSolvationParameters(
        "--psi4DDXSolvationParams", Options["--psi4DDXSolvationParams"]
    )
    Psi4DDXSolvationOptions = Psi4Util.SetupPsi4DDXSolvationOptions(
        OptionsInfo["Psi4DDXSolvationMode"], Psi4DDXSolvationParams
    )
    OptionsInfo["Psi4DDXSolvationParams"] = Psi4DDXSolvationParams
    OptionsInfo["Psi4DDXSolvationOptions"] = Psi4DDXSolvationOptions
    ...
    if OptionsInfo["SkipConformerGeneration"]:
        if OptionsInfo["SMILESInfileStatus"]:
            MiscUtil.PrintError(
                'The value, %s, specified for option "-c --confGeneration" is not allowed during, %s, value for option "-i, --infile" . The input file must be a 3D SD or MOL file. '
                % (Options["--confGeneration"], Options["--infile"])
            )

    if not OptionsInfo["SkipConformerGeneration"]:
        if OptionsInfo["Psi4ConstrainTorsionsMode"]:
            MiscUtil.PrintError(
                'The value, %s, specified for option "-c --confGeneration" is not allowed during, %s, value for option "--psi4ConstrainTorsions".'
                % (Options["--confGeneration"], Options["--psi4ConstrainTorsions"])
            )
    ...
    SolvationEnergyDataFieldLabel = "Psi4_DDX_Solvation_Energy (%s)" % Options["--energyUnits"]
    OptionsInfo["SolvationEnergyDataFieldLabel"] = SolvationEnergyDataFieldLabel
    ...
    if re.match("^Molecules$", MPLevel, re.I):
        ...
        if OptionsInfo["SkipConformerGeneration"]:
            MiscUtil.PrintError(
                'The value, %s, specified for option "--mpLevel" is not allowed during, %s, value of option "--confGeneration"  . '
                % (MPLevel, Options["--confGeneration"])
            )
...
def RetrieveOptions():
    ...
    # Handle listing of solvent information...
    if Options and Options["--psi4DDXListSolvents"]:
...
def ValidateOptions():
    ...
    MiscUtil.ValidateOptionTextValue("--psi4ConstrainTorsions", Options["--psi4ConstrainTorsions"], "yes no")

    MiscUtil.ValidateOptionTextValue("--psi4DDXSolvation", Options["--psi4DDXSolvation"], "yes no")
...
Usage:
    Psi4PerformMinimization.py [--basisSet <text>] [--confGeneration <yes or no> ] [--confParams <Name,Value,...>]
                               ...
                               [--psi4ConstrainTorsions <yes or no>] [--psi4ConstrainTorsionsParams <Name,Value,...>]
                               [--psi4DDXSolvation <yes or no>] [--psi4DDXSolvationParams <Name,Value,...>]
...
Options:
    ...
    --psi4ConstrainTorsions <yes or no>  [default: no]
        Constrain torsions around rotatable bonds during Psi4 minimization.
    --psi4ConstrainTorsionsParams <Name,Value,...>  [default: auto]
        A space delimited list of parameter name and value  pairs for constraining
        torsions around rotatable bonds during Psi4 minimization. The supported
        parameter names, along with their default values, are shown below:
            
            ignoreHydrogens, yes,  [ Possible values: yes or no ]
            rotBondsSMARTSMode, NonStrict,   [ Possible  values: NonStrict,
                SemiStrict, Strict, Specify ]
            rotBondsSMARTSPattern, auto  [ Possible values: Valid SMARTS ]
        
        ignoreHydrogens: Ignore torsions around rotatable bonds containing
        hydrogens during Psi4 minimization.
        
        rotBondsSMARTSMode: SMARTS pattern to use for identifying rotatable bonds
        for constraining torsions during Psi4 minimization. Possible values: NonStrict,
        SemiStrict, Strict or Specify. The following SMARTS patterns are used to identify
        rotatable bonds for different modes:
        ...
        choosing 'Specify' value for 'rotBondsSMARTSMode' parameter and providing
        its value via '--rotBondsSMARTSPattern'.
        
        rotBondsSMARTSPattern: SMARTS pattern for identifying rotatable bonds for
        constraining torsions during Psi4 minimization. This option is only valid for
        'Specify' value of 'rotBondsSMARTSMode' parameter.
    --psi4DDXSolvation <yes or no>  [default: no]
        Perform minimization in solution using domain-decomposition-based
        continuum solvation models [ Ref 160-161 ]. The script relies on Psi4
        interface to the DDX module to perform these calculations. The DDX library
        provides a linear-scaling implementation of standard continuum solvation
        models using a domain- decomposition ansatz. Two solvation models are
        supported: COnductor-like Screening MOdel (COSMO) [ Ref 162-163] and
        Polarizable Continuum Model (PCM) [ Ref 164-165 ].
        
        The solvation energy is included in the value of the total energy written to 
        the output SD file. In addition, the value of solvation energy is written to the
        output file under its own data field label.
        
        Psi4 relies on Python module PYDDX to calculate solvation energy. It must be
        present in your environment.
    --psi4DDXSolvationParams <Name,Value,...>  [default: auto]
        A space delimited list of parameter name and value  pairs for calculating
        solvation energy using the DDX module. The supported parameter names,
        along with their default values, are shown below:
            ...
            solventEpsilon: Dielectric constant of the solvent. The
            corresponding Psi4 option is DDX_SOLVENT_EPSILON.
            
            radiiSet: Radius set for cavity spheres. The corresponding Psi4
            option is DDX_RADII_SET.
            
            radiiScaling: Scaling factor for cavity spheres. The default value
            depends on radiiSet: 1.2 for Bondi; 1.1 for UFF. The corresponding
            Psi4 option is DDX_RADII_SCALING.
            
        These parameter names are automatically mapped to appropriate DDX keywords.
        
        You may specify the solvent either by directly providing a dielectric constant using
        'solventEpsilon' parameter or a solvent name corresponding to 'solvent' parameter.
        The solvent name must be a valid named supported by the DDX module. For example:
        water, ethanol, dimethylsulfoxide, cis-1,2-dimethylcyclohexane , etc. The 'solvent'
        parameter is ignored for non-zero value of 'solvent' option.
        
        The DDX module contains a variety of addtional options to confgure the calculation
        of solvation energy. For example: DDX_MAXITER, DDX_RADII_SCALING, etc. You may
        use '--psi4OptionsParams' to modify values for additional DDX options.
    --psi4DDXListSolvents
        List solvent names, along with dielectric values, supported by the DDX module
        for the calculation of solvent energy without performing any calculation.
...
Examples:
    ...
    combination of ETKDGv2 distance geometry methodology, applying RMSD cutoff
    ...
    To run the first example along with performing energy mimimization in solution
    using default DDX solvation parameters for water and write out a SD file, type:
    ...
    combination of ETKDGv2 distance geometry methodology and MMFF94s forcefield
