...
def ProcessSpecifiedMutations():
    ...
    for Mutation in MutationsWords:
        ...
        if not MatchedResults:
            MiscUtil.PrintWarning(
                'The format of mutation, "%s", specified using "-m, --mutations" option is not valid. Supported format: <ChainID>:<ResName><ResNum><ResName> or <ResName><ResNum><ResName>\nIgnoring mutation...'
                % (Mutation)
            )
            continue
        ...
        MutationSpec = "%s:%s%s%s" % (CurrentChainID, ResName, ResNum, NewResName)
        ...
        if CanonicalMutation in CanonicalMutationMap:
            ...
            continue
        CanonicalMutationMap[CanonicalMutation] = CanonicalMutation
