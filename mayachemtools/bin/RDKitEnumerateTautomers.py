...
def EnumerateTautomers():
    ...
    MolCount, ValidMolCount, TautomerizationFailedCount, TautomersCount, MinTautomersCount, MaxTautomersCount = (
    ...
    MiscUtil.PrintInfo(
        "Average number of tautomers for a molecule: %.1f"
        % (TautomersCount / (ValidMolCount - TautomerizationFailedCount))
    )
...
def ProcessMoleculesUsingMultipleProcesses(Mols, Writer):
    ...
    for Result in Results:
        ...
        if EncodedTautomerMols is not None:
            TautomerMols = [
                RDKitUtil.MolFromBase64EncodedMolString(EncodedTautomerMol)
                for EncodedTautomerMol in EncodedTautomerMols
            ]
...
def WorkerProcess(EncodedMolInfo):
    ...
    if TautomerMols is not None:
        EncodedTautomerMols = [
            ...
            for TautomerMol in TautomerMols
        ]
...
def SortMolTautomers(Mol, TautomerEnumerator, TautomerMols, TautomerScores=None):
    ...
    if TautomerScores is None:
        SortedResults = sorted(
            (Smiles, TautomerMol)
            for Smiles, TautomerMol in zip(TautomerSmiles, TautomerMols)
            if Smiles != CanonicalTautomerSmiles
        )
    else:
        SortedResults = sorted(
            (Smiles, TautomerMol, TautomerScore)
            for Smiles, TautomerMol, TautomerScore in zip(TautomerSmiles, TautomerMols, TautomerScores)
            if Smiles != CanonicalTautomerSmiles
        )
...
def ProcessTautomerizationParameters():
    ...
    OptionsInfo["TautomerizationParams"] = MiscUtil.ProcessOptionNameValuePairParameters(
        "--tautomerizationParams", Options["--tautomerizationParams"], ParamsDefaultInfo
    )
    ...
    for ParamName in ["MaxTautomers", "MaxTransforms"]:
        ...
        if ParamValue <= 0:
            MiscUtil.PrintError(
                'The parameter value, %s, specified for parameter name, %s, using "-t, --tautomerizationParams" option is not a valid value. Supported values: > 0'
...
Description:
    Enumerate tautomers for molecules and write them out to an output file.
