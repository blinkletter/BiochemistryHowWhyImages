...
def CalculateRMSDValue(RefMol, ProbeMol):
    ...
    try:
        if OptionsInfo["UseBestRMSD"]:
            RMSD = AllChem.GetBestRMS(ProbeMol, RefMol)
        else:
...
Examples:
    ...
    To calculate RMSD between all molecules in reference and probe input
    3D SD files and write out a CSV file containing calculated RMSD values along with
    appropriate molecule IDs, type:
    ...
    To calculate RMSD between all molecules in reference and probe molecules input
