...
def WorkerProcess(EncodedMolInfo):
    ...
    EncodedMolWithHs = RDKitUtil.MolToBase64EncodedMolString(
        MolWithHs, PropertyPickleFlags=Chem.PropertyPickleOptions.MolProps | Chem.PropertyPickleOptions.PrivateProps
    )
...
def CalculateMolPartialCharges(Mol, MolCount):
    ...
    if OptionsInfo["MMFFChargesMode"]:
        if AllChem.MMFFHasAllMoleculeParams(Mol):
            MMFFProp = AllChem.MMFFGetMoleculeProperties(Mol, mmffVariant=OptionsInfo["MMFFVariant"])
            PartialCharges = [MMFFProp.GetMMFFPartialCharge(AtomIndex) for AtomIndex in range(Mol.GetNumAtoms())]
        else:
            MolName = RDKitUtil.GetMolName(Mol, MolCount)
            MiscUtil.PrintWarning(
                "Failed to calculate MMFF partial charges for molecule, %s: Missing forcefield parameters" % MolName
            )
            return (False, PartialCharges)
    else:
        rdPartialCharges.ComputeGasteigerCharges(
            Mol, nIter=OptionsInfo["NumIters"], throwOnParamFailure=OptionsInfo["AllowParamFailure"]
        )
...
def ProcessOptions():
    ...
    MMFFVariant = "MMFF94" if re.match("^MMFF94$", Options["--mmffVariant"], re.I) else "MMFF94s"
...
def ValidateOptions():
    ...
    MiscUtil.ValidateOptionTextValue(" --mmffVariant", Options["--mmffVariant"], "MMFF94 MMFF94s")
...
Usage:
    RDKitCalculatePartialCharges.py [--allowParamFailure <yes or no>]
                                          ...
                                          [--mmffVariant <MMFF94 or MMFF94s>] [--mp <yes or no>]
                                          [--mpParams <Name,Value,...>] [--numIters <number>]
...
Options:
    ...
    --infileParams <Name,Value,...>  [default: auto]
        ...
        formats, along with their default values, are shown below:
            ...
            SD, MOL: sanitize,yes,strictParsing,yes
    ...
    --mmffVariant <MMFF94 or MMFF94s>  [default: MMFF94]
        Variant of MMFF forcefield to use for calculating charges.
