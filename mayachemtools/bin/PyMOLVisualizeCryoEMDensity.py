...
def WritePyMOLParameters(OutFH):
    ...
    if OptionsInfo["VolumeColorRampCreate"]:
        ...
        PMLCmds.append(
            """cmd.volume_ramp_new("%s", "%.2f blue 0.00 %.2f cyan 0.20 %.2f blue 0.00")"""
            % (ColorRampName, LowerContourLevel, ContourLevel, UpperContourLevel)
        )
...
def WriteChainComplexAndMeshViews(OutFH, FileIndex, PyMOLObjectNames, ChainID):
    ...
    if MeshChainComplex or VolumeChainComplex or SurfaceChainComplex:
        ...
        GenerateAndWritePMLForGroup(
            OutFH,
            PyMOLObjectNames["Chains"][ChainID]["ChainComplexCryoEMGroup"],
...
def SetupChainSelectionsView(OutFH, FileIndex, PyMOLObjectNames, ChainID):
    ...
    for Index in range(0, len(OptionsInfo["ChainSelectionsInfo"]["Names"])):
        ...
        PML = SetupPMLForCryoEMDensityVolume(
            MapName, CryoEMVolumeName, OptionsInfo["VolumeColorRampName"], Enable=False, Selection=SelectionObjectName
        )
        OutFH.write("\n%s\n" % PML)

        PML = SetupPMLForCryoEMDensityMesh(
            MapName, CryoEMMeshName, ContourLevel, Color, Enable=True, Selection=SelectionObjectName
        )
        OutFH.write("\n%s\n" % PML)

        PML = SetupPMLForCryoEMDensitySurface(
            MapName, CryoEMSurfaceName, ContourLevel, Color, Enable=False, Selection=SelectionObjectName
        )
        OutFH.write("\n%s\n" % PML)
...
def WriteChainLigandView(OutFH, FileIndex, PyMOLObjectNames, ChainID, LigandID):
    ...
    for GroupID in ["Ligand", "Pocket", "PocketSolvent", "PocketInorganic"]:
        ...
        PML = SetupPMLForCryoEMDensityVolume(
            CryoEMMapName,
            CryoEMVolumeName,
            OptionsInfo["VolumeColorRampName"],
            Enable=False,
...
def ProcessSpecifiedDensityMapFiles():
    ...
    if DensityMapFilesWordsCount != InfilesNamesCount:
        MiscUtil.PrintError(
            'The number of comma delimited cryo-EM density map files, %d, specified using "--DensityMapFiles" must be equal to the number of input files, %s, specified using "-i, --infiles" option.'
            % (DensityMapFilesWordsCount, InfilesNamesCount)
        )
    ...
    for Index in range(0, InfilesNamesCount):
        ...
        if not os.path.exists(DensityMapFile):
            MiscUtil.PrintError(
                'The cryo-EM density ED map file, %s, specified using option "--DensityMapFiles", corresponding to input file, %s,  doesn\'t exist.\n'
...
def ProcessAutoDensityMapFiles():
    ...
    for Index in range(0, InfilesNamesCount):
        ...
        if EMDBID is None:
            MiscUtil.PrintError(
                'Failed to retrieve EMDB ID from input file %s to automatically set density map file name. Use option "-d, --densityMapFiles " to specify density map file name and try again.'
                % Infile
            )
        ...
        if os.path.exists(MapFile1):
            ...
            MiscUtil.PrintError(
                'Density map files %s or %s don\'t exist. Use option "-d, --densityMapFiles " to specify density map file name and try again'
                % (MapFile1, MapFile2)
            )
...
def RetrieveRecommededContourLevel(Infile):
    ...
    if EMDBID is None:
        MiscUtil.PrintWarning(
            "Failed to retrieve EMDB ID from input file %s to detect local header file already downloaded from EMDB server..."
            % Infile
        )
    ...
    if not os.path.exists(MetadataHeaderFile):
        MiscUtil.PrintWarning(
            "Failed to find a local header file, %s, for EMDB ID %s..." % (MetadataHeaderFile, EMDBID)
        )
        OptionsInfo["InfilesRecommededContourLevels"][Infile] = RecommendedContourLevel
        return RecommendedContourLevel
...
def ProcessAutoMeshLevel():
    ...
    for Index in range(0, InfilesNamesCount):
        ...
        if RecommededContourLevel is None:
            MiscUtil.PrintWarning(
                'Failed to retrieve recommended mesh contour level from header. It\'s being set to 1.0. Use "--meshLevel" option to specify a different contour mesh level.'
            )
            MeshLevel = 1.0
        else:
...
def ProcessSpecifiedMeshLevel():
    ...
    if MeshLevelWordsCount != InfilesNamesCount:
        MiscUtil.PrintError(
            'The number of comma delimited mesh levels, %d, specified using "--meshLevel" must be equal to the number of input files, %s, specified using "-i, --infiles" option.'
            % (MeshLevelWordsCount, InfilesNamesCount)
        )
    ...
    for Index in range(0, InfilesNamesCount):
        ...
        if not MiscUtil.IsFloat(MeshLevel):
            MiscUtil.PrintError(
                'The mesh level, %s, specified using "--meshLevel" for input file, %s, must be a float.'
                % (MeshLevel, Infile)
            )
...
def ProcessVolumeColorRamp():
    ...
    if re.match("^auto$", ColorRamp, re.I):
        ...
        if RecommededContourLevel is None:
            ...
            MiscUtil.PrintWarning(
                "Failed to retrieve recommended contour level from header file corresponding to input file, %s, to create a new ramp. Using PyMOL default volume color ramp."
                % (FirstInfile)
            )
        else:
            ...
            MiscUtil.PrintInfo(
                ...
                % (ColorRampName, ColorRampContourLevel, FirstInfile)
            )
    else:
