        ...
        if not CalcStatus:
            if not OptionsInfo["QuietMode"]:
                MiscUtil.PrintWarning(
                    "Failed to calculate properties for molecule %s" % RDKitUtil.GetMolName(Mol, MolCount)
                )
...
def CalculatePropertyValues(Psi4Handle, WaveFunction, Mol, MolNum):
    ...
    return [
        PropertyNamesMap["CalcFunction"][Name](Name, Psi4Handle, WaveFunction, Mol, MolNum)
        for Name in OptionsInfo["SpecifiedPropertyNames"]
    ]
...
def CalculateDipole(PropertyName, Psi4Handle, WaveFunction, Mol, MolNum):
    ...
    try:
        ...
        if not OptionsInfo["QuietMode"]:
            MiscUtil.PrintWarning(
                "Psi4 failed to calculate %s values for molecule %s:\n%s\n"
...
def ProcessSpecifiedPropertyNames():
    ...
    for Name in SpecifiedNamesWords:
        ...
        if CanonicalName not in CanonicalPropertyNamesMap:
            MiscUtil.PrintError(
                'The property name specified, %s, using "-p, --property" option is not a valid name.' % Name
            )
...
def SetupPropertyNamesInfo(PrintInfo=True):
    ...
    Name = "Quadrupole"
    PropertyNamesMap["CalcValueLabels"][Name] = [
        "QuadrupoleXX",
        "QuadrupoleYY",
        "QuadrupoleZZ",
        "QuadrupoleXY",
    ...
    PropertyNamesMap["CalcValueLabels"][Name] = [
        "HOMOEnergy",
        "LUMOEnergy",
        "LUMO-HOMOGap (%s)" % OptionsInfo["EnergyUnits"],
    ]
