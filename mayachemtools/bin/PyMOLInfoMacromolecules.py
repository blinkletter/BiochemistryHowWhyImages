...
def ListHeaderInfo(Infile):
    ...
    if re.match("^pdb$", FileExt, re.I):
        Classification, DepositionDate, IDCode, ExperimentalTechnique, Resolution = (
            RetriveHeadAndExperimentalInfoFromPDBFile(Infile)
        )
    elif re.match("^cif$", FileExt, re.I):
        Classification, DepositionDate, IDCode, ExperimentalTechnique, Resolution = (
            RetriveHeadAndExperimentalInfoFromCIFFile(Infile)
        )
    ...
    for Line in Lines:
        if re.match("^_exptl.method", Line, re.I):
            ...
            ExperimentalTechnique = (
                ExperimentalTechnique.strip() if len(ExperimentalTechnique.strip()) else "Not Available"
            )
            break
...
def GetSelectionResiduesInfo(MolName, ChainID, SelectionType):
    ...
    if re.match("^Ligands$", SelectionType, re.I):
        ...
        SelectionInfo = (
            PyMOLUtil.GetAminoAcidResiduesInfo(MolName, ChainID, "NonStandard") if ChainID is not None else None
        )
    else:
        MiscUtil.PrintError(
            "Failed to retrieve residues information: Selection type %s is not valid..." % SelectionType
        )
...
def ListPocketsInfo(MolName, ChainIDs):
    ...
    if not (
        OptionsInfo["All"]
        or OptionsInfo["PocketLigands"]
        or OptionsInfo["PocketSolvents"]
        or OptionsInfo["PocketInorganics"]
    ):
        return
...
def ListPocketSelectionResiduesInfo(MolName, ChainID, LigandResName, LigandResNum, SelectionType):
    ...
    SelectionInfo = GetPocketSelectionResiduesInfo(
        MolName, ChainID, LigandResName, LigandResNum, PocketDistanceCutoff, SelectionType
    )
    ...
    MiscUtil.PrintInfo(
        "Chain ID: %s; Ligand ID: %s_%s\nResidue distribution: %s\nResidue IDs: %s"
        % (ChainID, LigandResName, LigandResNum, ResiduesDistribution, ResiduesIDs)
    )
...
def GetPocketSelectionResiduesInfo(MolName, ChainID, LigandResName, LigandResNum, PocketDistanceCutoff, SelectionType):
    ...
    if re.match("^Pockets$", SelectionType, re.I):
        SelectionInfo = PyMOLUtil.GetPocketPolymerResiduesInfo(
            MolName, ChainID, LigandResName, LigandResNum, PocketDistanceCutoff
        )
        ...
        MiscUtil.PrintError(
            "Failed to retrieve pocket residues information: Selection type %s is not valid..." % SelectionType
        )
    ...
    for ChainID in ChainIDs:
        if re.match("^Categories$", OptionsInfo["PhiPsiMode"], re.I):
            ...
            SetupAndListPhiPsiResiduesInfo(
                GeneralPhiPsiInfo, "General (All residues except glycine, proline, or pre-proline)", ChainID
            )
            ...
            SetupAndListPhiPsiResiduesInfo(
                PreProPhiPsiInfo, "Pre-Proline (Only residues before proline not including glycine or proline)", ChainID
            )
        else:
...
def ListInterfaceResiduesInfo(MolName, ChainIDs):
    ...
    for ChainIDsPair in ChainIDsPairs:
        ...
        InterfaceResiduesInfo1, InterfaceResiduesInfo2 = GetInterfaceChainsAndResiduesInfo(
            MolName, InterfaceChainIDs1, MolName, InterfaceChainIDs2, InterfaceResiduesMethod, InterfaceResiduesCutoff
        )
...
def SetupSelectionChainsResiduesInfo(SelectionInfo):
    ...
    for ChainID in SelectionInfo["ChainIDs"]:
        ...
        ResiduesCount = 0
        SortedResNames = sorted(
            SelectionInfo["ResNames"][ChainID],
            key=lambda ResName: SelectionInfo["ResCount"][ChainID][ResName],
            reverse=True,
        )
        for ResName in SortedResNames:
...
def ListBoundingBoxInfo(MolName):
    ...
    MiscUtil.PrintInfo(
        "Bounding box size in Angstroms: XSize - %.3f; YSize - %.3f; ZSize - %.3f" % (XSize, YSize, ZSize)
    )


def ListCentroid(MolName):
    """List centroid information."""

    if not (OptionsInfo["All"] or OptionsInfo["Centroid"]):
        return

    MolSelection = "(%s)" % MolName
    Centroid = PyMOLUtil.GetCentroid(MolSelection)
...
def GetPocketSelectionResiduesInfoLabel(SelectionType):
    ...
    if re.match("^Pockets$", SelectionType, re.I):
        ...
        MiscUtil.PrintError(
            "Failed to retrieve pocket residues label information: Selection type %s is not valid..." % SelectionType
        )
...
def ProcessOptions():
    ...
    OptionsInfo["Centroid"] = Options["--centroid"]
    ...
    if not re.match("^auto$", InterfaceResiduesChains, re.I):
        ...
        if len(InterfaceResiduesChainsWords) % 2:
            MiscUtil.PrintError(
                'The number of comma delimited chain IDs, %d, specified using "--interfaceResiduesChains" option, "%s",  must be a multple of 2.'
                % (len(InterfaceResiduesChainsWords), Options["--interfaceResiduesChains"])
            )
    ...
    if re.match("^auto$", InterfaceResiduesCutoff, re.I):
        if re.match("^BySASAChange$", InterfaceResiduesMethod, re.I):
            ...
            MiscUtil.PrintError(
                'The specified value, %s, for option "--interfaceResiduesMethod" is not supported.'
                % (InterfaceResiduesMethod)
            )
    else:
    ...
    for Name in OptionNames:
        if Name in OptionsInfo:
            ...
            MiscUtil.PrintError("Option name %s is not a valid name..." % Name)
...
def ValidateOptions():
    ...
    MiscUtil.ValidateOptionTextValue(
        "--interfaceResiduesMethod",
        Options["--interfaceResiduesMethod"],
...
Usage:
    PyMOLInfoMacromolecules.py [--all] [--boundingBox] [--centroid] [--chains]
...
Options:
    ...
    -c, --chains
     ...
     --centroid
        Centroid of atomic coordinates.  It corresponds to the mean of all 3D
