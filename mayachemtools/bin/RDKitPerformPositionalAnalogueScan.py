...
def PerformPositionalAnalogueScan():
    ...
    MolCount, ValidMolCount, SearchPatternMissingCount, AnaloguesGenerationFailedCount, AnaloguesCount = (
        ProcessMolecules(Mols, Writer)
    )
    ...
    MiscUtil.PrintInfo(
        ...
        % (MolCount - ValidMolCount + SearchPatternMissingCount + AnaloguesGenerationFailedCount)
    )
...
def ProcessMoleculesUsingMultipleProcesses(Mols, Writer):
    ...
    for Result in Results:
        ...
        MolAnalogues = [
            RDKitUtil.MolFromBase64EncodedMolString(EncodedMolAnalogue) for EncodedMolAnalogue in EncodedMolAnalogues
        ]
...
def WorkerProcess(EncodedMolInfo):
    ...
    if MolAnalogues is not None:
        EncodedMolAnalogues = [
            ...
            for MolAnalogue in MolAnalogues
        ]
...
def GenerateMolAnalogues(Mol, MolNum=None):
    ...
    if OptionsInfo["AttachAtomsMode"] or OptionsInfo["ReplaceAtomsMode"] or OptionsInfo["AttachSMILESMode"]:
        ...
        MiscUtil.PrintError(
            'Failed to generate analogues.  The mode value, %s, specified using "-m, --mode" is not supported.'
            % (OptionsInfo["Mode"])
        )
...
def GenerateMolAnaloguesForMultipleScanModes(Mol, MolNum=None):
    ...
    TargetPatternInfo = OptionsInfo["TargetPatternInfo"]
    for TargetPatternIndex, TargetPatternObject in enumerate(TargetPatternInfo["PatternObjects"]):
        for AtomIndicesCombination in combinations(MolMatchedAtomIndices, OptionsInfo["ComboSearchAtoms"]):
            ...
            for AtomIndex in AtomIndicesCombination:
                if OptionsInfo["AttachAtomsMode"]:
                    AtomicNumber = TargetPatternObject
                    MolAnalogue = AttachAtom(
                        MolAnalogue, AtomIndex, AtomicNumber, BondOrder=Chem.rdchem.BondType.SINGLE
                    )
                    ...
                    AttachAtomIndex = (
                        MolAnalogue.GetNumAtoms()
                        - 1
                        + TargetPatternInfo["AttachSMILESAtomIndexOffsets"][TargetPatternIndex]
                    )
                    SMILESMol = TargetPatternObject
                    MolAnalogue = AttachMol(
                        MolAnalogue, AtomIndex, SMILESMol, AttachAtomIndex, BondOrder=Chem.rdchem.BondType.SINGLE
                    )

            if SanitizeMolAnalogue(MolAnalogue, MolNum):
...
def GenerateMolAnaloguesForRxnSMARTSMode(Mol, MolNum=None):
    ...
    TargetPatternInfo = OptionsInfo["TargetPatternInfo"]
    for Rxn in TargetPatternInfo["PatternObjects"]:
...
def GenerateMolAnaloguesForTandemScanMode(Mol, MolNum=None):
    ...
    NumOfTandemOperations = 0
    if TargetPatternTandemScanInfo is not None and len(TargetPatternTandemScanInfo):
        NumOfTandemOperations = len(TargetPatternTandemScanInfo[0]["Names"])

    if len(MolMatchedAtomIndices) < NumOfTandemOperations:
        if not OptionsInfo["QuietMode"]:
            MolName = RDKitUtil.GetMolName(Mol, MolNum)
            MiscUtil.PrintWarning(
                "Number of matched search pattern atoms, %s, must be >= number of specified operations, %s, during TandemScan mode for molecule %s..."
                % (len(MolMatchedAtomIndices), NumOfTandemOperations, MolName)
            )
        SearchPatternMissingStatus = True
        return (MolAnalogues, SearchPatternMissingStatus, AnaloguesGenerationStatus)
    ...
    for AtomIndicesCombination in combinations(MolMatchedAtomIndices, NumOfTandemOperations):
        ...
        for AtomIndicesPermutation in AtomIndicesPermutations:
            for PatternInfo in TargetPatternTandemScanInfo:
                MolAnalogue = Chem.RWMol(Mol)

                for NameIndex, Name in enumerate(PatternInfo["Names"]):
                    AtomIndex = AtomIndicesPermutation[NameIndex]
                    PatternObject = PatternInfo["PatternObjects"][NameIndex]

                    if re.match("^AttachAtoms$", Name, re.I):
                        MolAnalogue = AttachAtom(
                            MolAnalogue, AtomIndex, PatternObject, BondOrder=Chem.rdchem.BondType.SINGLE
                        )
                    elif re.match("^ReplaceAtoms$", Name, re.I):
                        MolAnalogue = ReplaceAtom(MolAnalogue, AtomIndex, PatternObject)
                    elif re.match("^AttachSMILES$", Name, re.I):
                        AttachAtomIndex = (
                            MolAnalogue.GetNumAtoms() - 1 + PatternInfo["AttachSMILESAtomIndexOffsets"][NameIndex]
                        )
                        MolAnalogue = AttachMol(
                            MolAnalogue,
                            AtomIndex,
...
def SetupSearchAndTargetPatternsInfo():
    ...
    if SearchPattern is not None:
        ...
        if SearchPatternMol is None:
            MiscUtil.PrintError(
                'Failed to create search pattern molecule. The torsion SMILES/SMARTS pattern, "%s", specified using "-s, --searchPattern" option is not valid.'
                % (SearchPattern)
            )
    ...
    TargetPatternInfo = OptionsInfo["TargetPatternInfo"]
    if TargetPatternInfo is not None:
        TargetPatternInfo["PatternObjects"] = []
        TargetPatternInfo["AttachSMILESAtomIndexOffsets"] = []

    if re.match("^(AttachAtoms|ReplaceAtoms)$", Mode, re.I):
        for Pattern in TargetPatternInfo["Patterns"]:
            TargetPatternInfo["PatternObjects"].append(Chem.GetPeriodicTable().GetAtomicNumber(Pattern))
            TargetPatternInfo["AttachSMILESAtomIndexOffsets"].append(None)
    elif re.match("^AttachSMILES$", Mode, re.I):
        for Pattern in TargetPatternInfo["Patterns"]:
            SMILESMol = Chem.MolFromSmiles(Pattern)
            AttachAtomIndexOffset = GetAttachSMILESAtomIndexOffset(SMILESMol)

            TargetPatternInfo["PatternObjects"].append(SMILESMol)
            TargetPatternInfo["AttachSMILESAtomIndexOffsets"].append(AttachAtomIndexOffset)
    elif re.match("^RxnSMARTS$", Mode, re.I):
        for Pattern in TargetPatternInfo["Patterns"]:
            TargetPatternInfo["PatternObjects"].append(AllChem.ReactionFromSmarts(Pattern))
            TargetPatternInfo["AttachSMILESAtomIndexOffsets"].append(None)
    elif re.match("^TandemScan$", Mode, re.I):
        TargetPatternTandemScanInfo = OptionsInfo["TargetPatternTandemScanInfo"]
        for PatternInfo in TargetPatternTandemScanInfo:
            PatternInfo["PatternObjects"] = []
            PatternInfo["AttachSMILESAtomIndexOffsets"] = []
            for NameIndex, Name in enumerate(PatternInfo["Names"]):
                Pattern = PatternInfo["Pattern"][NameIndex]
                if re.match("^(AttachAtoms|ReplaceAtoms)$", Name, re.I):
                    PatternInfo["PatternObjects"].append(Chem.GetPeriodicTable().GetAtomicNumber(Pattern))
                    PatternInfo["AttachSMILESAtomIndexOffsets"].append(None)
                elif re.match("^AttachSMILES$", Name, re.I):
                    SMILESMol = Chem.MolFromSmiles(Pattern)
                    AttachAtomIndexOffset = GetAttachSMILESAtomIndexOffset(SMILESMol)

                    PatternInfo["PatternObjects"].append(SMILESMol)
                    PatternInfo["AttachSMILESAtomIndexOffsets"].append(AttachAtomIndexOffset)


def GetAttachSMILESAtomIndexOffset(Mol):
    """Get attach atom index offset and optionally clear atom map numbers."""

    AttachAtomIndexOffset = 1

    AtomMapIndices = RDKitUtil.GetAtomMapIndices(Mol)
    if AtomMapIndices is not None and len(AtomMapIndices):
        # Use the atom index corresponding to the lowest atom map number
        # as the attachement atom. The AtomMapIndices list contains atom indices
        # sorted by atom map numbers.

        AttachAtomIndexOffset = AtomMapIndices[0] + 1

        if OptionsInfo["ClearAtomMapNumbers"]:
            RDKitUtil.ClearAtomMapNumbers(Mol)

    return AttachAtomIndexOffset
...
def ProcessOptions():
    ...
    OptionsInfo["ClearAtomMapNumbers"] = True if re.match("^yes$", Options["--clearAtomMapNumbers"], re.I) else False
    ...
    if MiscUtil.IsEmpty(SearchPattern):
        MiscUtil.PrintError('No value specified using "-s, --searchPattern" option.')
    if re.match("^auto$", SearchPattern, re.I):
        ...
        if re.match("^RxnSMARTS$", Mode, re.I):
            MiscUtil.PrintError(
                'The specification of SMARTS search pattern, %s,  using "-s, --searchPattern" option is not allowed during, %s, value of "-m, --mode".'
                % (SearchPattern, Mode)
            )
    ...
    if MiscUtil.IsEmpty(TargetPattern):
        MiscUtil.PrintError('No value specified using "-t, --targetPattern" option.')
    TargetPattern = TargetPattern.strip()
    ...
    if re.match("^auto$", TargetPattern, re.I):
        ...
        if re.match("^AttachAtoms$", Mode, re.I):
            ...
            MiscUtil.PrintError(
                'Failed to setup default target pattern for mode, %s, specified using "-m, --mode"' % (Mode)
            )

    TargetPatternInfo = None
    TargetPatternTandemScanInfo = None

    if re.match("^(AttachAtoms|ReplaceAtoms)$", Mode, re.I):
        TargetPatternInfo = InitializeTargetPatternInfo(TargetPattern)
        for ElementSymbol in TargetPatternInfo["Patterns"]:
            if not RDKitUtil.IsValidElementSymbol(ElementSymbol):
                MiscUtil.PrintError(
                    'The target pattern, "%s", specified using "-t, --targetPattern" must be a valid atomic symbol during, %s, value of "-m, --mode" option.'
                    % (ElementSymbol, Mode)
                )
    elif re.match("^AttachSMILES$", Mode, re.I):
        TargetPatternInfo = InitializeTargetPatternInfo(TargetPattern)
        for SMILES in TargetPatternInfo["Patterns"]:
            ValidateSMILESTargetPattern(SMILES, Mode)
    elif re.match("^RxnSMARTS$", Mode, re.I):
        TargetPatternInfo = InitializeTargetPatternInfo(TargetPattern)
        for RxnPattern in TargetPatternInfo["Patterns"]:
            try:
                AllChem.ReactionFromSmarts(RxnPattern)
            except Exception as ErrMsg:
                MiscUtil.PrintError(
                    'The target pattern, "%s", specified using "-t, --targetPattern" must be a valid RxnSMARTS  during, %s, value of "-m, --mode" option.\nErrMsg: %s'
                    % (RxnPattern, Mode, ErrMsg)
                )
    elif re.match("^TandemScan$", Mode, re.I):
        TargetPatternTandemScanInfo = ProcessTandemScanTargetPattern(TargetPattern)

    # Track target pattern info...
    OptionsInfo["TargetPattern"] = TargetPattern
    OptionsInfo["TargetPatternInfo"] = TargetPatternInfo
    OptionsInfo["TargetPatternTandemScanInfo"] = TargetPatternTandemScanInfo


def InitializeTargetPatternInfo(TargetPattern):
    """Initialize target pattern information."""

    TargetPatternInfo = {}
    TargetPatternInfo["Patterns"] = TargetPattern.split()

    return TargetPatternInfo


def ProcessTandemScanTargetPattern(TargetPattern):
    """Process target pattern for tandem scan."""

    # Validate target pattern...
    ValidatedTargetPatternInfo = ValidateTandemScanTargetPattern(TargetPattern)

    # Expand target patterns to cover various combinations...
    #
    # For example: "ReplaceAtoms,N,Attach Atoms, F Cl" expands to
    # "ReplaceAtoms,N,AttachAtoms,F" and "ReplaceAtoms,N,AttachAtoms,Cl"
    #
    ExpandedPatterns = None
    for NameIndex, Name in enumerate(ValidatedTargetPatternInfo["Names"]):
        Patterns = ValidatedTargetPatternInfo["Patterns"][NameIndex]

        if ExpandedPatterns is None:
            ExpandedPatterns = []
            for Pattern in Patterns:
                ExpandedPatterns.append([Name, Pattern])
            continue
        ...
        for ExpandedPattern in ExpandedPatterns:
            for Pattern in Patterns:
                NewExpandedPattern = []
                NewExpandedPattern.extend(ExpandedPattern)

                NewExpandedPattern.append(Name)
                NewExpandedPattern.append(Pattern)

                NewExpandedPatterns.append(NewExpandedPattern)

        ExpandedPatterns = NewExpandedPatterns
    ...
    for ExpandedPattern in ExpandedPatterns:
        ...
        for Index in range(0, len(ExpandedPattern), 2):
            Name = ExpandedPattern[Index].strip()
            Pattern = ExpandedPattern[Index + 1].strip()
            PatternInfo["Names"].append(Name)
            PatternInfo["Pattern"].append(Pattern)

        TargetPatternTandemScanInfo.append(PatternInfo)

    return TargetPatternTandemScanInfo


def ValidateTandemScanTargetPattern(TargetPattern):
    """Validate target pattern for tandem scan."""

    ValidatedTargetPatternInfo = {}
    ValidatedTargetPatternInfo["Names"] = []
    ValidatedTargetPatternInfo["Patterns"] = []
    ...
    if len(TargetPatternWords) % 2:
        MiscUtil.PrintError(
            'The number of comma delimited paramater names and values, %d, specified using "-t --targetPattern" option must be an even number during, %s, value of "-m, --mode" option.'
            % (len(TargetPatternWords), Mode)
        )
    ...
    for Index in range(0, len(TargetPatternWords), 2):
        OperationName = TargetPatternWords[Index].strip()
        OperationPattern = TargetPatternWords[Index + 1].strip()

        CanonicalOperationName = OperationName.lower()
        if CanonicalOperationName not in CanonicalOperationNamesMap:
            MiscUtil.PrintError(
                'The operation name, %s, specified using "-t, --targetPattern" is not a valid during, %s, value of "-m, --mode" option. Supported operation  names: %s'
                % (OperationName, Mode, " ".join(ValidOperationNames))
            )

        if not OperationPattern:
            MiscUtil.PrintError(
                'Empty target pattern specified using "-t, --targetPattern" must be a valid atomic symbol for operation, %s, during, %s, value of "-m, --mode" option.'
                % (OperationName, Mode)
            )

        Name = CanonicalOperationNamesMap[CanonicalOperationName]
        OperationPatterns = OperationPattern.split()
        if re.match("^(AttachAtoms|ReplaceAtoms)$", OperationName, re.I):
            for Pattern in OperationPatterns:
                if not RDKitUtil.IsValidElementSymbol(Pattern):
                    MiscUtil.PrintError(
                        'The target pattern, "%s", specified using "-t, --targetPattern" must be a valid atomic symbol for operation, %s, during, %s, value of "-m, --mode" option.'
                        % (Pattern, OperationName, Mode)
                    )
        elif re.match("^AttachSMILES$", OperationName, re.I):
            for Pattern in OperationPatterns:
                ValidateSMILESTargetPattern(Pattern, Mode, OperationName)

        ValidatedTargetPatternInfo["Names"].append(Name)
        ValidatedTargetPatternInfo["Patterns"].append(OperationPatterns)

    return ValidatedTargetPatternInfo
...
def ValidateSMILESTargetPattern(TargetPattern, Mode, OperationName=None):
    ...
    if Mol is None:
        if OperationName is None:
            MiscUtil.PrintError(
                'The target pattern, "%s", specified using "-t, --targetPattern" must be a valid SMILES during, %s, value of "-m, --mode" option.'
                % (TargetPattern, Mode)
            )
        else:
            MiscUtil.PrintError(
                'The target pattern, "%s", specified using "-t, --targetPattern" must be a valid SMILES for operation, %s, during, %s, value of "-m, --mode" option.'
                % (TargetPattern, OperationName, Mode)
            )
...
def ValidateOptions():
    ...
    MiscUtil.ValidateOptionTextValue("--clearAtomMapNumbers", Options["--clearAtomMapNumbers"], "yes no")
    ...
    MiscUtil.ValidateOptionTextValue(
        "-m, --mode", Options["--mode"], "ReplaceAtoms AttachAtoms AttachSMILES RxnSMARTS TandemScan"
    )
...
Usage:
    RDKitPerformPositionalAnalogueScan.py [--comboSearchAtoms <number>] [--clearAtomMapNumbers <yes or no>]
                                          [--infileParams <Name,Value,...>] [--mode <ReplaceAtoms, AttachAtoms, AttachSMILES, RxnSMARTS, TandemScan>]
...
Options:
    ...
    --clearAtomMapNumbers <yes or no>  [default: yes]
        Clear any atom map numbers in target SMILES specification for 'AttachSMILES'
    ...
    -t, --targetPattern <ElementSymbol, SMILES, RxnSMARTS...>  [default: auto]
        ...
        atom map number. Otherwise, the first atom in SMILES target specification
...
Examples:
    ...
    for replacing aromatic carbons by nitogens in molecules in a SD file and write
    out a SD file, type:
    ...
    To run the previous example for attaching "C(C)(C)(C)" at a specific attachment
