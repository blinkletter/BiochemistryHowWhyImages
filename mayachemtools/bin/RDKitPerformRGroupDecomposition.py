...
def SetupCoreScaffolds(Mols):
    ...
    if re.match("^(BySMARTS|BySMILES)$", OptionsInfo["CoreScaffold"], re.I):
        ...
        MiscUtil.PrintError(
            'The  value, %s, specified for  "-c, --coreScaffold" option is not supported.'
            % (OptionsInfo["CoreScaffold"])
        )
    ...
    for DataLabel in sorted(RGroups):
        if re.match("^Core", DataLabel, re.I):
            ...
            MiscUtil.PrintWarning(
                "Ignoring unknown R group data label, %s, found during R group decomposition..." % DataLabel
            )
...
def SetupDecompositionParameters():
    ...
    OptionsInfo["DecompositionParams"] = {
        "RGroupCoreAlignment": rgd.RGroupCoreAlignment.MCS,
        "RGroupMatching": rgd.RGroupMatching.GreedyChunks,
        "chunkSize": 5,
        "matchOnlyAtRGroups": False,
        "removeHydrogenOnlyGroups": True,
        "removeHydrogensPostMatch": False,
    }
...
def ProcessSpecifiedDecompositionParameters():
    ...
    if not DecompositionParams:
        MiscUtil.PrintError(
            'No valid parameter name and value pairs specified using "-d, --decompositionParams" option.'
        )

    DecompositionParamsWords = DecompositionParams.split(",")
    if len(DecompositionParamsWords) % 2:
        MiscUtil.PrintError(
            'The number of comma delimited paramater names and values, %d, specified using "-d, --decompositionParams" option must be an even number.'
            % (len(DecompositionParamsWords))
        )
    ...
    for Index in range(0, len(DecompositionParamsWords), 2):
        ...
        if CanonicalName not in CanonicalParamNamesMap:
            MiscUtil.PrintError(
                'The parameter name, %s, specified using "-d, --decompositionParams" option is not a valid name. Supported parameter names: %s'
        ...
        if re.match("^RGroupCoreAlignment$", ParamName, re.I):
            if re.match("^MCS$", Value, re.I):
                ...
                MiscUtil.PrintError(
                    'The parameter value, %s, specified using "-d, --decompositionParams" option  for parameter, %s, is not a valid value. Supported values: MCS None'
                    % (Value, Name)
                )
            ...
            if re.match("^Greedy$", Value, re.I):
                ...
                MiscUtil.PrintError(
                    'The parameter value, %s, specified using "-d, --decompositionParams" option  for parameter, %s, is not a valid value. Supported values: Greedy GreedyChunks Exhaustive'
                    % (Value, Name)
                )
            ...
            if Value <= 0:
                MiscUtil.PrintError(
                    'The parameter value, %s, specified using "-d, --decompositionParams" option  for parameter, %s, is not a valid value. Supported values: > 0'
                    % (Value, Name)
                )
            ParamValue = Value
        else:
            if not re.match("^(Yes|No|True|False)$", Value, re.I):
                MiscUtil.PrintError(
                    'The parameter value, %s, specified using "-d, --decompositionParams" option  for parameter, %s, is not a valid value. Supported values: Yes No True False'
                    % (Value, Name)
                )
            ParamValue = False
...
def ProcessOptions():
    ...
    if not re.match("^none$", Options["--smartsOrSmilesCoreScaffold"], re.I) or len(
...
def ValidateOptions():
    ...
    if re.match("^none$", Options["--smartsOrSmilesCoreScaffold"], re.I) or not len(
        Options["--smartsOrSmilesCoreScaffold"]
    ):
        if re.match("^(BySMARTS|BySMILES)$", Options["--coreScaffold"], re.I):
            MiscUtil.PrintError(
        ...
        if not re.match("^(BySMARTS|BySMILES)$", Options["--coreScaffold"], re.I):
            MiscUtil.PrintError(
                ...
                % (Options["--smartsOrSmilesCoreScaffold"], Options["--coreScaffold"])
            )
