#!/usr/bin/env python
#
# File: Psi4GenerateConstrainedConformers.py
...
def main():
    ...
    GenerateConstrainedConformers()

    MiscUtil.PrintInfo("\n%s: Done...\n" % ScriptName)
    MiscUtil.PrintInfo("Total time: %s" % MiscUtil.GetFormattedElapsedTime(WallClockTime, ProcessorTime))


def GenerateConstrainedConformers():
    ...
    MolCount, ValidMolCount, CoreScaffoldMissingCount, ConfGenFailedCount = ProcessMolecules(RefMol, Mols, Writer)
    ...
    MiscUtil.PrintInfo(
        "Number of molecules failed during conformation generation or minimization: %d" % ConfGenFailedCount
    )
    MiscUtil.PrintInfo(
        "Number of ignored molecules: %d" % (MolCount - ValidMolCount + CoreScaffoldMissingCount + ConfGenFailedCount)
    )


def ProcessMolecules(RefMol, Mols, Writer):
    """Process molecules to generate constrained conformers."""
...
def ProcessMoleculesUsingSingleProcess(RefMol, Mols, Writer):
    """Process molecules to generate constrained conformers using a single process."""
    ...
    (MolCount, ValidMolCount, CoreScaffoldMissingCount, ConfGenFailedCount) = [0] * 4
    ...
    for Mol in Mols:
        ...
        ConfMols, CalcStatus, ConfIDs, ConfEnergyValues, ConfScaffoldEmbedRMSDValues = GenerateMolConformers(
            Psi4Handle, Mol, RefMolCore, MolCount
        )
        ...
        WriteMolConformers(Writer, Mol, MolCount, ConfMols, ConfIDs, ConfEnergyValues, ConfScaffoldEmbedRMSDValues)

    return (MolCount, ValidMolCount, CoreScaffoldMissingCount, ConfGenFailedCount)


def ProcessMoleculesUsingMultipleProcesses(RefMol, Mols, Writer):
    """Process molecules to generate constrained conformers using multiprocessing."""
...
def ProcessMoleculesUsingMultipleProcessesAtMoleculesLevel(RefMol, Mols, Writer):
    """Process molecules to generate constrained conformers using multiprocessing at molecules level."""
    ...
    for Result in Results:
        ...
        ConfMols = [RDKitUtil.MolFromBase64EncodedMolString(EncodedConfMol) for EncodedConfMol in EncodedConfMols]
...
def WorkerProcess(EncodedMolInfo):
    ...
    CalcStatus = False
    ConfIDs = None
    ConfEnergyValues = None
    ConfScaffoldEmbedRMSDValues = None
    ...
    # Generate conformers...
    ConfMols, CalcStatus, ConfIDs, ConfEnergyValues, ConfScaffoldEmbedRMSDValues = GenerateMolConformers(
        OptionsInfo["psi4"], Mol, RefMolCore, MolNum
    )

    EncodedConfMols = None
    if ConfMols is not None:
        EncodedConfMols = [
...
def ProcessMoleculesUsingMultipleProcessesAtConformersLevel(RefMol, Mols, Writer):
    ...
    for Mol in Mols:
        ...
        ConfMols, CalcStatus, ConfIDs, ConfEnergyValues, ConfScaffoldEmbedRMSDValues = (
            ProcessConformersUsingMultipleProcesses(Mol, RefMolCore, MolCount)
        )
...
def ProcessConformersUsingMultipleProcesses(Mol, RefMolCore, MolNum):
    """Generate constrained conformers and minimize them using multiple processes."""
    ...
    MolConfs, MolConfsStatus, MolConfIDs = ConstrainEmbedAndMinimizeMoleculeUsingRDKit(Mol, RefMolCore, MolNum)
    ...
    # Setup a encoded mols data iterable for a worker process...
    WorkerProcessDataIterable = RDKitUtil.GenerateBase64EncodedMolStringsWithIDs(MolConfs, MolConfIDs)
    ...
    MolConfs = []
    CalcEnergyMap = {}
    ScaffoldEmbedRMSDMap = {}
    CalcFailedCount = 0

    for Result in Results:
        ConfID, EncodedMolConf, CalcStatus, Energy, ScaffoldEmbedRMSD = Result
        ...
        MolConfIDs.append(ConfID)
        MolConfs.append(MolConf)
        CalcEnergyMap[ConfID] = Energy
        ScaffoldEmbedRMSDMap[ConfID] = ScaffoldEmbedRMSD

    if CalcFailedCount:
        return (None, False, None, None, None)

    # Filter conformers...
    SelectedMolConfs, SelectedMolConfIDs, SelectedConfEnergies, SelectedConfScaffoldEmbedRMSDValues = (
        FilterMolConformers(Mol, MolNum, MolConfs, MolConfIDs, CalcEnergyMap, ScaffoldEmbedRMSDMap)
    )

    return (SelectedMolConfs, True, SelectedMolConfIDs, SelectedConfEnergies, SelectedConfScaffoldEmbedRMSDValues)
...
def ConformerWorkerProcess(EncodedMolInfo):
    ...
    MolConfID, EncodedMolConf = EncodedMolInfo

    MolConfNum = MolConfID
    CalcStatus = False
    Energy = None
    ScaffoldEmbedRMSD = None

    if EncodedMolConf is None:
        return [MolConfNum, None, CalcStatus, Energy, ScaffoldEmbedRMSD]
    ...
    CalcStatus, Energy = ConstrainAndMinimizeMoleculeUsingPsi4(
        OptionsInfo["psi4"], MolConf, OptionsInfo["RefMolCore"], MolConfNum
    )
    if not CalcStatus:
        if not OptionsInfo["QuietMode"]:
    ...
    if OptionsInfo["ScaffoldRMSDOut"]:
        ScaffoldEmbedRMSD = "%.4f" % float(MolConf.GetProp("EmbedRMS"))
    MolConf.ClearProp("EmbedRMS")

    return [
        MolConfNum,
        RDKitUtil.MolToBase64EncodedMolString(
            MolConf, PropertyPickleFlags=Chem.PropertyPickleOptions.MolProps | Chem.PropertyPickleOptions.PrivateProps
        ),
        CalcStatus,
        Energy,
        ScaffoldEmbedRMSD,
...
def RetrieveReferenceMolecule():
    ...
    if ValidRefMolCount == 0:
        ...
        MiscUtil.PrintWarning(
            "The reference file, %s, contains, %d, valid molecules. Using first molecule as the reference molecule..."
            % (RefFile, ValidRefMolCount)
        )
    ...
    if OptionsInfo["UseScaffoldSMARTS"]:
        ...
        if ScaffoldPatternMol is None:
            MiscUtil.PrintError(
                'Failed to create scaffold pattern molecule. The scaffold SMARTS pattern, %s, specified using "-s, --scaffold" option is not valid.'
                % (OptionsInfo["ScaffoldSMARTS"])
            )

        if not RefMol.HasSubstructMatch(ScaffoldPatternMol):
            MiscUtil.PrintError(
                'The scaffold SMARTS pattern, %s, specified using "-s, --scaffold" option, is missing in the first valid reference molecule.'
                % (OptionsInfo["ScaffoldSMARTS"])
            )
...
def SetupCoreScaffold(RefMol, Mol, MolCount):
    ...
    if OptionsInfo["UseScaffoldMCS"]:
        ...
        MiscUtil.PrintError(
            'The  value, %s, specified for  "-s, --scaffold" option is not supported.' % (OptionsInfo["Scaffold"])
        )
...
def SetupCoreScaffoldBySMARTS(RefMol, Mol, MolCount):
    ...
    if not Mol.HasSubstructMatch(OptionsInfo["ScaffoldPatternMol"]):
        if not OptionsInfo["QuietMode"]:
            MiscUtil.PrintWarning(
                'The scaffold SMARTS pattern, %s, specified using "-s, --scaffold" option is missing in input molecule,  %s.'
                % (OptionsInfo["ScaffoldSMARTS"], RDKitUtil.GetMolName(Mol, MolCount))
            )
        return None
...
def GenerateMolConformers(Psi4Handle, Mol, RefMolCore, MolNum=None):
    """Generate constrained conformers."""
    ...
    for MolConfIndex, MolConf in enumerate(MolConfs):
        MolConfID = MolConfIDs[MolConfIndex]
        ...
        CalcEnergyMap[MolConfID] = Energy

        if OptionsInfo["ScaffoldRMSDOut"]:
            ScaffoldEmbedRMSDMap[MolConfID] = "%.4f" % float(MolConf.GetProp("EmbedRMS"))
        MolConf.ClearProp("EmbedRMS")
...
def FilterMolConformers(Mol, MolNum, MolConfs, ConfIDs, CalcEnergyMap, ScaffoldEmbedRMSDMap):
    """Filter conformers for a molecule."""

    # Sort conformers by energy...
    SortedConfIDs = sorted(ConfIDs, key=lambda ConfID: CalcEnergyMap[ConfID])

    # Setup a map from ConfID to MolConf...
    MolConfsMap = {}
    for MolConfIndex, ConfID in enumerate(ConfIDs):
        MolConfsMap[ConfID] = MolConfs[MolConfIndex]
    ...
    ApplyEnergyRMSDCutoff = True if EnergyRMSDCutoff > 0 else False

    EnergyRMSDCutoffLowest = OptionsInfo["EnergyRMSDCutoffModeLowest"]
    EnergyRMSDCalcModeBest = OptionsInfo["EnergyRMSDCalcModeBest"]
    ...
    for ConfID in SortedConfIDs:
        ...
        if ApplyEnergyRMSDCutoff:
            IgnoreConf = False
            ProbeMolConf = Chem.Mol(MolConfsMap[ConfID])

            if EnergyRMSDCutoffLowest:
                # Compare RMSD with the lowest energy conformation...
                RefMolConf = Chem.Mol(MinEnergyMolConf)
                if EnergyRMSDCalcModeBest:
                    CalcRMSD = AllChem.GetBestRMS(ProbeMolConf, RefMolConf)
                else:
                    CalcRMSD = rdMolAlign.AlignMol(ProbeMolConf, RefMolConf)
                if CalcRMSD < EnergyRMSDCutoff:
                    IgnoreConf = True
            else:
                for SelectedConfID in SelectedConfIDs:
                    RefMolConf = Chem.Mol(MolConfsMap[SelectedConfID])
                    if EnergyRMSDCalcModeBest:
                        CalcRMSD = AllChem.GetBestRMS(ProbeMolConf, RefMolConf)
                    else:
                        CalcRMSD = rdMolAlign.AlignMol(ProbeMolConf, RefMolConf)
    ...
    if not OptionsInfo["QuietMode"]:
        ...
        MiscUtil.PrintInfo(
            "Number of conformations ignored due to energy window cutoff: %d" % (IgnoredByEnergyConfCount)
        )
        if ApplyEnergyRMSDCutoff:
            MiscUtil.PrintInfo(
                "Number of conformations ignored due to energy RMSD cutoff:  %d" % (IgnoredByRMSDConfCount)
            )

    # Setup selected conformer molecules...
    SelectedConfMols = [MolConfsMap[ConfID] for ConfID in SelectedConfIDs]

    #  Setup energies...
    SelectedConfEnergies = None
    if OptionsInfo["EnergyOut"]:
        SelectedConfEnergies = []
        for ConfID in SelectedConfIDs:
            Energy = "%.*f" % (OptionsInfo["Precision"], CalcEnergyMap[ConfID])
            SelectedConfEnergies.append(Energy)

    # Setup scaffold RMSD values...
    SelectedConfScaffoldEmbedRMSDValues = None
    if OptionsInfo["ScaffoldRMSDOut"]:
        SelectedConfScaffoldEmbedRMSDValues = []
        for ConfID in SelectedConfIDs:
            # MolConf = MolConfsMap[ConfID]
            SelectedConfScaffoldEmbedRMSDValues.append(ScaffoldEmbedRMSDMap[ConfID])

    return (SelectedConfMols, SelectedConfIDs, SelectedConfEnergies, SelectedConfScaffoldEmbedRMSDValues)


def ConstrainAndMinimizeMoleculeUsingPsi4(Psi4Handle, Mol, RefMolCore, MolNum, ConfID=-1):
    """Constrain and Minimize molecule using Psi4."""
...
def ConstrainEmbedAndMinimizeMoleculeUsingRDKit(Mol, RefMolCore, MolNum=None):
    ...
    return FilterConstrainedConformationsByRMSD(Mol, MolConfs, ConfIDs, MolNum)


def FilterConstrainedConformationsByRMSD(Mol, MolConfs, MolConfIDs, MolNum=None):
    ...
    if EmbedRMSDCutoff is None or EmbedRMSDCutoff <= 0:
        ...
        return (MolConfs, True, MolConfIDs)
    ...
    for MolConfIndex, MolConf in enumerate(MolConfs):
        if FirstMolConf:
            ...
            SelectedMolConfIDs.append(MolConfIDs[MolConfIndex])
            continue
    ...
    return (SelectedMolConfs, True, SelectedMolConfIDs)
...
def WriteMolConformers(Writer, Mol, MolNum, ConfMols, ConfIDs, ConfEnergyValues=None, ConfScaffoldEmbedRMSDValues=None):
    """Write molecule coformers."""

    if ConfMols is None:
        return

    for Index, ConfMol in enumerate(ConfMols):
        ConfMolName = RDKitUtil.GetMolName(Mol, MolNum)
        SetConfMolName(ConfMol, ConfMolName, ConfIDs[Index])

        if ConfScaffoldEmbedRMSDValues is not None:
            ConfMol.SetProp("CoreScaffoldEmbedRMSD", ConfScaffoldEmbedRMSDValues[Index])

        if ConfEnergyValues is not None:
            ConfMol.SetProp(OptionsInfo["EnergyDataFieldLabel"], ConfEnergyValues[Index])

        Writer.write(ConfMol)
...
def ProcessSpecifiedMCSParameters():
    ...
    if len(MCSParamsWords) % 2:
        MiscUtil.PrintError(
            'The number of comma delimited paramater names and values, %d, specified using "-m, --mcsParams" option must be an even number.'
            % (len(MCSParamsWords))
        )
    ...
    for Index in range(0, len(MCSParamsWords), 2):
        ...
        if CanonicalName not in CanonicalParamNamesMap:
            MiscUtil.PrintError(
                'The parameter name, %s, specified using "-m, --mcsParams" option is not a valid name. Supported parameter names: %s'
                % (Name, " ".join(ValidParamNames))
            )
        ...
        if re.match("^Threshold$", ParamName, re.I):
            ...
            if Value <= 0.0 or Value > 1.0:
                MiscUtil.PrintError(
                    'The parameter value, %s, specified using "-m, --mcsParams" option  for parameter, %s, is not a valid value. Supported values: > 0 and <= 1.0'
                    % (Value, Name)
                )
            ParamValue = Value
            ...
            if Value <= 0:
                MiscUtil.PrintError(
                    'The parameter value, %s, specified using "-m, --mcsParams" option  for parameter, %s, is not a valid value. Supported values: > 0'
                    % (Value, Name)
                )
            ParamValue = Value
            ...
            if Value < 1:
                MiscUtil.PrintError(
                    'The parameter value, %s, specified using "-m, --mcsParams" option  for parameter, %s, is not a valid value. Supported values: >= 1'
                    % (Value, Name)
                )
            ParamValue = Value
            ...
            if re.match("^CompareAny$", Value, re.I):
                ...
                MiscUtil.PrintError(
                    'The parameter value, %s, specified using "-m, --mcsParams" option  for parameter, %s, is not a valid value. Supported values: CompareAny CompareElements CompareIsotopes'
                    % (Value, Name)
                )
            ...
            if re.match("^CompareAny$", Value, re.I):
                ...
                MiscUtil.PrintError(
                    'The parameter value, %s, specified using "-m, --mcsParams" option  for parameter, %s, is not a valid value. Supported values: CompareAny CompareOrder CompareOrderExact'
                    % (Value, Name)
                )
        elif re.match("^SeedSMARTS$", ParamName, re.I):
            if not len(Value):
                MiscUtil.PrintError(
                    'The parameter value specified using "-m, --mcsParams" option  for parameter, %s, is empty. '
                    % (Name)
                )
            ParamValue = Value
        else:
            if not re.match("^(Yes|No|True|False)$", Value, re.I):
                MiscUtil.PrintError(
                    'The parameter value, %s, specified using "-m, --mcsParams" option  for parameter, %s, is not a valid value. Supported values: Yes No True False'
                    % (Value, Name)
                )
            ParamValue = False
...
def ProcessOptions():
    ...
    OptionsInfo["EnergyRMSDCalcMode"] = Options["--energyRMSDCalcMode"]
    OptionsInfo["EnergyRMSDCalcModeBest"] = (
        True if re.match("^BestRMSD$", Options["--energyRMSDCalcMode"], re.I) else False
    )

    OptionsInfo["EnergyRMSDCutoff"] = float(Options["--energyRMSDCutoff"])

    OptionsInfo["EnergyRMSDCutoffMode"] = Options["--energyRMSDCutoffMode"]
    OptionsInfo["EnergyRMSDCutoffModeLowest"] = (
        True if re.match("^Lowest$", Options["--energyRMSDCutoffMode"], re.I) else False
    )

    # Process energy window...
    EnergyWindow = Options["--energyWindow"]
    if re.match("^auto$", EnergyWindow, re.I):
        # Set default energy window based on units...
        EnergyUnits = Options["--energyUnits"]
        if re.match(r"^kcal\/mol$", EnergyUnits, re.I):
            EnergyWindow = 20
        elif re.match(r"^kJ\/mol$", EnergyUnits, re.I):
            EnergyWindow = 83.68
            ...
            EnergyWindow = 0.8673
        elif re.match("^Hartrees$", EnergyUnits, re.I):
            EnergyWindow = 0.03188
        else:
            MiscUtil.PrintError(
                'Failed to set default value for "--energyWindow". The value, %s, specified for option "--energyUnits" is not valid. '
                % EnergyUnits
            )
    else:
        EnergyWindow = float(EnergyWindow)
    OptionsInfo["EnergyWindow"] = EnergyWindow
...
def ValidateOptions():
    ...
    MiscUtil.ValidateOptionTextValue(" --energyRMSDCalcMode", Options["--energyRMSDCalcMode"], "RMSD BestRMSD")

    MiscUtil.ValidateOptionFloatValue("--energyRMSDCutoff", Options["--energyRMSDCutoff"], {">=": 0})
    MiscUtil.ValidateOptionTextValue(" --energyRMSDCutoffMode", Options["--energyRMSDCutoffMode"], "All Lowest")

    if not re.match("^auto$", Options["--energyWindow"], re.I):
        MiscUtil.ValidateOptionFloatValue("--energyWindow", Options["--energyWindow"], {">": 0})
...
Usage:
    Psi4GenerateConstrainedConformers.py [--basisSet <text>] [--confParams <Name,Value,...>] [--energyOut <yes or no>]
                                         [--energyDataFieldLabel <text>] [--energyUnits <text>] [--energyRMSDCalcMode <RMSD or BestRMSD>]
                                         [--energyRMSDCutoff <number>] [--energyRMSDCutoffMode <All or Lowest>] [--energyWindow <number>]
                                         [--infileParams <Name,Value,...>] [--maxIters <number>] [--methodName <text>] [--mcsParams <Name,Value,...>]
...
Options:
    ...
    --energyRMSDCalcMode <RMSD or BestRMSD>  [default: RMSD]
        Methodology for calculating RMSD values during the application of RMSD
        cutoff for retaining conformations after the final energy minimization. Possible
        values: RMSD or BestRMSD. This option is ignore during 'None' value of
    ...
    --energyRMSDCutoff <number>  [default: 0.5]
        RMSD cutoff for retaining conformations after the final energy minimization.
        By default, only those conformations which are different from other low
        energy conformation by the specified RMSD cutoff and are with in the 
    ...
    --energyRMSDCutoffMode <All or Lowest>  [default: All]
        ...
        minimization. Possible values: All or Lowest. The RMSD values are compared
        ...
        low energy conformations by the specified RMSD cutoff and are with in the
...
Examples:
    ...
    in a SMILES file against a reference 3D molecule in a SD file using a common core
    scaffold between pairs of input and reference molecules identified using MCS,
    generating up to 50 conformations using ETKDGv2 methodology followed by initial
    MMFF forcefield minimization and  final energy minimization using B3LYP/6-31G**
    and B3LYP/6-31+G** for non-sulfur and sulfur containing molecules, applying
    energy RMSD cutoff of 0.5 and energy window value value of 20 kcal/mol, and
    write out a SD file:
    ...
    and a quantum chemistry method and write out a SD file type:
       ...
       %  Psi4GenerateConstrainedConformers.py  --confParams "confMethod,
          ETKDGv2, forceField,MMFF, forceFieldMMFFVariant,MMFF94s, maxConfs,20,
