...
def CalculateInteractionEnergy():
...
def ProcessMoleculesUsingSingleProcess(Mols, Writer):
    ...
    for Mol in Mols:
        ...
        CalcStatus, Energy, EnergyDetails = CalculateMoleculeInteractionEnergy(Psi4Handle, Psi4Mol, Mol, MolCount)

        if not CalcStatus:
            if not OptionsInfo["QuietMode"]:
                MiscUtil.PrintWarning(
                    "Failed to calculate interaction energy for molecule %s" % RDKitUtil.GetMolName(Mol, MolCount)
                )

            EnergyFailedCount += 1
            continue

        WriteMolecule(Writer, Mol, Energy, EnergyDetails)
...
def ProcessMoleculesUsingMultipleProcesses(Mols, Writer):
    ...
    for Result in Results:
        MolCount += 1
        MolIndex, EncodedMol, CalcStatus, Energy, EnergyDetails = Result
...
def WorkerProcess(EncodedMolInfo):
    ...
    if EncodedMol is None:
        return [MolIndex, None, CalcStatus, Energy, EnergyDetails]
    ...
    CalcStatus, Energy, EnergyDetails = CalculateMoleculeInteractionEnergy(OptionsInfo["psi4"], Psi4Mol, Mol, MolCount)
...
def CalculateMoleculeInteractionEnergy(Psi4Handle, Psi4Mol, Mol, MolNum=None):
    """Calculate interaction energy for a molecule containing two components."""
    ...
    MethodAndBasisSet = Psi4Util.JoinMethodNameAndBasisSet(MethodName, BasisSet)

    Status, Energy, EnergyDetails = [False, None, None]
    try:
        if OptionsInfo["BSSEType"] is None:
            Energy = Psi4Handle.energy(MethodAndBasisSet, molecule=Psi4Mol, return_wfn=False)
        else:
            Energy = Psi4Handle.energy(
                MethodAndBasisSet, bsse_type=OptionsInfo["BSSEType"], molecule=Psi4Mol, return_wfn=False
            )
        Status = True
    except Exception as ErrMsg:
        if not OptionsInfo["QuietMode"]:
            MiscUtil.PrintWarning("Failed to calculate interaction energy:\n%s\n" % ErrMsg)

    # Retrieve SAPT energy details...
    if Status and OptionsInfo["SAPTMode"]:
        EnergyTypeMap = OptionsInfo["SAPTEnergyIDsToTypes"]
        EnergyDetails = {}
        for EnergyType in EnergyTypeMap:
            try:
                EnergyDetails[EnergyType] = Psi4Handle.variable(EnergyTypeMap[EnergyType])
            except Exception as ErrMsg:
                EnergyDetails[EnergyType] = None
                if not OptionsInfo["QuietMode"]:
                    MiscUtil.PrintWarning(
                        "Failed to retrieve value for interaction energy components %s:\n%s\n" % (EnergyType, ErrMsg)
                    )

    # Convert energy units...
    if Status and OptionsInfo["ApplyEnergyConversionFactor"]:
        if Energy is not None:
            Energy = Energy * OptionsInfo["EnergyConversionFactor"]
        if EnergyDetails is not None:
            for EnergyType in EnergyDetails:
                if EnergyDetails[EnergyType] is not None:
                    EnergyDetails[EnergyType] = EnergyDetails[EnergyType] * OptionsInfo["EnergyConversionFactor"]
...
def SetupPsi4Mol(Psi4Handle, Mol, MolCount=None):
    ...
    MolGeometry = RDKitUtil.GetPsi4XYZFormatString(Mol, NoCom=True, NoReorient=True, CheckFragments=True)
...
def CheckAndValidateMolecule(Mol, MolCount=None):
    ...
    NumFragments = RDKitUtil.GetNumFragments(Mol)
    if OptionsInfo["SAPTMode"] or OptionsInfo["SNSMP2Mode"]:
        if NumFragments != 2:
            if not OptionsInfo["QuietMode"]:
                MiscUtil.PrintWarning(
                    "Ignoring molecule containing invalid number of fragments: %s. It must contain exaclty 2 fragments for calculation of interaction energy using SAPT or SNS-MP2 methodology.\n"
                    % NumFragments
                )
            return False
    else:
        if NumFragments == 1:
            if not OptionsInfo["QuietMode"]:
                MiscUtil.PrintWarning(
                    "Ignoring molecule containing invalid number of fragments: %s. It must contain more than 1 fragment for calculation of interaction energy using counterpoise correctio methodology.\n"
                    % NumFragments
                )
            return False
...
def SetupMethodNameAndBasisSet(Mol):
    ...
    return (OptionsInfo["MethodName"], OptionsInfo["BasisSet"])
...
def WriteMolecule(Writer, Mol, Energy, EnergyDetails):
    """Write molecule."""

    # Setup energy...
    EnergyValue = "%.*f" % (OptionsInfo["Precision"], Energy) if Energy is not None else ""
    Mol.SetProp(OptionsInfo["EnergyDataFieldLabel"], EnergyValue)

    # Setup SAPT energy details...
    if EnergyDetails is not None and OptionsInfo["SAPTMode"]:
        for EnergyID in OptionsInfo["SAPTEnergyIDs"]:
            Energy = EnergyDetails[EnergyID] if EnergyID in EnergyDetails else None
            EnergyValue = "%.*f" % (OptionsInfo["Precision"], Energy) if Energy is not None else ""

            Mol.SetProp(OptionsInfo["SAPTEnergyIDsToLabels"][EnergyID], EnergyValue)

    Writer.write(Mol)


def ProcessMethodNameAndBasisSetOptions():
    """Process method name and basis set options."""

    MethodName = Options["--methodName"]
    MethodNameAuto = True if re.match("^auto$", MethodName, re.I) else False
    if MethodNameAuto:
        MethodName = "SAPT0"
    SAPTMode = True if re.match("^SAPT", MethodName, re.I) else False

    SNSMP2Mode = True if re.match("^SNS-MP2$", MethodName, re.I) else False

    BasisSet = Options["--basisSet"]
    BasisSetAuto = True if re.match("^auto$", BasisSet, re.I) else False
    if BasisSetAuto:
        if SAPTMode:
            BasisSet = "jun-cc-pVDZ"
        elif SNSMP2Mode:
            BasisSet = ""
        else:
            MiscUtil.PrintError(
                'A basis set must be explicitly specified using "-b, --basisSet" option for, "%s", value of "-m, --methodName". '
                % (MethodName)
            )

    if SNSMP2Mode and not MiscUtil.IsEmpty(BasisSet):
        MiscUtil.PrintError(
            'The basis set value, "%s", specified using "-b, --basisSet" option is not valid. It must be an empty string SNS-MP2 calculations.'
            % (Options["--basisSet"])
        )

    OptionsInfo["MethodName"] = MethodName
    OptionsInfo["MethodNameAuto"] = MethodNameAuto
    OptionsInfo["SAPTMode"] = SAPTMode
    OptionsInfo["SNSMP2Mode"] = SNSMP2Mode

    OptionsInfo["BasisSet"] = BasisSet
    OptionsInfo["BasisSetAuto"] = BasisSetAuto


def ProcessBSSETypeOption():
    """Process basis set superposition energy correction option."""

    BSSEType = (
        None
        if re.match("^None$", Options["--bsseType"], re.I) or MiscUtil.IsEmpty(Options["--bsseType"])
        else Options["--bsseType"]
    )
    BSSETypeAuto = True if re.match("^auto$", Options["--bsseType"], re.I) else False

    if BSSETypeAuto:
        if OptionsInfo["SAPTMode"] or OptionsInfo["SNSMP2Mode"]:
            BSSEType = None
        elif re.match("^HF3c$", OptionsInfo["MethodName"], re.I):
            BSSEType = "noCP"
        else:
            BSSEType = "CP"
    else:
        if OptionsInfo["SAPTMode"] or OptionsInfo["SNSMP2Mode"]:
            if BSSEType is not None:
                MiscUtil.PrintError(
                    'The BSSE type value, "%s", specified using "--bsseType" option is not valid for SAPT or SNS-MP2 calculations. It must be set to None.'
                    % (Options["--bsseType"])
                )
        else:
            if BSSEType is None:
                MiscUtil.PrintWarning(
                    'The BSSE type value, "%s", specified using "--bsseType" option is not valid for "%s" calculations. Possible values: CP, noCP or VMFC'
                    % (Options["--bsseType"], OptionsInfo["MethodName"])
                )

    OptionsInfo["BSSEType"] = BSSEType
    OptionsInfo["BSSETypeAuto"] = BSSETypeAuto


def ProcessSAPTEnergyDataFieldLabelsOption():
    """Process data field labels for SAPT interaction energy components."""

    ParamsOptionName = "--energySAPTDataFieldLabels"
    ParamsOptionValue = Options["--energySAPTDataFieldLabels"]

    ParamsIDs = ["ElectrostaticEnergy", "ExchangeEnergy", "InductionEnergy", "DispersionEnergy"]
    ParamsIDsToTypes = {
        "ElectrostaticEnergy": "SAPT ELST ENERGY",
        "ExchangeEnergy": "SAPT EXCH ENERGY",
        "InductionEnergy": "SAPT IND ENERGY",
        "DispersionEnergy": "SAPT DISP ENERGY",
    }

    UnitsLabel = "(%s)" % Options["--energyUnits"]
    ParamsIDsToLabels = {
        "ElectrostaticEnergy": "Psi4_SAPT_Electrostatic_Energy %s" % UnitsLabel,
        "ExchangeEnergy": "Psi4_SAPT_Exchange_Energy %s" % UnitsLabel,
        "InductionEnergy": "Psi4_SAPT_Induction_Energy %s" % UnitsLabel,
        "DispersionEnergy": "Psi4_SAPT_Dispersion_Energy %s" % UnitsLabel,
    }

    if re.match("^auto$", ParamsOptionValue, re.I):
        OptionsInfo["SAPTEnergyIDs"] = ParamsIDs
        OptionsInfo["SAPTEnergyIDsToTypes"] = ParamsIDsToTypes
        OptionsInfo["SAPTEnergyIDsToLabels"] = ParamsIDsToLabels
        return
...
def ProcessOptions():
    ...
    ProcessMethodNameAndBasisSetOptions()

    # BSSEType option...
    ProcessBSSETypeOption()
    ...
    # Interaction energy units...
    OptionsInfo["EnergyUnits"] = Options["--energyUnits"]

    # Total interaction energy label...
    EnergyDataFieldLabel = Options["--energyDataFieldLabel"]
    if re.match("^auto$", EnergyDataFieldLabel, re.I):
        EnergyLabel = "Psi4_Interaction_Energy"
        if OptionsInfo["SAPTMode"]:
            EnergyLabel = "Psi4_SAPT_Interaction_Energy"
        elif OptionsInfo["SNSMP2Mode"]:
            EnergyLabel = "Psi4_SNS-MP2_Interaction_Energy"
        EnergyDataFieldLabel = "%s %s" % (EnergyLabel, Options["--energyUnits"])

    OptionsInfo["EnergyDataFieldLabel"] = EnergyDataFieldLabel

    ProcessSAPTEnergyDataFieldLabelsOption()
...
def ValidateOptions():
    ...
    MiscUtil.ValidateOptionTextValue("--bsseType", Options["--bsseType"], "CP noCP VMFC None auto")
...
Usage:
    Psi4CalculateInteractionEnergy.py [--basisSet <text>] [--bsseType <CP, noCP, VMFC, or None>]
                                      [--energyDataFieldLabel <text>] [--energySAPTDataFieldLabels <Type,Label,...,...>]
                                      [--energyUnits <text>] [--infileParams <Name,Value,...>] [--methodName <text>]
                                      [--mp <yes or no>] [--mpParams <Name, Value,...>] [ --outfileParams <Name,Value,...> ]
                                      [--overwrite] [--precision <number> ] [--psi4OptionsParams <Name,Value,...>]
                                      [--psi4RunParams <Name,Value,...>] [--quiet <yes or no>]
...
Description:
    ...
    components for Symmetry Adapted Perturbation Theory  (SAPT) [ Ref 154-155 ]
    and Spin-Network-Scaled MP2 (SNS-MP2) [ Ref 156] calculations and more than
    one fragment for all other calculations. An arbitrary number of fragments may be
    present in a molecule for Basis Set Superposition Energy (BSSE) correction
    calculations.

    The interaction energy is calculated at SAPT0 / jun-cc-pVDZ level of theory by
    default. The SAPT0 calculations are relatively fast but less accurate. You may
    want to consider calculating interaction energy at WB97X-D3 / aug-cc-pVTZ,
    B3LYP-D3 / aug-cc-pVTZ, or higher level of theory [ Ref 157 ] to improve the
    accuracy of your results. The  WB97X-D3 and B3LYP-D3 calculations rely on
    the presence of DFTD3 and gCP Psi4 plugins in your environment.
...
Options:
    -b, --basisSet <text>  [default: auto]
        ...
        SAPT calculations; None for SNS-MP2 calculations to use its default basis
    ...
    --bsseType <CP, noCP, VMFC, or None>  [default: auto]
        Type of Basis Set Superposition Energy (BSSE) correction to apply during the
    ...
    --energyDataFieldLabel <text>  [default: auto]
        ...
        Psi4_SAPT_Interaction_Energy (<Units>)  for SAPT calculation; 
        Psi4_SNS-MP2_Interaction_Energy (<Units>) for SNS-MP2 calculation;
    ...
    --energySAPTDataFieldLabels <Type,Label,...,...>  [default: auto]
        ...
        for writing individual components of total SAPT interaction energy.
        
        The supported SAPT energy types along with their default data field label
    ...
    -m, --methodName <text>  [default: auto]
        Method to use for interaction energy calculation. Default: SAPT0. The
    ...
    -r, --reference <text>  [default: auto]
        Reference wave function to use for interaction energy calculation. Default: RHF
        or UHF. The default values are Restricted Hartree-Fock (RHF) for closed-shell
        molecules with all electrons paired and Unrestricted Hartree-Fock (UHF) for
       open-shell molecules with unpaired electrons.
        ...
        The 'SpinMultiplicity' molecule property may contain multiples values
        corresponding to the number of fragments in a molecule. This property must
        not be set for automatic determination of the reference wave functionn.
    -w, --workingdir <dir>
        Location of working directory which defaults to the current directory.

Examples:
    To calculate interaction energy using SAPT0/aug-cc-pVDZ for molecules in a
    SD file, use RHF and UHF for closed-shell and open-shell molecules, and write
    a new SD file, type:

        % Psi4CalculateInteractionEnergy.py -i Psi4SampleDimers3D.sdf
          -o Psi4SampleDimers3DOut.sdf

    To run the first example for freezing core electrons and setting SCF type to DF
    and write out a new SD file, type:
    ...
    along with explicit Psi4 run time paramaters, for molecules in a SD containing
    ...
    default Psi4 run time paramaters for molecules in a SD containing 3D structures
    ...
    paramaters, for molecules in a SD containing 3D structures
    and write a new SD file, type:
    ...
    for molecules in a SD containing 3D structures and write a new SD file, type:

        % Psi4CalculateInteractionEnergy.py -m HF3c -b "" --bsseType noCP
          -i Psi4SampleDimers3D.sdf -o Psi4SampleDimers3DOut.sdf
      
    To calculate interaction energy at  CCSD(T)/aug-cc-pVDZ level of theory using
    default Psi4 run time paramaters for molecules in a SD containing 3D structures
    and write a new SD file, type:
