...
def PerformFiltering():
    ...
    PAINSPatternMols = SetupPAINSPatternMols()
...
def ProcessMoleculesUsingSingleProcess(Mols, PAINSPatternMols, Writer, WriterFiltered):
    ...
    for Mol in Mols:
        ...
        MolMatched, AlertsInfo = DoesMoleculeContainsPAINSPattern(Mol, PAINSPatternMols)
...
def ProcessMoleculesUsingMultipleProcesses(Mols, PAINSPatternMols, Writer, WriterFiltered):
    ...
    OptionsInfo["EncodedPAINSPatternMols"] = [
        RDKitUtil.MolToBase64EncodedMolString(PatternMol) for PatternMol in PAINSPatternMols
    ]
...
def InitializeWorkerProcess(*EncodedArgs):
    ...
    OptionsInfo["PAINSPatternMols"] = [
        RDKitUtil.MolFromBase64EncodedMolString(EncodedMol) for EncodedMol in OptionsInfo["EncodedPAINSPatternMols"]
    ]
...
def WorkerProcess(EncodedMolInfo):
    ...
    MolMatched, AlertsInfo = DoesMoleculeContainsPAINSPattern(Mol, OptionsInfo["PAINSPatternMols"])
...
def SetupPAINSPatternMols():
    """Set up PAINS pattern mols for substructure search corresponding to PAINS mode."""

    PatternMols = []
    for FilterType in OptionsInfo["SpecifiedFilterTypes"]:
        for Index, Pattern in enumerate(OptionsInfo["PAINSFiltersMap"]["SMARTS"][FilterType]):
            ID = OptionsInfo["PAINSFiltersMap"]["IDs"][FilterType][Index]
...
def ProcessPAINSMode():
    ...
    for FilterType in PAINSMode.split(","):
        ...
        if CanonicalFilterType not in CanonicalFilterTypesMap:
            MiscUtil.PrintError(
                'The PAINS mode, %s, specified using "-p, --PAINSMode" is not valid. Supported PAINS modes: %s'
                % (FilterType, ", ".join(OptionsInfo["PAINSFiltersMap"]["FilterTypes"]))
            )
...
def ProcessPAINSMatch():
    """Process specified PAINS match."""

    PAINSMatch = Options["--painsMatch"]

    MatchFirstAlert, MatchAllAlerts = [False] * 2
    if re.match("^First$", PAINSMatch, re.I):
        MatchFirstAlert = True
    elif re.match("^All$", PAINSMatch, re.I):
        MatchAllAlerts = True
    else:
        MiscUtil.PrintError(
            'The value %s, specified using "--painsMatch" option is not valid. Supported values: First or All'
            % (PAINSMatch)
        )

    OptionsInfo["PAINSMatch"] = PAINSMatch
    ...
    OptionsInfo["AlertsCountLabel"] = "PAINSAlertsCount"
    OptionsInfo["AlertsLabel"] = "FirstPAINSAlert" if MatchFirstAlert else "PAINSAlerts"
...
def RetrievePAINSFiltersInfo():
    ...
    for LineWords in FilterLinesWords:
        ...
        if FilterType not in PAINSFiltersMap["FilterTypes"]:
            PAINSFiltersMap["FilterTypes"].append(FilterType)
            PAINSFiltersMap["IDs"][FilterType] = []
            PAINSFiltersMap["SMARTS"][FilterType] = []

        PAINSFiltersMap["IDs"][FilterType].append(ID)
    ...
    for FilterType in PAINSFiltersMap["FilterTypes"]:
        MiscUtil.PrintInfo(
            "Filter family type: %s; Number of filters: %d" % (FilterType, len(PAINSFiltersMap["IDs"][FilterType]))
        )
...
def ValidateOptions():
    ...
    MiscUtil.ValidateOptionTextValue("--painsMatch", Options["--painsMatch"], "First All")
...
Usage:
    RDKitFilterPAINS.py  [--infileParams <Name,Value,...>] [--mode <filter or count>]
                         ...
                         [--painsMode <All or A, B, C>] [--painsMatch <First or All>] [--negate <yes or no>]
...
Options:
    ...
    -p, --painsMode <All or A, B, or C>  [default: All]
        ...
        filtering molecules. 
    --painsMatch <First or All>  [default: First]
        Stop after matching  only first PAINS pattern or match all patterns for
        filtering molecules.
        
        The 'PAINSAlertCount' and 'PAINSAlerts' data fields are added to
        SD file containing filtered molecules for 'All' value of '-painsMatch'. In
...
Examples:
    ...
    molecules along with the alerts information for filtered molecules matching
    first pattern, type: 

        % RDKitFilterPAINS.py  --outfileFiltered yes --outfileParams
    ...
    to PAINS SMARTS patterns and write out comma delmited SMILES files containing
    these and filtered molecules along with the alerts information for filtered
    molecules matching all patterns, type: 

        % RDKitFilterPAINS.py --painsMatch All --outfileFiltered yes
          --outfileParams "SMILESDelimiter,comma" -i Sample.sdf
          -o SampleOut.smi

    To count the number of molecules not containing any substructure corresponding
    to PAINS SMARTS patterns and write out comma delmited SD files containing
    these and filtered molecules along with the alerts information for filtered
    molecules matching all patterns, type: 
