...
def AlignMolecules():
    ...
    if not re.match("^(OneToOne|FirstToAll)$", OptionsInfo["Mode"], re.I):
        MiscUtil.PrintError(
            "Alignment couldn't be performed: Specified mode, %s, is not supported" % OptionsInfo["Mode"]
        )
    ...
    MiscUtil.PrintInfo(
        ...
        % ((RefMolCount - ValidRefMolCount), (ProbeMolCount - ValidProbeMolCount + AlignmentFailedCount))
    )
...
def PerformAlignmentAndWrieOutput(RefMol, ProbeMol, RefMolName, ProbeMolName, Writer):
    ...
    try:
        if OptionsInfo["UseRMSD"]:
            rdMolAlign.AlignMol(ProbeMol, RefMol, maxIters=OptionsInfo["MaxIters"])
        elif OptionsInfo["UseBestRMSD"]:
            AllChem.GetBestRMS(ProbeMol, RefMol)
...
def ValidateOptions():
    ...
    MiscUtil.ValidateOptionsDistinctFileNames(
        "-r, --reffile", Options["--reffile"], "-o, --outfile", Options["--outfile"]
    )
    MiscUtil.ValidateOptionsDistinctFileNames(
        "-p, --probefile", Options["--probefile"], "-o, --outfile", Options["--outfile"]
    )
...
Examples:
    To perform shape alignment using Open3A methodology between pairs of molecules in
    ...
    To perform alignment using RMSD methodology between pairs of molecules in
    reference and probe input 3D SD files and write out a SD file containing aligned
    molecules, type:
