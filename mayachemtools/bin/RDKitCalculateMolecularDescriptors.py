...
def ProcessMoleculesUsingSingleProcess(Mols, Writer):
    ...
    MiscUtil.PrintInfo(
        ...
        % (DescriptorsCount, ("descroptors" if DescriptorsCount > 1 else "descriptor"))
    )
...
def CalculateDescriptorValues(MolIndex, Mol):
    ...
    return [
        FormatCalculatedValue(CalculateDescriptorValue(MolIndex, Mol, Name), OptionsInfo["Precision"])
        for Name in OptionsInfo["SpecifiedDescriptorNames"]
    ]
...
def FormatCalculatedValue(Value, Precision):
    ...
    if (type(Value) is float) or (type(Value) is numpy.float64):
        ...
        FormattedValue = re.sub(r"\[|\]|,", "", FormattedValue)
    else:
        FormattedValue = "%s" % Value
...
def ProcessSpecifiedDescriptorNames():
    ...
    for Name in DescriptorNames.split(","):
        ...
        if CanonicalName in CanonicalNameMap:
            ...
            MiscUtil.PrintError(
                'The descriptor name, %s, specified using "-d, --descriptorNames" option is not a valid name.' % (Name)
            )
        if SMILESInfile:
            if CanonicalName in Canonical3DNameMap:
                MiscUtil.PrintError(
                    'The 3D descriptor name, %s, specified using "-d, --descriptorNames" option is not a valid for SMILES input file.'
                    % (Name)
                )
...
def RetrieveMolecularDescriptorsInfo(PrintInfo=True):
    ...
    Autocorr2DExclude = OptionsInfo["Autocorr2DExclude"] if "Autocorr2DExclude" in OptionsInfo else False
    ...
    if not Autocorr2DExclude:
        try:
            ...
            if Name not in DescriptorNamesMap["ComputeFunction"]:
                DescriptorNamesMap[Type]["Names"].append(Name)
                DescriptorNamesMap["ComputeFunction"][Name] = ComputeFunction
...
def ValidateOptions():
    ...
    if re.match("^Specify$", Options["--mode"], re.I):
        if re.match("^none$", Options["--descriptorNames"], re.I):
            MiscUtil.PrintError(
                'The name(s) of molecular descriptors must be specified using "-d, --descriptorNames" option during "Specify" value of "-m, --mode" option.'
            )
    ...
    if re.match("^3D|All$", Options["--mode"], re.I):
        if MiscUtil.CheckFileExt(Options["--infile"], "smi"):
            MiscUtil.PrintError(
                'The input SMILES file, %s, is not valid for  "3D or All" value of "-m, --mode" option.'
            )
...
Description:
    ...
    2D descriptors: Autocorr2D, AvgIpc, BCUT2D_*, BalabanJ, BertzCT, Chi0, Chi1, Chi0n - Chi4n,
    Chi0v - Chi4v, EState_VSA1 - EState_VSA11, ExactMolWt, FpDensityMorgan1, FpDensityMorgan2,
    FpDensityMorgan3, FractionCSP3, HallKierAlpha, HeavyAtomCount, HeavyAtomMolWt, Ipc,
    Kappa1 - Kappa3, LabuteASA, MaxAbsEStateIndex, MaxAbsPartialCharge, MaxEStateIndex,
