...
def PerformEnumeration():
    ...
    if RxnByNameMode:
        RxnSMARTSPattern = OptionsInfo["RxnNameSMARTS"]
    else:
        RxnSMARTSPattern = OptionsInfo["SpecifiedSMARTS"]

    # Set up a reaction and match number of reactants in rxn SMARTS against number of
    # reactant files...
    MiscUtil.PrintInfo("\nValidating reaction SMARTS...")
    try:
        Rxn = AllChem.ReactionFromSmarts(RxnSMARTSPattern)
    except Exception as ErrMsg:
        MiscUtil.PrintError("Failed to validate reaction SMARTS %s\n%s\n" % (RxnSMARTSPattern, ErrMsg))
    ...
    if ReactantFilesCount != RxnReactantsCount:
        MiscUtil.PrintError(
            "The number of specified reactant files, %d, must match number of reactants, %d, in reaction SMARTS"
            % (ReactantFilesCount, RxnReactantsCount)
        )
...
def ProcessSpecifiedReactionName():
    ...
    OptionsInfo["RxnNameSMARTS"] = None
    ...
    if CanonicalRxnName in CanonicalRxnNameMap:
        ...
        OptionsInfo["RxnNameSMARTS"] = RxnNamesMap["SMARTSPattern"][Name]
    else:
        MiscUtil.PrintError(
            'The rxn name name, %s, specified using "-r, --rxnName" option is not a valid name.'
            % (OptionsInfo["RxnName"])
        )
...
def ProcessListReactionNamesOption():
    ...
    ProcessReactionNamesFileOption()
    ProcessColumnOptions()

    RetrieveReactionNamesInfo()
    ListAndValidateReactionNamesInfo()


def RetrieveReactionNamesInfo():
    """Retrieve reaction names information."""

    RxnNamesFile = OptionsInfo["RxnNamesFile"]

    MiscUtil.PrintInfo("\nRetrieving reaction names and SMARTS patterns from file %s" % (RxnNamesFile))

    if not os.path.exists(RxnNamesFile):
        MiscUtil.PrintError("The reaction names file, %s, doesn't exist.\n" % (RxnNamesFile))

    IgnoreHeaderLine = True
    RxnLinesWords = MiscUtil.GetTextLinesWords(
        RxnNamesFile, OptionsInfo["RxnNamesFileDelimiter"], OptionsInfo["RxnNamesFileQuote"], IgnoreHeaderLine
    )

    RxnNamesMap["Names"] = []
    RxnNamesMap["SMARTSPattern"] = {}

    RxnNameColindex = OptionsInfo["RxnNameColnum"] - 1
    RxnSMARTSColindex = OptionsInfo["RxnSMARTSColnum"] - 1

    for LineWords in RxnLinesWords:
        Name = LineWords[RxnNameColindex]
        SMARTSPattern = LineWords[RxnSMARTSColindex]

        if Name in RxnNamesMap["SMARTSPattern"]:
            MiscUtil.PrintWarning("Ignoring duplicate reaction name: %s..." % Name)
        else:
            RxnNamesMap["Names"].append(Name)
            RxnNamesMap["SMARTSPattern"][Name] = SMARTSPattern

    if not len(RxnNamesMap["Names"]):
        MiscUtil.PrintError("Failed to retrieve any reaction names and SMARTS patterns...")

    MiscUtil.PrintInfo(
        "Total number of reactions present in reaction names and SMARTS file: %d" % (len(RxnNamesMap["Names"]))
    )


def ListAndValidateReactionNamesInfo():
    """List and validate reaction names information."""

    ListReactionNamesInfo()
    ValidateReactionNamesInfo()
...
def ListReactionNamesInfo():
    ...
    for Name in sorted(RxnNamesMap["Names"]):
        RxnCount += 1
        SMARTSPattern = RxnNamesMap["SMARTSPattern"][Name]
        MiscUtil.PrintInfo("%s\t%s" % (Name, SMARTSPattern))

    MiscUtil.PrintInfo("\nTotal number of reactions: %s" % RxnCount)


def ValidateReactionNamesInfo():
    """Validate reaction names information."""
    ...
    for Name in sorted(RxnNamesMap["Names"]):
        ...
        SMARTSPattern = RxnNamesMap["SMARTSPattern"][Name]
        try:
            AllChem.ReactionFromSmarts(SMARTSPattern)
            ValidRxnCount += 1
        except Exception as ErrMsg:
            MiscUtil.PrintInfo(
    ...
    InvalidRxnCount = RxnCount - ValidRxnCount
    MiscUtil.PrintInfo(
        "\nTotal number of reactions: %s\nNumber of valid reactions: %s\nNumber of invalid reactions: %s"
        % (RxnCount, ValidRxnCount, InvalidRxnCount)
    )

    MiscUtil.PrintInfo("")


def ProcessReactionNamesFileOption():
    """Process reaction names file option."""

    RxnNamesFile = None
    if not re.match("^auto$", Options["--rxnNamesFile"], re.I):
        MiscUtil.ValidateOptionFilePath("--rxnNamesFile", Options["--rxnNamesFile"])
        RxnNamesFile = Options["--rxnNamesFile"]

    if RxnNamesFile is None:
        MayaChemToolsDataDir = MiscUtil.GetMayaChemToolsLibDataPath()
        RxnNamesFile = os.path.join(MayaChemToolsDataDir, "ReactionNamesAndSMARTS.csv")

    OptionsInfo["RxnNamesFile"] = RxnNamesFile
    OptionsInfo["RxnNamesFileDelimiter"] = ","
    OptionsInfo["RxnNamesFileQuote"] = '"'


def ProcessColumnOptions():
    """Process column options."""

    ProcessColumnModeOption()
    RetrieveColumnNames()

    ProcessReactionNameColOption()
    ProcessReactionSMARTSColOption()
...
def RetrieveColumnNames():
    ...
    Colnames = RxnLinesWords[0]

    if len(Colnames) == 0:
        MiscUtil.PrintError(
            "The first line in reaction names, %s, is empty. It must contain column names.\n"
...
def ProcessReactionNameColOption():
    """Process reaction name column option."""

    RxnNameCol = Options["--colRxnName"]
    if re.match("^auto$", RxnNameCol, re.I):
        Colname = "RxnName"
        if Colname not in OptionsInfo["ColnameToColnumMap"]:
            MiscUtil.PrintError(
                'The reaction name column name, %s, doen\'t exist in reaction names file. You must specify a valid reaction name column name or number using "--colRxnName" option.\n'
                % Colname
            )

        Colnum = OptionsInfo["ColnameToColnumMap"][Colname]
        RxnNameColSpec = Colnum if OptionsInfo["ColnumMode"] else Colname
    else:
        RxnNameColSpec = RxnNameCol

    RxnNameColname, RxnNameColnum = ProcessColumnSpecification("--colRxnName", RxnNameColSpec)

    OptionsInfo["RxnNameCol"] = RxnNameCol
    OptionsInfo["RxnNameColname"] = RxnNameColname
    OptionsInfo["RxnNameColnum"] = RxnNameColnum


def ProcessReactionSMARTSColOption():
    """Process reaction SMARTS column option."""

    RxnSMARTSCol = Options["--colRxnSMARTS"]
    if re.match("^auto$", RxnSMARTSCol, re.I):
        Colname = "RxnSMARTS"
        if Colname not in OptionsInfo["ColnameToColnumMap"]:
            MiscUtil.PrintError(
                'The reaction SMARTS column name, %s, doen\'t exist in reaction names file. You must specify a valid reaction name column name or number using "--colRxnSMARTS" option.\n'
                % Colname
            )

        Colnum = OptionsInfo["ColnameToColnumMap"][Colname]
        RxnSMARTSColSpec = Colnum if OptionsInfo["ColnumMode"] else Colname
    else:
        RxnSMARTSColSpec = RxnSMARTSCol

    RxnSMARTSColname, RxnSMARTSColnum = ProcessColumnSpecification("--colRxnSMARTS", RxnSMARTSColSpec)

    OptionsInfo["RxnSMARTSCol"] = RxnSMARTSCol
    OptionsInfo["RxnSMARTSColname"] = RxnSMARTSColname
    OptionsInfo["RxnSMARTSColnum"] = RxnSMARTSColnum
...
def ProcessColumnSpecification(OptionName, Colspec):
    ...
    if OptionsInfo["ColnumMode"]:
        ...
        if Colnum not in OptionsInfo["ColnumToColnameMap"]:
            MiscUtil.PrintError(
                'The column number, %s, specified using "%s" option doesn\'t exist in reaction names file. You must specify a valid column number. Valid values: >= 1 and <= %s\n'
...
def ProcessOptions():
    ...
    OptionsInfo["SpecifiedSMARTS"] = Options["--smartsRxn"]
    if not OptionsInfo["RxnByNameMode"]:
        if not Options["--smartsRxn"]:
            MiscUtil.PrintError(
                'No rxn SMARTS pattern specified using "-r, --rxnName" option during "RxnByName" value of "-m, --mode" option'
            )
...
def ValidateOptions():
    ...
    MiscUtil.ValidateOptionTextValue("-m, --mode", Options["--mode"], "RxnByName RxnBySMARTS")
...
Usage:
    RDKitEnumerateCompoundLibrary.py  [--colmode <collabel or colnum>] [--colRxnName <text or number>]
                                      [--colRxnSMARTS <text or number>] [--compute2DCoords <yes or no>] [--infileParams <Name,Value,...>]
                                      [--mode <RxnByName or RxnBySMARTS>] [--outfileParams <Name,Value,...>] [--overwrite]
                                      [--prodMolNames <UseReactants or Sequential>] [--rxnName <text>]
                                      [--rxnNamesFile <FileName or auto>] [--smartsRxn <text>] [--sanitize <yes or no>]
                                      [-w <dir>] -i  <ReactantFile1,...> -o <outfile>
    RDKitEnumerateCompoundLibrary.py [--colmode <collabel or colnum>] [--colRxnName <text or number>] [--colRxnSMARTS <text or number>]
                                      [--rxnNamesFile <FileName or auto>] -l | --list
...
Description:
    ...
    using a reaction name or SMARTS pattern and reactant input files.

    The SMARTS patterns for supported reactions names [ Ref 134 ] are retrieved from file,
    ReactionNamesAndSMARTS.csv, available in MayaChemTools data directory. The current
...
Options:
    -c, --colmode <collabel or colnum>  [default: collabel]
        Use column number or name for the specification of columns in a CSV
        file containing reaction names along with reaction SMARTS. You may
        specify a reaction names file using '--rxnNamesFile' option.
    --colRxnName <text or number>  [default: auto]
        Column name or number corresponding to reaction names. The default value
        is automatically set based on the value of '-c, --colmode': 'RxnName'  for
        'collabel'; Reaction name column number for 'colnum'.
    --colRxnSMARTS <text or number>  [default: auto]
        Column name or number corresponding to reaction SMARTS strings. The default
        value is automatically set based on the value of '-c, --colmode': 'RxnSMARTS'
        for 'collabel'; Reacton SMARTS column number for 'colnum'.
    ...
    -i, --infiles <ReactantFile1, ReactantFile2...>
        ...
        using reaction SMARTS. The number of reactant files must match number of
        reaction components in reaction SMARTS. All reactant input files must have
        the same format.
    ...
    -l, --list
        List available reaction names along with corresponding SMARTS patterns without
        performing any enumeration. In addition, reaction SMARTS patterns are validated.
    -m, --mode <RxnByName or RxnBySMARTS>  [default: RxnByName]
        Indicate whether a reaction is specified by a reaction name or a SMARTS pattern.
        Possible values: RxnByName or RxnBySMARTS.
    -o, --outfile <outfile>
        Output file name.
    ...
    --rxnNamesFile <FileName or auto>  [default: auto]
        Specify a file name containing data for names of reactions and SMARTS patterns or
        use default file, ReactionNamesAndSMARTS.csv, available in MayaChemTools data
        directory.
        
        Default reactions SMARTS file format: RxnName,RxnSMARTS.
        
        The local file format is assumed to be same as the default file format. You may
        explicitly specify column names or numbers for reaction name and reaction
        SMARTS using '--colRxnName' and '--colRxnSMARTS' options.
    -s, --smartsRxn <text>
        SMARTS pattern of a reaction to use for enumerating a compound library. This
        option is only used during 'RxnBySMARTS' value of '-m, --mode' option.
...
Examples:
    ...
    To run the previous command using a local reaction names file with explicit
    specification of column names containing reaction names and SMARTS, and write
     out a SMILES file type:

        % RDKitEnumerateCompoundLibrary.py -r Schotten_Baumann_amide
          --rxnNamesFile ReactionNamesAndSMARTS.csv
          --colmode collabel --colRxnName RxnName --colRxnSMARTS RxnSMARTS
          -i 'SampleAcids.smi,SampleAmines.smi' -o SampleOutCmpdLibrary.smi

    To perform a combinatorial enumeration of a virtual compound library corresponding
    to an amide reaction specified using a SMARTS pattern and write out a SD file containing
    sanitized molecules, computed 2D coordinates, and generation of molecule names from
    reactant names, type:

        % RDKitEnumerateCompoundLibrary.py -m RxnBySMARTS
    ...
    of molecule names, type:
        ...
        % RDKitEnumerateCompoundLibrary.py -m RxnBySMARTS -c no --sanitize no
