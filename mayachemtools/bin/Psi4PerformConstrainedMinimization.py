...
def PerformConstrainedMinimization():
    ...
    MolCount, ValidMolCount, CoreScaffoldMissingCount, MinimizationFailedCount, WriteFailedCount = ProcessMolecules(
        RefMol, Mols, Writer
    )
    ...
    MiscUtil.PrintInfo(
        ...
        % (MolCount - ValidMolCount + CoreScaffoldMissingCount + MinimizationFailedCount + WriteFailedCount)
    )
...
def WorkerProcess(EncodedMolInfo):
    ...
    Mol, CalcStatus, Energy, ScaffoldEmbedRMSD = ConstrainAndMinimizeMolecule(
...
def ProcessConformersUsingMultipleProcesses(Mol, RefMolCore, MolNum):
    ...
    MinEnergy = (
        "%.*f" % (OptionsInfo["Precision"], CalcEnergyMap[MinEnergyConfNum]) if OptionsInfo["EnergyOut"] else None
    )
...
Examples:
    ...
    using ETKDGv2 methodology followed by initial MMFF forcefield minimization and final
