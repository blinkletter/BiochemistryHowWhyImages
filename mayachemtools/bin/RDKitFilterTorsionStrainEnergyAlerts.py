...
try:
    ...
    import MiscUtil
    import RDKitUtil
    from TorsionAlerts.TorsionStrainEnergyAlerts import TorsionStrainEnergyAlerts
except ImportError as ErrMsg:
...
def PerformFiltering():
    ...
    MolCount, ValidMolCount, RemainingMolCount, WriteFailedCount = ProcessMolecules(Mols)
...
def ProcessMoleculesUsingSingleProcess(Mols):
    ...
    # Instantiate torsion strain energy alerts class...
    TorsionStrainEnergyAlertsHandle = InstantiateTorsionStrainEnergyAlertsClass()
    ...
    for Mol in Mols:
        ...
        RotBondsAlertsStatus, RotBondsAlertsInfo = (
            TorsionStrainEnergyAlertsHandle.IdentifyTorsionLibraryAlertsForRotatableBonds(Mol)
        )
...
def ProcessMoleculesUsingMultipleProcesses(Mols):
    ...
    # Instantiate torsion strain energy alerts class to list torsion library information...
...
def InitializeWorkerProcess(*EncodedArgs):
    ...
    # Instantiate torsion strain energy alerts class...
    OptionsInfo["TorsionStrainEnergyAlertsHandle"] = InstantiateTorsionStrainEnergyAlertsClass(Quiet=True)
...
def WorkerProcess(EncodedMolInfo):
    ...
    # Identify torsion library alerts for rotatable bonds..
    TorsionStrainEnergyAlertsHandle = OptionsInfo["TorsionStrainEnergyAlertsHandle"]
    RotBondsAlertsStatus, RotBondsAlertsInfo = (
        TorsionStrainEnergyAlertsHandle.IdentifyTorsionLibraryAlertsForRotatableBonds(Mol)
    )
...
def TrackTorsionAlertsSummaryInfo(TorsionAlertsSummaryInfo, RotBondsAlertsInfo):
    ...
    for ID in RotBondsAlertsInfo["IDs"]:
        ...
        MaxSingleEnergyAlertType = SetupMaxSingleEnergyAlertStatusValue(
            RotBondsAlertsInfo["MaxSingleEnergyAlertStatus"][ID]
        )
        ...
        if TorsionRuleNodeID not in TorsionAlertsSummaryInfo["RuleSMARTS"]:
            ...
            TorsionAlertsSummaryInfo["HierarchyClassName"][TorsionRuleNodeID] = RotBondsAlertsInfo[
                "HierarchyClassName"
            ][ID]
            TorsionAlertsSummaryInfo["HierarchySubClassName"][TorsionRuleNodeID] = RotBondsAlertsInfo[
                "HierarchySubClassName"
            ][ID]
...
def WriteTorsionAlertsSummaryInfo(Writer, TorsionAlertsSummaryInfo):
    ...
    Values = [
        ...
        "HierarchySubClass",
        "EnergyMethod",
        "MaxSingleEnergyTorsionAlertTypes",
        "MaxSingleEnergyTorsionAlertCount",
    ...
    for ID in SortedRuleIDs:
        ...
        for MaxSingleEnergyAlertType in sorted(TorsionAlertsSummaryInfo["MaxSingleEnergyAlertTypes"][ID]):
            ...
            MaxSingleEnergyAlertTypesCount.append(
                "%s" % TorsionAlertsSummaryInfo["MaxSingleEnergyAlertTypes"][ID][MaxSingleEnergyAlertType]
            )
            MaxSingleEnergyAlertTypesMolCount.append(
                "%s" % TorsionAlertsSummaryInfo["MaxSingleEnergyAlertTypesMolCount"][ID][MaxSingleEnergyAlertType]
            )
        ...
        Values = [
            ...
            HierarchySubClassName,
            EnergyMethod,
            "%s" % MiscUtil.JoinWords(MaxSingleEnergyAlertTypes, ","),
            "%s" % (MiscUtil.JoinWords(MaxSingleEnergyAlertTypesCount, ",")),
            "%s" % (MiscUtil.JoinWords(MaxSingleEnergyAlertTypesMolCount, ",")),
        ]
        Writer.write("%s\n" % MiscUtil.JoinWords(Values, ",", QuoteValues))
...
def WriteTorsionAlertsFilteredByRulesInfo(TorsionAlertsSummaryInfo):
    ...
    for Mol in FilteredMols:
        ...
        for TorsionRuleID in TorsionRuleIDs:
            ...
            WriteMoleculeFilteredByRuleID(
                ByRuleOutfilesWriters[TorsionRuleID], Mol, TorsionRuleID, TorsionAlertsSummaryInfo, TorsionAlertsInfo
            )
...
def RetrieveTorsionAlertsInfo(Mol, TorsionAlertsSummaryInfo):
    ...
    if len(TorsionAlertsWords) % TorsionAlertsSetSize:
        MiscUtil.PrintError(
            "The number of space delimited values, %s, for TorsionAlerts data field in filtered SD file must be a multiple of %s."
            % (len(TorsionAlertsWords), TorsionAlertsSetSize)
        )
    ...
    for Index in range(0, len(TorsionAlertsWords), TorsionAlertsSetSize):
        ...
        (
            ...
            AngleNotObserved,
            MaxSingleEnergyAlertType,
        ) = TorsionAlertsWords[Index : Index + TorsionAlertsSetSize]
        ...
        if TorsionRule not in TorsionAlertsSummaryInfo["SMARTSToRuleIDs"]:
            MiscUtil.PrintWarning(
                "The SMARTS pattern, %s, for TorsionAlerts data field in filtered SD file doesn't map to any torsion rule..."
                % TorsionRule
            )
            continue
...
def SetupMolPropertiesForAlertsInformation(Mol, RotBondsAlertsInfo):
    ...
    if RotBondsAlertsInfo is not None:
        ...
        Mol.SetProp(
            SDFieldIDsToLabels["TotalEnergyLabel"],
            "%s" % SetupEnergyValueForSDField(RotBondsAlertsInfo["TotalEnergy"], Precision),
        )
        Mol.SetProp(
            SDFieldIDsToLabels["TotalEnergyLowerBoundCILabel"],
            "%s" % SetupEnergyValueForSDField(RotBondsAlertsInfo["TotalEnergyLowerBound"], Precision),
        )
        Mol.SetProp(
            SDFieldIDsToLabels["TotalEnergyUpperBoundCILabel"],
            "%s" % SetupEnergyValueForSDField(RotBondsAlertsInfo["TotalEnergyUpperBound"], Precision),
        )

        # Setup max single energy and alert count...
        if OptionsInfo["MaxSingleEnergyMode"] or OptionsInfo["TotalOrMaxSingleEnergyMode"]:
            Mol.SetProp(
                SDFieldIDsToLabels["MaxSingleEnergyLabel"],
                "%s" % SetupEnergyValueForSDField(RotBondsAlertsInfo["MaxSingleEnergy"], Precision),
            )
            Mol.SetProp(
                SDFieldIDsToLabels["MaxSingleEnergyAlertsCountLabel"],
                "%s"
                % (
                    "NA"
                    if RotBondsAlertsInfo["MaxSingleEnergyAlertsCount"] is None
                    else RotBondsAlertsInfo["MaxSingleEnergyAlertsCount"]
                ),
            )

        Mol.SetProp(
            SDFieldIDsToLabels["AnglesNotObservedCountLabel"],
            "%s"
            % (
                "NA"
                if RotBondsAlertsInfo["AnglesNotObservedCount"] is None
                else RotBondsAlertsInfo["AnglesNotObservedCount"]
            ),
        )
    ...
    if RotBondsAlertsInfo is not None:
        for ID in RotBondsAlertsInfo["IDs"]:
            ...
            TorsionAtomIndices = SetupTorsionAtomIndicesValues(
                RotBondsAlertsInfo["TorsionAtomIndices"][ID], ValuesDelim
            )
            ...
            RotBondValues.append(
                "%s" % SetupMaxSingleEnergyAlertStatusValue(RotBondsAlertsInfo["MaxSingleEnergyAlertStatus"][ID])
            )
    ...
    if len(AlertsInfoValues):
        Mol.SetProp(
            OptionsInfo["SDFieldIDsToLabels"]["TorsionAlertsLabel"],
            "%s" % ("%s" % AlertsInfoValuesDelim.join(AlertsInfoValues)),
        )
...
def WriteMoleculeFilteredByRuleID(Writer, Mol, TorsionRuleID, TorsionAlertsSummaryInfo, TorsionAlertsInfo):
    ...
    SetupMolPropertiesForFilteredByRuleIDAlertsInformation(
        Mol, TorsionRuleID, TorsionAlertsSummaryInfo, TorsionAlertsInfo
    )

    Writer.write(Mol)


def SetupMolPropertiesForFilteredByRuleIDAlertsInformation(
    Mol, TorsionRuleID, TorsionAlertsSummaryInfo, TorsionAlertsInfo
):
    ...
    Mol.SetProp(
        OptionsInfo["SDFieldIDsToLabels"]["TorsionRuleLabel"],
        "%s" % ("%s" % InterSetValuesDelim.join(AlertRuleInfoValues)),
    )

    # Setup max single energy alert count for torsion rule...
    Mol.SetProp(
        OptionsInfo["SDFieldIDsToLabels"]["TorsionRuleMaxSingleEnergyAlertsCountLabel"],
        "%s" % TorsionAlertsInfo["MaxSingleEnergyAlertsCount"][TorsionRuleID],
    )

    # Setup angle not observed count for torsion rule...
    Mol.SetProp(
        OptionsInfo["SDFieldIDsToLabels"]["TorsionRuleAnglesNotObservedCountLabel"],
        "%s" % TorsionAlertsInfo["AnglesNotObservedCount"][TorsionRuleID],
    )
    ...
    Mol.SetProp(
        OptionsInfo["SDFieldIDsToLabels"]["TorsionRuleAlertsLabel"], "%s" % (InterSetValuesDelim.join(AlertsInfoValues))
    )
...
def SkipRotatableBondAlertInfo(ID, RotBondsAlertsInfo):
    ...
    if OptionsInfo["TotalEnergyMode"]:
        ...
        if (
            RotBondsAlertsInfo["MaxSingleEnergyAlertStatus"][ID] is None
            or not RotBondsAlertsInfo["MaxSingleEnergyAlertStatus"][ID]
        ):
            Status = True
...
def SetupByRuleOutfilesWriters(RuleIDs):
    ...
    MsgTxt = (
        "all"
        if OptionsInfo["OutfilesFilteredByRulesAllMode"]
        else "top %s" % OptionsInfo["OutfilesFilteredByRulesMaxCount"]
    )
    MiscUtil.PrintInfo(
        "\nGenerating output files %s*.%s for %s torsion rules triggering alerts..."
        % (OutfilesRoot, OutfilesExt, MsgTxt)
    )
...
def InstantiateTorsionStrainEnergyAlertsClass(Quiet=False):
    """Initialize torsion strain energy alerts class."""

    try:
        TorsionStrainEnergyAlertsHandle = TorsionStrainEnergyAlerts(
            AlertsMode=OptionsInfo["AlertsMode"],
            TotalEnergyCutoff=OptionsInfo["TotalEnergyCutoff"],
            MaxSingleEnergyCutoff=OptionsInfo["MaxSingleEnergyCutoff"],
            RotBondsSMARTSMode=OptionsInfo["RotBondsSMARTSMode"],
            RotBondsSMARTSPattern=OptionsInfo["RotBondsSMARTSPattern"],
            TorsionLibraryFilePath=OptionsInfo["TorsionEnergyLibraryFile"],
            AlertTorsionsNotObserved=OptionsInfo["FilterTorsionsNotObserved"],
        )
    except Exception as ErrMsg:
        MiscUtil.PrintError("Failed to instantiate TorsionStrainEnergyAlerts:\n%s\n" % (ErrMsg))

    if not Quiet:
        MiscUtil.PrintInfo(
            "\nRetrieving data from torsion strain energy library file %s..."
            % TorsionStrainEnergyAlertsHandle.GetTorsionLibraryFilePath()
        )
        TorsionStrainEnergyAlertsHandle.ListTorsionLibraryInfo()

    return TorsionStrainEnergyAlertsHandle
...
def ProcessRotatableBondsSMARTSMode():
    ...
    RotBondsSMARTSPatternSpecified = OptionsInfo["RotBondsSMARTSPatternSpecified"]

    if re.match("^(NonStrict|SemiStrict|Strict)$", RotBondsMode, re.I):
        RotBondsSMARTSPattern = None
    elif re.match("Specify", RotBondsMode, re.I):
        RotBondsSMARTSPatternSpecified = RotBondsSMARTSPatternSpecified.strip()
        if not len(RotBondsSMARTSPatternSpecified):
            MiscUtil.PrintError(
                'Empty value specified for SMILES/SMARTS pattern in  "--rotBondsSMARTSPattern" option, %s.'
                % RotBondsMode
            )

        RotBondsPatternMol = Chem.MolFromSmarts(RotBondsSMARTSPatternSpecified)
        if RotBondsPatternMol is None:
            MiscUtil.PrintError(
                'Failed to create rotatable bonds pattern molecule. The rotatable bonds SMARTS pattern, "%s", specified using "--rotBondsSMARTSPattern" option is not valid.'
                % (RotBondsSMARTSPatternSpecified)
            )
    else:
        MiscUtil.PrintError(
            'The value, %s, specified for option "-r, --rotBondsSMARTSMode" is not valid. ' % RotBondsMode
        )
...
def ProcessSDFieldLabelsOption():
    ...
    ParamsIDsToLabels = {
        ...
        "MaxSingleEnergyAlertsCountLabel": "MaxSingleEnergyAlertsCount",
        "AnglesNotObservedCountLabel": "AnglesNotObservedCount",
        "TorsionAlertsLabel": "TorsionAlerts(RotBondIndices TorsionIndices TorsionAngle Energy EnergyLowerBoundCI EnergyUpperBoundCI HierarchyClass HierarchySubClass TorsionRule EnergyMethod AngleNotObserved MaxSingleEnergyAlert)",
        "TorsionRuleLabel": "TorsionRule (HierarchyClass HierarchySubClass TorsionRule EnergyMethod)",
        "TorsionRuleMaxSingleEnergyAlertsCountLabel": "TorsionRuleMaxSingleEnergyAlertsCount",
        "TorsionRuleAnglesNotObservedCountLabel": "TorsionRuleAnglesNotObservedCount",
        "TorsionRuleAlertsLabel": "TorsionRuleAlerts (RotBondIndices TorsionIndices TorsionAngle Energy EnergyLowerBoundCI EnergyUpperBoundCI AngleNotObserved MaxSingleEnergyAlert)",
    }
...
def ProcessOptions():
    ...
    OptionsInfo["FilterTorsionsNotObserved"] = (
        True if re.match("^yes$", Options["--filterTorsionsNotObserved"], re.I) else False
    )
    ...
    if re.match("^yes$", Options["--outfileSummary"], re.I):
        if not re.match("^MaxSingleEnergy$", Options["--alertsMode"], re.I):
            MiscUtil.PrintError(
                'The value "%s" specified for "--outfileSummary" option is not valid. The specified value is only allowed during "MaxSingleEnergy" value of "-a, --alertsMode" option.'
                % (Options["--outfileSummary"])
            )
    ...
    if re.match("^yes$", Options["--outfilesFilteredByRules"], re.I):
        if not re.match("^MaxSingleEnergy$", Options["--alertsMode"], re.I):
            MiscUtil.PrintError(
                'The value "%s" specified for "--outfilesFilteredByRules" option is not valid. The specified value is only allowed during "MaxSingleEnergy" value of "-a, --alertsMode" option.'
                % (Options["--outfileSummary"])
            )

    OptionsInfo["TrackAlertsSummaryInfo"] = (
        True if (OptionsInfo["OutfileSummaryMode"] or OptionsInfo["OutfilesFilteredByRulesMode"]) else False
    )
    ...
    OptionsInfo["OutfilesFilteredByRulesAllMode"] = (
        True if re.match("^All$", Options["--outfilesFilteredByRulesMaxCount"], re.I) else False
    )
    ...
    if re.match("^yes$", Options["--outfilesFilteredByRules"], re.I):
        if not re.match("^yes$", Options["--outfileAlerts"], re.I):
            MiscUtil.PrintError(
                'The value "%s" specified for "--outfilesFilteredByRules" option is not valid. The specified value is only allowed during "yes" value of "--outfileAlerts" option.'
                % (Options["--outfilesFilteredByRules"])
            )
...
def ProcessListTorsionLibraryOption():
    ...
    # Instantiate TorsionStrainEnergyAlerts using defaults...
    TorsionStrainEnergyAlertsHandle = TorsionStrainEnergyAlerts(
        TorsionLibraryFilePath=OptionsInfo["TorsionEnergyLibraryFile"]
    )
    MiscUtil.PrintInfo(
...
def ValidateOptions():
    ...
    MiscUtil.ValidateOptionTextValue(
        "-a, --alertsMode", Options["--alertsMode"], "TotalEnergy MaxSingleEnergy TotalOrMaxSingleEnergy"
    )

    MiscUtil.ValidateOptionFloatValue(
        "--alertsMaxSingleEnergyCutoff", Options["--alertsMaxSingleEnergyCutoff"], {">": 0.0}
    )
    ...
    if not re.match("^All$", Options["--outfilesFilteredByRulesMaxCount"], re.I):
        MiscUtil.ValidateOptionIntegerValue(
            "--outfilesFilteredByRulesMaxCount", Options["--outfilesFilteredByRulesMaxCount"], {">": 0}
        )
    ...
    MiscUtil.ValidateOptionTextValue(
        "-r, --rotBondsSMARTSMode", Options["--rotBondsSMARTSMode"], "NonStrict SemiStrict Strict Specify"
    )
    if re.match("^Specify$", Options["--rotBondsSMARTSMode"], re.I):
        if not Options["--rotBondsSMARTSPattern"]:
            MiscUtil.PrintError(
                'The SMARTS pattern must be specified using "--rotBondsSMARTSPattern" during "Specify" value of "-r, --rotBondsSMARTS" option'
            )
    else:
        if Options["--rotBondsSMARTSPattern"]:
            MiscUtil.PrintError(
                'The SMARTS pattern must not be specified using "--rotBondsSMARTSPattern" during "%s" value of "-r, --rotBondsSMARTS" option'
                % (Options["--rotBondsSMARTSMode"])
            )
...
Description:
    ...
    is available under MAYACHEMTOOLS/lib/python/TorsionAlerts directory.
...
Options:
    ...
    -r, --rotBondsSMARTSMode <NonStrict, SemiStrict,...>  [default: SemiStrict]
        ...
        derived from 'Strict' SMARTS pattern for its usage in this script.
    ...
    -t, --torsionEnergyLibraryFile <FileName or auto>  [default: auto]
        ...
        MAYACHEMTOOLS/lib/Python/TorsionAlerts directory.
        
        The format of data in local XML file must match format of the data in Torsion
        Library [ Ref 153 ] file available in MAYACHEMTOOLS directory.
    -w, --workingdir <dir>
