#!/usr/bin/env python
#
# File: RDKitGenerateConstrainedConformers.py
...
def GenerateMolConformers(Mol, RefMolCore, MolNum=None):
    """Generate constrained conformers."""
    ...
    for ConfID in ConfIDs:
        try:
            ...
            if not OptionsInfo["QuietMode"]:
                ...
                MiscUtil.PrintWarning(
                    "Constrained embedding coupldn't  be performed for molecule %s:\n%s\n"
        ...
        if OptionsInfo["ScaffoldRMSDOut"]:
            ScaffoldEmbedRMSDMap[ConfID] = "%.4f" % float(MolConf.GetProp("EmbedRMS"))
        MolConf.ClearProp("EmbedRMS")

        if OptionsInfo["RemoveHydrogens"]:
            MolConf = Chem.RemoveHs(MolConf)
    ...
    if ApplyEnergyRMSDCutoff:
        ...
        for ConfID in SortedConfIDs:
            if FirstConf:
                FirstConf = False
                CalcRMSDMap[ConfID] = 0.0
                continue

            # Make a copy for probe molecule. It gets updated during the calculation...
            ProbeMolConf = Chem.Mol(MolConfsMap[ConfID])
            RMSD = rdMolAlign.AlignMol(ProbeMolConf, MinEnergyMolConf)
    ...
    # Setup selected conformer energy values...
    SelectedConfEnergyValues = None
    if OptionsInfo["EnergyOut"]:
        SelectedConfEnergyValues = ["%.2f" % CalcEnergyMap[ConfID] for ConfID in SelectedConfIDs]

    # Setup selected conformer scaffold RMSD values...
    ...
    return (SelectedConfMols, True, SelectedConfIDs, SelectedConfEnergyValues, SelectedConfScaffoldEmbedRMSDValues)
...
def WriteMolConformers(Writer, Mol, MolNum, ConfMols, ConfIDs, ConfEnergyValues=None, ConfScaffoldEmbedRMSDValues=None):
    ...
    for Index, ConfMol in enumerate(ConfMols):
        ...
        if ConfEnergyValues is not None:
            ConfMol.SetProp(OptionsInfo["EnergyLabel"], ConfEnergyValues[Index])
...
Usage:
    RDKitGenerateConstrainedConformers.py [--addHydrogens <yes or no>] [--conformerGenerator <SDG, KDG, ETDG, ETKDG, ETKDGv2>]
                                          ...
                                          [--energyWindow <number> ] [--infileParams <Name,Value,...>] [--maxConfs <number>]
                                          [--mcsParams <Name,Value,...>] [--mp <yes or no>] [--mpParams <Name,Value,...>]
...
Options:
    ...
    --energyRMSDCutoff <number>  [default: 0.5]
        RMSD cutoff for retaining conformations after embedding and energy minimization.
        Possible values: A number or None
        
        The default is to keep only those conformations which are different from the
        lowest energy conformation by the specified RMSD cutoff. The None value may
...
Examples:
    ...
    generating up to 50 conformations using ETKDG methodology followed by MMFF
    forcefield minimization within energy window of 20 kcal/mol and RMSD of greater
    than 0.5 from the lowest energy conformation, and write out a SD file:
    ...
    To generate constrained conformers for molecules in a SD file against a reference
    3D molecule in a SD file using a common core scaffold between pairs of input and
    reference molecules identified using MCS, generating up to 10 conformations
    using SDG methodology followed by UFF forcefield minimization, conformations 
    with in an energy window of 10 kcal/mol and RMSD of greater that 1, and write out
    a SD file containing minimum energy structure along with energy and embed RMS
