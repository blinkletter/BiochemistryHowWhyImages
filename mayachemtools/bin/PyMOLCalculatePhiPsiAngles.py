...
def CalculatePhiPsiAngles():
    ...
    for ChainID in OptionsInfo["SpecifiedChainsAndLigandsInfo"]["ChainIDs"]:
        ...
        if OptionsInfo["MultipleOutFiles"]:
            ...
            for Index, Category in enumerate(OptionsInfo["Categories"]):
                ...
                WritePhiPsiInfo(
                    OptionsInfo["CategoriesOutFHs"][Category],
                    MolName,
                    ChainID,
                    PhiPsiInfoList[Index],
                    OutDelim,
                    Precision,
                )
    ...
    MiscUtil.PrintInfo(
        ...
        % (OptionsInfo["Outfile"], OptionsInfo["OutfileResCount"])
    )
    if OptionsInfo["MultipleOutFiles"]:
        MiscUtil.PrintInfo("")
        for Index, Category in enumerate(OptionsInfo["Categories"]):
            MiscUtil.PrintInfo(
                "Number of phi and psi angles in output file %s: %d"
                % (OptionsInfo["CategoriesOutfiles"][Category], OptionsInfo["CategoriesResCount"][Category])
            )
    ...
    for Category in OptionsInfo["Categories"]:
        ...
        if os.path.exists(CategoryOutfile):
            if not OptionsInfo["Overwrite"]:
                MiscUtil.PrintError(
                    'The category output file, %s, already exist. Use option "--ov" or "--overwrite" and try again.\n'
                    % (CategoryOutfile)
                )
