#!/usr/bin/env python
#
# File: Psi4VisualizeDualDescriptors.py
...
def main():
    ...
    GenerateAndVisualizeDualDescriptors()

    MiscUtil.PrintInfo("\n%s: Done...\n" % ScriptName)
    MiscUtil.PrintInfo("Total time: %s" % MiscUtil.GetFormattedElapsedTime(WallClockTime, ProcessorTime))


def GenerateAndVisualizeDualDescriptors():
    """Generate and visualize dual descriptors."""

    if OptionsInfo["GenerateCubeFiles"]:
        GenerateDualDescriptors()

    if OptionsInfo["VisualizeCubeFiles"]:
        VisualizeDualDescriptors()


def GenerateDualDescriptors():
    """Generate cube files for dual descriptors."""
...
def ProcessMolecules(Mols):
    """Process and generate cube files for molecules."""
...
def ProcessMoleculesUsingSingleProcess(Mols):
    """Process and generate dual descriptors cube files for molecules using a single process."""
        ...
        if not CalcStatus:
            if not OptionsInfo["QuietMode"]:
                MiscUtil.PrintWarning(
                    "Failed to generate cube files for molecule %s" % RDKitUtil.GetMolName(Mol, MolCount)
                )
...
def ProcessMoleculesUsingMultipleProcesses(Mols):
    """Process and generate dual descriptors cube files for molecules using  multiple processes."""
...
def GenerateMolCubeFiles(Psi4Handle, Psi4Mol, Mol, MolNum=None):
    ...
    Status, Energy, WaveFunction = Psi4Util.CalculateSinglePointEnergy(
    ...
    # Generate cube files...
    Status = GenerateCubeFilesForDualDescriptors(Psi4Handle, WaveFunction, Mol, MolNum)
...
def GenerateCubeFilesForDualDescriptors(Psi4Handle, WaveFunction, Mol, MolNum):
    """Generate cube files for dual descriptors."""
    ...
    Status, Psi4CubeFiles = GenerateCubeFilesUsingCubeprop(
        Psi4Handle, WaveFunction, Mol, MolNum, CubeFilesDir, CubeFilesDirPath
    )
    if not Status:
        return False
...
def GenerateCubeFilesUsingCubeprop(Psi4Handle, WaveFunction, Mol, MolNum, CubeFilesDir, CubeFilesDirPath):
    ...
    # Collect cube files...
    Psi4CubeFiles = glob.glob(os.path.join(CubeFilesDir, "DUAL*.cube"))
    if len(Psi4CubeFiles) == 0:
        shutil.rmtree(CubeFilesDir)
        if not OptionsInfo["QuietMode"]:
            MiscUtil.PrintWarning("Failed to create cube files for dual descriptors...")
            MiscUtil.PrintWarning("Ignoring molecule: %s" % RDKitUtil.GetMolName(Mol, MolNum))
        return (False, None)
...
def VisualizeDualDescriptors():
    """Visualize cube files for dual descriptors."""
    ...
    for Mol in Mols:
        ...
        # Setup dual descriptor views...
        WriteDualDescriptorView(OutFH, CubeFilesInfo, PyMOLObjectNames)

        # Setup dual descriptor level group...
        Enable, Action = [True, "open"]
        GenerateAndWritePMLForGroup(
            OutFH, PyMOLObjectNames["DualGroup"], PyMOLObjectNames["DualGroupMembers"], Enable, Action
        )
        ...
        if FirstMol:
            ...
            Enable, Action = [True, "open"]
        GenerateAndWritePMLForGroup(
            OutFH, PyMOLObjectNames["MolGroup"], PyMOLObjectNames["MolGroupMembers"], Enable, Action
        )
...
def WritePyMOLParameters(OutFH):
    ...
    if OptionsInfo["PyMOLViewParams"]["SetupGlobalVolumeColorRamp"]:
        ...
        GenerateAndWritePMLForVolumeColorRamp(
            OutFH,
            PyMOLViewParams["GlobalVolumeColorRampName"],
            PyMOLViewParams["ContourLevel1"],
            PyMOLViewParams["ContourColor1"],
            PyMOLViewParams["ContourLevel2"],
            PyMOLViewParams["ContourColor2"],
            PyMOLViewParams["VolumeContourWindowFactor"],
            PyMOLViewParams["VolumeColorRampOpacity"],
        )
...
def WriteMolView(OutFH, CubeFilesInfo, PyMOLObjectNames):
    ...
    # MolAlone group...
    GenerateAndWritePMLForGroup(
        OutFH, PyMOLObjectNames["MolAloneGroup"], PyMOLObjectNames["MolAloneGroupMembers"], True, "open"
    )


def WriteDualDescriptorView(OutFH, CubeFilesInfo, PyMOLObjectNames):
    """Write out PML for viewing dual descriptors in a molecule."""

    OutFH.write("""\n""\n"Setting up views for dual descriptor..."\n""\n""")

    # Cube...
    DualCubeFile = CubeFilesInfo["FileName"]
    DualCubeName = PyMOLObjectNames["DualCube"]
    PMLCmds = []
    PMLCmds.append("""cmd.load("%s", "%s")""" % (DualCubeFile, DualCubeName))
    PMLCmds.append("""cmd.disable("%s")""" % (DualCubeName))
    PMLCmds.append("")
    PML = "\n".join(PMLCmds)
    OutFH.write("%s\n" % PML)

    ContourLevel1 = CubeFilesInfo["ContourLevel1"]
    ContourLevel2 = CubeFilesInfo["ContourLevel2"]

    ContourColor1 = CubeFilesInfo["ContourColor1"]
    ContourColor2 = CubeFilesInfo["ContourColor2"]

    ContourWindowFactor = CubeFilesInfo["ContourWindowFactor"]
    ContourColorOpacity = CubeFilesInfo["ContourColorOpacity"]

    # Volume ramp...
    if CubeFilesInfo["SetupLocalVolumeColorRamp"]:
        GenerateAndWritePMLForVolumeColorRamp(
            OutFH,
    ...
    # Volume...
    DualVolumeName = PyMOLObjectNames["DualVolume"]
    PMLCmds = []
    PMLCmds.append(
        """cmd.volume("%s", "%s", "%s")""" % (DualVolumeName, DualCubeName, CubeFilesInfo["VolumeColorRampName"])
    )
    PMLCmds.append("""cmd.disable("%s")""" % (DualVolumeName))
    PMLCmds.append("")
    PML = "\n".join(PMLCmds)
    OutFH.write("%s\n" % PML)
    ...
    # Mesh...
    PMLCmds = []
    DualMeshName = PyMOLObjectNames["DualMeshPositive"]
    PMLCmds.append("""cmd.isomesh("%s", "%s", "%s")""" % (DualMeshName, DualCubeName, ContourLevel2))
    PMLCmds.append("""util.color_deep("%s", "%s")""" % (ContourColor2, DualMeshName))
    PMLCmds.append("""cmd.enable("%s")""" % (DualMeshName))
    PMLCmds.append("")

    DualMeshName = PyMOLObjectNames["DualMeshNegative"]
    PMLCmds.append("""cmd.isomesh("%s", "%s", "%s")""" % (DualMeshName, DualCubeName, ContourLevel1))
    PMLCmds.append("""util.color_deep("%s", "%s")""" % (ContourColor1, DualMeshName))
    PMLCmds.append("""cmd.enable("%s")""" % (DualMeshName))
    PMLCmds.append("")
    PML = "\n".join(PMLCmds)
    OutFH.write("%s\n" % PML)
    ...
    GenerateAndWritePMLForGroup(
        OutFH, PyMOLObjectNames["DualMeshGroup"], PyMOLObjectNames["DualMeshGroupMembers"], False, "open"
    )

    # Surface...
    PMLCmds = []
    DualSurfaceName = PyMOLObjectNames["DualSurfacePositive"]
    PMLCmds.append("""cmd.isosurface("%s", "%s", "%s")""" % (DualSurfaceName, DualCubeName, ContourLevel2))
    PMLCmds.append("""util.color_deep("%s", "%s")""" % (ContourColor2, DualSurfaceName))
    PMLCmds.append("""cmd.enable("%s")""" % (DualSurfaceName))
    PMLCmds.append("")

    DualSurfaceName = PyMOLObjectNames["DualSurfaceNegative"]
    PMLCmds.append("""cmd.isosurface("%s", "%s", "%s")""" % (DualSurfaceName, DualCubeName, ContourLevel1))
    PMLCmds.append("""util.color_deep("%s", "%s")""" % (ContourColor1, DualSurfaceName))
    PMLCmds.append("""cmd.enable("%s")""" % (DualSurfaceName))
    PMLCmds.append("")
    PML = "\n".join(PMLCmds)
    OutFH.write("%s\n" % PML)
    ...
    GenerateAndWritePMLForGroup(
        OutFH, PyMOLObjectNames["DualSurfaceGroup"], PyMOLObjectNames["DualSurfaceGroupMembers"], True, "open"
    )


def GenerateAndWritePMLForVolumeColorRamp(
    OutFH,
    ColorRampName,
    ContourLevel1,
    ContourColor1,
    ...
    PML = """\
cmd.volume_ramp_new("%s", [%.4f, "%s", 0.00, %.4f, "%s", %s, %.4f, "%s", 0.00, %4f, "%s", 0.00, %.4f, "%s", %s, %4f, "%s", 0.00])""" % (
        ColorRampName,
        (ContourLevel1 - RampWindowOffset1),
        ContourColor1,
        ContourLevel1,
        ContourColor1,
        ContourColorOpacity,
        (ContourLevel1 + RampWindowOffset1),
        ContourColor1,
        (ContourLevel2 - RampWindowOffset2),
        ContourColor2,
        ContourLevel2,
        ContourColor2,
        ContourColorOpacity,
...
def RetrieveMolCubeFilesInfo(Mol, MolNum):
    ...
    CubeFilesInfo = {}
    CubeFilesInfo["FileName"] = None
    CubeFilesInfo["IsocontourRangeMin"] = None
    CubeFilesInfo["IsocontourRangeMax"] = None

    CubeFilesInfo["ContourLevel1"] = None
    CubeFilesInfo["ContourLevel2"] = None

    CubeFilesInfo["ContourColor1"] = None
    CubeFilesInfo["ContourColor2"] = None

    CubeFilesInfo["ContourWindowFactor"] = None
    CubeFilesInfo["ContourColorOpacity"] = None

    CubeFilesInfo["VolumeColorRampName"] = None
    CubeFilesInfo["SetupLocalVolumeColorRamp"] = False
    ...
    # Retrieve cube files...
    CubeFiles = glob.glob("%s*DUAL*.cube" % (CubeFilesInfo["MolPrefix"]))

    if len(CubeFiles) == 0:
        return (False, CubeFilesInfo)

    CubeFileName = CubeFiles[0]
    if len(CubeFiles) > 1:
        if not OptionsInfo["QuietMode"]:
            MiscUtil.PrintWarning(
                "Multiple cube files available for molecule %s; Using first cube file, %s, to visualize dual descriptor..."
                % (RDKitUtil.GetMolName(Mol, MolNum), CubeFileName)
            )
    CubeFilesInfo["FileName"] = CubeFileName

    IsocontourRangeMin, IsocontourRangeMax, ContourLevel1, ContourLevel2 = SetupIsocontourRangeAndContourLevels(
        CubeFileName
    )
    CubeFilesInfo["IsocontourRangeMin"] = IsocontourRangeMin
    CubeFilesInfo["IsocontourRangeMax"] = IsocontourRangeMax

    CubeFilesInfo["ContourLevel1"] = ContourLevel1
    CubeFilesInfo["ContourLevel2"] = ContourLevel2
    CubeFilesInfo["ContourColor1"] = OptionsInfo["PyMOLViewParams"]["ContourColor1"]
    CubeFilesInfo["ContourColor2"] = OptionsInfo["PyMOLViewParams"]["ContourColor2"]

    CubeFilesInfo["ContourWindowFactor"] = OptionsInfo["PyMOLViewParams"]["VolumeContourWindowFactor"]
    CubeFilesInfo["ContourColorOpacity"] = OptionsInfo["PyMOLViewParams"]["VolumeColorRampOpacity"]

    VolumeColorRampName = SetupVolumeColorRampName(CubeFilesInfo)
    CubeFilesInfo["VolumeColorRampName"] = VolumeColorRampName
    CubeFilesInfo["SetupLocalVolumeColorRamp"] = (
        False if OptionsInfo["PyMOLViewParams"]["SetupGlobalVolumeColorRamp"] else True
    )
...
def SetupIsocontourRangeAndContourLevels(CubeFileName):
    ...
    if IsocontourRangeMin is None:
        ...
        if not OptionsInfo["QuietMode"]:
            MiscUtil.PrintInfo(
                "Failed to retrieve isocontour range from the cube file. Setting min isocontour value to %s..."
                % (IsocontourRangeMin)
            )
    ...
    if IsocontourRangeMax is None:
        ...
        if not OptionsInfo["QuietMode"]:
            MiscUtil.PrintInfo(
                "Failed to retrieve isocontour range from the cube file. Setting max isocontour value to %s..."
                % (IsocontourRangeMax)
            )
    ...
    if not OptionsInfo["QuietMode"]:
        if IsocontourRangeMin is not None and IsocontourRangeMax is not None:
            MiscUtil.PrintInfo(
                "CubeFileName: %s; Isocontour range for %s percent of the density: %.4f to %.4f; ContourLevel1: %s; ContourLevel2: %s"
                % (
                    CubeFileName,
                    (100 * OptionsInfo["Psi4CubeFilesParams"]["IsoContourThreshold"]),
                    IsocontourRangeMin,
                    IsocontourRangeMax,
                    ContourLevel1,
                    ContourLevel2,
...
def SetupPyMOLObjectNames(Mol, MolNum, CubeFilesInfo):
    ...
    SetupPyMOLObjectNamesForDualDescriptors(Mol, MolNum, CubeFilesInfo, PyMOLObjectNames)
...
def SetupPyMOLObjectNamesForDualDescriptors(Mol, MolNum, CubeFilesInfo, PyMOLObjectNames):
    """Setup groups and objects for dual descriptors."""

    MolGroupName = PyMOLObjectNames["MolGroup"]

    # Setup dual group..
    DualGroupName = "%s.Dual" % (MolGroupName)
    PyMOLObjectNames["DualGroup"] = DualGroupName
    PyMOLObjectNames["MolGroupMembers"].append(DualGroupName)
    PyMOLObjectNames["DualGroupMembers"] = []

    # Cube...
    DualCubeName = "%s.Cube" % DualGroupName
    PyMOLObjectNames["DualCube"] = DualCubeName
    PyMOLObjectNames["DualGroupMembers"].append(DualCubeName)

    # Volume...
    DualVolumeName = "%s.Volume" % DualGroupName
    PyMOLObjectNames["DualVolume"] = DualVolumeName
    PyMOLObjectNames["DualGroupMembers"].append(DualVolumeName)

    # Mesh...
    DualMeshGroupName = "%s.Mesh" % DualGroupName
    DualMeshPositiveName = "%s.Positive_Electrophilic" % DualMeshGroupName
    DualMeshNegativeName = "%s.Negative_Nucleophilic" % DualMeshGroupName

    PyMOLObjectNames["DualMeshGroup"] = DualMeshGroupName
    PyMOLObjectNames["DualGroupMembers"].append(DualMeshGroupName)

    PyMOLObjectNames["DualMeshPositive"] = DualMeshPositiveName
    PyMOLObjectNames["DualMeshNegative"] = DualMeshNegativeName
    PyMOLObjectNames["DualMeshGroupMembers"] = []
    PyMOLObjectNames["DualMeshGroupMembers"].append(DualMeshPositiveName)
    PyMOLObjectNames["DualMeshGroupMembers"].append(DualMeshNegativeName)

    # Surface....
    DualSurfaceGroupName = "%s.Surface" % DualGroupName
    DualSurfacePositiveName = "%s.Positive_Electrophilic" % DualSurfaceGroupName
    DualSurfaceNegativeName = "%s.Negative_Nucleophilic" % DualSurfaceGroupName

    PyMOLObjectNames["DualSurfaceGroup"] = DualSurfaceGroupName
    PyMOLObjectNames["DualGroupMembers"].append(DualSurfaceGroupName)

    PyMOLObjectNames["DualSurfacePositive"] = DualSurfacePositiveName
    PyMOLObjectNames["DualSurfaceNegative"] = DualSurfaceNegativeName
    PyMOLObjectNames["DualSurfaceGroupMembers"] = []
    PyMOLObjectNames["DualSurfaceGroupMembers"].append(DualSurfacePositiveName)
...
def CheckAndValidateMolecule(Mol, MolCount=None):
    ...
    # Check for spin multiplicity to warn about open-shell systems...
    if RDKitUtil.GetSpinMultiplicity(Mol) > 1:
        if not OptionsInfo["QuietMode"]:
            MiscUtil.PrintWarning(
                "Generation of dual descriptors not supported for open-shell systems. Ignoring  molecule containing unpaired electrons: %s\n"
                % MolName
            )
        return False
    ...
    if OptionsInfo["GenerateCubeFiles"]:
        if os.path.isdir(OptionsInfo["OutfilesDir"]):
            if not Options["--overwrite"]:
                MiscUtil.PrintError(
                    'The outfiles directory, %s, corresponding to output file specified, %s, for option "-o, --outfile" already exist. Use option "--overwrite" or "--ov"  and try again to generate and visualize cube files...\n'
    ...
    if OptionsInfo["VisualizeCubeFiles"]:
        if not Options["--overwrite"]:
            if os.path.exists(os.path.join(OptionsInfo["OutfilesDir"], OptionsInfo["Outfile"])):
                MiscUtil.PrintError(
                    'The outfile file specified, %s, for option "-o, --outfile" already exist in outfiles dir, %s. Use option "--overwrite" or "--ov" to overwrite and try again to generate and visualize cube files.\n'
        ...
        if not OptionsInfo["GenerateCubeFiles"]:
            if not os.path.isdir(OptionsInfo["OutfilesDir"]):
                MiscUtil.PrintError(
                    'The outfiles directory, %s, corresponding to output file specified, %s, for option "-o, --outfile" doesn\'t exist. Use value, GenerateCubeFiles or Both, for option "--mode" and try again to generate and visualize cube files...\n'
                    % (OptionsInfo["OutfilesDir"], OptionsInfo["Outfile"])
                )
            ...
            if not len(CubeFiles):
                MiscUtil.PrintError(
                    'The outfiles directory, %s, contains no cube files, "*.cube". Use value, GenerateCubeFiles or Both, for option "--mode" and try again to generate and visualize cube...\n'
...
def ProcessOptions():
    ...
    if re.match("^auto$", OutfilesDir, re.I):
        OutfilesDir = "%s_DualDescriptors" % OptionsInfo["OutfileRoot"]
    ...
    OptionsInfo["Psi4CubeFilesParams"] = Psi4Util.ProcessPsi4CubeFilesParameters(
        "--psi4CubeFilesParams", Options["--psi4CubeFilesParams"], ParamsDefaultInfo=ParamsDefaultInfoOverride
    )
    ...
    ParamsDefaultInfoOverride = {
        ...
        "DisplaySphereScale": 0.2,
        "DisplayStickRadius": 0.1,
        "HideHydrogens": True,
        "MeshWidth": 0.5,
        "MeshQuality": 2,
        "SurfaceQuality": 2,
        "SurfaceTransparency": 0.25,
        "VolumeColorRamp": "auto",
        "VolumeColorRampOpacity": 0.2,
        "VolumeContourWindowFactor": 0.05,
    }
    OptionsInfo["PyMOLViewParams"] = MiscUtil.ProcessOptionPyMOLCubeFileViewParameters(
        "--pymolViewParams", Options["--pymolViewParams"], ParamsDefaultInfo=ParamsDefaultInfoOverride
    )
    ...
    if OptionsInfo["PyMOLViewParams"]["VolumeColorRampAuto"]:
        ...
        if not (
            OptionsInfo["PyMOLViewParams"]["ContourLevel1Auto"] or OptionsInfo["PyMOLViewParams"]["ContourLevel2Auto"]
        ):
...
Usage:
    Psi4VisualizeDualDescriptors.py [--basisSet <text>] [--infileParams <Name,Value,...>] [--methodName <text>]
...
Description:
    ...
    A set of cube and SD output files is generated for each closed-shell molecule
...
Options:
    ...
    -r, --reference <text>  [default: auto]
        ...
        orbitals. Default: RHF or UHF. The default values are Restricted Hartree-Fock (RHF)
        for closed-shell molecules with all electrons paired and Unrestricted artree-Fock (UHF)
...
Examples:
    ...
    non-sulfur and sulfur containing closed-shell molecules in a SD file with 3D
    structures, and write a new PML file, type:
    ...
    a PML file to visualize dual descriptors for frontier molecular orbitals, type:
    ...
    visualize dual descriptors for frontier molecular orbitals and write out a PML file,
    type:
