...
def ProcessMoleculesUsingSingleProcess(Mols, Writer):
    ...
    for Mol in Mols:
        ...
        CalcStatus, Energy, SolvationEnergy = CalculateMoleculeEnergy(Psi4Handle, Psi4Mol, Mol, MolCount)
        ...
        WriteMolecule(Writer, Mol, Energy, SolvationEnergy)
...
def ProcessMoleculesUsingMultipleProcesses(Mols, Writer):
    ...
    for Result in Results:
        MolCount += 1
        MolIndex, EncodedMol, CalcStatus, Energy, SolvationEnergy = Result
...
def WorkerProcess(EncodedMolInfo):
    ...
    CalcStatus, Energy, SolvationEnergy = CalculateMoleculeEnergy(OptionsInfo["psi4"], Psi4Mol, Mol, MolCount)
...
def CalculateMoleculeEnergy(Psi4Handle, Psi4Mol, Mol, MolNum=None):
    ...
    return (Status, Energy, SolvationEnergy)
...
def WriteMolecule(Writer, Mol, Energy, SolvationEnergy):
    """Write molecule."""
...
Description:
    ...
    geometry is not optimized before the calculation. In addition, the hydrogens
    must be present for all molecules in input file. The 3D coordinates are not
...
Options:
    ...
    --psi4DDXSolvation <yes or no>  [default: no]
        Perform energy calculation in solution using domain-decomposition-based
...
Examples:
    ...
    To run the first example along with calculating DDX solvation energy using
    default solvent parameters for water and write out a SD file, type:

        % Psi4CalculateEnergy.py --mp yes -i Psi4Sample3D.sdf
          -o Psi4Sample3DOut.sdf --psi4DDXSolvation yes

    To run the first example along with calculating DDX solvation energy using
    specific solvartion parameters and write out a SD file, type:
