    ...
    try:
        if OptionsInfo["UseUFF"]:
            ...
            Status = AllChem.MMFFOptimizeMolecule(
                Mol, maxIters=OptionsInfo["MaxIters"], mmffVariant=OptionsInfo["MMFFVariant"]
            )
        else:
            MiscUtil.PrintError(
...
def ProcesssConformerGeneratorOption():
    ...
    OptionsInfo["SkipConformerGeneration"] = ConfGenParams["SkipConformerGeneration"]
...
def ValidateOptions():
    ...
    MiscUtil.ValidateOptionTextValue(
        "-c, --conformerGenerator", Options["--conformerGenerator"], "SDG ETDG KDG ETKDG ETKDGv2 None"
    )
...
Usage:
    RDKitPerformMinimization.py [--addHydrogens <yes or no>] [--conformerGenerator <SDG, KDG, ETDG,  ETKDG, ETKDGv2, None>]
