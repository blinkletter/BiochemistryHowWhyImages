...
def CompareMoleculeShapes():
    ...
    if not re.match("^(OneToOne|AllToAll|FirstToAll)$", OptionsInfo["Mode"], re.I):
        MiscUtil.PrintError(
            "Shape comparison couldn't be performed: Specified mode, %s, is not supported" % OptionsInfo["Mode"]
        )

    if not re.match("^(Open3A|CrippenOpen3A)$", OptionsInfo["Alignment"], re.I):
        MiscUtil.PrintError(
            "Shape couldn't be performed: Specified alignment mode, %s, is not supported" % OptionsInfo["Alignment"]
        )
    ...
    if re.match("^OneToOne$", OptionsInfo["Mode"], re.I):
        ...
        MiscUtil.PrintError(
            "Shape comaprison couldn't be performed: Specified mode, %s, is not supported" % OptionsInfo["Mode"]
        )
    ...
    MiscUtil.PrintInfo(
        ...
        % ((RefMolCount - ValidRefMolCount), (ProbeMolCount - ValidProbeMolCount))
    )
...
Examples:
    ...
    reference and probe input in 3D SD files, calculate both Tanimoto and protrude
