...
def ProcessOptionChargesRespParameters():
    ...
    ParamsInfo = {
        "MaxIter": 25,
        "RestrainHydrogens": False,
        "RemoveGridFiles": True,
        "RespA": 0.0005,
        "RespB": 0.1,
        "Tolerance": 1e-5,
        "VDWRadii": VDWRadii,
        "VDWScaleFactors": VDWScaleFactors,
        "VDWPointDensity": 1.0,
    }
    ...
    for Index in range(0, len(ParamsOptionValueWords), 2):
        ...
        if re.match("^MaxIter$", ParamName, re.I):
            ...
            if not ScaleFactorsValue:
                MiscUtil.PrintError(
                    'No parameter value specified for parameter name, %s, using "%s" option.' % (Name, ParamsOptionName)
                )
            ...
            for ScaleFactor in ScaleFactorsWords:
                if not MiscUtil.IsNumber(ScaleFactor):
                    MiscUtil.PrintError(
                        'The value, %s, in parameter value, %s, specified for parameter name, %s, using "%s" option must be a float.'
                        % (ScaleFactor, Value, Name, ParamsOptionName)
                    )
                ScaleFactor = float(ScaleFactor)
                if ScaleFactor <= 0:
                    MiscUtil.PrintError(
                        'The value, %s, in parameter value, %s, specified for parameter name, %s, using "%s" option is not a valid value. Supported values: > 0'
                        % (ScaleFactor, Value, Name, ParamsOptionName)
                    )
                if len(ScaleFactors):
                    if ScaleFactor <= LastScaleFactor:
                        MiscUtil.PrintError(
                            'The value, %s, in parameter value, %s, specified for parameter name, %s, using "%s" option is not a valid value. It must be greater than the previous value, %s, specified for the scale factor.'
                            % (ScaleFactor, Value, Name, ParamsOptionName, LastScaleFactor)
                        )
            ...
            if len(RadiiWords) % 2:
                MiscUtil.PrintError(
                    'The number of space delimited values, %s, in parameter value, %s, specified for parameter name, %s, using "%s" option is not valid. It must be an even number.'
                    % (len(RadiiWords), Value, Name, ParamsOptionName)
                )
            ...
            for RadiiWordsIndex in range(0, len(RadiiWords), 2):
                ...
                if not MiscUtil.IsNumber(VDWRadius):
                    MiscUtil.PrintError(
                        'The vdw radius value, %s, in parameter value, %s, specified for parameter name, %s, using "%s" option is not a valid.'
                        % (VDWRadius, Value, Name, ParamsOptionName)
                    )

                if not RDKitUtil.IsValidElementSymbol(ElementSymbol.capitalize()):
                    MiscUtil.PrintWarning(
                        'The element symbol, %s, in parameter value, %s, specified for parameter name, %s, using "%s" option is not a valid.'
                        % (ElementSymbol, Value, Name, ParamsOptionName)
                    )
...
def SetupChargesRespOptions(ParamsInfo):
    ...
    # Setup ESP options...
    ParamNameToESPOptionID = {
        "MaxIter": "MAX_IT",
        "RespA": "RESP_A",
        "RespB": "RESP_B",
        "Tolerance": "TOLER",
        "VDWRadii": "VDW_RADII",
        "VDWScaleFactors": "VDW_SCALE_FACTORS",
        "VDWPointDensity": "VDW_POINT_DENSITY",
    }
