...
def WritePMLHeader(OutFH, ScriptName):
    """Write out PML header for setting up complex view."""
...
"" """
    ...
    if OptionsInfo["VolumeComplex"]:
        PML = SetupPMLForElectronDensityVolume(
            MapName, VolumeName, VolumeColorRamp, Enable=EnableVolume, Selection=ComplexName
        )
        OutFH.write("\n%s\n" % PML)

    if OptionsInfo["MeshComplex"]:
        PML = SetupPMLForElectronDensityMesh(
            MapName, MeshName, ContourLevel, Color, Enable=EnableMesh, Selection=ComplexName
        )
        OutFH.write("\n%s\n" % PML)

    if OptionsInfo["SurfaceComplex"]:
        PML = SetupPMLForElectronDensitySurface(
            MapName, SurfaceName, ContourLevel, Color, Enable=EnableSurface, Selection=ComplexName
        )
        OutFH.write("\n%s\n" % PML)

    GenerateAndWritePMLForGroup(
        OutFH,
        PyMOLObjectNames["ComplexCompositeEDGroup"],
...
"" """
    ...
    if OptionsInfo["MeshComplex"]:
        PML = SetupPMLForElectronDensityMesh(
            MapName, Mesh1Name, ContourLevel1, Color1, Enable=EnableMesh, Selection=ComplexName
        )
        OutFH.write("\n%s\n" % PML)

    if OptionsInfo["SurfaceComplex"]:
        PML = SetupPMLForElectronDensitySurface(
            MapName, Surface1Name, ContourLevel1, Color1, Enable=EnableSurface, Selection=ComplexName
        )
        OutFH.write("\n%s\n" % PML)

    if OptionsInfo["MeshComplex"]:
        PML = SetupPMLForElectronDensityMesh(
            MapName, Mesh2Name, ContourLevel2, Color2, Enable=EnableMesh, Selection=ComplexName
        )
        OutFH.write("\n%s\n" % PML)

    if OptionsInfo["SurfaceComplex"]:
        PML = SetupPMLForElectronDensitySurface(
            MapName, Surface2Name, ContourLevel2, Color2, Enable=EnableSurface, Selection=ComplexName
        )
        OutFH.write("\n%s\n" % PML)

    GenerateAndWritePMLForGroup(
        OutFH, PyMOLObjectNames["ComplexDiffEDGroup"], PyMOLObjectNames["ComplexDiffEDGroupMembers"], True, "close"
    )
...
def WriteChainComplexAndMeshViews(OutFH, FileIndex, PyMOLObjectNames, ChainID):
    ...
    if MeshChainComplex or VolumeChainComplex or SurfaceChainComplex:
        ...
        if VolumeChainComplex:
            PML = SetupPMLForElectronDensityVolume(
                MapName, VolumeName, VolumeColorRamp, Enable=EnableVolumeChainComplex, Selection=ChainComplexName
            )
            OutFH.write("\n%s\n" % PML)

        if MeshChainComplex:
            PML = SetupPMLForElectronDensityMesh(
                MapName, MeshName, ContourLevel, Color, Enable=EnableMeshChainComplex, Selection=ChainComplexName
            )
            OutFH.write("\n%s\n" % PML)

        if SurfaceChainComplex:
            PML = SetupPMLForElectronDensitySurface(
                MapName, SurfaceName, ContourLevel, Color, Enable=EnableSurfaceChainComplex, Selection=ChainComplexName
            )
            OutFH.write("\n%s\n" % PML)

        GenerateAndWritePMLForGroup(
            OutFH,
            PyMOLObjectNames["Chains"][ChainID]["ChainComplexCompositeEDGroup"],
        ...
        if PyMOLObjectNames["SetupDiffEDMapObjects"]:
            ...
            if MeshChainComplex:
                PML = SetupPMLForElectronDensityMesh(
                    MapName, Mesh1Name, ContourLevel1, Color1, Enable=EnableMeshChainComplex, Selection=ChainComplexName
                )
                OutFH.write("\n%s\n" % PML)
            ...
            if SurfaceChainComplex:
                PML = SetupPMLForElectronDensitySurface(
                    ...
                    ContourLevel1,
                    Color1,
                    Enable=EnableSurfaceChainComplex,
                    Selection=ChainComplexName,
                )
                OutFH.write("\n%s\n" % PML)

            if MeshChainComplex:
                PML = SetupPMLForElectronDensityMesh(
                    MapName, Mesh2Name, ContourLevel2, Color2, Enable=EnableMeshChainComplex, Selection=ChainComplexName
                )
                OutFH.write("\n%s\n" % PML)
            ...
            GenerateAndWritePMLForGroup(
                OutFH,
                PyMOLObjectNames["Chains"][ChainID]["ChainComplexDiffEDGroup"],
...
def WriteChainAloneViews(OutFH, FileIndex, PyMOLObjectNames, ChainID):
    ...
    if OptionsInfo["BFactorChainCartoonPutty"]:
        ...
        PML = PyMOLUtil.SetupPMLForBFactorPuttyView(
            BFactorPuttyName, ChainName, ColorPalette=OptionsInfo["BFactorColorPalette"], Enable=False
        )
        OutFH.write("\n%s\n" % PML)
...
def SetupChainSelectionsView(OutFH, FileIndex, PyMOLObjectNames, ChainID):
    ...
    for Index in range(0, len(OptionsInfo["ChainSelectionsInfo"]["Names"])):
        ...
        PML = PyMOLUtil.SetupPMLForSelectionDisplayView(
            SelectionObjectName, SelectionCmd, OptionsInfo["SelectionsChainStyle"], Enable=True
        )
        OutFH.write("\n%s\n" % PML)
        ...
        PML = SetupPMLForElectronDensityVolume(
            CompositeMapName,
            CompositeVolumeName,
            OptionsInfo["VolumeColorRampCompositeMap"],
            Enable=False,
            Selection=SelectionObjectName,
        )
        OutFH.write("\n%s\n" % PML)
        ...
        PML = SetupPMLForElectronDensityMesh(
            CompositeMapName,
            CompositeMeshName,
            OptionsInfo["MeshLevelCompositeMap"],
            OptionsInfo["MeshColorCompositeMap"],
            Enable=True,
            Selection=SelectionObjectName,
        )
        OutFH.write("\n%s\n" % PML)
        ...
        if PyMOLObjectNames["SetupDiffEDMapObjects"]:
            ...
            PML = SetupPMLForElectronDensityVolume(
                DiffMapName,
                DiffVolumeName,
                OptionsInfo["VolumeColorRampDiffMap"],
                Enable=False,
                Selection=SelectionObjectName,
            )
            OutFH.write("\n%s\n" % PML)
            ...
            PML = SetupPMLForElectronDensityMesh(
                DiffMapName,
                DiffMesh1Name,
                OptionsInfo["Mesh1LevelDiffMap"],
                OptionsInfo["Mesh1ColorDiffMap"],
                Enable=True,
                Selection=SelectionObjectName,
            )
            OutFH.write("\n%s\n" % PML)
            ...
            PML = SetupPMLForElectronDensityMesh(
                DiffMapName,
                DiffMesh2Name,
                OptionsInfo["Mesh2LevelDiffMap"],
                OptionsInfo["Mesh2ColorDiffMap"],
                Enable=True,
                Selection=SelectionObjectName,
            )
            OutFH.write("\n%s\n" % PML)
    ...
    for GroupID in ["Ligand", "Pocket", "PocketSolvent", "PocketInorganic"]:
        ...
        if re.match("^Pocket$", GroupID, re.I) and OptionsInfo["PocketSurface"]:
            ...
            PML = PyMOLUtil.SetupPMLForHydrophobicSurfaceView(
                HydrophobicSurfaceName, GroupTypeObjectName, ColorPalette="RedToWhite", Enable=False
            )
            OutFH.write("\n%s\n" % PML)
...
def SetupPMLForElectronDensityMesh(MapName, MeshName, SigmaLevel, Color, Enable=True, Selection=None):
    ...
    if Selection is None:
        ...
        PMLCmds.append(
            """cmd.isomesh("%s", "%s", %.1f, "(%s)", carve = %.1f)"""
            % (MeshName, MapName, SigmaLevel, Selection, Carve)
        )
    PMLCmds.append(PyMOLUtil.SetupPMLForDeepColoring(MeshName, Color))
...
def SetupPMLForElectronDensityVolume(MapName, VolumeName, VolumeColorRamp, Enable=True, Selection=None):
    ...
    if Selection is None:
        ...
        PMLCmds.append(
            """cmd.volume("%s", "%s", "%s", "(%s)", carve = %.1f)"""
            % (VolumeName, MapName, VolumeColorRamp, Selection, Carve)
        )
...
def SetupPMLForElectronDensitySurface(MapName, SurfaceName, SigmaLevel, Color, Enable=True, Selection=None):
    ...
    if Selection is None:
        ...
        PMLCmds.append(
            """cmd.isosurface("%s", "%s", %.1f, "(%s)", carve = %.1f)"""
            % (SurfaceName, MapName, SigmaLevel, Selection, Carve)
        )
    PMLCmds.append(PyMOLUtil.SetupPMLForDeepColoring(SurfaceName, Color))
...
def SetupPyMOLObjectNamesForChain(FileIndex, PyMOLObjectNames, ChainID):
    ...
    if OptionsInfo["ChainSelections"]:
        ...
        for SelectionName in OptionsInfo["ChainSelectionsInfo"]["Names"]:
            ...
            CompositeMeshGroupMembersID = "%s%sCompositeEDGroupMembers" % (
...
def SetupPyMOLObjectNamesForLigand(FileIndex, PyMOLObjectNames, ChainID, LigandID):
    ...
    for GroupType in ["Ligand", "Pocket", "Pocket_Solvent", "Pocket_Inorganic"]:
        ...
        if PyMOLObjectNames["SetupDiffEDMapObjects"]:
            ...
            PyMOLObjectNames["Ligands"][ChainID][LigandID][DiffMeshGroupMembersID].extend(
                [DiffVolumeName, DiffMesh1Name, DiffSurface1Name, DiffMesh2Name, DiffSurface2Name]
            )
...
def ProcessEDMapFilesAndSuffixes():
    ...
    if (not re.match("^auto$", EDMapFiles, re.I)) and (not re.match("^auto$", EDMapSuffixes, re.I)):
        MiscUtil.PrintError(
            'The values, "%s" and "%s", specified using "--EDMapFiles" and "--EDMapSuffixes" are not valid. Both of these options can\'t be specified simultaneously.'
            % (EDMapFiles, EDMapSuffixes)
        )
...
def ProcessEDMapFiles():
    ...
    if EDMapFilesWordsCount % 2:
        MiscUtil.PrintError(
            'The number of comma delimited ED map files, %d, specified using "--EDMapFiles" option must be an even number.'
    ...
    if EDMapFilesWordsCount != 2 * InfilesNamesCount:
        MiscUtil.PrintError(
            'The number of comma delimited ED map files, %d, specified using "--EDMapFiles" must be twice the number of input files, %s, specified using "-i, --infiles" option.'
            % (EDMapFilesWordsCount, InfilesNamesCount)
        )
    ...
    for Index in range(0, InfilesNamesCount):
        ...
        if not os.path.exists(CompositeEDMapFile):
            MiscUtil.PrintError(
                'The composite ED map file, %s, specified using option "--EDMapFiles", corresponding to input file, %s,  doesn\'t exist.\n'
        ...
        if not re.match("^None$", DiffEDMapFile, re.I):
            if os.path.exists(DiffEDMapFile):
                ...
                MiscUtil.PrintWarning(
                    "The difference ED map file, %s, specified using option \"--EDMapFiles\", corresponding to input file, %s,  doesn't exist. PyMOL groups and objectes related to difference maps won't be created.\n"
...
def ProcessEDMapSuffixes():
    ...
    if len(EDMapSuffixesWords) % 2:
        MiscUtil.PrintError(
            'The number of comma delimited ED map types names and suffixes, %d, specified using "--EDMapSuffixes" option must be an even number.'
            % (len(EDMapSuffixesWords))
        )
    ...
    for Index in range(0, len(EDMapSuffixesWords), 2):
        ...
        if re.match("^CompositeMap$", EDMapType, re.I):
            ...
            MiscUtil.PrintError(
                'The ED map type, %s, specified using "--EDMapSuffixes" option is not a valid ED map type. Supported ED map types: CompositeMap, DifferenceMap'
                % (EDMapType)
            )
...
def SetupEDMapFileNamesUsingSuffixes():
    ...
    for Index in range(0, InfilesNamesCount):
        ...
        if not os.path.exists(CompositeEDMapFile):
            MiscUtil.PrintError(
                'The composite ED map file, %s, for option "--EDMapSuffixes", corresponding to input file, %s, doesn\'t exist.\n'
                % (CompositeEDMapFile, Infile)
            )
        ...
        if os.path.exists(DiffEDMapFile):
            ...
            MiscUtil.PrintWarning(
                "The difference ED map file, %s, for option \"--EDMapSuffixes\", corresponding to input file, %s,  doesn't exist. PyMOL groups and objectes related to difference maps won't be created.\n"
                % (DiffEDMapFile, Infile)
            )
...
def ProcessOptions():
    ...
    OptionsInfo["BFactorChainCartoonPutty"] = (
        True if re.match("^Yes$", Options["--BFactorChainCartoonPutty"], re.I) else False
    )
...
Usage:
    PyMOLVisualizeElectronDensity.py  [--align <yes or no>] [--alignMethod <align, cealign, super>]
                                     ...
                                     [--ignoreHydrogens <yes or no>] [--ligandIDs <Largest, All or ID1,ID2...>] [--labelFontID <number>]
