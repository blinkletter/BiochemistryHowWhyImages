...
def WorkerProcess(EncodedMolInfo):
    ...
    EncodedMol = RDKitUtil.MolToBase64EncodedMolString(
...
def RemoveMolSalts(Mol, Remover, MolNum):
    ...
    if SaltyStatus:
        ...
        for DataLabel in Mol.GetPropNames(includePrivate=False, includeComputed=False):
            DataProp = Mol.GetProp(DataLabel)
            UnsaltedMol.SetProp(DataLabel, DataProp)
...
def SetupSaltRemover():
    ...
    return SaltRemover(
        defnFilename=OptionsInfo["SaltsFile"], defnData=OptionsInfo["SaltsSMARTS"], defnFormat=InputFormat.SMARTS
    )
...
def ProcessOptions():
    ...
    if re.match("^ByComponent$", Options["--saltsMode"], re.I):
        ...
        MiscUtil.PrintError(
            'The salts mode specified, %s, using "--saltsMode" option is not valid.' % Options["--saltsMode"]
        )
    ...
    if re.match("^BySMARTS$", Options["--saltsMode"], re.I):
        if not Options["--saltsSMARTS"]:
            MiscUtil.PrintError(
                'No salts SMARTS pattern specified using "--saltsSMARTS" option during "BySMARTS" value of "-s, --saltsMode" option'
            )
        SaltsSMARTS = Options["--saltsSMARTS"].strip(" ")
        if not len(SaltsSMARTS):
            MiscUtil.PrintError(
                'Empty SMARTS pattern specified using "--saltsSMARTS" option during "BySMARTS" value of "-s, --saltsMode" option'
            )
...
Examples:
    ...
    To count number of molecules containing salts from in a SD file, using largest
