...
def SelectMolecules(Mols, MolsFingerprints):
    ...
    if OptionsInfo["NumMols"] > len(Mols):
        MiscUtil.PrintError(
            'The number of diverse molecules to pick, %d, specified using "-n, --numMols" must be less than total number of valid molecules, %d'
            % (OptionsInfo["NumMols"], len(Mols))
        )
...
def SelectMoleculesUsingHierarchicalClustering(Mols, MolsFingerprints):
    ...
    try:
        ...
        MiscUtil.PrintError(
            "Failed to import numpy python module. This is required for picking diverse molecules using hierarchical for clustering."
        )
...
def ProcessClusteringMethodParameter():
    ...
    if CanonicalName not in CanonicalClusteringMethodNameMap:
        MiscUtil.PrintError(
            'The cluster method, %s, specified using "-c, --clusteringMethod" option is not a valid name.'
...
def ValidateOptions():
    ...
    MiscUtil.ValidateOptionTextValue(
        "-c, --clusteringMethod", Options["--clusteringMethod"], "Centroid CLink Gower McQuitty SLink UPGMA Ward"
    )
    MiscUtil.ValidateOptionTextValue(
        "-f, --fingerprints",
