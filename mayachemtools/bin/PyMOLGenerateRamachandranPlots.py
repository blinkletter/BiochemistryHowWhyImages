...
def GenerateSinglePlotFile():
    ...
    # Setup figure...
    PlotFigure, Axes = plt.subplots(
        2, 2, figsize=(OptionsInfo["FigWidth"], OptionsInfo["FigHeight"]), dpi=OptionsInfo["FigDPI"]
    )
...
def GenerateMultiplePlotFiles():
    ...
    for PlotType in OptionsInfo["PlotTypesInfo"]["Types"]:
        ...
        PlotFigure, PlotAxis = plt.subplots(
            1, 1, figsize=(OptionsInfo["FigWidth"], OptionsInfo["FigHeight"]), dpi=OptionsInfo["FigDPI"]
        )
...
def DrawPlot(PlotAxis, PlotType):
    ...
    PlotAxis.contourf(
        ...
        PhiPsiContourInfo["Y"],
        PhiPsiContourInfo["Z"],
        levels=PlotTypesInfo["Levels"][PlotType],
        colors=PlotTypesInfo["Colors"][PlotType],
    )

    # Draw scatter plot for phi and psi angles...
    if PlotTypesInfo["ResCount"][PlotType]:
        PlotAxis.scatter(
            PlotTypesInfo["PhiAngles"][PlotType],
            PlotTypesInfo["PsiAngles"][PlotType],
            s=OptionsInfo["ScatterMarkerSize"],
            c=OptionsInfo["ScatterMarkerColor"],
            marker=OptionsInfo["ScatterMarkerStyle"],
        )
    ...
    PlotAxis.set_xticklabels(
        PlotTypesInfo["MajorTickLabels"][PlotType],
        fontdict={"fontsize": OptionsInfo["FontTicksSize"], "fontweight": OptionsInfo["FontTicksWeight"]},
    )
    PlotAxis.set_yticks(PlotTypesInfo["MajorTickPositions"][PlotType])
    PlotAxis.set_yticklabels(
        PlotTypesInfo["MajorTickLabels"][PlotType],
        fontdict={"fontsize": OptionsInfo["FontTicksSize"], "fontweight": OptionsInfo["FontTicksWeight"]},
    )
    ...
    if OptionsInfo["Grid"]:
        PlotAxis.grid(
            True,
            color=OptionsInfo["GridLineColor"],
            linestyle=OptionsInfo["GridLineStyle"],
            linewidth=OptionsInfo["GridLineWidth"],
        )

    # Setup title...
    PlotAxis.set_title(
        PlotTypesInfo["Titles"][PlotType],
        fontsize=OptionsInfo["FontTitleSize"],
        fontweight=OptionsInfo["FontTitleWeight"],
    )
    ...
    if PlotTypesInfo["DrawYLabel"][PlotType]:
        ...
        PlotAxis.set_ylabel(
            YLabel,
            fontsize=OptionsInfo["FontAxesSize"],
            fontweight=OptionsInfo["FontAxesWeight"],
            rotation=YRotation,
            labelpad=0,
        )
...
def ReadPhiAndPsiDensityFile(DensityFile):
    ...
    for LineWords in DensityReader:
        ...
        if Phi not in PhiValuesMap:
            PhiValuesMap[Phi] = float(Phi)
        if Psi not in PsiValuesMap:
            PsiValuesMap[Psi] = float(Psi)

        # Track density data...
        if Phi not in DensityInfo["Density"]:
    ...
    MiscUtil.PrintInfo(
        "Phi - Min: %s; Max: %s; Bin size: %s; Count: %s"
        % (
            DensityInfo["PhiValues"][0],
            DensityInfo["PhiValues"][-1],
            abs(DensityInfo["PhiValues"][1] - DensityInfo["PhiValues"][0]),
            len(DensityInfo["PhiValues"]),
        )
    )
    MiscUtil.PrintInfo(
        "Psi - Min: %s; Max: %s; Bin size: %s; Count: %s"
        % (
            DensityInfo["PsiValues"][0],
            DensityInfo["PsiValues"][-1],
            abs(DensityInfo["PsiValues"][1] - DensityInfo["PsiValues"][0]),
            len(DensityInfo["PsiValues"]),
        )
    )
...
def CalculatePhiPsiAngles():
    ...
    for ChainID in OptionsInfo["SpecifiedChainsAndLigandsInfo"]["ChainIDs"]:
        PhiPsiInfoList = []
        GeneralPhiPsiInfo, GlycinePhiPsiInfo, ProlinePhiPsiInfo, PreProlinePhiPsiInfo = (
            PyMOLUtil.GetPhiPsiCategoriesResiduesInfo(MolName, ChainID)
        )
    ...
    for PlotType in OptionsInfo["PlotTypesInfo"]["Types"]:
        MiscUtil.PrintInfo(
            'Number of "%s" phi and psi angles: %s' % (PlotType, OptionsInfo["PlotTypesInfo"]["ResCount"][PlotType])
        )
...
def ProcessChainIDs():
    ...
    SpecifiedChainsAndLigandsInfo = PyMOLUtil.ProcessChainsAndLigandsOptionsInfo(
        ChainsAndLigandsInfo, "-c, --chainIDs", OptionsInfo["ChainIDs"], None, None
    )
...
def ProessContourLevelsAndColors():
    ...
    for TypeLevelsColorsWord in LevelsAndColors.split(";"):
        if not len(TypeLevelsColorsWord):
            MiscUtil.PrintError(
                'The plot types, levels, and colors, "%s" specified using "-l, --levelsAndColors" option in, "%s", is empty.'
                % (TypeLevelsColorsWord, LevelsAndColors)
            )

        TypeLevelsColorsWords = TypeLevelsColorsWord.split(":")
        if len(TypeLevelsColorsWords) != 2:
            MiscUtil.PrintError(
                'The format of plot type, levels, and colors specification, "%s", specified using "-l, --levelsAndColors" option, in "%s",  is not valid: Supported format: <PlotType>: <Level>, <Color>, <Level>,...'
                % (TypeLevelsColorsWord, OptionsInfo["LevelsAndColors"])
            )

        PlotType = TypeLevelsColorsWords[0]
        if not len(PlotType):
            MiscUtil.PrintError(
                'The plot type, "%s" specified using "-l, --levelsAndColors" option in, "%s", is empty.'
                % (PlotType, TypeLevelsColorsWord)
            )
        CanonicalPlotType = PlotType.lower()

        if CanonicalPlotType not in CanonicalPlotTypes:
            MiscUtil.PrintError(
                'The plot type, "%s" specified using "-l, --levelsAndColors" option in, "%s", is not valid. Supported valus: %s'
                % (PlotType, TypeLevelsColorsWord, ", ".join(OptionsInfo["PlotTypesInfo"]["Types"]))
            )
        ...
        if not (len(LevelsColorsWords) % 2):
            MiscUtil.PrintError(
                'The format of levels and colors specifification, "%s", specified using "-l, --levelsAndColors" option in, "%s",  is not valid. It must contain odd number of values. Supported format: <PlotType>: <Level>, <Color>, <Level>,...'
                % (", ".join(LevelsColorsWords), TypeLevelsColorsWord)
            )
        ...
        for Index, SpecWord in enumerate(LevelsColorsWords):
            if not len(SpecWord):
                MiscUtil.PrintError(
                    'The level or color, "%s" specified using "-l, --levelsAndColors" option in, "%s", is empty.'
                    % (SpecWord, TypeLevelsColorsWord)
                )
            ...
            if not MiscUtil.IsFloat(SpecWord):
                MiscUtil.PrintError(
                    'The level, "%s" specified using "-l, --levelsAndColors" option in, "%s", must be a number.'
                    % (SpecWord, TypeLevelsColorsWord)
                )
            ...
            if len(Levels):
                ...
                for PreviousLevel in Levels:
                    if Level <= PreviousLevel:
                        MiscUtil.PrintError(
                            'The level, "%s" specified using "-l, --levelsAndColors" option in, "%s", must be greater than all previous levels.'
                            % (SpecWord, TypeLevelsColorsWord)
                        )
