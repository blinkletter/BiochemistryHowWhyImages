...
def PerformButinaClustering(Mols, MolsFingerprints):
    ...
    ClusteredMolIndices = Butina.ClusterData(
        DistanceMatrix, FingerprintsCount, DistanceCutoff, reordering=Reordering, isDistData=True
    )
...
def PerformHierarchicalClustering(Mols, MolsFingerprints):
    ...
    try:
        ...
        MiscUtil.PrintError(
            "Failed to import numpy python module. This is required to cluster molecules using hierarchical clustering methodology."
        )

    if OptionsInfo["NumClusters"] > len(Mols):
        MiscUtil.PrintError(
            'The number of clusters, %d, specified using "-n, --numClusters" must be less than total number of valid molecules, %d'
            % (OptionsInfo["NumClusters"], len(Mols))
        )

    MiscUtil.PrintInfo(
        "\nCluster molecules using %s hierarchical clustering methodology and %s similarity metric..."
        % (OptionsInfo["SpecifiedHierarchicalClusteringMethod"], OptionsInfo["SimilarityMetric"])
    )
    ...
    if re.match("^BitVect$", OptionsInfo["SpecifiedFingerprintsType"], re.I):
        ...
        MolsFingerprints = [
            rdMolDescriptors.GetAtomPairFingerprint(
                Mol, minLength=MinLength, maxLength=MaxLength, includeChirality=UseChirality
            )
            for Mol in Mols
        ]
...
def GenerateMorganFingerprints(Mols):
    ...
    FPSize = OptionsInfo["FingerprintsParams"]["Morgan"]["FPSize"]
    UseFeatures = False
    ...
    if re.match("^BitVect$", OptionsInfo["SpecifiedFingerprintsType"], re.I):
        ...
        MolsFingerprints = [
            rdMolDescriptors.GetMorganFingerprintAsBitVect(
                Mol, Radius, useFeatures=UseFeatures, useChirality=UseChirality, nBits=FPSize
            )
            for Mol in Mols
        ]
    else:
        ...
        MolsFingerprints = [
            rdMolDescriptors.GetMorganFingerprint(Mol, Radius, useFeatures=UseFeatures, useChirality=UseChirality)
            for Mol in Mols
        ]
...
def GenerateTopologicalTorsionsFingerprints(Mols):
    ...
    BitsPerHash = OptionsInfo["FingerprintsParams"]["TopologicalTorsions"]["BitsPerHash"]
    ...
    if re.match("^BitVect$", OptionsInfo["SpecifiedFingerprintsType"], re.I):
        ...
        MolsFingerprints = [
            rdMolDescriptors.GetHashedTopologicalTorsionFingerprintAsBitVect(
                Mol, includeChirality=UseChirality, nBits=FPSize, nBitsPerEntry=BitsPerHash
            )
            for Mol in Mols
        ]
    else:
        ...
        MolsFingerprints = [
            rdMolDescriptors.GetTopologicalTorsionFingerprint(Mol, includeChirality=UseChirality) for Mol in Mols
        ]
...
def GenerateLowerTriangularDistanceMatrix(MolsFingerprints):
    ...
    for Index1 in range(0, NumFPs):
        for Index2 in range(0, Index1):
            Distance = 1 - SimilarityFunction(
                MolsFingerprints[Index1],
                MolsFingerprints[Index2],
            )
    ...
    if OptionsInfo["SingleOutFileMode"]:
        ...
        if ClustersCount > 4:
            MiscUtil.PrintInfo(
                ...
                % (ClustersCount, OptionsInfo["OutfileBasename"], OptionsInfo["OutfileExt"])
            )
        else:
            Delmiter = ","
...
def ProcessFingerprintsParameters():
    ...
    ProcessSpecifiedFingerprintsType()
...
def SetupFingerprintsNamesAndParameters():
    ...
    OptionsInfo["FingerprintsParams"]["AtomPairs"] = {
        "MinLength": 1,
        "MaxLength": 30,
        "UseChirality": False,
        "FPSize": 2048,
    ...
    OptionsInfo["FingerprintsParams"]["Morgan"] = {"Radius": 2, "UseChirality": False, "FPSize": 2048}
    OptionsInfo["FingerprintsParams"]["MorganFeatures"] = {"Radius": 2, "UseChirality": False, "FPSize": 2048}
    OptionsInfo["FingerprintsParams"]["TopologicalTorsions"] = {"UseChirality": False, "FPSize": 2048, "BitsPerHash": 4}
...
def ProcessSpecifiedFingerprintsType():
    ...
    if re.match("^auto$", FingerprintsType, re.I):
        ...
        if re.match("^(MACCS166Keys|PathLength)$", FingerprintsName, re.I):
            MiscUtil.PrintError(
                'The fingerprints Type, %s, specified using "--fingerprintsType" is not allowed for fingerprints %s.'
                % (FingerprintsType, FingerprintsName)
            )
        ...
        if not re.match("^(Tanimoto|Dice)$", SimilarityName, re.I):
            MiscUtil.PrintError(
                'The fingerprints Type, %s, specified using "--fingerprintsType" is not allowed for similarity metric %s.\nSupported similarity metrics: Tanimoto or Dice'
                % (FingerprintsType, SimilarityName)
            )
...
def ProcessClusteringMethodParameter():
    ...
    if CanonicalName not in CanonicalClusteringMethodNameMap:
        MiscUtil.PrintError(
    ...
    OptionsInfo["SpecifiedHierarchicalClusteringMethodID"] = ClusteringMethodInfoMap[
...
def ValidateOptions():
    ...
    MiscUtil.ValidateOptionFloatValue(
        "-b, --butinaSimilarityCutoff", Options["--butinaSimilarityCutoff"], {">": 0.0, "<=": 1.0}
    )
    MiscUtil.ValidateOptionTextValue("--butinaReordering", Options["--butinaReordering"], "yes no")

    MiscUtil.ValidateOptionTextValue(
        "-c, --clusteringMethod", Options["--clusteringMethod"], "Butina Centroid CLink Gower McQuitty SLink UPGMA Ward"
    )
    MiscUtil.ValidateOptionTextValue(
        "-f, --fingerprints",
    ...
    MiscUtil.ValidateOptionTextValue("--fingerprintsType", Options["--fingerprintsType"], "IntVect BitVect  auto")
...
Usage:
    RDKitClusterMolecules.py [--butinaSimilarityCutoff <number>]  [--butinaReordering <yes or no>]
                             [--clusteringMethod <Butina, Centroid, CLink...>] [--fingerprints <MACCS166Keys, Morgan, PathLength...> ]
                             [--fingerprintsType <IntVect, BitVect, or Auto>] [--infileParams <Name,Value,...>]
...
Examples:
    ...
    BitVect, fingerprint BitVect size of 4096, and write out a single SMILES file
