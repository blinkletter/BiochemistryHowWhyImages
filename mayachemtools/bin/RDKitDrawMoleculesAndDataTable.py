...
def WriteTableDataRows(Writer, ValidMols, DataMap):
    ...
    for Mol in ValidMols:
        ...
        for DataLabel in DataMap["DataLabels"]:
            if DataLabel not in AvailableDataLabelsMap:
                Writer.write("""            <td></td>\n""")
                continue
...
def WriteHTMLPageHeader(Writer, DataMap):
    ...
    if OptionsInfo["ColVisibility"]:
        if ColCount > MaxDataColVisColCount:
            ...
            MiscUtil.PrintWarning(
                "The number of data columns, %d, is more than %d. Only first %d data columns will be available in column visibility pulldown."
                % (ColCount, MaxColVisColCount, MaxColVisColCount)
            )
...
def WriteHTMLTableHeader(Writer):
    ...
    if OptionsInfo["TableStyle"] is None:
        ...
        Writer.write(
            """    <table id="MolsAndDataTable" class="%s" cellspacing="0" width="100%s">\n"""
...
def RetrieveMoleculesAndData():
    ...
    if MiscUtil.CheckFileExt(OptionsInfo["Infile"], "smi csv tsv txt"):
        ...
        if not re.search("SMILES", Line, re.I):
            MiscUtil.PrintError(
                "The input file, %s, must contain a title line containing a column name with SMILES in its name."
                % OptionsInfo["Infile"]
            )
    ...
    if MiscUtil.CheckFileExt(OptionsInfo["Infile"], "sdf sd smi"):
        ValidMols, MolCount, ValidMolCount = RDKitUtil.ReadAndValidateMolecules(
            OptionsInfo["Infile"], **OptionsInfo["InfileParams"]
        )
    else:
...
def RetrieveMoleculesFromTextFile(Infile):
    ...
    if MolColIndex is None:
        MiscUtil.PrintError(
            "The input file, %s, must contain a title line containing a column name with SMILES in its name." % Infile
        )
    ...
    for LineIndex in range(1, len(TextLinesWords)):
        ...
        if len(LineWords) != ColCount:
            MiscUtil.PrintWarning(
                "Ignoring text line number %d: Number of columns, %d, must match number of columns, %d, in title line.\nLine: %s"
                % (MolCount, len(LineWords), ColCount, Delimiter.join(LineWords))
            )
            continue
        ...
        if Mol is None:
            MiscUtil.PrintWarning(
                "Ignoring text line number %d: SMILES parsing failed\nLine: %s" % (MolCount, Delimiter.join(LineWords))
            )
            continue
...
def ValidateShowMolNameOption(DataMap):
    ...
    if CanonicalDataLabel in DataMap["CanonicalDataLabelsMap"]:
        ...
        if not OptionsInfo["ShowMolNameAuto"]:
            MiscUtil.PrintWarning(
                'Ignoring "--showMolName" option: Data label "Name" corresponding to molecule name is already present in input file.'
            )
...
def ValidateSpecifiedDataLabelsForHighlightSMARTS(DataMap):
    ...
    for DataLabel in OptionsInfo["HighlightSMARTSDataLabels"]:
        ...
        if CanonicalDataLabel not in DataMap["CanonicalDataLabelsMap"]:
            MiscUtil.PrintError(
                'The data label specified, %s, using option "--highlightSMARTS" doesn\'t exist in input file.'
                % DataLabel
            )
        ...
        if not DataMap["StructureDataMap"][Label]:
            MiscUtil.PrintError(
                'The data label specified, %s, using option "--highlightSMARTS" doesn\'t correspond to structure data: Valid structure data labels: SMILES in data label.'
                % DataLabel
            )
...
def ValidateSpecifiedDataLabelsForHighlightClasses(DataMap):
    ...
    for Label in OptionsInfo["HighlightClassesLabels"]:
        ...
        for LabelSynonym in OptionsInfo["HighlightClassesSynonymsMap"][Label]:
            ...
            if CanonicalLabel in OptionsInfo["HighlightValuesCanonicalLabelsMap"]:
                MiscUtil.PrintInfo("")
                MiscUtil.PrintWarning(
                    'The data label, %s, for class, %s , in option "--highlightValuesClasses" has already been used in "--highlightValues" option. It\'ll be ignored during highlighting.'
                    % (LabelSynonym, OptionsInfo["HighlightClasses"])
                )
                continue
            ...
            if CanonicalLabel in OptionsInfo["HighlightRangesCanonicalLabelsMap"]:
                MiscUtil.PrintInfo("")
                MiscUtil.PrintWarning(
                    'The data label, %s, for class, %s , in option "--highlightValuesClasses" has already been used in "--highlightValuesRanges" option. It\'ll be ignored during highlighting.'
                    % (LabelSynonym, OptionsInfo["HighlightClasses"])
                )
                continue
        ...
        if ValidCanonicalLabel is None:
            MiscUtil.PrintWarning(
                'The data label or its synonyms - %s - for class, %s , in option "--highlightValuesClasses" either don\'t exist in input file or have already been used for highlighting in option "--highlightValuesClasses" or "--highlightValuesRanges". It\'ll be ignored during highlighting.'
                % (
                    MiscUtil.JoinWords(OptionsInfo["HighlightClassesSynonymsMap"][Label], ", "),
                    OptionsInfo["HighlightClasses"],
                )
            )
    ...
    if ValidDataLabelsCount == 0:
        MiscUtil.PrintInfo("")
        MiscUtil.PrintWarning(
            'The data labels and their synonyms for class, %s , in option "--highlightValuesClasses" either don\'t exists in input file or have already been used for highlighting in option "--highlightValuesClasses" or "--highlightValuesRanges". No class highlighting will be performed. Missing data labels:  %s'
            % (OptionsInfo["HighlightClasses"], MiscUtil.JoinWords(OptionsInfo["HighlightClassesLabels"], ", "))
        )
    elif ValidDataLabelsCount < DataLabelsCount:
        MiscUtil.PrintInfo("")
        MiscUtil.PrintWarning(
            'The class, %s, based highlighting specified using "--highlightValuesClasses" option will be performed using only, %d, out of, %d, data labels: %s\nThe rest of the data label(s) - %s - either don\'t exist in the input file or have aready been used for highlighting in option "--highlightValuesClasses" or "--highlightValuesRanges".'
            % (
                OptionsInfo["HighlightClasses"],
                ValidDataLabelsCount,
                DataLabelsCount,
                MiscUtil.JoinWords(ValidDataLabelsList, ", "),
                MiscUtil.JoinWords(NotValidDataLabelsList, ", "),
            )
        )
...
def ValidateDataLabels(OptionName, DataMap, DataLabels):
    ...
    for DataLabel in DataLabels:
        if re.match("^Structure$", DataLabel, re.I):
            MiscUtil.PrintError(
                'The data label specified, %s, using option "-%s" must not correspond to structure data. Structure label is not allowed.'
                % (DataLabel, OptionName)
            )

        CanonicalDataLabel = DataLabel.lower()
        if CanonicalDataLabel not in DataMap["CanonicalDataLabelsMap"]:
            MiscUtil.PrintError(
                'The data label specified, %s, using option "%s" doesn\'t exist in input file.'
                % (DataLabel, OptionName)
            )
        ...
        if DataMap["StructureDataMap"][Label]:
            MiscUtil.PrintError(
                'The data label specified, %s, using option "%s" must not correspond to structure data: Valid structure data labels contain "SMILES" in their name..'
                % (DataLabel, OptionName)
            )
...
def SetupMolInLineSVGImageTag(Mol, HightlightAtomList):
    ...
    SVGText = RDKitUtil.GetInlineSVGForMolecule(
        Mol,
        OptionsInfo["MolImageWidth"],
        OptionsInfo["MolImageHeight"],
        AtomListToHighlight=HightlightAtomList,
        Base64Encoded=OptionsInfo["MolImageEncoded"],
    )

    if OptionsInfo["MolImageEncoded"]:
        SVGInlineImageTag = 'img src="data:image/svg+xml;base64,\n%s"' % SVGText
    else:
...
def SetupAtomListToHighlight(Mol, DataLabel):
    ...
    if OptionsInfo["HighlightSMARTSAllMode"]:
        ...
        CanonicalDataLabel = DataLabel.lower()
        if CanonicalDataLabel not in OptionsInfo["HighlightSMARTSCanonicalDataLabelsMap"]:
            return HighlightAtomList
...
def GetBackgroundColorUsingHighlightValuesMode(DataLabel, DataValue, DataMap):
    ...
    CanonicalDataLabel = DataLabel.lower()
    if CanonicalDataLabel not in OptionsInfo["HighlightValuesCanonicalLabelsMap"]:
        return (BackgroundColor, BackgroundColorType)
...
def GetBackgroundColorUsingHighlightClassesMode(DataLabel, DataValue, DataMap):
    ...
    CanonicalDataLabel = DataLabel.lower()
    if CanonicalDataLabel not in OptionsInfo["HighlightClassesCanonicalLabelsMap"]:
        return (BackgroundColor, BackgroundColorType)
...
def GetBackgroundColorForHighlightingValue(DataLabel, DataValue, DataType, Criterion, CriterionValue):
    ...
    if re.match("^numeric$", DataType, re.I):
        if not MiscUtil.IsNumber(DataValue):
            MiscUtil.PrintWarning(
                "Ignoring data value, %s, for data label, %s, during numeric highlighting: It must be a number"
                % (DataValue, DataLabel)
            )
            return (BackgroundColor, BackgroundColorType)
...
def GetBackgroundColorUsingHighlightRangesMode(DataLabel, DataValue, DataMap):
    ...
    CanonicalDataLabel = DataLabel.lower()
    if CanonicalDataLabel not in OptionsInfo["HighlightRangesCanonicalLabelsMap"]:
        return (BackgroundColor, BackgroundColorType)
...
def ProcessHighlightSMARTSOption():
    ...
    OptionsInfo["HighlightSMARTSDelim"] = Options["--highlightSMARTSDelim"]
    ...
    HighlightSMARTSWords = HighlightSMARTS.split(OptionsInfo["HighlightSMARTSDelim"])

    if len(HighlightSMARTSWords) == 1:
        PatternMol = Chem.MolFromSmarts(HighlightSMARTS)
        if PatternMol is None:
            MiscUtil.PrintError(
                'The value specified, %s, using option "--highlightSMARTS" is not a valid SMARTS: Failed to create pattern molecule'
                % Options["--highlightSMARTS"]
            )
    ...
    if len(HighlightSMARTSWords) % 2:
        MiscUtil.PrintError(
            'The number of comma delimited paramater names and values, %d, specified using "--highlightSMARTS" option must be an even number.'
            % (len(HighlightSMARTSWords))
        )
    ...
    for Index in range(0, len(HighlightSMARTSWords), 2):
        ...
        if DataLabel in OptionsInfo["HighlightSMARTSDataLabelsMap"]:
            MiscUtil.PrintError(
                'The datalabel, %s, specified in pair, "%s, %s", using option "--highlightSMARTS" is not a valid: Multiple occurences of data label'
                % (DataLabel, DataLabel, SMARTSPattern)
            )
...
def ProcessHighlightValuesOption():
    ...
    if len(HighlightValuesWords) % 4:
        MiscUtil.PrintError(
            'The number of comma delimited paramater names and values, %d, specified using "--highlightValues" option must be a multiple of 4.'
            % (len(HighlightValuesWords))
        )
    ...
    for Index in range(0, len(HighlightValuesWords), 4):
        ...
        if not re.match("^(numeric|text|regex)$", DataType, re.I):
            MiscUtil.PrintError(
                'The data type, %s, specified in quratet "%s,%s,%s,%s", using "--highlightValues" option is not valid. Supported values: numeric, regex or text.'
                % (DataType, DataLabel, DataType, DataCriterion, DataValue)
            )

        if re.match("^regex$", DataType, re.I):
            if not re.match("^(eq|ne)$", DataCriterion, re.I):
                MiscUtil.PrintError(
                    'The data criterion, %s, specified in quratet "%s,%s,%s,%s", using "--highlightValues" option is not valid. Supported values: eq or ne'
                    % (DataType, DataLabel, DataType, DataCriterion, DataValue)
                )
        else:
            if not re.match("^(gt|lt|ge|le|eq|ne)$", DataCriterion, re.I):
                MiscUtil.PrintError(
                    'The data criterion, %s, specified in quratet "%s,%s,%s,%s", using "--highlightValues" option is not valid. Supported values: gt, lt, ge, le, eq, or  ne.'
                    % (DataType, DataLabel, DataType, DataCriterion, DataValue)
                )
        ...
        if re.match("^numeric$", DataType, re.I):
            if not MiscUtil.IsNumber(DataValue):
                MiscUtil.PrintError(
                    'The data value, %s, specified in quratet "%s,%s,%s,%s", using "--highlightValues" option is not valid. It must be a number for data type, %s'
                    % (DataType, DataLabel, DataType, DataCriterion, DataValue, DataType)
                )
            DataValue = float(DataValue)
        ...
        if DataLabel in OptionsInfo["HighlightValuesLabelsMap"]:
            MiscUtil.PrintError(
                'The data label, %s, specified in quratet "%s,%s,%s,%s", using "--highlightValues" option is not valid: Multiple occurences of data label'
                % (DataLabel, DataLabel, DataType, DataCriterion, DataValue)
            )
...
def ProcessHighlightValuesRangesOption():
    ...
    if len(HighlightRangesWords) % 6:
        MiscUtil.PrintError(
            'The number of comma delimited paramater names and values, %d, specified in sextet "%s" using "--highlightValuesRanges" option must be a multiple of 6.'
            % (len(HighlightRangesWords), HighlightRanges)
        )
    ...
    for Index in range(0, len(HighlightRangesWords), 6):
        ...
        SpecifiedSextet = "%s,%s,%s,%s,%s,%s" % (
            DataLabel,
            DataType,
            LowerBoundDataCriterion,
            LowerBoundDataValue,
            UpperBoundDataCriterion,
            UpperBoundDataValue,
        )
        ...
        if CanonicalDataLabel in OptionsInfo["HighlightValuesCanonicalLabelsMap"]:
            MiscUtil.PrintError(
                'The data label specified, %s, using option "--highlightRanges" has already been used in "--highlightValues" option'
                % DataLabel
            )

        if not re.match("^(numeric|text)$", DataType, re.I):
            MiscUtil.PrintError(
                'The data type, %s, specified in sextet "%s" using "--highlightValuesRanges" option is not valid. Supported values: numeric text.'
                % (DataType, SpecifiedSextet)
            )

        if not re.match("^(lt|le)$", LowerBoundDataCriterion, re.I):
            MiscUtil.PrintError(
                'The lower bound criterion, %s, specified in sextet "%s" using "--highlightValuesRanges" option is not valid. Supported values: lt or le.'
                % (LowerBoundDataCriterion, SpecifiedSextet)
            )

        if not re.match("^(gt|ge)$", UpperBoundDataCriterion, re.I):
            MiscUtil.PrintError(
                'The upper bound criterion, %s, specified in sextet "%s" using "--highlightValuesRanges" option is not valid. Supported values: gt or ge.'
                % (UpperBoundDataCriterion, SpecifiedSextet)
            )

        if re.match("^numeric$", DataType, re.I):
            if not MiscUtil.IsNumber(LowerBoundDataValue):
                MiscUtil.PrintError(
                    'The lower bound data value, %s, specified in sextet "%s", using "--highlightValuesRanges" option is not valid. It must be a number for "%s" data type.'
                    % (LowerBoundDataValue, SpecifiedSextet, DataType)
                )

            if not MiscUtil.IsNumber(UpperBoundDataValue):
                MiscUtil.PrintError(
                    'The upper bound data value, %s, specified in sextet "%s", using "--highlightValuesRanges" option is not valid. It must be a number for "%s"data type.'
                    % (UpperBoundDataValue, SpecifiedSextet, DataType)
                )

            if float(LowerBoundDataValue) >= float(UpperBoundDataValue):
                MiscUtil.PrintError(
                    'The lower bound data value, %s, must be less than upper bound value, %s, specified in sextet "%s" using "--highlightValuesRanges" option.'
                    % (LowerBoundDataValue, UpperBoundDataValue, SpecifiedSextet)
                )
            ...
            if LowerBoundDataValue >= UpperBoundDataValue:
                MiscUtil.PrintError(
                    'The lower bound data value, %s, must be less than upper bound value, %s, specified in sextet "%s", using "--highlightValuesRanges" option is not valid. It must be a number for data type, %s'
                    % (LowerBoundDataValue, UpperBoundDataValue, SpecifiedSextet, DataType)
                )

        # Track values...
        if DataLabel in OptionsInfo["HighlightRangesLabelsMap"]:
            MiscUtil.PrintError(
                'The data label, %s, specified in sextet "%s", using "--highlightValuesRanges" option is not valid. Multiple occurences of data label'
                % (DataLabel, SpecifiedSextet)
            )
...
def ProcessHighlightValuesClassesOption():
    ...
    if re.match("^RuleOf5$", HighlightClasses, re.I):
        ...
        if OptionsInfo["HighlightValues"] is not None:
            MiscUtil.PrintError(
                'The value specified, %s, using option "--highlightValuesClasses" is not allowed in conjunction with "--highlightValues" option.'
                % HighlightClasses
            )
        if OptionsInfo["HighlightRanges"] is not None:
            MiscUtil.PrintError(
                'The value specified, %s, using option "--highlightValuesClasses" is not allowed in conjunction with "--highlightRanges" option .'
                % HighlightClasses
            )
        ...
        MiscUtil.PrintError(
            'The value specified, %d, using option "--highlightValuesClasses" is not supported.' % HighlightClasses
        )
        return
    ...
    for Index in range(0, len(HighlightClassesWords), 4):
        ...
        if DataLabel in OptionsInfo["HighlightClassesLabelsMap"]:
            MiscUtil.PrintWarning(
                'Ignoring duplicate datalabel, %s, specified in highlighting values rule for class, %s, in "--highlightClassesValue" option...'
                % (DataLabel, HighlightClasses)
            )
            continue
...
def ProcessHighlightColorsOption():
    ...
    if len(HighlightColorsList) != 3:
        MiscUtil.PrintError(
            'The number of comma delimited paramater names and values, %d, specified using "--highlightColors" option must be 3.'
            % (len(HighlightColorsList))
        )

    ColorsType, Color1, Color2 = HighlightColorsList
    if not re.match("^(colorclass|colorspec)$", ColorsType, re.I):
        MiscUtil.PrintError(
            'The color type, %s, specified using "--highlightColors" option is not valid. Supported values: colorclass or colorspec.'
            % ColorsType
        )
...
def ProcessHighlightColorsRangesOption():
    ...
    if len(HighlightColorsList) != 4:
        MiscUtil.PrintError(
            'The number of comma delimited paramater names and values, %d, specified using "--highlightColorsRanges" option must be 4.'
            % (len(HighlightColorsList))
        )
    ...
    if not re.match("^(colorclass|colorspec)$", ColorsType, re.I):
        MiscUtil.PrintError(
            'The color type, %s, specified using "--highlightColorsRanges" option is not valid. Supported values: colorclass or colorspec.'
            % ColorsType
        )
...
def ProcessHighlightColorsRandomOption():
    ...
    if len(HighlightColorsList) <= 1:
        MiscUtil.PrintError(
            'The number of comma delimited paramater names and values, %d, specified using "--highlightColorsRandom" option must be > 1.'
            % (len(HighlightColorsList))
        )
    ...
    if not re.match("^(colorclass|colorspec)$", ColorsType, re.I):
        MiscUtil.PrintError(
            'The color type, %s, specified using "--highlightColorsRandim" option is not valid. Supported values: colorclass or colorspec.'
            % ColorsType
        )
...
def CheckOptionTableClassColorValues(OptionName, ColorsList):
    ...
    for Color in ColorsList:
        if Color not in TableClassColors:
            MiscUtil.PrintWarning(
                'The color class name, %s, specified using option "%s" appears to be a unknown name...'
                % (Color, OptionName)
            )
...
def ProcessOptions():
    ...
    OptionsInfo["InfileParams"] = MiscUtil.ProcessOptionInfileParameters(
        "--infileParams", Options["--infileParams"], OptionsInfo["Infile"]
    )
    ...
    if not re.match("^(numbers|simple|simple_numbers|full|full_numbers|simple_number)$", Options["--pagingType"], re.I):
        MiscUtil.PrintWarning(
            'The paging type name, %s, specified using option "--pagingType" appears to be a unknown type...'
            % (PagingType)
        )
    ...
    if re.match("vh$", Options["--scrollYSize"], re.I):
        ...
        if ScrollYSize <= 0:
            MiscUtil.PrintError(
                'The value specified, %s, for option "--scrollYSize" is not valid. Supported value: > 0 followed by "vh"'
                % Options["--scrollYSize"]
            )
        ...
        if re.match("^All$", Options["--tableStyle"], re.I):
            ...
            for Style in [Style for Style in TableStyle.split(",")]:
                if not re.match("^(table|table-striped|table-bordered|table-hover|table-dark|table-sm)$", Style, re.I):
                    MiscUtil.PrintWarning(
                        'The table style name, %s, specified using option "-t, --tableStyle" appears to be a unknown style...'
                        % (Style)
                    )
...
def ValidateOptions():
    ...
    if not re.match("^None$", Options["--alignmentSMARTS"], re.I):
        ...
        if PatternMol is None:
            MiscUtil.PrintError(
                'The value specified, %s, using option "--alignmentSMARTS" is not a valid SMARTS: Failed to create pattern molecule'
                % Options["--alignmentSMARTS"]
            )
    ...
    MiscUtil.ValidateOptionTextValue(
        "--highlightValuesClasses", Options["--highlightValuesClasses"], "RuleOf5 RuleOf3 DrugLike Random None"
    )
...
Usage:
    RDKitDrawMoleculesAndDataTable.py [--alignmentSMARTS <SMARTS>]
                             ...
                             [--headerStyle <text>] [--highlightSMARTS <SMARTS,...>] [--highlightSMARTSDelim <text>]
                             ...
                             [--molImageSize <width,height>] [--molImageEncoded <yes or no> ] [--overwrite]
...
Options:
    ...
    --highlightSMARTSDelim <text>  [default: ,]
        Delimiter for parsing SMARTS patterns specified using '--highlightSMARTS'
    ...
    --molImageEncoded <yes or no>  [default: yes]
        Base64 encode SVG image of a molecule for inline embedding in a HTML page.
        The inline SVG image may fail to display in browsers without encoding.
    -o, --outfile <outfile>
        Output file name.
    --overwrite
