...
def GenerateMacromolecularVisualization():
    ...
    for FileIndex in range(0, len(OptionsInfo["InfilesInfo"]["InfilesNames"])):
        ...
        # Setup trajectories views...
        if GetTrajectoriesStatus(FileIndex):
            WriteTrajectoriesView(OutFH, FileIndex, PyMOLObjectNames)
        ...
        for ChainID in SpecifiedChainsAndLigandsInfo["ChainIDs"]:
            ...
            for LigandID in SpecifiedChainsAndLigandsInfo["LigandIDs"][ChainID]:
                ...
                GenerateAndWritePMLForGroup(
                    OutFH,
                    PyMOLObjectNames["Ligands"][ChainID][LigandID]["ChainLigandGroup"],
                    PyMOLObjectNames["Ligands"][ChainID][LigandID]["ChainLigandGroupMembers"],
                    Enable,
                    Action,
                )

            # Setup docked poses views...
            if GetChainAloneDockedPosesStatus(FileIndex, ChainID):
                WriteChainDockedPosesView(OutFH, FileIndex, PyMOLObjectNames, ChainID)
...
def WriteComplexView(OutFH, FileIndex, PyMOLObjectNames, FirstComplex):
    ...
    if OptionsInfo["SurfaceComplex"]:
        ...
        PML = PyMOLUtil.SetupPMLForHydrophobicSurfaceView(
            PyMOLObjectNames["ComplexHydrophobicSurface"],
            PyMOLObjectNames["Complex"],
...
def WriteChainComplexViews(OutFH, FileIndex, PyMOLObjectNames, ChainID):
    ...
    if OptionsInfo["SurfaceChainComplex"]:
        # Setup hydrophobic surface...
        PML = PyMOLUtil.SetupPMLForHydrophobicSurfaceView(
            PyMOLObjectNames["Chains"][ChainID]["ChainComplexHydrophobicSurface"],
            ChainComplexName,
            ColorPalette=OptionsInfo["SurfaceColorPalette"],
            Enable=False,
        )
...
def WriteChainAloneViews(OutFH, FileIndex, PyMOLObjectNames, ChainID):
    ...
    WriteChainAloneBFactorViews(OutFH, FileIndex, PyMOLObjectNames, ChainID)
    ...
    if GetChainAloneContainsSurfacesStatus(FileIndex, ChainID):
        ...
        if GetChainAloneSurfaceChainElectrostaticsStatus(FileIndex, ChainID):
            ...
            WriteSurfaceElectrostaticsView(
                "Surface",
                OutFH,
                SelectionObjectName,
                ElectrostaticsGroupName,
                ElectrostaticsGroupMembers,
                DisplayAs="cartoon",
            )
        ...
        # Setup disulfide group...
        WriteChainAloneDisulfideBondsView(OutFH, FileIndex, PyMOLObjectNames, ChainID)

        # Setup salt bridges group...
        WriteChainAloneSaltBridgesView(OutFH, FileIndex, PyMOLObjectNames, ChainID)
...
def WriteChainLigandView(OutFH, FileIndex, PyMOLObjectNames, ChainID, LigandID):
    ...
    for GroupID in ["Ligand", "Pocket", "PocketSolvent", "PocketInorganic"]:
        ...
        if re.match("^Ligand$", GroupID, re.I):
            ...
            PML = PyMOLUtil.SetupPMLForLigandView(
                GroupTypeObjectName, ComplexName, LigandID, Enable=True, IgnoreHydrogens=OptionsInfo["IgnoreHydrogens"]
            )
            OutFH.write("%s\n" % PML)
            ...
            PML = PyMOLUtil.SetupPMLForLigandPocketView(
                GroupTypeObjectName,
                ComplexName,
                LigandName,
                OptionsInfo["PocketDistanceCutoff"],
                Enable=True,
                IgnoreHydrogens=OptionsInfo["IgnoreHydrogens"],
            )
            OutFH.write("%s\n" % PML)
            OutFH.write(
                """cmd.set("label_color", "%s", "%s")\n""" % (OptionsInfo["PocketLabelColor"], GroupTypeObjectName)
            )
            ...
            PML = PyMOLUtil.SetupPMLForLigandPocketSolventView(
                GroupTypeObjectName, ComplexName, LigandName, OptionsInfo["PocketDistanceCutoff"], Enable=True
            )
            OutFH.write("%s\n" % PML)
            ...
            PML = PyMOLUtil.SetupPMLForLigandPocketInorganicView(
                GroupTypeObjectName, ComplexName, LigandName, OptionsInfo["PocketDistanceCutoff"], Enable=True
            )
            OutFH.write("%s\n" % PML)
        ...
        if re.match("^(Pocket|PocketSolvent|PocketInorganic)$", GroupID, re.I):
            ...
            PML = PyMOLUtil.SetupPMLForPolarContactsView(
                PolarContactsName,
                LigandName,
                GroupTypeObjectName,
                Enable=False,
                Color=PolarContactsColor,
                Cutoff=OptionsInfo["PocketContactsCutoff"],
            )
            OutFH.write("\n%s\n" % PML)

            OutFH.write("""cmd.set("label_color", "%s", "%s")\n""" % (PolarContactsColor, PolarContactsName))

        if re.match("^PocketInorganic$", GroupID, re.I):
            # Setup pi cation contacts...
            PiCationContactsID = "%sPiCationContacts" % (GroupID)
            PiCationContactsName = PyMOLObjectNames["Ligands"][ChainID][LigandID][PiCationContactsID]
            PiCationContactsColor = OptionsInfo["PocketContactsInorganicPiCationColor"]

            PML = PyMOLUtil.SetupPMLForPiCationContactsView(
                PiCationContactsName,
                LigandName,
                GroupTypeObjectName,
                Enable=False,
                Color=PiCationContactsColor,
                Cutoff=OptionsInfo["PocketContactsCutoff"],
            )
            OutFH.write("\n%s\n" % PML)
        ...
        if re.match("^Pocket$", GroupID, re.I):
            ...
            PML = PyMOLUtil.SetupPMLForHydrophobicContactsView(
                HydrophobicContactsName,
                LigandName,
                GroupTypeObjectName,
                Enable=False,
                Color=HydrophobicContactsColor,
                Cutoff=OptionsInfo["PocketContactsCutoff"],
            )
            OutFH.write("\n%s\n" % PML)
            OutFH.write(
            ...
            # Setup pi pi contacts...
            PiPiContactsID = "%sPiPiContacts" % (GroupID)
            PiPiContactsName = PyMOLObjectNames["Ligands"][ChainID][LigandID][PiPiContactsID]
            PiPiContactsColor = OptionsInfo["PocketContactsLigandPiPiColor"]

            PML = PyMOLUtil.SetupPMLForPiPiContactsView(
                PiPiContactsName,
                LigandName,
                GroupTypeObjectName,
                Enable=False,
                Color=PiPiContactsColor,
                Cutoff=OptionsInfo["PocketContactsCutoff"],
            )
            OutFH.write("\n%s\n" % PML)
            ...
            # Setup halogen contacts...
            HalogenContactsID = "%sHalogenContacts" % (GroupID)
            HalogenContactsName = PyMOLObjectNames["Ligands"][ChainID][LigandID][HalogenContactsID]
            HalogenContactsColor = OptionsInfo["PocketContactsLigandHalogenColor"]

            PML = PyMOLUtil.SetupPMLForHalogenContactsView(
                HalogenContactsName,
                LigandName,
                GroupTypeObjectName,
                Enable=False,
                Color=HalogenContactsColor,
                Cutoff=OptionsInfo["PocketContactsCutoff"],
            )
            OutFH.write("\n%s\n" % PML)
            OutFH.write("""cmd.set("label_color", "%s", "%s")\n""" % (HalogenContactsColor, HalogenContactsName))

            # Setup pocket selections...
            WritePocketSelectionsView(OutFH, FileIndex, PyMOLObjectNames, ChainID, LigandID, GroupTypeObjectID)
...
def WriteChainDockedPosesView(OutFH, FileIndex, PyMOLObjectNames, ChainID):
    """Write out PML for viewing docked poses for input files in a chain."""

    if not GetChainAloneDockedPosesStatus(FileIndex, ChainID):
        return

    SpecifiedChainsAndLigandsInfo = OptionsInfo["InfilesInfo"]["SpecifiedChainsAndLigandsInfo"][FileIndex]
    for InputFileIndex, InputFile in enumerate(SpecifiedChainsAndLigandsInfo["DockedPosesInputFiles"][ChainID]):
        WriteChainDockedPosesViewForInputFile(OutFH, FileIndex, PyMOLObjectNames, ChainID, InputFileIndex)

    GenerateAndWritePMLForGroup(
        OutFH,
        PyMOLObjectNames["Chains"][ChainID]["DockedPosesGroup"],
        PyMOLObjectNames["Chains"][ChainID]["DockedPosesGroupMembers"],
        False,
        "close",
    )


def WriteChainDockedPosesViewForInputFile(OutFH, PDBFileIndex, PyMOLObjectNames, ChainID, InputFileIndex):
    """Write out PML for viewing docked poses for an input file in a chain."""

    DockedPosesInfo = OptionsInfo["DockedPosesInfo"]
    DockedPosesDistanceContactsInfo = OptionsInfo["DockedPosesDistanceContactsInfo"]

    LigandID = DockedPosesInfo["LigandID"][PDBFileIndex]
    UseInputFileAsLigandID = DockedPosesInfo["UseInputFileAsLigandID"][PDBFileIndex]
    InputFile = DockedPosesInfo["InputFiles"][PDBFileIndex][InputFileIndex]
    InputFileID = DockedPosesInfo["InputFilesIDs"][PDBFileIndex][InputFileIndex]
    ...
    # Setup poses view...
    PosesName = PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID]["Poses"]
    PML = PyMOLUtil.SetupPMLForLigandsInputFileView(
        PosesName, InputFile, Enable=True, IgnoreHydrogens=OptionsInfo["IgnoreHydrogens"]
    )
    OutFH.write("%s\n" % PML)

    for GroupID in ["Pocket", "PocketSolvent", "PocketInorganic"]:
        ComplexName = PyMOLObjectNames["Chains"][ChainID]["ChainComplex"]
        LigandName = PosesName if UseInputFileAsLigandID else PyMOLObjectNames["Ligands"][ChainID][LigandID]["Ligand"]

        # Setup  pocket object...
        PocketID = "%sPocket" % (GroupID)
        PocketName = PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID][PocketID]
        ...
        if re.match("^Pocket$", GroupID, re.I):
            ...
            PML = PyMOLUtil.SetupPMLForLigandPocketSolventView(
                PocketName, ComplexName, LigandName, OptionsInfo["PocketDistanceCutoff"], Enable=True
            )
            OutFH.write("%s\n" % PML)
        elif re.match("^PocketInorganic$", GroupID, re.I):
            PML = PyMOLUtil.SetupPMLForLigandPocketInorganicView(
        ...
        if DockedPosesInfo["DistanceContacts"] and re.match("^Pocket$", GroupID, re.I):
            # Setup distance contacts...
            for ContactIndex, ContactID in enumerate(DockedPosesDistanceContactsInfo["ContactIDs"]):
                ContactCutoff = DockedPosesDistanceContactsInfo["ContactCutoff"][ContactID]

                DistanceContactID = "%sPocketDistanceContacts%s" % (GroupID, ContactID)
                DistanceContactName = PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID][DistanceContactID]

                DistanceContactsColor = OptionsInfo["DockedPosesDistanceContactsColor"]
                EnableContact = True if ContactIndex == 0 else False
                PML = PyMOLUtil.SetupPMLForDistanceContactsView(
                    DistanceContactName,
                    PosesName,
                    PocketName,
                    Enable=EnableContact,
                    Color=DistanceContactsColor,
            ...
            # Setup distance conatcts group...
            DistanceContactGroupNameID = "%sDistanceContactsGroup" % GroupID
            DistanceContactGroupMembersID = "%sDistanceContactsGroupMembers" % GroupID

            GenerateAndWritePMLForGroup(
                OutFH,
                PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID][DistanceContactGroupNameID],
                PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID][DistanceContactGroupMembersID],
                True,
                "open",
            )

        # Setup polar contacts...
        if re.match("^(Pocket|PocketSolvent|PocketInorganic)$", GroupID, re.I):
            PolarContactsID = "%sPolarContacts" % (GroupID)
            PolarContactsName = PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID][PolarContactsID]
        ...
        if re.match("^PocketInorganic$", GroupID, re.I):
            # Setup pi cation contacts...
            PiCationContactsID = "%sPiCationContacts" % (GroupID)
            PiCationContactsName = PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID][PiCationContactsID]
            PiCationContactsColor = OptionsInfo["PocketContactsInorganicPiCationColor"]
        ...
        if re.match("^Pocket$", GroupID, re.I):
            # Setup hydrophobic contacts...
            HydrophobicContactsID = "%sHydrophobicContacts" % (GroupID)
            HydrophobicContactsName = PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID][
                HydrophobicContactsID
            ]
            HydrophobicContactsColor = OptionsInfo["PocketContactsLigandHydrophobicColor"]
            ...
            # Setup pi pi contacts...
            PiPiContactsID = "%sPiPiContacts" % (GroupID)
            PiPiContactsName = PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID][PiPiContactsID]
            PiPiContactsColor = OptionsInfo["PocketContactsLigandPiPiColor"]
            ...
            # Setup halogen contacts...
            HalogenContactsID = "%sHalogenContacts" % (GroupID)
            HalogenContactsName = PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID][HalogenContactsID]
            HalogenContactsColor = OptionsInfo["PocketContactsLigandHalogenColor"]
        ...
        # Setup group for an input file...
        GroupNameID = "%sGroup" % (GroupID)
        GroupMembersID = "%sGroupMembers" % (GroupID)
        Enable = True if re.match("^Pocket$", GroupID, re.I) else False
        Action = "open" if re.match("^Pocket$", GroupID, re.I) else "close"

        GenerateAndWritePMLForGroup(
            OutFH,
            PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID][GroupNameID],
            PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID][GroupMembersID],
            Enable,
            Action,
        )

    # Setup docked poses group for an input file...
    Action = "open" if InputFileIndex == 0 else "close"
    Enable = True if InputFileIndex == 0 else False
    GenerateAndWritePMLForGroup(
        OutFH,
        PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID]["DockedPosesGroupName"],
        PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID]["DockedPosesGroupMembers"],
        Enable,
        Action,
    )


def WritePocketSelectionsView(OutFH, FileIndex, PyMOLObjectNames, ChainID, LigandID, PocketObjectID):
    """Write out PML for viewing selections for a lgand pocket."""

    if not GetPocketContainsSelectionsStatus(FileIndex, ChainID, LigandID):
        return

    PocketObjectName = PyMOLObjectNames["Ligands"][ChainID][LigandID][PocketObjectID]
    SelectionsGroupIDPrefix = "PocketSelectionsGroup"

    for Index in range(0, len(OptionsInfo["PocketChainSelectionsInfo"]["Names"])):
        SelectionName = OptionsInfo["PocketChainSelectionsInfo"]["Names"][Index]
        Selection = OptionsInfo["PocketChainSelectionsInfo"]["Selections"][Index]
        ...
        SelectionObjectName = PyMOLObjectNames["Ligands"][ChainID][LigandID][SelectionObjectID]
        SelectionCmd = "(%s and (%s))" % (PocketObjectName, Selection)
        PML = PyMOLUtil.SetupPMLForSelectionDisplayView(
            SelectionObjectName,
            SelectionCmd,
            OptionsInfo["SelectionsPocketStyle"],
            Enable=True,
            IgnoreHydrogens=OptionsInfo["IgnoreHydrogens"],
        )
        OutFH.write("\n%s\n" % PML)

        if GetPocketSelectionSurfaceChainStatus(FileIndex, ChainID, LigandID):
            ...
            HydrophobicChargeSurfaceID = "%s%sSurfaceHydrophobicityCharge" % (
            ...
            GenerateAndWritePMLForGroup(
                OutFH,
                PyMOLObjectNames["Ligands"][ChainID][LigandID][SurfaceGroupID],
        ...
        GenerateAndWritePMLForGroup(
            OutFH,
            PyMOLObjectNames["Ligands"][ChainID][LigandID][SelectionsNameGroupID],
            PyMOLObjectNames["Ligands"][ChainID][LigandID][SelectionsNameGroupMembersID],
            True,
            "open",
        )

    # Setup a group for selections...
    SelectionsGroupID = "%s" % (SelectionsGroupIDPrefix)
    SelectionsGroupMembersID = "%sGroupMembers" % (SelectionsGroupIDPrefix)
    GenerateAndWritePMLForGroup(
        OutFH,
        PyMOLObjectNames["Ligands"][ChainID][LigandID][SelectionsGroupID],
        PyMOLObjectNames["Ligands"][ChainID][LigandID][SelectionsGroupMembersID],
        True,
        "close",
    )
...
def WritePocketSurfacesTypesView(OutFH, FileIndex, PyMOLObjectNames, ChainID, LigandID, PocketObjectID):
    ...
    for SubGroupType in ["Surface", "Cavity"]:
        ...
        SurfaceName = PyMOLObjectNames["Ligands"][ChainID][LigandID][SurfaceID]
        PML = PyMOLUtil.SetupPMLForSurfaceView(
            SurfaceName, PocketObjectName, Enable=False, Color=OptionsInfo["SurfaceColor"], DisplayAs=DisplayStyle
        )
        OutFH.write("\n%s\n" % PML)
        ...
        if GetPocketSurfaceChainStatus(FileIndex, ChainID, LigandID):
            ...
            if GetPocketSurfaceChainElectrostaticsStatus(FileIndex, ChainID, LigandID):
                ...
                ElectrostaticsGroupMembers = PyMOLObjectNames["Ligands"][ChainID][LigandID][
                    ElectrostaticsGroupMembersID
                ]
                WriteSurfaceElectrostaticsView(
                    SubGroupType,
                    OutFH,
                    ...
                    ElectrostaticsGroupMembers,
                    DisplayAs=DisplayStyle,
                    SurfaceCavityMode=CavityMode,
                )

            # Setup surfaces sub group...
            GenerateAndWritePMLForGroup(
                OutFH,
                PyMOLObjectNames["Ligands"][ChainID][LigandID][SurfacesSubGroupID],
    ...
    GenerateAndWritePMLForGroup(
        OutFH,
        PyMOLObjectNames["Ligands"][ChainID][LigandID][SurfacesGroupID],
...
def WritePocketResidueTypesView(OutFH, FileIndex, PyMOLObjectNames, ChainID, LigandID, PocketObjectID):
    ...
    for SubGroupType in ["Aromatic", "Hydrophobic", "Polar", "Positively_Charged", "Negatively_Charged", "Other"]:
        ...
        WriteResidueTypesResiduesAndSurfaceView(
            OutFH,
            PocketObjectName,
            ResiduesObjectName,
            ResiduesSurfaceObjectName,
            ResiduesColor,
            ResiduesNames,
            NegateResidueNames,
        )
        ...
        GenerateAndWritePMLForGroup(
            OutFH,
            PyMOLObjectNames["Ligands"][ChainID][LigandID][ResiduesSubGroupID],
    ...
    GenerateAndWritePMLForGroup(
        OutFH,
        PyMOLObjectNames["Ligands"][ChainID][LigandID][ResiduesGroupID],
...
def WriteChainAloneChainSelectionsView(OutFH, FileIndex, PyMOLObjectNames, ChainID):
    ...
    for Index in range(0, len(OptionsInfo["ChainSelectionsInfo"]["Names"])):
        ...
        PML = PyMOLUtil.SetupPMLForSelectionDisplayView(
            SelectionObjectName,
            SelectionCmd,
            OptionsInfo["SelectionsChainStyle"],
            Enable=True,
            IgnoreHydrogens=OptionsInfo["IgnoreHydrogens"],
        )
        OutFH.write("\n%s\n" % PML)
        ...
        if GetChainAloneContainsChainSelectionSurfacesStatus(FileIndex, ChainID):
            ...
            GenerateAndWritePMLForGroup(
                OutFH,
                PyMOLObjectNames["Chains"][ChainID][SurfaceGroupID],
                PyMOLObjectNames["Chains"][ChainID][SurfaceGroupMembersID],
                True,
                "close",
            )
        ...
        GenerateAndWritePMLForGroup(
            OutFH,
            PyMOLObjectNames["Chains"][ChainID][SelectionsNameGroupID],
    ...
    GenerateAndWritePMLForGroup(
        OutFH,
        PyMOLObjectNames["Chains"][ChainID][SelectionsGroupID],
...
def WriteChainAloneResidueTypesView(OutFH, FileIndex, PyMOLObjectNames, ChainID):
    ...
    for SubGroupType in ["Aromatic", "Hydrophobic", "Polar", "Positively_Charged", "Negatively_Charged", "Other"]:
        ...
        GenerateAndWritePMLForGroup(
            OutFH,
            PyMOLObjectNames["Chains"][ChainID][ResiduesSubGroupID],
    ...
    GenerateAndWritePMLForGroup(
        OutFH,
        PyMOLObjectNames["Chains"][ChainID]["ChainAloneResiduesGroup"],
        PyMOLObjectNames["Chains"][ChainID]["ChainAloneResiduesGroupMembers"],
        False,
        "close",
    )


def WriteChainAloneBFactorViews(OutFH, FileIndex, PyMOLObjectNames, ChainID):
    """Write out PML for viewing B factor values for a chain."""

    if not GetChainAloneBFactorStatus(FileIndex, ChainID):
        return

    ChainName = PyMOLObjectNames["Chains"][ChainID]["ChainAlone"]

    # Setup cartoon...
    Name = PyMOLObjectNames["Chains"][ChainID]["ChainAloneBFactorCartoon"]
    PML = PyMOLUtil.SetupPMLForBFactorCartoonView(
        Name, ChainName, ColorPalette=OptionsInfo["BFactorColorPalette"], Enable=False
    )
    OutFH.write("\n%s\n" % PML)

    # Setup putty...
    Name = PyMOLObjectNames["Chains"][ChainID]["ChainAloneBFactorPutty"]
    PML = PyMOLUtil.SetupPMLForBFactorPuttyView(
        Name, ChainName, ColorPalette=OptionsInfo["BFactorColorPalette"], Enable=True
    )
    OutFH.write("\n%s\n" % PML)

    # Setup B factor group...
    GenerateAndWritePMLForGroup(
        OutFH,
        PyMOLObjectNames["Chains"][ChainID]["ChainAloneBFactorGroup"],
        PyMOLObjectNames["Chains"][ChainID]["ChainAloneBFactorGroupMembers"],
        False,
        "close",
    )


def WriteChainAloneDisulfideBondsView(OutFH, FileIndex, PyMOLObjectNames, ChainID):
    """Write out PML for viewing disulfide bonds for a chain."""

    if not GetChainAloneDisulfideBondsStatus(FileIndex, ChainID):
        return

    ChainName = PyMOLObjectNames["Chains"][ChainID]["ChainAlone"]
    Name = PyMOLObjectNames["Chains"][ChainID]["ChainAloneDisulfideBondsResidues"]

    PML = PyMOLUtil.SetupPMLForDisulfideBondsView(Name, ChainName, "sticks", Enable=True)
    OutFH.write("\n%s\n" % PML)

    # Setup disulfide bonds group...
    GenerateAndWritePMLForGroup(
        OutFH,
        PyMOLObjectNames["Chains"][ChainID]["ChainAloneDisulfideBondsGroup"],
        PyMOLObjectNames["Chains"][ChainID]["ChainAloneDisulfideBondsGroupMembers"],
        True,
        "close",
    )


def WriteChainAloneSaltBridgesView(OutFH, FileIndex, PyMOLObjectNames, ChainID):
    """Write out PML for viewing salt bridges for a chain."""

    if not GetChainAloneSaltBridgesStatus(FileIndex, ChainID):
        return

    ChainName = PyMOLObjectNames["Chains"][ChainID]["ChainAlone"]

    # Seup salt bridges resiudes group...
    PositivelyChargedResiduesName = PyMOLObjectNames["Chains"][ChainID][
        "ChainAloneSaltBridgesResiduesPositivelyCharged"
    ]
    PML = PyMOLUtil.SetupPMLForSaltBridgesResiduesView(
        PositivelyChargedResiduesName,
        ChainName,
        OptionsInfo["SaltBridgesChainResiduesInfo"]["Positively_Charged"],
        "lines",
        Enable=True,
    )
    OutFH.write("\n%s\n" % PML)

    NegativelyChargedResiduesName = PyMOLObjectNames["Chains"][ChainID][
        "ChainAloneSaltBridgesResiduesNegativelyCharged"
    ]
    PML = PyMOLUtil.SetupPMLForSaltBridgesResiduesView(
        NegativelyChargedResiduesName,
        ChainName,
        OptionsInfo["SaltBridgesChainResiduesInfo"]["Negatively_Charged"],
        "lines",
        Enable=True,
    )
    OutFH.write("\n%s\n" % PML)
    ...
    GenerateAndWritePMLForGroup(
        OutFH,
        PyMOLObjectNames["Chains"][ChainID]["ChainAloneSaltBridgesResiduesGroup"],
        PyMOLObjectNames["Chains"][ChainID]["ChainAloneSaltBridgesResiduesGroupMembers"],
        True,
        "open",
    )

    # Setup salt bridges contacts...
    ContactsColor = OptionsInfo["SaltBridgesChainContactsColor"]
    ContactsName = PyMOLObjectNames["Chains"][ChainID]["ChainAloneSaltBridgesContacts"]
    PML = PyMOLUtil.SetupPMLForPolarContactsView(
        ContactsName,
        PositivelyChargedResiduesName,
        NegativelyChargedResiduesName,
        Enable=True,
        Color=ContactsColor,
        Cutoff=OptionsInfo["SaltBridgesChainCutoff"],
    )
    OutFH.write("\n%s\n" % PML)
    ...
    GenerateAndWritePMLForGroup(
        OutFH,
        PyMOLObjectNames["Chains"][ChainID]["ChainAloneSaltBridgesGroup"],
        PyMOLObjectNames["Chains"][ChainID]["ChainAloneSaltBridgesGroupMembers"],
        False,
        "close",
    )


def WriteResidueTypesResiduesAndSurfaceView(
    OutFH, SelectionObjectName, Name, SurfaceName, ResiduesColor, ResiduesNames, NegateResidueNames
):
    ...
    # Setup residues...
    PML = PyMOLUtil.SetupPMLForSelectionDisplayView(
        Name, Selection, "lines", ResiduesColor, True, IgnoreHydrogens=OptionsInfo["IgnoreHydrogens"]
    )
    OutFH.write("\n%s\n" % PML)

    # Setup surface...
    PML = PyMOLUtil.SetupPMLForSelectionDisplayView(
        SurfaceName, Selection, "surface", ResiduesColor, True, IgnoreHydrogens=OptionsInfo["IgnoreHydrogens"]
    )
    OutFH.write("\n%s\n" % PML)


def WriteSurfaceElectrostaticsView(
    Mode,
    OutFH,
    SelectionObjectName,
    ElectrostaticsGroupName,
...
def WriteTrajectoriesView(OutFH, FileIndex, PyMOLObjectNames):
    """Write out PML for viewing trajectories for a PDB file."""

    if not GetTrajectoriesStatus(FileIndex):
        return

    # Setup topology view...
    Name = PyMOLObjectNames["Trajectories"]["Topology"]

    PMLCmds = []
    PMLCmds.append("""cmd.load("%s", "%s")""" % (OptionsInfo["InfilesInfo"]["InfilesNames"][FileIndex], Name))
    ...
    PMLCmds.append(PyMOLUtil.SetupPMLForEnableDisable(Name, Enable=False))

    PML = "\n".join(PMLCmds)
    OutFH.write("\n%s\n" % PML)

    # Write view for each trajectory file...
    for TrajFileIndex, TrajFile in enumerate(OptionsInfo["TrajectoriesInfo"]["TrajFiles"][FileIndex]):
        WriteTrajectoryViewTrajectoryFile(OutFH, FileIndex, PyMOLObjectNames, TrajFileIndex)

    GenerateAndWritePMLForGroup(
        OutFH,
        PyMOLObjectNames["Trajectories"]["TrajectoriesGroup"],
        PyMOLObjectNames["Trajectories"]["TrajectoriesGroupMembers"],
        False,
        "close",
    )


def WriteTrajectoryViewTrajectoryFile(OutFH, PDBFileIndex, PyMOLObjectNames, TrajFileIndex):
    """Write out PML for viewing a trajectory file."""

    TrajFile = OptionsInfo["TrajectoriesInfo"]["TrajFiles"][PDBFileIndex][TrajFileIndex]
    TrajFileID = OptionsInfo["TrajectoriesInfo"]["TrajFilesIDs"][PDBFileIndex][TrajFileIndex]

    OutFH.write("""\n""\n"Setting up views for trajectory file ID %s..."\n""\n""" % (TrajFileID))

    # Setup trajectory view...
    Name = PyMOLObjectNames["Trajectories"][TrajFileID]["Trajectory"]
    PDBFile = OptionsInfo["InfilesInfo"]["InfilesNames"][PDBFileIndex]

    PMLCmds = []
    PMLCmds.append("""cmd.load("%s", "%s")""" % (PDBFile, Name))
    PMLCmds.append("""cmd.load_traj("%s", "%s", state = 1)""" % (TrajFile, Name))
    PMLCmds.append("""cmd.show("cartoon", "%s")""" % (Name))
    PMLCmds.append("""util.cba(33, "%s", _self = cmd)""" % (Name))
    PMLCmds.append("""cmd.show("sticks", "(organic and (%s))")""" % (Name))
    PMLCmds.append(PyMOLUtil.SetupPMLForEnableDisable(Name, Enable=True))

    PML = "\n".join(PMLCmds)
    OutFH.write("%s\n" % PML)

    # Setup group for a trajectory file...
    Action = "open" if TrajFileIndex == 0 else "close"
    Enable = True if TrajFileIndex == 0 else False
    GenerateAndWritePMLForGroup(
        OutFH,
        PyMOLObjectNames["Trajectories"][TrajFileID]["TrajectoryGroup"],
        PyMOLObjectNames["Trajectories"][TrajFileID]["TrajectoryGroupMembers"],
        Enable,
        Action,
    )
...
def DeleteEmptyPyMOLObjects(OutFH, FileIndex, PyMOLObjectNames):
    ...
    for ChainID in SpecifiedChainsAndLigandsInfo["ChainIDs"]:
        ...
        # Delete disulfide bonds objects...
        DeleteEmptyChainDisulfideBondsObjects(OutFH, FileIndex, PyMOLObjectNames, ChainID)

        # Delete salt bridges objects...
        DeleteEmptyChainSaltBridgesObjects(OutFH, FileIndex, PyMOLObjectNames, ChainID)

        # Delete ligand objects...
        for LigandID in SpecifiedChainsAndLigandsInfo["LigandIDs"][ChainID]:
            ...
            for GroupID in ["Pocket", "PocketSolvent", "PocketInorganic"]:
                ...
                if re.match("^Pocket$", GroupID, re.I):
                    DeleteEmptyPocketSelectionsObjects(
                        OutFH, FileIndex, PyMOLObjectNames, ChainID, LigandID, GroupTypeObjectID
                    )
                    DeleteEmptyPocketResidueTypesObjects(
                        OutFH, FileIndex, PyMOLObjectNames, ChainID, LigandID, GroupTypeObjectID
                    )

        # Delete docked poses objects...
        DeleteEmptyChainDockedPosesObjects(OutFH, FileIndex, PyMOLObjectNames, ChainID)
...
def DeleteEmptyChainSelectionsObjects(OutFH, FileIndex, PyMOLObjectNames, ChainID):
    ...
    for SelectionName in OptionsInfo["ChainSelectionsInfo"]["Names"]:
        ...
        if GetChainAloneContainsChainSelectionSurfacesStatus(FileIndex, ChainID):
            SurfaceGroupID = "%s%sSurfaceGroup" % (SelectionsGroupIDPrefix, SelectionNameGroupID)
            SurfaceGroupMembersID = "%s%sSurfaceGroupMembers" % (SelectionsGroupIDPrefix, SelectionNameGroupID)
            WritePMLToCheckAndDeleteEmptyObjects(
                OutFH,
                ",".join(PyMOLObjectNames["Chains"][ChainID][SurfaceGroupMembersID]),
                PyMOLObjectNames["Chains"][ChainID][SurfaceGroupID],
            )
        ...
        WritePMLToCheckAndDeleteEmptyObjects(
            OutFH,
            PyMOLObjectNames["Chains"][ChainID][SelectionObjectID],
            PyMOLObjectNames["Chains"][ChainID][SelectionsGroupID],
        )
...
def DeleteEmptyChainDisulfideBondsObjects(OutFH, FileIndex, PyMOLObjectNames, ChainID):
    """Delete empty chain disulfide bonds objects."""
    ...
    WritePMLToCheckAndDeleteEmptyObjects(
        OutFH,
        PyMOLObjectNames["Chains"][ChainID]["ChainAloneDisulfideBondsResidues"],
        PyMOLObjectNames["Chains"][ChainID]["ChainAloneDisulfideBondsGroup"],
    )


def DeleteEmptyChainSaltBridgesObjects(OutFH, FileIndex, PyMOLObjectNames, ChainID):
    """Delete empty chain salt bridges objects."""

    if not GetChainAloneSaltBridgesStatus(FileIndex, ChainID):
        return

    Names = []
    Names.append(PyMOLObjectNames["Chains"][ChainID]["ChainAloneSaltBridgesResiduesPositivelyCharged"])
    Names.append(PyMOLObjectNames["Chains"][ChainID]["ChainAloneSaltBridgesResiduesNegativelyCharged"])

    WritePMLToCheckAndDeleteEmptyObjects(
        OutFH, ",".join(Names), PyMOLObjectNames["Chains"][ChainID]["ChainAloneSaltBridgesGroup"]
    )


def DeleteEmptyPocketSelectionsObjects(OutFH, FileIndex, PyMOLObjectNames, ChainID, LigandID, PocketObjectID):
    """Delete empty pocket selection objects."""
    ...
    for SelectionName in OptionsInfo["PocketChainSelectionsInfo"]["Names"]:
        SelectionNameGroupID = SelectionName
        ...
        if GetPocketSelectionSurfaceChainStatus(FileIndex, ChainID, LigandID):
            ...
            WritePMLToCheckAndDeleteEmptyObjects(
                OutFH,
                ",".join(PyMOLObjectNames["Ligands"][ChainID][LigandID][SurfaceGroupMembersID]),
                PyMOLObjectNames["Ligands"][ChainID][LigandID][SurfaceGroupID],
            )
        ...
        WritePMLToCheckAndDeleteEmptyObjects(
            OutFH,
            PyMOLObjectNames["Ligands"][ChainID][LigandID][SelectionObjectID],
            PyMOLObjectNames["Ligands"][ChainID][LigandID][SelectionsGroupID],
        )
...
def DeleteEmptyChainDockedPosesObjects(OutFH, PDBFileIndex, PyMOLObjectNames, ChainID):
    """Delete empty chain docked poses objest."""

    if not GetChainAloneDockedPosesStatus(PDBFileIndex, ChainID):
        return

    SpecifiedChainsAndLigandsInfo = OptionsInfo["InfilesInfo"]["SpecifiedChainsAndLigandsInfo"][PDBFileIndex]
    DockedPosesInfo = OptionsInfo["DockedPosesInfo"]
    ...
    for InputFileIndex, InputFile in enumerate(SpecifiedChainsAndLigandsInfo["DockedPosesInputFiles"][ChainID]):
        ...
        for GroupID in ["Pocket", "PocketSolvent", "PocketInorganic"]:
            GroupNameID = "%sGroup" % (GroupID)
            GroupName = PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID][GroupNameID]

            GroupTypeObjectID = "%sPocket" % (GroupID)
...
def SetupPyMOLObjectNames(FileIndex):
    ...
    PyMOLObjectNames["DockedPosesInputFile"] = {}
    PyMOLObjectNames["Trajectories"] = {}

    # Setup groups and objects for complex...
    SetupPyMOLObjectNamesForComplex(FileIndex, PyMOLObjectNames)

    # Setup groups and object for trajectories...
    if GetTrajectoriesStatus(FileIndex):
        SetupPyMOLObjectNamesForTrajectories(FileIndex, PyMOLObjectNames)
    ...
    for ChainID in SpecifiedChainsAndLigandsInfo["ChainIDs"]:
        ...
        # Setup group and objects for docked poses...
        if GetChainAloneDockedPosesStatus(FileIndex, ChainID):
            SetupPyMOLObjectNamesForDockedPoses(FileIndex, PyMOLObjectNames, ChainID)
...
def SetupPyMOLObjectNamesForTrajectories(FileIndex, PyMOLObjectNames):
    """Setup groups and objects for a PDB file."""

    if not GetTrajectoriesStatus(FileIndex):
        return

    # Setup a group for trajectories....
    TrajectoriesGroupName = "%s.Trajectories" % PyMOLObjectNames["PDBGroup"]
    PyMOLObjectNames["Trajectories"]["TrajectoriesGroup"] = TrajectoriesGroupName
    PyMOLObjectNames["PDBGroupMembers"].append(TrajectoriesGroupName)

    PyMOLObjectNames["Trajectories"]["TrajectoriesGroupMembers"] = []

    # Setup a topolgy object...
    TopologyName = "%s.Topology" % TrajectoriesGroupName
    PyMOLObjectNames["Trajectories"]["Topology"] = TopologyName
    PyMOLObjectNames["Trajectories"]["TrajectoriesGroupMembers"].append(TopologyName)

    # Setup trajectory objects...
    for TrajFileIndex, TrajFile in enumerate(OptionsInfo["TrajectoriesInfo"]["TrajFiles"][FileIndex]):
        SetupPyMOLObjectNamesForTrajectoryFile(FileIndex, PyMOLObjectNames, TrajFileIndex)


def SetupPyMOLObjectNamesForTrajectoryFile(PDBFileIndex, PyMOLObjectNames, TrajFileIndex):
    """Setup groups and objest for a trajectory file for a PDB file."""

    TrajFileID = OptionsInfo["TrajectoriesInfo"]["TrajFilesIDs"][PDBFileIndex][TrajFileIndex]

    PyMOLObjectNames["Trajectories"][TrajFileID] = {}

    TrajectoriesGroupName = PyMOLObjectNames["Trajectories"]["TrajectoriesGroup"]

    # Setup a trajectories level trajectory file group...
    TrajectoryFileGroupName = "%s.%s" % (TrajectoriesGroupName, TrajFileID)
    PyMOLObjectNames["Trajectories"][TrajFileID]["TrajectoryGroup"] = TrajectoryFileGroupName
    PyMOLObjectNames["Trajectories"]["TrajectoriesGroupMembers"].append(TrajectoryFileGroupName)

    PyMOLObjectNames["Trajectories"][TrajFileID]["TrajectoryGroupMembers"] = []

    # Setup object for a trajectory file...
    TrajectoryName = "%s.Trajectory" % (TrajectoryFileGroupName)
    PyMOLObjectNames["Trajectories"][TrajFileID]["Trajectory"] = TrajectoryName
    PyMOLObjectNames["Trajectories"][TrajFileID]["TrajectoryGroupMembers"].append(TrajectoryName)
...
def SetupPyMOLObjectNamesForChain(FileIndex, PyMOLObjectNames, ChainID):
    ...
    if GetChainAloneBFactorStatus(FileIndex, ChainID):
        # Setup B factor group and add it to chain alone group...
        BFactorGroupName = "%s.BFactor" % (ChainAloneGroupName)
        PyMOLObjectNames["Chains"][ChainID]["ChainAloneBFactorGroup"] = BFactorGroupName
        PyMOLObjectNames["Chains"][ChainID]["ChainAloneGroupMembers"].append(BFactorGroupName)

        PyMOLObjectNames["Chains"][ChainID]["ChainAloneBFactorGroupMembers"] = []

        # Setup cartoon...
        Name = "%s.Cartoon" % (BFactorGroupName)
        PyMOLObjectNames["Chains"][ChainID]["ChainAloneBFactorCartoon"] = Name
        PyMOLObjectNames["Chains"][ChainID]["ChainAloneBFactorGroupMembers"].append(Name)

        # Setup putty...
        Name = "%s.Putty" % (BFactorGroupName)
    ...
    if GetChainAloneContainsSelectionsStatus(FileIndex, ChainID):
        ...
        for SelectionName in OptionsInfo["ChainSelectionsInfo"]["Names"]:
            ...
            if GetChainAloneContainsChainSelectionSurfacesStatus(FileIndex, ChainID):
                ...
                # Add selection surface group to selection name group...
                ...
                SelectionsNameSurfaceGroupMembersID = "%s%sSurfaceGroupMembers" % (
                    SelectionsGroupIDPrefix,
                    SelectionNameGroupID,
                )
                ...
                if GetChainAloneSurfaceChainSelectionStatus(FileIndex, ChainID):
                    ...
                    for MemberType in ["Hydrophobicity", "Hydrophobicity_Charge"]:
                        ...
                        PyMOLObjectNames["Chains"][ChainID][SubGroupMemberID] = SubGroupMemberName
                        PyMOLObjectNames["Chains"][ChainID][SelectionsNameSurfaceGroupMembersID].append(
                            SubGroupMemberName
                        )
    ...
    if GetChainAloneDisulfideBondsStatus(FileIndex, ChainID):
        # Setup disulfide bonds group and add it to chain alone group...
        DisulfideBondsGroupName = "%s.Disulfide_Bonds" % (ChainAloneGroupName)
        PyMOLObjectNames["Chains"][ChainID]["ChainAloneDisulfideBondsGroup"] = DisulfideBondsGroupName
        PyMOLObjectNames["Chains"][ChainID]["ChainAloneGroupMembers"].append(DisulfideBondsGroupName)

        PyMOLObjectNames["Chains"][ChainID]["ChainAloneDisulfideBondsGroupMembers"] = []

        # Setup a residues object for disulfide bonds...
        Name = "%s.Residues" % (DisulfideBondsGroupName)
        PyMOLObjectNames["Chains"][ChainID]["ChainAloneDisulfideBondsResidues"] = Name
        PyMOLObjectNames["Chains"][ChainID]["ChainAloneDisulfideBondsGroupMembers"].append(Name)

    if GetChainAloneSaltBridgesStatus(FileIndex, ChainID):
        # Setup salt bridges group and add it to chain alone group...
        SaltBridgesGroupName = "%s.Salt_Bridges" % (ChainAloneGroupName)
        PyMOLObjectNames["Chains"][ChainID]["ChainAloneSaltBridgesGroup"] = SaltBridgesGroupName
        PyMOLObjectNames["Chains"][ChainID]["ChainAloneGroupMembers"].append(SaltBridgesGroupName)

        PyMOLObjectNames["Chains"][ChainID]["ChainAloneSaltBridgesGroupMembers"] = []

        # Setup residues group and add it to salt bridges group...
        ResiduesGroupName = "%s.Residues" % (SaltBridgesGroupName)
        PyMOLObjectNames["Chains"][ChainID]["ChainAloneSaltBridgesResiduesGroup"] = ResiduesGroupName
        PyMOLObjectNames["Chains"][ChainID]["ChainAloneSaltBridgesGroupMembers"].append(ResiduesGroupName)

        PyMOLObjectNames["Chains"][ChainID]["ChainAloneSaltBridgesResiduesGroupMembers"] = []

        # Setup objects for residues group...
        Name = "%s.Positively_Charged" % (ResiduesGroupName)
        PyMOLObjectNames["Chains"][ChainID]["ChainAloneSaltBridgesResiduesPositivelyCharged"] = Name
        PyMOLObjectNames["Chains"][ChainID]["ChainAloneSaltBridgesResiduesGroupMembers"].append(Name)

        Name = "%s.Negatively_Charged" % (ResiduesGroupName)
        PyMOLObjectNames["Chains"][ChainID]["ChainAloneSaltBridgesResiduesNegativelyCharged"] = Name
        PyMOLObjectNames["Chains"][ChainID]["ChainAloneSaltBridgesResiduesGroupMembers"].append(Name)

        # Add contacts object to salt bridges group...
        Name = "%s.Contacts" % (SaltBridgesGroupName)
        PyMOLObjectNames["Chains"][ChainID]["ChainAloneSaltBridgesContacts"] = Name
        PyMOLObjectNames["Chains"][ChainID]["ChainAloneSaltBridgesGroupMembers"].append(Name)
...
def SetupPyMOLObjectNamesForLigand(FileIndex, PyMOLObjectNames, ChainID, LigandID):
    ...
    for GroupType in ["Ligand", "Pocket", "Pocket_Solvent", "Pocket_Inorganic"]:
        ...
        if re.match("^Pocket_Inorganic$", GroupType, re.I):
            PiCationContactsName = "%s.Pi_Cation_Contacts" % (GroupName)
            PiCationContactsID = "%sPiCationContacts" % (GroupID)
            PyMOLObjectNames["Ligands"][ChainID][LigandID][PiCationContactsID] = PiCationContactsName
            PyMOLObjectNames["Ligands"][ChainID][LigandID][GroupMembersID].append(PiCationContactsName)
        ...
        if re.match("^Pocket$", GroupType, re.I):
            ...
            PiPiContactsName = "%s.Pi_Pi_Contacts" % (GroupName)
            PiPiContactsID = "%sPiPiContacts" % (GroupID)
            PyMOLObjectNames["Ligands"][ChainID][LigandID][PiPiContactsID] = PiPiContactsName
            PyMOLObjectNames["Ligands"][ChainID][LigandID][GroupMembersID].append(PiPiContactsName)
            ...
            HalogenContactsName = "%s.Halogen_Contacts" % (GroupName)
            HalogenContactsID = "%sHalogenContacts" % (GroupID)
            PyMOLObjectNames["Ligands"][ChainID][LigandID][HalogenContactsID] = HalogenContactsName
            PyMOLObjectNames["Ligands"][ChainID][LigandID][GroupMembersID].append(HalogenContactsName)

            if GetPocketContainsSelectionsStatus(FileIndex, ChainID, LigandID):
                # Setup selections group and its subgroups..
                SelectionsGroupName = "%s.Selections" % (GroupName)
                SelectionsGroupID = "%sSelectionsGroup" % (GroupID)
                SelectionsGroupMembersID = "%sGroupMembers" % (SelectionsGroupID)

                PyMOLObjectNames["Ligands"][ChainID][LigandID][SelectionsGroupID] = SelectionsGroupName
                PyMOLObjectNames["Ligands"][ChainID][LigandID][GroupMembersID].append(SelectionsGroupName)
                ...
                for SelectionName in OptionsInfo["PocketChainSelectionsInfo"]["Names"]:
                    ...
                    SelectionsNameGroupName = "%s.%s" % (SelectionsGroupName, SelectionName)
                    SelectionsNameGroupID = "%s%sGroup" % (SelectionsGroupID, SelectionNameGroupID)
                    SelectionsNameGroupMembersID = "%s%sGroupMembers" % (SelectionsGroupID, SelectionNameGroupID)

                    # Add selections name sub group to selections group...
                    PyMOLObjectNames["Ligands"][ChainID][LigandID][SelectionsNameGroupID] = SelectionsNameGroupName
                    PyMOLObjectNames["Ligands"][ChainID][LigandID][SelectionsGroupMembersID].append(
                        SelectionsNameGroupName
                    )
                    ...
                    # Add selection member to selections name group...
                    SubGroupMemberName = "%s.Selection" % (SelectionsNameGroupName)
                    SubGroupMemberID = "%s%sSelection" % (SelectionsGroupID, SelectionNameGroupID)

                    PyMOLObjectNames["Ligands"][ChainID][LigandID][SubGroupMemberID] = SubGroupMemberName
                    PyMOLObjectNames["Ligands"][ChainID][LigandID][SelectionsNameGroupMembersID].append(
                        SubGroupMemberName
                    )
                    ...
                    if GetPocketSelectionSurfaceChainStatus(FileIndex, ChainID, LigandID):
                        # Setup a surface sub group and add it to selections name group...
                        SelectionsNameSurfaceGroupName = "%s.Surface" % (SelectionsNameGroupName)
                        SelectionsNameSurfaceGroupID = "%s%sSurfaceGroup" % (SelectionsGroupID, SelectionNameGroupID)

                        # Add selection surface group to selection name group...
                        PyMOLObjectNames["Ligands"][ChainID][LigandID][SelectionsNameSurfaceGroupID] = (
                            SelectionsNameSurfaceGroupName
                        )
                        PyMOLObjectNames["Ligands"][ChainID][LigandID][SelectionsNameGroupMembersID].append(
                            SelectionsNameSurfaceGroupName
                        )
                        ...
                        PyMOLObjectNames["Ligands"][ChainID][LigandID][SelectionsNameSurfaceGroupMembersID] = []

                        # Setup surfaces...
                        for MemberType in ["Surface", "Hydrophobicity", "Hydrophobicity_Charge"]:
                            MemberID = re.sub("_", "", MemberType)
                            ...
                            PyMOLObjectNames["Ligands"][ChainID][LigandID][SubGroupMemberID] = SubGroupMemberName
                            PyMOLObjectNames["Ligands"][ChainID][LigandID][SelectionsNameSurfaceGroupMembersID].append(
                                SubGroupMemberName
                            )
            ...
            if GetPocketResidueTypesStatus(FileIndex, ChainID, LigandID):
                ...
                for SubGroupType in [
                    ...
                    for MemberType in ["Residues", "Surface"]:
                        ...
                        PyMOLObjectNames["Ligands"][ChainID][LigandID][SubGroupMemberID] = SubGroupMemberName
                        PyMOLObjectNames["Ligands"][ChainID][LigandID][ResiduesSubMembersGroupID].append(
                            SubGroupMemberName
                        )
            ...
            if GetPocketContainsSurfaceStatus(FileIndex, ChainID, LigandID):
                ...
                for SubGroupType in ["Surface", "Cavity"]:
                    ...
                    if GetPocketSurfaceChainStatus(FileIndex, ChainID, LigandID):
                        ...
                        PyMOLObjectNames["Ligands"][ChainID][LigandID][SurfacesSubGroupMembersID].append(
                            HydrophobicSurfaceName
                        )
                        ...
                        HydrophobicChargeSurfaceID = "%sHydrophobicChargeSurface" % (SurfacesSubGroupID)
                        PyMOLObjectNames["Ligands"][ChainID][LigandID][HydrophobicChargeSurfaceID] = (
                            HydrophobicChargeSurfaceName
                        )
                        PyMOLObjectNames["Ligands"][ChainID][LigandID][SurfacesSubGroupMembersID].append(
                            HydrophobicChargeSurfaceName
...
def SetupPyMOLObjectNamesForDockedPoses(FileIndex, PyMOLObjectNames, ChainID):
    """Stetup groups and objects for docked poses in input files for a chain."""

    PyMOLObjectNames["DockedPosesInputFile"][ChainID] = {}

    if not GetChainAloneDockedPosesStatus(FileIndex, ChainID):
        return

    # Setup group for docked poses...
    if "DockedPosesGroup" not in PyMOLObjectNames["Chains"][ChainID]:
        # Setup docked poses group at a chain level...
        ChainGroupName = PyMOLObjectNames["Chains"][ChainID]["ChainGroup"]
        DockedPosesGroupName = "%s.%s" % (ChainGroupName, OptionsInfo["DockedPosesGroupName"])

        PyMOLObjectNames["Chains"][ChainID]["DockedPosesGroup"] = DockedPosesGroupName
        PyMOLObjectNames["Chains"][ChainID]["ChainGroupMembers"].append(DockedPosesGroupName)
        PyMOLObjectNames["Chains"][ChainID]["DockedPosesGroupMembers"] = []

    SpecifiedChainsAndLigandsInfo = OptionsInfo["InfilesInfo"]["SpecifiedChainsAndLigandsInfo"][FileIndex]
    for InputFileIndex, InputFile in enumerate(SpecifiedChainsAndLigandsInfo["DockedPosesInputFiles"][ChainID]):
        SetupPyMOLObjectNamesForDockedPosesInputFile(FileIndex, PyMOLObjectNames, ChainID, InputFileIndex)


def SetupPyMOLObjectNamesForDockedPosesInputFile(PDBFileIndex, PyMOLObjectNames, ChainID, InputFileIndex):
    """Stetup groups and objects for docked poses in an input file for a chain."""
    ...
    PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID] = {}

    DockedPosesGroupName = PyMOLObjectNames["Chains"][ChainID]["DockedPosesGroup"]

    # Setup a docked poses level InputFile group...
    DockedPosesInputFileGroupName = "%s.%s" % (DockedPosesGroupName, InputFileID)
    PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID]["DockedPosesGroupName"] = (
        DockedPosesInputFileGroupName
    )
    PyMOLObjectNames["Chains"][ChainID]["DockedPosesGroupMembers"].append(DockedPosesInputFileGroupName)

    PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID]["DockedPosesGroupMembers"] = []

    # Setup objects for docked poses...
    PosesName = "%s.%s" % (DockedPosesInputFileGroupName, OptionsInfo["DockedPosesName"])
    PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID]["Poses"] = PosesName
    PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID]["DockedPosesGroupMembers"].append(PosesName)

    for GroupType in ["Pocket", "Pocket_Solvent", "Pocket_Inorganic"]:
        GroupID = re.sub("_", "", GroupType)
        GroupName = "%s.%s" % (DockedPosesInputFileGroupName, GroupType)

        GroupNameID = "%sGroup" % (GroupID)
        GroupMembersID = "%sGroupMembers" % (GroupID)

        PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID][GroupNameID] = GroupName
        PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID]["DockedPosesGroupMembers"].append(GroupName)

        PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID][GroupMembersID] = []

        PocketID = "%sPocket" % GroupID
        PocketName = "%s.Pocket" % GroupName
        PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID][PocketID] = PocketName
        PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID][GroupMembersID].append(PocketName)

        if DockedPosesInfo["DistanceContacts"] and re.match("^Pocket$", GroupType, re.I):
            # Setup distance contacts group and add it to pocket group...
            DistanceContactGroupName = "%s.Distance_Contacts" % GroupName

            DistanceContactGroupNameID = "%sDistanceContactsGroup" % GroupID
            DistanceContactGroupMembersID = "%sDistanceContactsGroupMembers" % GroupID

            PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID][DistanceContactGroupNameID] = (
                DistanceContactGroupName
            )
            PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID][GroupMembersID].append(
                DistanceContactGroupName
            )

            # Setup distance contacts group members...
            PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID][DistanceContactGroupMembersID] = []

            for ContactID in DockedPosesDistanceContactsInfo["ContactIDs"]:
                DistanceContactID = "%sPocketDistanceContacts%s" % (GroupID, ContactID)
                DistanceContactName = "%s.Distance_Contacts.%s" % (GroupName, ContactID)

                PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID][DistanceContactID] = DistanceContactName
                PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID][DistanceContactGroupMembersID].append(
                    DistanceContactName
                )

        PolarContactsID = "%sPolarContacts" % GroupID
        PolarContactsName = "%s.Polar_Contacts" % GroupName
        PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID][PolarContactsID] = PolarContactsName
        PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID][GroupMembersID].append(PolarContactsName)

        if re.match("^Pocket_Inorganic$", GroupType, re.I):
            PiCationContactsID = "%sPiCationContacts" % GroupID
            PiCationContactsName = "%s.Pi_Cation_Contacts" % GroupName
            PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID][PiCationContactsID] = PiCationContactsName
            PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID][GroupMembersID].append(PiCationContactsName)

        if re.match("^Pocket$", GroupType, re.I):
            HydrophobicContactsID = "%sHydrophobicContacts" % GroupID
            HydrophobicContactsName = "%s.Hydrophobic_Contacts" % GroupName
            PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID][HydrophobicContactsID] = (
                HydrophobicContactsName
            )
            PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID][GroupMembersID].append(
                HydrophobicContactsName
            ...
            PiPiContactsID = "%sPiPiContacts" % GroupID
            PiPiContactsName = "%s.Pi_Pi_Contacts" % GroupName
            PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID][PiPiContactsID] = PiPiContactsName
            PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID][GroupMembersID].append(PiPiContactsName)
            ...
            HalogenContactsID = "%sHalogenContacts" % GroupID
            HalogenContactsName = "%s.Halogen_Contacts" % GroupName
            PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID][HalogenContactsID] = HalogenContactsName
            PyMOLObjectNames["DockedPosesInputFile"][ChainID][InputFileID][GroupMembersID].append(HalogenContactsName)
...
def ProcessChainAndLigandIDs():
    ...
    for FileIndex in range(0, len(OptionsInfo["InfilesInfo"]["InfilesNames"])):
        MiscUtil.PrintInfo(
            ...
            % OptionsInfo["InfilesInfo"]["InfilesNames"][FileIndex]
        )
        ...
        SpecifiedChainsAndLigandsInfo = PyMOLUtil.ProcessChainsAndLigandsOptionsInfo(
            ChainsAndLigandsInfo, "-c, --chainIDs", OptionsInfo["ChainIDs"], "-l, --ligandIDs", OptionsInfo["LigandIDs"]
        )
...
def ProcessResidueTypesAndSurfaceAndChainSelectionsOptions(FileIndex, SpecifiedChainsAndLigandsInfo):
    ...
    SpecifiedChainsAndLigandsInfo["PocketChainSelections"] = {}
    SpecifiedChainsAndLigandsInfo["PocketChainSelectionsSurfaces"] = {}
    SpecifiedChainsAndLigandsInfo["SurfacePocketChainSelections"] = {}
    ...
    SpecifiedChainsAndLigandsInfo["DisulfideBondsChain"] = {}
    SpecifiedChainsAndLigandsInfo["SaltBridgesChain"] = {}

    SpecifiedChainsAndLigandsInfo["BFactorChain"] = {}
    ...
    for ChainID in SpecifiedChainsAndLigandsInfo["ChainIDs"]:
        ...
        # Process BFactors for chains...
        BFactorChain = True if re.match("^yes$", OptionsInfo["BFactorChain"], re.I) else False
        SpecifiedChainsAndLigandsInfo["BFactorChain"][ChainID] = BFactorChain
        ...
        if re.match("^auto$", OptionsInfo["SurfaceChainElectrostatics"], re.I):
            ...
            SurfaceChainElectrostatics = (
                True if re.match("^yes$", OptionsInfo["SurfaceChainElectrostatics"], re.I) else False
            )
        SpecifiedChainsAndLigandsInfo["SurfaceChainElectrostatics"][ChainID] = SurfaceChainElectrostatics

        SpecifiedChainsAndLigandsInfo["ChainSurfaces"][ChainID] = SurfaceChain

        # Process disulfide bonds for chains...
        if re.match("^auto$", OptionsInfo["DisulfideBondsChain"], re.I):
            DisulfideBondsChain = True if AminoAcidsPresent else False
        else:
            DisulfideBondsChain = True if re.match("^yes$", OptionsInfo["DisulfideBondsChain"], re.I) else False
        SpecifiedChainsAndLigandsInfo["DisulfideBondsChain"][ChainID] = DisulfideBondsChain

        # Process salt bridges bonds for chains...
        if re.match("^auto$", OptionsInfo["SaltBridgesChain"], re.I):
            SaltBridgesChain = True if AminoAcidsPresent else False
        else:
            SaltBridgesChain = True if re.match("^yes$", OptionsInfo["SaltBridgesChain"], re.I) else False
        SpecifiedChainsAndLigandsInfo["SaltBridgesChain"][ChainID] = SaltBridgesChain
        ...
        SpecifiedChainsAndLigandsInfo["ChainSelectionsSurfaces"][ChainID] = SurfaceChainSelections

        # Process selections, residue types and surfaces for pockets...
        SpecifiedChainsAndLigandsInfo["PocketChainSelections"][ChainID] = {}
        SpecifiedChainsAndLigandsInfo["PocketChainSelectionsSurfaces"][ChainID] = {}
        SpecifiedChainsAndLigandsInfo["SurfacePocketChainSelections"][ChainID] = {}
        ...
        for LigandID in SpecifiedChainsAndLigandsInfo["LigandIDs"][ChainID]:
            # Process pocket chain selections and surfaces...
            PocketChainSelections = True if len(OptionsInfo["PocketChainSelectionsInfo"]["Names"]) else False
            SpecifiedChainsAndLigandsInfo["PocketChainSelections"][ChainID][LigandID] = PocketChainSelections

            # Process surfaces for chain selections...
            if re.match("^auto$", OptionsInfo["SelectionsPocketSurface"], re.I):
                PocketChainSelectionsSurface = True if AminoAcidsPresent else False
            else:
                PocketChainSelectionsSurface = (
                    True if re.match("^yes$", OptionsInfo["SelectionsPocketSurface"], re.I) else False
                )

            SpecifiedChainsAndLigandsInfo["PocketChainSelectionsSurfaces"][ChainID][LigandID] = (
                PocketChainSelectionsSurface
            )

            SpecifiedChainsAndLigandsInfo["SurfacePocketChainSelections"][ChainID][LigandID] = (
                PocketChainSelectionsSurface
            )

            # Process pocket surfaces...
            ...
            if re.match("^auto$", OptionsInfo["PocketSurfaceElectrostatics"], re.I):
                ...
                PocketSurfaceElectrostatics = (
                    True if re.match("^yes$", OptionsInfo["PocketSurfaceElectrostatics"], re.I) else False
                )
            SpecifiedChainsAndLigandsInfo["SurfacePocketElectrostatics"][ChainID][LigandID] = (
                PocketSurfaceElectrostatics
            )

            # Process pocket residue types...
            SpecifiedChainsAndLigandsInfo["PocketSurfaces"][ChainID][LigandID] = PocketSurface
...
def GetChainAloneBFactorStatus(FileIndex, ChainID):
    """Get status of B factors for chain alone object."""

    Status = OptionsInfo["InfilesInfo"]["SpecifiedChainsAndLigandsInfo"][FileIndex]["BFactorChain"][ChainID]

    return Status


def GetChainAloneDisulfideBondsStatus(FileIndex, ChainID):
    """Get status of disulfide bonds for chain alone object."""

    Status = OptionsInfo["InfilesInfo"]["SpecifiedChainsAndLigandsInfo"][FileIndex]["DisulfideBondsChain"][ChainID]

    return Status


def GetChainAloneSaltBridgesStatus(FileIndex, ChainID):
    """Get status of salt bridges for chain alone object."""

    Status = OptionsInfo["InfilesInfo"]["SpecifiedChainsAndLigandsInfo"][FileIndex]["SaltBridgesChain"][ChainID]

    return Status


def GetChainAloneDockedPosesStatus(FileIndex, ChainID):
    """Get status of docked for chain alone object."""

    Status = OptionsInfo["InfilesInfo"]["SpecifiedChainsAndLigandsInfo"][FileIndex]["DockedPoses"][ChainID]

    return Status


def GetTrajectoriesStatus(FileIndex):
    """Get status of trajectories for a PDB object."""

    if OptionsInfo["TrajectoriesInfo"] is None:
        return False

    Status = True if FileIndex in OptionsInfo["TrajectoriesInfo"]["PDBFileIndices"] else False
...
def GetPocketResidueTypesStatus(FileIndex, ChainID, LigandID):
    ...
    return OptionsInfo["InfilesInfo"]["SpecifiedChainsAndLigandsInfo"][FileIndex]["ResidueTypesPocket"][ChainID][
        LigandID
    ]
...
def GetPocketContainsSelectionsStatus(FileIndex, ChainID, LigandID):
    """Get status of selections present in a pocket."""

    return OptionsInfo["InfilesInfo"]["SpecifiedChainsAndLigandsInfo"][FileIndex]["PocketChainSelections"][ChainID][
        LigandID
    ]
...
def GetPocketSelectionSurfaceChainStatus(FileIndex, ChainID, LigandID):
    """Get status of surfaces for a pocket selection in a chain."""

    return OptionsInfo["InfilesInfo"]["SpecifiedChainsAndLigandsInfo"][FileIndex]["SurfacePocketChainSelections"][
        ChainID
    ][LigandID]
...
def GetPocketSurfaceChainElectrostaticsStatus(FileIndex, ChainID, LigandID):
    ...
    return OptionsInfo["InfilesInfo"]["SpecifiedChainsAndLigandsInfo"][FileIndex]["SurfacePocketElectrostatics"][
        ChainID
    ][LigandID]
...
def CheckPresenceOfValidLigandIDs(ChainsAndLigandsInfo, SpecifiedChainsAndLigandsInfo):
    ...
    for ChainID in SpecifiedChainsAndLigandsInfo["ChainIDs"]:
        if len(SpecifiedChainsAndLigandsInfo["LigandIDs"][ChainID]):
            MiscUtil.PrintInfo(
                "Chain ID: %s; Specified LigandIDs: %s"
                % (ChainID, ", ".join(SpecifiedChainsAndLigandsInfo["LigandIDs"][ChainID]))
            )
        else:
            MiscUtil.PrintInfo("Chain IDs: %s; Specified LigandIDs: None" % (ChainID))
            MiscUtil.PrintWarning(
                "No valid ligand IDs found for chain ID, %s. PyMOL groups and objects related to ligand and binding pockect won't be created."
                % (ChainID)
            )
...
def ProcessDockedPoses():
    """Process docked poses."""

    OptionsInfo["DockedPosesGroupName"] = Options["--dockedPosesGroupName"]
    OptionsInfo["DockedPosesName"] = Options["--dockedPosesName"]

    DockedPosesInfo = ProcessDockedPosesOptionsInfo("--dockedPoses", OptionsInfo["DockedPoses"])
    OptionsInfo["DockedPosesInfo"] = DockedPosesInfo

    ProcessDockedPosesInfoForInfiles()

    OptionsInfo["DockedPosesDistanceContactsCutoffs"] = Options["--dockedPosesDistanceContactsCutoffs"]
    OptionsInfo["DockedPosesDistanceContactsColor"] = Options["--dockedPosesDistanceContactsColor"]

    DockedPosesDistanceContactsInfo = ProcessDockedPosesDistanceContactsOptionsInfo()
    OptionsInfo["DockedPosesDistanceContactsInfo"] = DockedPosesDistanceContactsInfo

    # Setup distance conatcts status for dockes poses...
    if DockedPosesInfo is not None:
        DockedPosesInfo["DistanceContacts"] = False
        if DockedPosesDistanceContactsInfo is not None:
            if len(DockedPosesDistanceContactsInfo["ContactIDs"]):
                DockedPosesInfo["DistanceContacts"] = True


def ProcessDockedPosesInfoForInfiles():
    """Process docked poses info for infiles."""

    DockedPosesInfo = OptionsInfo["DockedPosesInfo"]
    for FileIndex in range(0, len(OptionsInfo["InfilesInfo"]["InfilesNames"])):
        SpecifiedChainsAndLigandsInfo = OptionsInfo["InfilesInfo"]["SpecifiedChainsAndLigandsInfo"][FileIndex]
        ProcessDockedPosesInfoForInfileAndChains(FileIndex, SpecifiedChainsAndLigandsInfo, DockedPosesInfo)


def ProcessDockedPosesInfoForInfileAndChains(FileIndex, SpecifiedChainsAndLigandsInfo, DockedPosesInfo):
    """Process docked poses info for a specific infile."""

    SpecifiedChainsAndLigandsInfo["DockedPoses"] = {}
    SpecifiedChainsAndLigandsInfo["DockedPosesInputFiles"] = {}

    for ChainID in SpecifiedChainsAndLigandsInfo["ChainIDs"]:
        DockedPoses = False
        if DockedPosesInfo is not None:
            if FileIndex in DockedPosesInfo["PDBFileIndices"]:
                if ChainID == DockedPosesInfo["ChainID"][FileIndex]:
                    DockedPoses = True

        SpecifiedChainsAndLigandsInfo["DockedPoses"][ChainID] = DockedPoses

        SpecifiedChainsAndLigandsInfo["DockedPosesInputFiles"][ChainID] = []
        if DockedPoses:
            SpecifiedChainsAndLigandsInfo["DockedPosesInputFiles"][ChainID].extend(
                DockedPosesInfo["InputFiles"][FileIndex]
            )


def ProcessDockedPosesOptionsInfo(DockedPosesOptionName, DockedPosesOptionValue):
    """Process docked poses options info."""

    DockedPoses = DockedPosesOptionValue
    if re.match("^none$", DockedPoses, re.I):
        return None

    MiscUtil.PrintInfo("\nProcessing docked poses...")

    # Initialize docked poses info...
    DockedPosesInfo = {}
    DockedPosesInfo["PDBFileIndices"] = []

    DockedPosesInfo["PDBFile"] = {}
    DockedPosesInfo["ChainID"] = {}
    DockedPosesInfo["LigandID"] = {}
    DockedPosesInfo["UseInputFileAsLigandID"] = {}
    DockedPosesInfo["InputFiles"] = {}
    DockedPosesInfo["InputFilesRoots"] = {}
    DockedPosesInfo["InputFilesIDs"] = {}

    # Parse docked poses values...
    DockedPosesWords = DockedPoses.split(",")
    if len(DockedPosesWords) % 4:
        MiscUtil.PrintError(
            'The number of comma delimited docked poses values, %d, specified using "%s" option must be a multple of 4.'
            % (len(DockedPosesWords), DockedPosesOptionName)
        )

    # Validate and process specified values...
    for Index in range(0, len(DockedPosesWords), 4):
        PDBFile = DockedPosesWords[Index].strip()
        ChainID = DockedPosesWords[Index + 1].strip()
        LigandID = DockedPosesWords[Index + 2].strip()
        InputFiles = DockedPosesWords[Index + 3].strip()

        InputFiles = re.sub("[ ]+", " ", InputFiles)
        InputFiles = InputFiles.split(" ")

        # Process PDB file...
        MiscUtil.ValidateOptionFilePath("--dockedPoses", PDBFile)

        PDBFileIndex = None
        for FileIndex in range(0, len(OptionsInfo["InfilesInfo"]["InfilesNames"])):
            PDBInfile = OptionsInfo["InfilesInfo"]["InfilesNames"][FileIndex]
            if PDBFile == PDBInfile:
                PDBFileIndex = FileIndex
                break
        if PDBFileIndex is None:
            MiscUtil.PrintError(
                'The PDB file specified, %s, using option "--dockedPoses" is not valid. It must be specified as an input file.'
                % (PDBFile)
            )

        if PDBFileIndex in DockedPosesInfo["PDBFileIndices"]:
            MiscUtil.PrintError(
                'The PDB file specified, %s, using option "--dockedPoses" is not valid. It is a duplicate and has already been specified.'
                % (PDBFile)
            )

        # Process chain ID...
        SpecifiedChainsAndLigandsInfo = OptionsInfo["InfilesInfo"]["SpecifiedChainsAndLigandsInfo"][PDBFileIndex]
        if ChainID not in SpecifiedChainsAndLigandsInfo["ChainIDs"]:
            MiscUtil.PrintError(
                'The chain ID, %s, specified for PDB file, %s, using option "--dockedPoses" is not valid. It must be present in valid chain IDs corresponding to option "-c, --chainIDs" .'
                % (ChainID, PDBFile)
            )

        # Process ligand ID...
        UseInputFileAsLigandID = True if re.match("^UseInputFile$", LigandID, re.I) else False
        if not UseInputFileAsLigandID:
            if LigandID not in SpecifiedChainsAndLigandsInfo["LigandIDs"][ChainID]:
                MiscUtil.PrintError(
                    'The ligand ID, %s, specified for chain ID, %s, and PDB file, %s, using option "--dockedPoses" is not valid. It must be present in valid ligand IDs corresponding to option "-l, --ligandIDs" .'
                    % (LigandID, ChainID, PDBFile)
                )
        ...
        for InputFile in InputFiles:
            MiscUtil.ValidateOptionFilePath("--dockedPoses", InputFile)
            MiscUtil.ValidateOptionFileExt("--dockedPoses", InputFile, "sdf sd")

            FileDir, FileName, FileExt = MiscUtil.ParseFileName(InputFile)
            InputFileRoot = FileName

            # Clean up input file names for generating PyMOL object names...
            InputFileID = re.sub("[^a-zA-Z0-9]", "_", FileName)

            if InputFileRoot in InputFilesRoots:
                MiscUtil.PrintError(
                    'The input file specified, %s, using option "--dockedPoses" is not valid. It is a duplicate and has already been specified for a PDB file.'
                    % (InputFile)
                )

            InputFilesRoots.append(InputFileRoot)
            InputFilesIDs.append(InputFileID)

        # Track values...
        DockedPosesInfo["PDBFileIndices"].append(PDBFileIndex)

        DockedPosesInfo["PDBFile"][PDBFileIndex] = PDBFile
        DockedPosesInfo["ChainID"][PDBFileIndex] = ChainID

        DockedPosesInfo["LigandID"][PDBFileIndex] = LigandID
        DockedPosesInfo["UseInputFileAsLigandID"][PDBFileIndex] = UseInputFileAsLigandID

        DockedPosesInfo["InputFiles"][PDBFileIndex] = InputFiles
        DockedPosesInfo["InputFilesRoots"][PDBFileIndex] = InputFilesRoots
        DockedPosesInfo["InputFilesIDs"][PDBFileIndex] = InputFilesIDs

    return DockedPosesInfo


def ProcessDockedPosesDistanceContactsOptionsInfo():
    """Process dockes poses distance contacts info."""

    DistanceContactsCutoffs = OptionsInfo["DockedPosesDistanceContactsCutoffs"]
    if re.match("^none$", DistanceContactsCutoffs):
        return None

    DistanceContactsInfo = {}
    DistanceContactsInfo["ContactIDs"] = []
    DistanceContactsInfo["ContactCutoff"] = {}

    DistanceContactsCutoffs = re.sub(" ", "", DistanceContactsCutoffs)
    if not DistanceContactsCutoffs:
        MiscUtil.PrintError('No value specified using "--dockedPosesDistanceContactsCutoffs" option.')

    ContactCutoffValues = DistanceContactsCutoffs.split(",")
    for Index, ContactCutoff in enumerate(ContactCutoffValues):
        MiscUtil.ValidateOptionFloatValue(
            "--dockedPosesDistanceContactsCutoffs", ContactCutoff, {">": 0.0, "<=": OptionsInfo["PocketDistanceCutoff"]}
        )
        ContactCutoff = float(ContactCutoff.strip())

        ContactID = "Contact%s_At_%s" % ((Index + 1), ContactCutoff)
        ContactID = re.sub(r"\.", "pt", ContactID)

        DistanceContactsInfo["ContactIDs"].append(ContactID)
        DistanceContactsInfo["ContactCutoff"][ContactID] = ContactCutoff

    return DistanceContactsInfo


def ProcessTrajectories():
    """Process trajectories."""

    TrajectoriesInfo = ProcessTrajectoriesOptionsInfo("--Trajectories", OptionsInfo["Trajectories"])
    OptionsInfo["TrajectoriesInfo"] = TrajectoriesInfo


def ProcessTrajectoriesOptionsInfo(TrajectoriesOptionName, TrajectoriesOptionValue):
    """Process trajectories options info."""

    Trajectories = TrajectoriesOptionValue
    if re.match("^none$", Trajectories, re.I):
        return None

    MiscUtil.PrintInfo("\nProcessing trajectories...")

    # Initialize trajectories info...
    TrajectoriesInfo = {}
    TrajectoriesInfo["PDBFileIndices"] = []

    TrajectoriesInfo["PDBFile"] = {}
    TrajectoriesInfo["TrajFiles"] = {}
    TrajectoriesInfo["TrajFilesRoots"] = {}
    TrajectoriesInfo["TrajFilesIDs"] = {}

    # Parse trajectories values...
    TrajectoriesWords = Trajectories.split(",")
    if len(TrajectoriesWords) % 2:
        MiscUtil.PrintError(
            'The number of comma delimited trajectories values, %d, specified using "%s" option must be a multple of 2.'
            % (len(TrajectoriesWords), TrajectoriesOptionName)
        )

    # Validate and process specified values...
    for Index in range(0, len(TrajectoriesWords), 2):
        PDBFile = TrajectoriesWords[Index].strip()
        TrajFiles = TrajectoriesWords[Index + 1].strip()

        TrajFiles = re.sub("[ ]+", " ", TrajFiles)
        TrajFiles = TrajFiles.split(" ")

        # Process PDB file...
        MiscUtil.ValidateOptionFilePath(TrajectoriesOptionName, PDBFile)
        ...
        if PDBFileIndex is None:
            MiscUtil.PrintError(
                'The topology PDB file specified, %s, using option "%s" is not valid. It must be specified as an input file.'
                % (PDBFile, TrajectoriesOptionName)
            )

        if PDBFileIndex in TrajectoriesInfo["PDBFileIndices"]:
            MiscUtil.PrintError(
                'The topology PDB file specified, %s, using option "%s" is not valid. It is a duplicate and has already been specified.'
                % (PDBFile, TrajectoriesOptionName)
            )
        ...
        for TrajFile in TrajFiles:
            MiscUtil.ValidateOptionFilePath(TrajectoriesOptionName, TrajFile)

            FileDir, FileName, FileExt = MiscUtil.ParseFileName(TrajFile)
            TrajFileRoot = FileName

            # Clean up trajectory file names for generating PyMOL object names...
            TrajFileID = re.sub("[^a-zA-Z0-9]", "_", FileName)

            if TrajFileRoot in TrajFilesRoots:
                MiscUtil.PrintError(
                    'The trajectory file specified, %s, using option "%s" is not valid. It is a duplicate and has already been specified for a PDB file.'
                    % (TrajFile, TrajectoriesOptionName)
                )

            TrajFilesRoots.append(TrajFileRoot)
            TrajFilesIDs.append(TrajFileID)

        # Track values...
        TrajectoriesInfo["PDBFileIndices"].append(PDBFileIndex)

        TrajectoriesInfo["PDBFile"][PDBFileIndex] = PDBFile

        TrajectoriesInfo["TrajFiles"][PDBFileIndex] = TrajFiles
        TrajectoriesInfo["TrajFilesRoots"][PDBFileIndex] = TrajFilesRoots
        TrajectoriesInfo["TrajFilesIDs"][PDBFileIndex] = TrajFilesIDs
...
def ProcessResidueTypes():
    ...
    ResidueTypesNamesInfo, ResidueTypesParamsInfo = PyMOLUtil.ProcessResidueTypesOptionsInfo(
        "-r, --residueTypes", OptionsInfo["ResidueTypes"]
    )
    OptionsInfo["ResidueTypesNames"] = ResidueTypesNamesInfo
    OptionsInfo["ResidueTypesParams"] = ResidueTypesParamsInfo


def ProcessSaltBridgesChainResidues():
    """Process salt bridges chain residues."""

    SaltBridgesChainResiduesInfo = PyMOLUtil.ProcessSaltBridgesChainResiduesOptionsInfo(
        "--saltBridgesChainResidues", OptionsInfo["SaltBridgesChainResidues"]
    )
    OptionsInfo["SaltBridgesChainResiduesInfo"] = SaltBridgesChainResiduesInfo
...
def ProcessPockectChainSelections():
    """Process custom selections for pocket chains."""

    PocketChainSelectionsInfo = PyMOLUtil.ProcessChainSelectionsOptionsInfo(
        "--selectionsPocket", OptionsInfo["SelectionsPocket"]
    )
    OptionsInfo["PocketChainSelectionsInfo"] = PocketChainSelectionsInfo


def ProcessChainSelections():
    """Process custom selections for chains."""

    ChainSelectionsInfo = PyMOLUtil.ProcessChainSelectionsOptionsInfo(
        "--selectionsChain", OptionsInfo["SelectionsChain"]
    )
...
def ProcessOptions():
    ...
    OptionsInfo["BFactorChain"] = Options["--BFactorChain"]
    ...
    OptionsInfo["IgnoreHydrogens"] = True if re.match("^Yes$", Options["--ignoreHydrogens"], re.I) else False
    ...
    if re.match("^pml$", FileExt, re.I):
        ...
        if os.path.exists(OptionsInfo["PMLOutfile"]) and (not OptionsInfo["Overwrite"]):
            MiscUtil.PrintError(
                'The intermediate output file to be generated, %s, already exist. Use option "--ov" or "--overwrite" and try again.'
                % OptionsInfo["PMLOutfile"]
            )

    OptionsInfo["DisulfideBondsChain"] = Options["--disulfideBondsChain"]
    ...
    OptionsInfo["PocketContactsLigandPiPiColor"] = Options["--pocketContactsLigandPiPiColor"]
    OptionsInfo["PocketContactsLigandPiCationColor"] = Options["--pocketContactsLigandPiCationColor"]
    OptionsInfo["PocketContactsLigandHalogenColor"] = Options["--pocketContactsLigandHalogenColor"]
    ...
    OptionsInfo["PocketContactsInorganicPiCationColor"] = Options["--pocketContactsInorganicPiCationColor"]
    ...
    OptionsInfo["SaltBridgesChain"] = Options["--saltBridgesChain"]
    OptionsInfo["SaltBridgesChainContactsColor"] = Options["--saltBridgesChainContactsColor"]
    OptionsInfo["SaltBridgesChainCutoff"] = float(Options["--saltBridgesChainCutoff"])
    OptionsInfo["SaltBridgesChainResidues"] = Options["--saltBridgesChainResidues"]
    ProcessSaltBridgesChainResidues()
    ...
    OptionsInfo["SelectionsPocket"] = Options["--selectionsPocket"]
    OptionsInfo["SelectionsPocketSurface"] = Options["--selectionsPocketSurface"]
    OptionsInfo["SelectionsPocketStyle"] = Options["--selectionsPocketStyle"]
    ProcessPockectChainSelections()
    ...
    OptionsInfo["DockedPoses"] = Options["--dockedPoses"]
    ProcessDockedPoses()

    OptionsInfo["Trajectories"] = Options["--trajectories"]
    ProcessTrajectories()
...
def ValidateOptions():
    ...
    MiscUtil.ValidateOptionTextValue("--BFactorChain", Options["--BFactorChain"], "yes no")
    ...
    MiscUtil.ValidateOptionTextValue("--disulfideBondsChain", Options["--disulfideBondsChain"], "yes no auto")

    MiscUtil.ValidateOptionTextValue("--ignoreHydrogens", Options["--ignoreHydrogens"], "yes no")
    ...
    if float(Options["--pocketContactsCutoff"]) > float(Options["--pocketDistanceCutoff"]):
        MiscUtil.PrintError(
            'The value, %s, specified using option "--pocketContactsCutoff" must be less than value, %s, specified using "-pocketDistanceCutoff" option.'
            % (Options["--pocketContactsCutoff"], Options["--pocketDistanceCutoff"])
        )
    ...
    MiscUtil.ValidateOptionTextValue(
        "--pocketSurfaceElectrostatics", Options["--pocketSurfaceElectrostatics"], "yes no auto"
    )

    MiscUtil.ValidateOptionTextValue("--residueTypesChain", Options["--residueTypesChain"], "yes no auto")

    MiscUtil.ValidateOptionTextValue("--saltBridgesChain", Options["--saltBridgesChain"], "yes no auto")
    MiscUtil.ValidateOptionFloatValue("--saltBridgesChainCutoff", Options["--saltBridgesChainCutoff"], {">": 0.0})

    MiscUtil.ValidateOptionTextValue("--selectionsChainSurface", Options["--selectionsChainSurface"], "yes no auto")
    MiscUtil.ValidateOptionTextValue("--selectionsPocketSurface", Options["--selectionsPocketSurface"], "yes no auto")
    ...
    MiscUtil.ValidateOptionTextValue(
        "--surfaceChainElectrostatics", Options["--surfaceChainElectrostatics"], "yes no auto"
    )
...
Usage:
    PyMOLVisualizeMacromolecules.py [--align <yes or no>] [--alignMethod <align, cealign, super>]
                                    ...
                                    [--allowEmptyObjects <yes or no>] [--BFactorChain <yes or no>] [--BFactorColorPalette <text>]
                                    [--chainIDs <First, All or ID1,ID2...>] [--disulfideBondsChain <yes or no>]
                                    [--dockedPoses <PDBFile,ChainID,LigandID,InputFiles,...>] [--dockedPosesDistanceContactsCutoffs <number1,number2...>]
                                    [--dockedPosesDistanceContactsColor <text>] [--dockedPosesGroupName <text>]
                                    [--dockedPosesName <text>] [--ignoreHydrogens <yes or no>] [--ligandIDs <Largest, All or ID1,ID2...>]
                                    [--labelFontID <number>] [--PMLOut <yes or no>] [--pocketContactsInorganicColor <text>]
                                    [--pocketContactsInorganicPiCationColor <text>] [--pocketContactsLigandColor <text>]
                                    ...
                                    [--saltBridgesChain <yes or no>] [--saltBridgesChainContactsColor <text>]
                                    [--saltBridgesChainCutoff <number>] [--saltBridgesChainResidues <Type, ResNames,...>]
                                    ...
                                    [--selectionsPocketSurface <yes or no>] [--selectionsPocketStyle <DisplayStyle>]
                                    ...
                                    [--surfaceTransparency <number>] [--trajectories <PDBToplogyFile,TrajFiles,...> ]
...
Options:
    -a, --align <yes or no>  [default: no]
        Align input files to a reference file before visualization. The docked poses
    ...
    -b, --BFactorChain <yes or no>  [default: yes]
        A cartoon and putty around individual chains colored by an arbitrary set
        of B factor values. The minimum and maximum values for B factors are
        automatically detected. These values may indicate spread of electron
        density around atoms or correspond to any other property mapped to
        B factors in input file.
    --BFactorColorPalette <text>  [default: blue_white_red]
        Color palette for coloring cartoon and putty around chains generated using B
        factor values. Any valid PyMOL color palette name is allowed. No validation is
    ...
    --dockedPoses <PDBFile,ChainID,LigandID,InputFiles,...>  [default: none]
        PDB file name, pocket chain ID, ligand specification, and input files to use
        for creating pockets to visualize docked poses or any arbitrary set of
        molecules. Any valid PyMOL input file format is allowed.
        
        It's a quartet of comma limited values corresponding to PDF file name,
        pocket chain ID, ligand ID, and input files. Multiple input file names are
        delimited by spaces.
        ...
        The 'LigandID' must be a valid ligand ID in 'ChainID'. Alternatively, you may use
        input files by specifying 'UseInputFile' to select residues in 'ChainID' for creating
        pockets to visualize docked poses.
    --dockedPosesDistanceContactsCutoffs <number1,number2...>  [default: none]
        A comma delimited list of distances in Angstroms for identifying and displaying
        distance contacts between heavy atoms in pocket residues and docked poses. A
        PyMOL distance contact object is created for each specified distance. You may
        find it helpful for identifying steric clashes between docked poses and pocket
        residues. The maximum distance cutoff value must be less than the specified
        value for '--pocketDistanceCutoff' option.
    --dockedPosesDistanceContactsColor <text>  [default: red]
        Color for drawing distance contacts between docked poses and pocket residues.
        The specified value must be valid color. No validation is performed.
    --dockedPosesGroupName <text>  [default: Docked_Poses]
        PyMOL object name for docked poses group. You may use an artbitray name
        to reflect data in input file(s) specified in '--dockedPoses' option. It must be a valid
        PyMOL object name. No validation is performed.
    --dockedPosesName <text>  [default: Poses]
        PyMOL object name for docked poses object. You may use an artbitray name
        to reflect data in input file(s) specified in '--dockedPoses' option. It must be a
        valid PyMOL object name. No validation is performed.
    -e, --examples
        Print examples.
    -h, --help
    ...
    --pocketContactsInorganicPiCationColor <text>  [default: limon]
        Color for drawing pi cation contacts between inorganic and pocket residues.
        The specified value must be valid color. No validation is performed. The pi
        cation contacts are drawn using PyMOL distance command with support for
        mode 7 and may require incentive version of PyMOL.
    ...
    --pocketContactsLigandHalogenColor <text>  [default: magenta]
        Color for drawing halogen contacts between ligand and pocket residues.
        The specified value must be valid color. No validation is performed.
    ...
    --pocketContactsLigandPiCationColor <text>  [default: yelloworange]
        Color for drawing pi cation contacts between ligand and pocket residues.
        The specified value must be valid color. No validation is performed. The pi
        cation contacts are drawn using PyMOL distance command with support for
        mode 7 and may require incentive version of PyMOL.
    --pocketContactsLigandPiPiColor <text>  [default: cyan]
        Color for drawing pi pi contacts between ligand and pocket residues.
        The specified value must be valid color. No validation is performed. The pi
        pi contacts are drawn using PyMOL distance command with support for
        mode 6 and may require incentive version of PyMOL.
    ...
    --pocketContactsCutoff <number>  [default: 4.0]
        Distance in Angstroms for identifying polar, hyrdophobic contacts, pi pi,
        pi cation, and halogen contacts  between atoms in pocket residues and
        ligands.
    ...
    --selectionsPocket <ObjectName,SelectionSpec,...>  [default: none]
        Custom selections for pocket residues. It is a pairwise list of comma delimited
        values corresponding to PyMOL object names and selection specifications.  The
        selection specification must be a valid PyMOL specification. No validation is
        performed.
        
        The PyMOL objects are created for each pocket corresponding to the
        specified selections. The display style for PyMOL objects is set using
        value of '--selectionsChainStyle' option.
        
        The specified selection specification is automatically appended to appropriate
        pocket specification before creating PyMOL objects.
        
        For example, the following specification for '--selectionsPocket' option will
        generate PyMOL objects for pockets containing Tyrosines and Serines:
            
            Tyrosines,resn TYR,Serines,resn SER
            
    --selectionsPocketSurface <yes or no>  [default: auto]
        Surfaces around individual pocket chain selections colored by hydrophobicity
        alone and both hydrophobicity and charge. This option is similar to '--surfaceChain'
        options for creating surfaces for chain. Additional details are available in the
        documentation section for '--surfaceChain' options.
    --selectionsPocketStyle <DisplayStyle>  [default: sticks]
        Display style for PyMOL objects created for '--selectionsPocket' option. It
        must be a valid PyMOL display style. No validation is performed.
    ...
    -t, --trajectories <PDBToplogyFile,TrajFiles,...>  [default: none]
        PDB topology file name and MD trajectories files for visualizing trajectories
        for PDB files.
        
        It's a pair of comma limited values corresponding to a PDB file name and
        MD trajectory files. Multiple trajectory file names are delimited by spaces.
        ...
        The trajectory files must correspond to the specified PDB topology file. In
        addition, the format of trajectory files must be a valid PyMOL format. PyMOL
        uses Molfile Plugin, which supports a variety of trajectory file formats. For
        example: HARMM, NAMD, X-PLOR (.dcd), Gromacs TRR/XTC (.trr, .xtc),
        XYZ (.xyz) etc.
    --overwrite
...
Examples:
    ...
    To visualize the first chain along with all serines and tyrosines in binding
    pockets, the largest ligand in the first chain, and ligand binding pockets
    to highlight ligand interaction with pocket resiudes, solvents and inorganics,
    in a PDB file, and generate a PML file, type:

        % PyMOLVisualizeMacromolecules.py -i Sample4.pdb -o Sample4.pml
          --selectionsPocket "Serines,resn ser,Tyrosines,resn tyr"

    To visualize docking poses from a SD file in a pocket corresponding to a SD file
    for a chain, along with visualization of other information, in a PDB file,
    and generate a PML file, type:
    ...
    To visualize docking poses from a SD file in a pocket corresponding to SD file
    for a chain, along with visualization of distance contacts at 3.0 and 3.5 Angstroms,
    in a PDB file, and generate a PML file, type:
    ...
    To visualize docking poses from a SD file in a pocket corresponding to a specific
    ligand a chain, along with visualization of other information, in a PDB file,
    and generate a PML file, type:
    ...
    To visualize docking poses from multiple SDs file in a pocket corresponding SD file
    a for a chain, along with visualization of other information, in a PDB file, and
    ...
    To visualize trajectory from a DCD file,  along with visualization of other
    information, corresponding to a PDB topology file and generate a PML file,
    type:
