...
def PerformTorsionScan():
    ...
    MiscUtil.PrintInfo(
        "Generating output files %s_*.sdf, %s_*Torsion*Match*.sdf, %s_*Torsion*Match*Energies.csv, %s_*Torsion*Match*Plot.%s..."
        % (FileName, FileName, FileName, FileName, PlotExt)
    )

    MolCount, ValidMolCount, MinimizationFailedCount, TorsionsMissingCount, TorsionsScanFailedCount = ProcessMolecules(
        Mols
    )
    ...
    MiscUtil.PrintInfo(
        "Number of ignored molecules: %d"
        % (MolCount - ValidMolCount + TorsionsMissingCount + MinimizationFailedCount + TorsionsScanFailedCount)
    )
...
def ProcessMoleculesUsingSingleProcess(Mols):
    ...
    # Intialize Psi4...
    MiscUtil.PrintInfo("\nInitializing Psi4...")
    Psi4Handle = Psi4Util.InitializePsi4(
        Psi4RunParams=OptionsInfo["Psi4RunParams"],
        Psi4OptionsParams=OptionsInfo["Psi4OptionsParams"],
        PrintVersion=True,
        PrintHeader=True,
    )
    ...
    for Mol in Mols:
        ...
        Mol, MinimizationCalcStatus, TorsionsMatchStatus, TorsionsScanCalcStatus = PerformMinimizationAndTorsionScan(
            Mol, MolCount
        )
...
def WorkerProcess(EncodedMolInfo):
    ...
    return [
        ...
        RDKitUtil.MolToBase64EncodedMolString(
            Mol, PropertyPickleFlags=Chem.PropertyPickleOptions.MolProps | Chem.PropertyPickleOptions.PrivateProps
        ),
        MinimizationCalcStatus,
        TorsionsMatchStatus,
        TorsionsScanCalcStatus,
    ]
...
def ProcessMoleculesUsingMultipleProcessesAtTorsionAnglesLevel(Mols):
    ...
    for Mol in Mols:
        ...
        Mol, MinimizationCalcStatus, TorsionsMatchStatus, TorsionsScanCalcStatus = PerformMinimizationAndTorsionScan(
            Mol, MolCount, UseMultiProcessingAtTorsionAnglesLevel=True
        )
...
def ScanSingleTorsionInMolUsingMultipleProcessesAtTorsionAnglesLevel(
    Mol, TorsionID, TorsionPattern, TorsionPatternMol, TorsionMatches, MolNum
):
    ...
    if OptionsInfo["MPLevelMoleculesMode"]:
        MiscUtil.PrintError(
            "Single torison scanning for a molecule is not allowed in multiprocessing mode at molecules level.\n"
        )
    ...
    # Setup a encoded mols data iterable for a worker process...
    WorkerProcessDataIterable = GenerateBase64EncodedMolStringsWithTorsionScanInfo(
        Mol, (MolNum - 1), Mols, Angles, TorsionID, TorsionPattern, TorsionPatternMol, TorsionMatches
    )
...
def TorsionAngleWorkerProcess(EncodedMolInfo):
    ...
    TorsionMol, CalcStatus, Energy = MinimizeCalculateEnergyForTorsionMol(
        Mol, TorsionMol, TorsionAngle, TorsionID, TorsionPattern, TorsionPatternMol, TorsionMatches, (MolIndex + 1)
    )
...
def GenerateBase64EncodedMolStringsWithTorsionScanInfo(
    Mol,
    MolIndex,
    TorsionMols,
    TorsionAngles,
    ...
    TorsionPatternMol,
    TorsionMatches,
    PropertyPickleFlags=Chem.PropertyPickleOptions.AllProps,
):
    ...
    for Index, TorsionMol in enumerate(TorsionMols):
        yield (
            [MolIndex, None, None, TorsionAngles[Index], TorsionID, TorsionPattern, None, TorsionMatches]
            if (Mol is None or TorsionMol is None)
            else [
                MolIndex,
                RDKitUtil.MolToBase64EncodedMolString(Mol, PropertyPickleFlags),
                RDKitUtil.MolToBase64EncodedMolString(TorsionMol, PropertyPickleFlags),
                TorsionAngles[Index],
                TorsionID,
                TorsionPattern,
                RDKitUtil.MolToBase64EncodedMolString(TorsionPatternMol, PropertyPickleFlags),
                TorsionMatches,
            ]
        )
...
def InitializePsi4ForWorkerProcess():
    ...
    if OptionsInfo["MPLevelTorsionAnglesMode"] and re.match(
        "auto", OptionsInfo["Psi4RunParams"]["OutputFileSpecified"], re.I
    ):
        ...
        # Update output file...
        OptionsInfo["Psi4RunParams"]["OutputFile"] = Psi4Util.UpdatePsi4OutputFileUsingPID(
            OptionsInfo["Psi4RunParams"]["OutputFile"], os.getpid()
        )

    # Intialize Psi4...
    OptionsInfo["psi4"] = Psi4Util.InitializePsi4(
        Psi4RunParams=OptionsInfo["Psi4RunParams"],
        Psi4OptionsParams=OptionsInfo["Psi4OptionsParams"],
        PrintVersion=False,
        PrintHeader=True,
    ...
    for TorsionID in TorsionsPatternsInfo["IDs"]:
        ...
        if FirstTorsionMode and TorsionPatternCount > 1:
            if not OptionsInfo["QuietMode"]:
                MiscUtil.PrintWarning(
                    'Already scaned first torsion pattern, "%s" for molecule %s during "%s" value of "--modeTorsions" option . Abandoning torsion scan...\n'
        ...
        for Index, TorsionMatches in enumerate(TorsionsMatches):
            ...
            if TorsionMatchCount > TorsionMaxMatches:
                if not OptionsInfo["QuietMode"]:
                    MiscUtil.PrintWarning(
                        'Already scaned a maximum of %s torsion matches for molecule %s specified by "--torsionMaxMatches" option. Abandoning torsion scan...\n'
                        % (TorsionMaxMatches, MolName)
                    )
                break

            TmpMol, TorsionScanStatus, TorsionMols, TorsionEnergies, TorsionAngles = ScanSingleTorsionInMol(
                Mol,
                TorsionID,
                TorsionPattern,
                TorsionPatternMol,
                ...
                TorsionMatchNum,
                MolNum,
                UseMultiProcessingAtTorsionAnglesLevel,
            )
...
def ScanSingleTorsionInMol(
    ...
    if not OptionsInfo["QuietMode"]:
        MiscUtil.PrintInfo(
            ...
            % (TorsionPattern, TorsionMatchNum, RDKitUtil.GetMolName(Mol, MolNum))
        )

        if OptionsInfo["TorsionMinimize"]:
            MiscUtil.PrintInfo(
                "Generating initial ensemble of constrained conformations by distance geometry and forcefield followed by Psi4 constraned minimization to select the lowest energy structure at specific torsion angles for molecule %s..."
                % (RDKitUtil.GetMolName(Mol, MolNum))
            )
        else:
            MiscUtil.PrintInfo(
                "Calculating single point energy using Psi4 for molecule, %s, at specific torsion angles..."
                % (RDKitUtil.GetMolName(Mol, MolNum))
            )

    if UseMultiProcessingAtTorsionAnglesLevel:
        return ScanSingleTorsionInMolUsingMultipleProcessesAtTorsionAnglesLevel(
            Mol, TorsionID, TorsionPattern, TorsionPatternMol, TorsionMatches, MolNum
        )
    else:
        return ScanSingleTorsionInMolUsingSingleProcess(
            Mol, TorsionID, TorsionPattern, TorsionPatternMol, TorsionMatches, MolNum
        )
...
def ScanSingleTorsionInMolUsingSingleProcess(Mol, TorsionID, TorsionPattern, TorsionPatternMol, TorsionMatches, MolNum):
    ...
    for Index, Angle in enumerate(Angles):
        ...
        TorsionMol, CalcStatus, Energy = MinimizeCalculateEnergyForTorsionMol(
            Mol, TorsionMol, Angle, TorsionID, TorsionPattern, TorsionPatternMol, TorsionMatches, MolNum
        )
...
def SetupMolsForSingleTorsionScanInMol(Mol, TorsionMatches, MolNum=None):
    ...
    TorsionMols = []
    TorsionAngles = OptionsInfo["TorsionAngles"]
...
def MinimizeCalculateEnergyForTorsionMol(
    Mol, TorsionMol, TorsionAngle, TorsionID, TorsionPattern, TorsionPatternMol, TorsionMatches, MolNum
):
    ...
    if OptionsInfo["TorsionMinimize"]:
        ...
        TorsionMol, CalcStatus, Energy = ConstrainAndMinimizeMolecule(
            TorsionMol, TorsionAngle, TorsionMatchesMol, TorsionMatches, MolNum
        )
        ...
        if not CalcStatus:
            if not OptionsInfo["QuietMode"]:
                MolName = RDKitUtil.GetMolName(Mol, MolNum)
                MiscUtil.PrintWarning(
                    "Failed to perform constrained minimization for molecule %s with torsion angle set to %s during torsion scan for torsion pattern %s. Abandoning torsion scan..."
                    % (MolName, TorsionAngle, TorsionPattern)
                )
        ...
        if not CalcStatus:
            if not OptionsInfo["QuietMode"]:
                MolName = RDKitUtil.GetMolName(Mol, MolNum)
                MiscUtil.PrintWarning(
                    "Failed to calculate Psi4 energy for molecule %s with torsion angle set to %s during torsion scan for torsion pattern %s. Abandoning torsion scan..."
                    % (MolName, TorsionAngle, TorsionPattern)
                )
            return (TorsionMol, False, None)
...
def SetupTorsionsMolInfo(Mol, MolNum=None):
    ...
    for TorsionID in TorsionsPatternsInfo["IDs"]:
        ...
        TorsionPattern = TorsionsPatternsInfo["Pattern"][TorsionID]
        TorsionPatternMol = TorsionsPatternsInfo["Mol"][TorsionID]
        TorsionsMatches = RDKitUtil.FilterSubstructureMatchesByAtomMapNumbers(
            Mol, TorsionPatternMol, Mol.GetSubstructMatches(TorsionPatternMol, useChirality=UseChirality)
        )
        ...
        for Index, TorsionMatch in enumerate(TorsionsMatches):
            if len(TorsionMatch) != 4:
                if not OptionsInfo["QuietMode"]:
                    MiscUtil.PrintWarning(
                        "Ignoring invalid torsion match to atom indices, %s, for torsion pattern, %s, in molecule %s: It must match exactly 4 atoms."
                        % (TorsionMatch, TorsionPattern, MolName)
                    )
                continue
            ...
            if not RDKitUtil.AreAtomIndicesSequentiallyConnected(Mol, TorsionMatch):
                if not OptionsInfo["QuietMode"]:
                    MiscUtil.PrintInfo("")
                    MiscUtil.PrintWarning(
                        "Invalid torsion match to atom indices, %s, for torsion pattern, %s, in molecule %s: Matched atom indices must be sequentially connected."
                        % (TorsionMatch, TorsionPattern, MolName)
                    )
                    MiscUtil.PrintWarning("Reordering matched atom indices in a sequentially connected manner...")

                Status, ReorderdTorsionMatch = RDKitUtil.ReorderAtomIndicesInSequentiallyConnectedManner(
                    Mol, TorsionMatch
                )
                if Status:
                    ...
                    if not OptionsInfo["QuietMode"]:
                        MiscUtil.PrintWarning(
                            "Successfully reordered torsion match to atom indices, %s, for torsion pattern, %s, in molecule %s: Matched atom indices are now sequentially connected."
                            % (TorsionMatch, TorsionPattern, MolName)
                        )
                else:
                    if not OptionsInfo["QuietMode"]:
                        MiscUtil.PrintWarning(
                            "Ignoring torsion match. Failed to reorder torsion match to atom indices, %s, for torsion pattern, %s, in molecule %s: Matched atom indices are not sequentially connected."
                            % (TorsionMatch, TorsionPattern, MolName)
                        )
                    continue
            ...
            if Bond.IsInRing():
                if not OptionsInfo["QuietMode"]:
                    MiscUtil.PrintWarning(
                        "Ignoring invalid torsion match to atom indices, %s, for torsion pattern, %s, in molecule %s: Matched atom indices, %s and %s, are not allowed to be in a ring."
                        % (TorsionMatch, TorsionPattern, MolName, TorsionMatch[1], TorsionMatch[2])
                    )
                continue
            ...
            if OptionsInfo["FilterTorsionsByAtomIndicesMode"]:
                ...
                if len(InvalidAtomIndices):
                    if not OptionsInfo["QuietMode"]:
                        MiscUtil.PrintWarning(
                            'Ignoring invalid torsion match to atom indices, %s, for torsion pattern, %s, in molecule %s: Matched atom indices, %s,  must be present in the list, %s, specified using option "--torsionsFilterbyAtomIndices".'
                            % (
                                TorsionMatch,
                                TorsionPattern,
                                MolName,
                                InvalidAtomIndices,
                                OptionsInfo["TorsionsFilterByAtomIndicesList"],
                            )
                        )
                    continue
...
def SetupTorsionsPatternsInfo():
    ...
    for TorsionPattern in OptionsInfo["TorsionPatternsList"]:
        ...
        if TorsionMol is None:
            MiscUtil.PrintError(
                'Failed to create torsion pattern molecule. The torsion SMILES/SMARTS pattern, "%s", specified using "-t, --torsions" option is not valid.'
                % (TorsionPattern)
            )
...
def GenerateAndMinimizeConformersUsingForceField(Mol, MolNum=None):
    ...
    if not OptionsInfo["QuietMode"]:
        MiscUtil.PrintInfo(
            "Performing initial minimization of molecule, %s, using forcefield by generating a conformation ensemble and selecting the lowest energy conformer - EmbedRMSDCutoff: %s; Size: %s; Size after RMSD filtering: %s"
            % (
                MolName,
                OptionsInfo["ConfGenerationParams"]["EmbedRMSDCutoff"],
                OptionsInfo["ConfGenerationParams"]["MaxConfs"],
                len(ConfIDs),
            )
        )
    ...
    for ConfID in ConfIDs:
        ...
        if ConvergeStatus != 0:
            if not OptionsInfo["QuietMode"]:
                MiscUtil.PrintWarning(
                    'Minimization using forcefield failed to converge for molecule %s in %d steps. Try using higher value for "maxIters" in "--confParams" option...\n'
...
def ConstrainAndMinimizeMolecule(Mol, TorsionAngle, RefMolCore, RefMolMatches, MolNum=None):
    ...
    if not MolConfsStatus:
        if not OptionsInfo["QuietMode"]:
            MiscUtil.PrintWarning(
                "Conformation generation couldn't be performed for molecule %s: Constrained embedding failed...\n"
                % MolName
            )
    ...
    for ConfNum, MolConf in enumerate(MolConfs):
        if not OptionsInfo["QuietMode"]:
            MiscUtil.PrintInfo(
                ...
                % (MolName, ConfNum, TorsionAngle)
            )

        CalcStatus, Energy = ConstrainAndMinimizeMoleculeUsingPsi4(
            OptionsInfo["psi4"], MolConf, RefMolCore, RefMolMatches, MolNum
        )
        if not CalcStatus:
            if not OptionsInfo["QuietMode"]:
...
def ConstrainAndMinimizeMoleculeUsingPsi4(Psi4Handle, Mol, RefMolCore, RefMolMatches, MolNum, ConfID=-1):
    ...
    # Setup freeze list for constrained torsion...
    FreezeDihedralList = [("%s" % AtomIdex) for AtomIdex in ConstrainedAtomIndices]
    FreezeDihedralString = "%s" % " ".join(FreezeDihedralList)
    Psi4Handle.set_options({"OPTKING__frozen_dihedral": FreezeDihedralString})

    # Optimize geometry...
    Status, Energy, WaveFunction = Psi4Util.PerformGeometryOptimization(
        Psi4Handle, Psi4Mol, MethodName, BasisSet, ReturnWaveFunction=True, Quiet=OptionsInfo["QuietMode"]
    )
...
def ConstrainEmbedAndMinimizeMoleculeUsingRDKit(Mol, RefMolCore, RefMolMatches, MolNum=None):
    ...
    if OptionsInfo["ConfGenerationParams"]["UseUFF"]:
        ...
        ForceFieldFunction = lambda mol, confId=-1: AllChem.MMFFGetMoleculeForceField(
            mol,
            AllChem.MMFFGetMoleculeProperties(mol, mmffVariant=OptionsInfo["ConfGenerationParams"]["MMFFVariant"]),
            confId=confId,
        )
        ForceFieldName = "MMFF"
    ...
    ETVersion = OptionsInfo["ConfGenerationParams"]["ETVersion"]
    ...
    for ConfID in ConfIDs:
        try:
            ...
            RDKitUtil.ConstrainAndEmbed(
                MolConf,
                RefMolCore,
                coreMatchesMol=RefMolMatches,
                useTethers=UseTethers,
                coreConfId=-1,
                randomseed=ConfID,
                getForceField=ForceFieldFunction,
            ...
            if not OptionsInfo["QuietMode"]:
                MiscUtil.PrintWarning(
                    "Constrained embedding couldn't  be performed for molecule %s:\n%s\n"
...
def FilterConstrainedConformationsByRMSD(Mol, MolConfs, MolNum=None):
    ...
    if EmbedRMSDCutoff is None or EmbedRMSDCutoff <= 0:
        if not OptionsInfo["QuietMode"]:
            MiscUtil.PrintInfo(
                ...
                % (RDKitUtil.GetMolName(Mol, MolNum), len(MolConfs))
            )
    ...
    if not OptionsInfo["QuietMode"]:
        MiscUtil.PrintInfo(
            ...
            % (RDKitUtil.GetMolName(Mol, MolNum), EmbedRMSDCutoff, len(MolConfs), len(SelectedMolConfs))
        )
...
def EmbedMolecule(Mol, MolNum=None):
    ...
    try:
        ConfIDs = AllChem.EmbedMultipleConfs(
            ...
            pruneRmsThresh=EmbedRMSDCutoff,
            enforceChirality=EnforceChirality,
            useExpTorsionAnglePrefs=UseExpTorsionAnglePrefs,
            useBasicKnowledge=UseBasicKnowledge,
            ETversion=ETVersion,
...
def MinimizeMoleculeUsingForceField(Mol, MolNum, ConfID=-1):
    ...
    try:
        if OptionsInfo["ConfGenerationParams"]["UseUFF"]:
            ConvergeStatus = AllChem.UFFOptimizeMolecule(
                Mol, confId=ConfID, maxIters=OptionsInfo["ConfGenerationParams"]["MaxIters"]
            )
        elif OptionsInfo["ConfGenerationParams"]["UseMMFF"]:
            ConvergeStatus = AllChem.MMFFOptimizeMolecule(
                Mol,
                confId=ConfID,
                maxIters=OptionsInfo["ConfGenerationParams"]["MaxIters"],
                mmffVariant=OptionsInfo["ConfGenerationParams"]["MMFFVariant"],
            )
        else:
            MiscUtil.PrintError(
                "Minimization couldn't be performed: Specified forcefield, %s, is not supported"
                % OptionsInfo["ConfGenerationParams"]["ForceField"]
            )
    except (ValueError, RuntimeError, Chem.rdchem.KekulizeException) as ErrMsg:
        if not OptionsInfo["QuietMode"]:
            MolName = RDKitUtil.GetMolName(Mol, MolNum)
            MiscUtil.PrintWarning(
                "Minimization using forcefield couldn't be performed for molecule %s:\n%s\n" % (MolName, ErrMsg)
            )
        return (False, None)
...
def CalculateEnergyUsingForceField(Mol, ConfID=None):
    ...
    if OptionsInfo["ConfGenerationParams"]["UseUFF"]:
        ...
        MMFFMoleculeProperties = AllChem.MMFFGetMoleculeProperties(
            Mol, mmffVariant=OptionsInfo["ConfGenerationParams"]["MMFFVariant"]
        )
...
def CalculateEnergyUsingPsi4(Psi4Handle, Psi4Mol, Mol, MolNum=None):
    ...
    Status, Energy = Psi4Util.CalculateSinglePointEnergy(
        Psi4Handle, Psi4Mol, MethodName, BasisSet, Quiet=OptionsInfo["QuietMode"]
    )
...
def SetupConstrainedAtomIndicesForPsi4(Mol, RefMolCore, RefMolMatches):
    ...
    AtomIndices = [AtomIndex + 1 for AtomIndex in ConstrainAtomIndices]
...
def SetupEnergyConversionFactor(Psi4Handle):
    ...
    if re.match(r"^kcal\/mol$", EnergyUnits, re.I):
        ConversionFactor = Psi4Handle.constants.hartree2kcalmol
    elif re.match(r"^kJ\/mol$", EnergyUnits, re.I):
...
def GenerateOutputFiles(Mol, MolNum, TorsionID, TorsionMatchNum, TorsionMols, TorsionEnergies, TorsionAngles):
    ...
    GenerateScannedTorsionsStructureOutfile(
        StructureOutfile, Mol, MolNum, TorsionID, TorsionMatchNum, TorsionMols, TorsionEnergies, TorsionAngles
    )
    GenerateEnergyTextOutfile(
        EnergyTextOutfile, Mol, MolNum, TorsionID, TorsionMatchNum, TorsionMols, TorsionEnergies, TorsionAngles
    )
    GeneratePlotOutfile(
        PlotOutfile, Mol, MolNum, TorsionID, TorsionMatchNum, TorsionMols, TorsionEnergies, TorsionAngles
    )


def GenerateScannedTorsionsStructureOutfile(
    Outfile, Mol, MolNum, TorsionID, TorsionMatchNum, TorsionMols, TorsionEnergies, TorsionAngles
):
...
def ProcessTorsionRangeOptions():
    """Process tosion range options."""

    TosionRangeMode = Options["--torsionRangeMode"]
    OptionsInfo["TosionRangeMode"] = TosionRangeMode

    if re.match("^Range$", TosionRangeMode, re.I):
        ProcessTorsionRangeValues()
    elif re.match("^Angles$", TosionRangeMode, re.I):
        ProcessTorsionAnglesValues()
    else:
        MiscUtil.PrintError('The value, %s,  option "--torsionRangeMode" is not supported.' % TosionRangeMode)


def ProcessTorsionRangeValues():
    """Process tosion range values."""

    TorsionRange = Options["--torsionRange"]
    if re.match("^auto$", TorsionRange, re.I):
        TorsionRange = "0,360,5"
    ...
    if TorsionStart >= TorsionStop:
        MiscUtil.PrintError(
            'The start value, %d, specified for option "--torsionRange" in string "%s" must be less than stop value, %s.'
            % (TorsionStart, Options["--torsionRange"], TorsionStop)
        )
    if TorsionStep == 0:
        MiscUtil.PrintError(
            'The step value, %d, specified for option "--torsonRange" in string "%s" must be > 0.'
            % (TorsionStep, Options["--torsionRange"])
        )
    if TorsionStep >= (TorsionStop - TorsionStart):
        MiscUtil.PrintError(
            'The step value, %d, specified for option "--torsonRange" in string "%s" must be less than, %s.'
            % (TorsionStep, Options["--torsionRange"], (TorsionStop - TorsionStart))
        )

    if TorsionStart < 0:
        if TorsionStart < -180:
            MiscUtil.PrintError(
                'The start value, %d, specified for option "--torsionRange" in string "%s" must be  >= -180 to use scan range from -180 to 180.'
                % (TorsionStart, Options["--torsionRange"])
            )
        if TorsionStop > 180:
            MiscUtil.PrintError(
                'The stop value, %d, specified for option "--torsionRange" in string "%s" must be <= 180 to use scan range from -180 to 180.'
                % (TorsionStop, Options["--torsionRange"])
            )
    else:
        if TorsionStop > 360:
            MiscUtil.PrintError(
                'The stop value, %d, specified for option "--torsionRange" in string "%s" must be  <= 360 to use scan range from 0 to 360.'
                % (TorsionStop, Options["--torsionRange"])
            )

    TorsionAngles = [Angle for Angle in range(TorsionStart, TorsionStop, TorsionStep)]
    TorsionAngles.append(TorsionStop)
    ...
    OptionsInfo["TorsionAngles"] = TorsionAngles


def ProcessTorsionAnglesValues():
    """Process tosion angle values."""

    TorsionRange = Options["--torsionRange"]
    if re.match("^auto$", TorsionRange, re.I):
        MiscUtil.PrintError('The value specified, %s, for option "--torsionRange" is not valid.' % (TorsionRange))

    TorsionAngles = []

    for TorsionAngle in TorsionRange.split(","):
        TorsionAngle = int(TorsionAngle)

        if TorsionAngle < -180:
            MiscUtil.PrintError(
                'The stop value, %d, specified for option "--torsionRange" in string "%s" must be  >= -180.'
                % (TorsionAngle, TorsionRange)
            )

        if TorsionAngle > 360:
            MiscUtil.PrintError(
                'The stop value, %d, specified for option "--torsionRange" in string "%s" must be  <= 360.'
                % (TorsionAngle, TorsionRange)
            )
    ...
    OptionsInfo["TorsionRange"] = TorsionRange
    OptionsInfo["TorsionStart"] = None
    OptionsInfo["TorsionStop"] = None
    OptionsInfo["TorsionStep"] = None

    OptionsInfo["TorsionAngles"] = sorted(TorsionAngles)
...
def ProcessOptions():
    ...
    OptionsInfo["Psi4OptionsParams"] = Psi4Util.ProcessPsi4OptionsParameters(
        "--psi4OptionsParams", Options["--psi4OptionsParams"]
    )
    OptionsInfo["Psi4RunParams"] = Psi4Util.ProcessPsi4RunParameters(
        "--psi4RunParams", Options["--psi4RunParams"], InfileName=OptionsInfo["Infile"]
    )

    # Conformer generation paramaters...
    ParamsDefaultInfoOverride = {"MaxConfs": 250, "MaxConfsTorsions": 50}
    OptionsInfo["ConfGenerationParams"] = MiscUtil.ProcessOptionConformerParameters(
        "--confParams", Options["--confParams"], ParamsDefaultInfoOverride
    )
    ...
    OptionsInfo["OutPlotRelativeEnergy"] = (
        True if re.match("^yes$", Options["--outPlotRelativeEnergy"], re.I) else False
    )
    OptionsInfo["OutPlotTitleTorsionSpec"] = (
        True if re.match("^yes$", Options["--outPlotTitleTorsionSpec"], re.I) else False
    )
    ...
    for TorsionPattern in TorsionPatterns.split(","):
        ...
        if not len(TorsionPattern):
            MiscUtil.PrintError(
                'Empty value specified for SMILES/SMARTS pattern in  "-t, --torsions" option: %s' % TorsionPatterns
            )

        TorsionMol = Chem.MolFromSmarts(TorsionPattern)
        if TorsionMol is None:
            MiscUtil.PrintError(
                'Failed to create torsion pattern molecule. The torsion SMILES/SMARTS pattern, "%s", specified using "-t, --torsions" option, "%s",  is not valid.'
                % (TorsionPattern, TorsionPatterns)
            )
    ...
    if not re.match("^None$", TorsionsFilterByAtomIndices, re.I):
        for AtomIndex in TorsionsFilterByAtomIndices.split(","):
            ...
            if not MiscUtil.IsInteger(AtomIndex):
                MiscUtil.PrintError(
                    'The value specified, %s, for option "--torsionsFilterbyAtomIndices" must be an integer.'
                    % AtomIndex
                )
            AtomIndex = int(AtomIndex)
            if AtomIndex < 0:
                MiscUtil.PrintError(
                    'The value specified, %s, for option "--torsionsFilterbyAtomIndices" must be >= 0.' % AtomIndex
                )
            TorsionsFilterByAtomIndicesList.append(AtomIndex)

        if len(TorsionsFilterByAtomIndicesList) < 4:
            MiscUtil.PrintError(
                'The number of values, %s,  specified, %s, for option "--torsionsFilterbyAtomIndices" must be >=4.'
                % (len(TorsionsFilterByAtomIndicesList), TorsionsFilterByAtomIndices)
            )
...
def ValidateOptions():
    ...
    if not Options["--overwrite"]:
        ...
        if len(FileNames):
            MiscUtil.PrintError(
                'The outfile names, %s_*, generated from file specified, %s, for option "-o, --outfile" already exist. Use option "--overwrite" or "--ov"  and try again.\n'
    ...
    MiscUtil.ValidateOptionTextValue("--torsionRangeMode", Options["--torsionRangeMode"], "Range or Angles")
    TorsionRange = Options["--torsionRange"]
    if re.match("^Range$", Options["--torsionRangeMode"], re.I):
        if not re.match("^auto$", TorsionRange, re.I):
            MiscUtil.ValidateOptionNumberValues("--torsionRange", TorsionRange, 3, ",", "integer", {})
    else:
        if re.match("^auto$", TorsionRange, re.I):
            MiscUtil.PrintError(
                'The value, %s, specified for option "-torsionRange" is not valid for "%s" value of "--torsionRangeMode" option. You must specify a torsion angle or a comma delimited list of torsion angles.'
                % (TorsionRange, Options["--torsionRangeMode"])
            )
        TorsionAngles = []
        for TorsionAngle in TorsionRange.split(","):
            TorsionAngle = TorsionAngle.strip()
            if not MiscUtil.IsInteger(TorsionAngle):
                MiscUtil.PrintError(
                    'The value specified, %s, for option "--torsionRange" in string "%s" must be an integer.'
                    % (TorsionAngle, TorsionRange)
                )
            if TorsionAngle in TorsionAngles:
                MiscUtil.PrintError(
                    'The value specified, %s, for option "--torsionRange" in string "%s" is a duplicate value.'
                    % (TorsionAngle, TorsionRange)
                )
            TorsionAngles.append(TorsionAngle)
...
Usage:
    Psi4PerformTorsionScan.py [--basisSet <text>] [--confParams <Name,Value,...>] [--energyDataFieldLabel <text>]
                              ...
                              [--torsionMaxMatches <number>] [--torsionMinimize <yes or no>] [--torsionRangeMode <Range or Angles>]
                              [--torsionRange <Start,Stop,Step or Angle1,Angle2,...>] [--useChirality <yes or no>]
...
Options:
    ...
    --confParams <Name,Value,...>  [default: auto]
        ...
        along with basic Knowledge-terms and Distance Geometry (ETKDG or
        ETKDGv2) [Ref 129, 167] .
    ...
    --torsionRangeMode <Range or Angles>  [default: Range]
        Perform torsion scan using torsion angles corresponding to a torsion range
        or an explicit list of torsion angles. Possible values: Range or Angles. You
        may use '--torsionRange' option to specify values for torsion angle or
        torsion angles.
    --torsionRange <Start,Stop,Step or Angle1,Angle2...>  [default: auto]
        Start, stop, and step size angles or a comma delimited list of angles in
        degrees for a torsion scan.
        
        This value is '--torsionRangeMode' specific. It must be a triplet corresponding
        to 'start,Stop,Step' for 'Range' value of '--torsionRange' option. Otherwise, it
        is comma delimited list of one or more torsion angles for 'Angles' value of
        '--torsionRange' option.
        
        The default values, based on '--torsionRangeMode' option, are shown below:
            
            TorsionRangeMode       Default value
            Range                  0,360,5
            Angles                 None
            
        You must explicitly provide a list of  torsion angle(s) for 'Angles' of
        '--torsionRangeMode' option.
...
Examples:
    ...
    To run the previous example for performing a torsion scan using a specific list
    of torsion angles, type:
    
        % Psi4PerformTorsionScan.py  -t "CCCC" -i Psi4SampleTorsionScan.smi 
          -o SampleOut.sdf --torsionRangeMode Angles
          --torsionRange "0,180,360"
    ...
    for specific torsion angles using ETKDGv2 methodology followed by initial MMFF
