...
def MinimizeCalculateEnergyForTorsionMol(
    ...
    if OptionsInfo["TorsionMinimize"]:
        ...
        TorsionMol, CalcStatus, Energy = ConstrainAndMinimizeMolecule(
            TorsionMol, TorsionMatchesMol, TorsionMatches, MolNum
        )
        ...
        if not CalcStatus:
            if not OptionsInfo["QuietMode"]:
                MolName = RDKitUtil.GetMolName(Mol, MolNum)
                MiscUtil.PrintWarning(
                    "Failed to retrieve calculated energy for molecule %s with torsion angle set to %s during torsion scan for torsion pattern %s. Abandoning torsion scan..."
                    % (MolName, TorsionAngle, TorsionPattern)
                )
            return (TorsionMol, False, None)
...
def MinimizeMolecule(Mol, MolNum=None):
    ...
    if not OptionsInfo["QuietMode"]:
        MiscUtil.PrintInfo("Minimizing molecule %s..." % (RDKitUtil.GetMolName(Mol, MolNum)))
        ...
        try:
            if OptionsInfo["UseUFF"]:
                ...
                Status = AllChem.MMFFOptimizeMolecule(
                    Mol, confId=ConfID, maxIters=OptionsInfo["MaxIters"], mmffVariant=OptionsInfo["MMFFVariant"]
                )
            else:
                MiscUtil.PrintError(
...
def ConstrainAndMinimizeMolecule(Mol, RefMolCore, RefMolMatches=None, MolNum=None):
    ...
    if OptionsInfo["UseUFF"]:
        ...
        ForceFieldFunction = lambda mol, confId=-1: AllChem.MMFFGetMoleculeForceField(
            mol, AllChem.MMFFGetMoleculeProperties(mol, mmffVariant=OptionsInfo["MMFFVariant"]), confId=confId
        )
        ForceFieldName = "MMFF"
    ...
    ETVersion = OptionsInfo["ETVersion"]
...
def GetEnergy(Mol, ConfID=None):
    ...
    if OptionsInfo["UseUFF"]:
        ...
        MiscUtil.PrintError(
            "Couldn't retrieve conformer energy: Specified forcefield, %s, is not supported" % OptionsInfo["ForceField"]
        )
...
def GenerateScannedTorsionsStructureOutfile(
    ...
    for Index, TorsionMol in enumerate(TorsionMols):
        ...
        RelativeTorsionEnergy = "%.2f" % RelativeTorsionEnergies[Index]
        TorsionMol.SetProp(OptionsInfo["RelativeEnergyLabel"], RelativeTorsionEnergy)
...
def ProcesssConformerGeneratorOption():
    """Process comformer generator option."""

    ConfGenParams = MiscUtil.ProcessOptionConformerGenerator("--conformerGenerator", Options["--conformerGenerator"])

    OptionsInfo["ConformerGenerator"] = ConfGenParams["ConformerGenerator"]
    OptionsInfo["UseBasicKnowledge"] = ConfGenParams["UseBasicKnowledge"]
    OptionsInfo["UseExpTorsionAnglePrefs"] = ConfGenParams["UseExpTorsionAnglePrefs"]
    OptionsInfo["ETVersion"] = ConfGenParams["ETVersion"]
...
def ProcessOptions():
    ...
    # The default width and height, 10.0 and 5.6, map to aspect raito of 16/9 (1.778)...
    DefaultValues = {
        "Type": "linepoint",
        "Width": 10.0,
        "Height": 5.6,
        "Title": "RDKit Torsion Scan",
        "XLabel": "Torsion Angle (degrees)",
        "YLabel": "Energy (kcal/mol)",
    }
    if OptionsInfo["OutPlotRelativeEnergy"]:
        DefaultValues["YLabel"] = "Relative Energy (kcal/mol)"
    ...
    ProcesssConformerGeneratorOption()
    ...
    if re.match("^UFF$", Options["--forceField"], re.I):
        ...
        MiscUtil.PrintError(
            'The value, %s, specified for "--forceField" is not supported.' % (Options["--forceField"],)
        )
    ...
    if UseMMFF:
        ...
        OptionsInfo["RelativeEnergyLabel"] = "%s_Relative_Energy" % MMFFVariant
    else:
        OptionsInfo["EnergyLabel"] = "%s_Energy" % ForceField
        OptionsInfo["RelativeEnergyLabel"] = "%s_Relative_Energy" % ForceField
...
def ValidateOptions():
    ...
    MiscUtil.ValidateOptionTextValue(
        "-c, --conformerGenerator", Options["--conformerGenerator"], "SDG KDG ETDG ETKDG ETKDGv2"
    )
...
Usage:
    RDKitPerformTorsionScan.py [--addHydrogens <yes or no>] [--conformerGenerator <SDG, KDG, ETDG, ETKDG, ETKDGv2>]
                               ...
                               [--maxConfsTorsion <number>] [--maxIters <number>] [--mp <yes or no>]
                               ...
                               [--outPlotRelativeEnergy <yes or no>] [--outPlotTitleTorsionSpec <yes or no>] [--overwrite]  [--quiet <yes or no>]
                               [--removeHydrogens <yes or no>] [--randomSeed <number>] [--torsionsFilterbyAtomIndices <Index1, Index2, ...>]
                               [--torsionMaxMatches <number>] [--torsionMinimize <yes or no>] [--torsionRangeMode <Range or Angles>]
                               [--torsionRange <Start,Stop,Step or Angle1,Angle2,...>] [--useChirality <yes or no>] [--useTethers  <yes or no>]
...
Options:
    ...
    -c, --conformerGenerator <text>  [default: ETKDGv2]
        ...
        The possible values along with a brief description are shown below:
...
Examples:
    To perform a torsion scan from 0 to 360 degrees with a stepsize of 5 on the
    first molecule in a SMILES file using a minimum energy structure of the molecule
    selected from an ensemble of conformations, skipping generation of conformation
    ensembles for specific torsion angles and constrained energy minimization of the
    ensemble, generate output files corresponding to structure, energy and torsion
    plot, type:
    ...
    of torsion angles, type:
        ...
        % RDKitPerformTorsionScan.py  -t "O=CNC" -i SampleSeriesD3R.smi 
          -o SampleOut.sdf --torsionRangleMode Angles
          --torsionRange "160,220,280"
