...
def GenerateSVGImageFile(ValidMols, MolNames, AtomLists, BondLists):
    ...
    MolsSVGText = RDKitUtil.GetSVGForMolecules(
        ValidMols,
        OptionsInfo["NumOfMolsPerRow"],
        OptionsInfo["MolImageWidth"],
        OptionsInfo["MolImageHeight"],
        Legends=MolNames,
        AtomListsToHighlight=AtomLists,
        BondListsToHighlight=BondLists,
        BoldText=OptionsInfo["FontBold"],
    )
...
def GenerateImageFile(ValidMols, MolNames, AtomLists, BondLists):
    ...
    try:
        MolsImage = Draw.MolsToGridImage(
            ValidMols,
        ...
        # MolsToGridImage doesn't appear to handle the following parameters in the latest version of RDKit:
        #      . kekulize = OptionsInfo["Kekulize"], options = UpdatedDrawingOptions
        MolsImage = Draw.MolsToGridImage(
            ValidMols,
            molsPerRow=NumOfMolsPerRow,
            subImgSize=(Width, Height),
...
def SetupStructureDataDrawing(Writer, MolIndex, Mols, MolNames, HighlightAtomLists, HighlightBondLists):
    ...
    SVGText = RDKitUtil.GetInlineSVGForMolecule(
        ...
        OptionsInfo["MolImageHeight"],
        Legend=MolName,
        AtomListToHighlight=HighlightAtomList,
        BondListToHighlight=HighlightBondList,
        BoldText=OptionsInfo["FontBold"],
    ...
    if OptionsInfo["MolImageEncoded"]:
        SVGInlineImageTag = '%s src="data:image/svg+xml;base64,\n%s"' % (ImageTag, SVGText)
    else:
...
def WriteHTMLPageHeader(Writer, MolCount):
    ...
    ColVisibilityExcludeColIndices = (
        MiscUtil.JoinWords(ColVisibilityExcludeColIndicesList, ", ") if len(ColVisibilityExcludeColIndicesList) else ""
    )
    ...
    if OptionsInfo["ColVisibility"]:
        if ColCount > MaxDataColVisColCount:
            ...
            MiscUtil.PrintWarning(
                "The number of data columns, %d, is quite large. Only first %d data columns will be available in column visibility pulldown."
                % (ColCount, MaxDataColVisColCount)
            )
...
def WriteHTMLTableHeader(Writer):
    ...
    if OptionsInfo["TableStyle"] is None:
        ...
        Writer.write(
            """    <table id="MolsTable" class="%s" cellspacing="0" width="100%s">\n"""
...
def GetMolPopoverTag(Mol):
    ...
    PopoverTag = (
        """class="MolPopover" data-toggle="popover" data-html="true" data-trigger="click" data-placement="right" title="<span class='small'><b>Additional Information</b></span>" data-content="<span class='small'>%s</span>" """
        % DataContent
    )
...
Usage:
    RDKitDrawMolecules.py [--alignmentSMARTS <SMARTS>] [--atomLabelFontSize <number>]
                             ...
                             [--molImageSize <width,height>] [--molImageEncoded <yes or no> ]
...
Description:
    ...
    The options '--atomLabelFontSize' and '--bondLineWidth' don't appear to work
    during the generation of a SVG image. In addition, these may not work for other
    image types in the latest version of RDKIT.
...
Options:
    ...
    --atomLabelFontSize <number>  [default: 12]
        ...
        a SVG and HTML output file. This option may not work in the latest version of RDKit.
    -b, --bondLineWidth <number>  [default: 1.2]
        Line width for drawing bonds. This option is ignored during the generation of a SVG
        and HTML output file. This option may not work in the latest version of RDKit.
    ...
    -f --fontBold <yes or no>  [default: yes]
        ...
        option is ignored for all other output files. This option may not work in the latest
        version of RDKit.
