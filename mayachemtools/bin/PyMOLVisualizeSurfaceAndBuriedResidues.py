...
def WriteChainAloneViews(OutFH, FileIndex, PyMOLObjectNames, ChainID):
    ...
    for GroupType in ["Surface_Residues", "Buried_Residues"]:
        ...
        WriteChainAloneResiduesGroupResiduesTypesView(
            GroupType, OutFH, FileIndex, PyMOLObjectNames, ChainID, ResiduesGroupID
        )
        WriteChainAloneResiduesGroupResiduesSurfacesView(
            GroupType, OutFH, FileIndex, PyMOLObjectNames, ChainID, ResiduesGroupID
        )
...
def GetChainResiduesSelection(FileIndex, ChainID, GroupType):
    ...
    SpecifiedChainsAndLigandsInfo = OptionsInfo["InfilesInfo"]["SpecifiedChainsAndLigandsInfo"][FileIndex]
    ResiduesSelection = (
        SpecifiedChainsAndLigandsInfo["SurfaceResiduesSelection"][ChainID]
        if re.match("^Surface_Residues$", GroupType, re.I)
        else SpecifiedChainsAndLigandsInfo["BuriedResiduesSelection"][ChainID]
    )
...
def WriteChainAloneResiduesGroupResiduesSurfacesView(
    GroupType, OutFH, FileIndex, PyMOLObjectNames, ChainID, MainGroupID
):
    ...
    if GetChainAloneSurfaceChainStatus(FileIndex, ChainID):
        ...
        if GetChainAloneSurfaceChainElectrostaticsStatus(FileIndex, ChainID):
            ...
            WriteSurfaceElectrostaticsView(
                OutFH, ChainName, ElectrostaticsGroupName, ElectrostaticsGroupMembers, DisplayAs=None
            )

    # Setup surfaces group...
    GenerateAndWritePMLForGroup(
        OutFH,
        PyMOLObjectNames["Chains"][ChainID][SurfacesGroupID],
        PyMOLObjectNames["Chains"][ChainID][SurfacesGroupMembersID],
        True,
        "close",
    )
...
def WriteSurfaceElectrostaticsView(
    OutFH, SelectionObjectName, ElectrostaticsGroupName, ElectrostaticsGroupMembers, DisplayAs="lines"
):
...
def DeleteEmptyPyMOLObjects(OutFH, FileIndex, PyMOLObjectNames):
    ...
    for ChainID in SpecifiedChainsAndLigandsInfo["ChainIDs"]:
        ...
        for GroupType in ["Surface_Residues", "Buried_Residues"]:
            ...
            DeleteEmptyChainAloneResiduesGroupResiduesTypesObjects(
                GroupType, OutFH, FileIndex, PyMOLObjectNames, ChainID, ResiduesGroupID
            )

            # Delete empty surface objects...
            DeleteEmptyAloneResiduesGroupResiduesSurfacesObjects(
                GroupType, OutFH, FileIndex, PyMOLObjectNames, ChainID, ResiduesGroupID
            )


def DeleteEmptyChainAloneResiduesGroupResiduesTypesObjects(
    GroupType, OutFH, FileIndex, PyMOLObjectNames, ChainID, MainGroupID
):
    ...
    DeleteResiduesGroup = (
        False if AreGroupTypeResiduesPresent(GroupType, FileIndex, PyMOLObjectNames, ChainID) else True
    )
...
def SetupPyMOLObjectNamesForChain(FileIndex, PyMOLObjectNames, ChainID):
    ...
    for GroupType in ["Surface_Residues", "Buried_Residues"]:
        ...
        SetupPyMOLObjectNamesForChainResiduesTypes(
            FileIndex, PyMOLObjectNames, ChainID, ResiduesGroupName, ResiduesGroupID, ResiduesGroupMembersID
        )
        SetupPyMOLObjectNamesForChainResiduesSurfaces(
            FileIndex, PyMOLObjectNames, ChainID, ResiduesGroupName, ResiduesGroupID, ResiduesGroupMembersID
        )
...
def SetupPyMOLObjectNamesForChainResiduesTypes(
    FileIndex, PyMOLObjectNames, ChainID, MainGroupName, MainGroupID, MainGroupMembersID
):
...
def SetupPyMOLObjectNamesForChainResiduesSurfaces(
    FileIndex, PyMOLObjectNames, ChainID, MainGroupName, MainGroupID, MainGroupMembersID
):
...
def RetrieveSurfaceAndBuriedResiduesInfo():
    ...
    for FileIndex in range(0, len(OptionsInfo["InfilesInfo"]["InfilesNames"])):
        ...
        for ChainID in SpecifiedChainsAndLigandsInfo["ChainIDs"]:
            SurfaceResiduesInfo, BuriedResiduesInfo = PyMOLUtil.GetSurfaceAndBuriedResiduesInfo(
                MolName, ChainID, OptionsInfo["CutoffSASA"]
            )
            ...
            SurfaceResiduesSelection, BuriedResiduesSelection = SetupSurfaceAndBuriedResiduesSelection(
                SurfaceResiduesNums, BuriedResiduesNums
            )

            # Track surface and buried residues...
            SpecifiedChainsAndLigandsInfo["SurfaceResiduesPresent"][ChainID] = (
                True if len(SurfaceResiduesNums) else False
            )
            ...
            SurfaceResiduesCount, SurfaceResiduesDistribution, SurfaceResiduesIDs = FormatResiduesInfo(
                SurfaceResiduesInfo
            )
            MiscUtil.PrintInfo(
                "\nInput file: %s; ChainID: %s\nNumber of surface residues: %d"
                % (Infile, ChainID, SurfaceResiduesCount)
            )
...
def FormatResiduesInfo(SelectionInfo):
    ...
    SortedResNames = sorted(
        SelectionInfo["ResNames"], key=lambda ResName: SelectionInfo["ResCount"][ResName], reverse=True
    )
...
def ProcessResidueTypesAndSurfaceOptions(FileIndex, SpecifiedChainsAndLigandsInfo):
    ...
    for ChainID in SpecifiedChainsAndLigandsInfo["ChainIDs"]:
        ...
        # A generic color surface is always created...
        ChainSurfaces = True
...
def GetChainAloneResidueTypesStatus(FileIndex, ChainID):
    ...
    Status = (
        True
        if OptionsInfo["InfilesInfo"]["SpecifiedChainsAndLigandsInfo"][FileIndex]["ResidueTypesChain"][ChainID]
        else False
    )
...
def ProcessOptions():
    ...
    if len(SurfaceColorsWords) != 2:
        MiscUtil.PrintError(
            'The number of comma delimited color names, %d, specified using "--surfaceColors" option, "%s",  must be a 2.'
...
Usage:
    PyMOLVisualizeSurfaceAndBuriedResidues.py [--align <yes or no>] [--alignMethod <align, cealign, super>]
                                    ...
                                    [--ignoreHydrogens <yes or no>] [--ligandIDs <Largest, All or ID1,ID2...> ] [--PMLOut <yes or no>]
                                    [--residueIDs <yes or no> ] [--residueTypes <Type,Color,ResNames,...>] [--residueTypesChain <yes or no>]
