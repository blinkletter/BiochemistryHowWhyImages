...
def ApplyMutation(Mutation, MolName, ChainID, ResNum, NewBaseName):
    ...
    try:
        ...
        MiscUtil.PrintError(
            "The nucleic acids mutageneis wizard is not available in your PyMOL environment: %s\n" % ErrMsg
        )
...
def ProcessSpecifiedMutations():
    ...
    if not len(MutationsWords):
        MiscUtil.PrintError(
            'The number of comma delimited mutations specified using "-m, --mutations" option, "%s",  must be > 0.'
            % (OptionsInfo["Mutations"])
        )
    ...
    for Mutation in MutationsWords:
        ...
        if not len(Mutation):
            MiscUtil.PrintWarning(
                'The mutation, "%s", specified using "-m, --mutations" option is empty.\nIgnoring mutation...'
                % (Mutation)
            )
            continue
        ...
        if not MatchedResults:
            MiscUtil.PrintWarning(
                'The format of mutation, "%s", specified using "-m, --mutations" option is not valid. Supported format: <ChainID>:<ResNum><BaseName> or <ResNum><BaseName>\nIgnoring mutation...'
                % (Mutation)
            )
            continue
        ...
        if FirstMutation:
            ...
            if CurrentChainID is None:
                MiscUtil.PrintError(
                    'The first mutation, "%s", specified using "-m, --mutations" option must be colon delimited and contain only two values, the first value corresponding to chain ID'
                    % (Mutation)
                )
        ...
        # Check for duplicate mutation specifications...
        MutationSpec = "%s:%s%s" % (CurrentChainID, ResNum, NewBaseName)
        CanonicalMutation = MutationSpec.lower()
        ...
        if (ResiduesInfo is None) or (not len(ResiduesInfo["ResNames"])):
            MiscUtil.PrintWarning(
                'The residue number, %s, in mutation, "%s", specified using "-m, --mutations" option appears to be missing in input file.\nIgnoring mutation...'
                % (ResNum, Mutation)
            )
            continue
    ...
    if not len(SpecifiedMutationsInfo):
        MiscUtil.PrintError(
            'No valid mutations, "%s" specified using "-m, --mutations" option.' % (OptionsInfo["Mutations"])
        )
