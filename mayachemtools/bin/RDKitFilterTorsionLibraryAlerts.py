...
#
# Acknowledgments: Wolfgang Guba, Patrick Penner, and Levi Pierce
#
...
def ProcessMoleculesUsingSingleProcess(Mols):
    ...
    # Instantiate torsion library alerts class...
    ...
    for Mol in Mols:
        ...
        RotBondsAlertsStatus, RotBondsAlertsInfo = (
            TorsionLibraryAlertsHandle.IdentifyTorsionLibraryAlertsForRotatableBonds(Mol)
        )
...
def ProcessMoleculesUsingMultipleProcesses(Mols):
    ...
    InstantiateTorsionLibraryAlertsClass()
...
def InitializeWorkerProcess(*EncodedArgs):
    ...
    # Instantiate torsion slibrary alerts class...
    OptionsInfo["TorsionLibraryAlertsHandle"] = InstantiateTorsionLibraryAlertsClass(Quiet=True)
...
def WorkerProcess(EncodedMolInfo):
    ...
    # Identify torsion library alerts for rotatable bonds..
    TorsionTorsionLibraryAlertsHandle = OptionsInfo["TorsionLibraryAlertsHandle"]
    RotBondsAlertsStatus, RotBondsAlertsInfo = (
        TorsionTorsionLibraryAlertsHandle.IdentifyTorsionLibraryAlertsForRotatableBonds(Mol)
    )
...
def TrackTorsionAlertsSummaryInfo(TorsionAlertsSummaryInfo, RotBondsAlertsInfo):
    ...
    for ID in RotBondsAlertsInfo["IDs"]:
        ...
        if TorsionRuleNodeID not in TorsionAlertsSummaryInfo["RuleSMARTS"]:
            ...
            TorsionAlertsSummaryInfo["TorsionRuleTolerances1"][TorsionRuleNodeID] = RotBondsAlertsInfo[
                "TorsionRuleTolerances1"
            ][ID]
            TorsionAlertsSummaryInfo["TorsionRuleTolerances2"][TorsionRuleNodeID] = RotBondsAlertsInfo[
                "TorsionRuleTolerances2"
            ][ID]
...
def WriteTorsionAlertsSummaryInfo(Writer, TorsionAlertsSummaryInfo):
    ...
    for ID in SortedRuleIDs:
        ...
        TorsionPeaks = MiscUtil.JoinWords(
            ["%s" % Value for Value in TorsionAlertsSummaryInfo["TorsionRulePeaks"][ID]], ","
        )
        TorsionRuleTolerances1 = MiscUtil.JoinWords(
            ["%s" % Value for Value in TorsionAlertsSummaryInfo["TorsionRuleTolerances1"][ID]], ","
        )
        TorsionRuleTolerances2 = MiscUtil.JoinWords(
            ["%s" % Value for Value in TorsionAlertsSummaryInfo["TorsionRuleTolerances2"][ID]], ","
        )
        ...
        Values = [
            ...
            HierarchyClassName,
            HierarchySubClassName,
            "%s" % MiscUtil.JoinWords(AlertTypes, ","),
            "%s" % (MiscUtil.JoinWords(AlertTypeCount, ",")),
            "%s" % (MiscUtil.JoinWords(AlertTypeMolCount, ",")),
        ]
        Writer.write("%s\n" % MiscUtil.JoinWords(Values, ",", QuoteValues))
...
def SetupMolPropertiesForAlertsInformation(Mol, RotBondsAlertsInfo):
    ...
    if len(AlertsCount):
        Mol.SetProp(
            OptionsInfo["SDFieldIDsToLabels"]["TorsionAlertsCountLabel"], "%s" % MiscUtil.JoinWords(AlertsCount, " ")
        )
...
def SetupMolPropertiesForFilteredByRuleIDAlertsInformation(
    ...
    Mol.SetProp(
        OptionsInfo["SDFieldIDsToLabels"]["TorsionRuleAlertsCountLabel"], "%s" % (InterSetValuesDelim.join(AlertsCount))
    )
    ...
    Mol.SetProp(
        OptionsInfo["SDFieldIDsToLabels"]["TorsionRuleMaxAngleViolationLabel"], "%.2f" % (max(TorsionAngleViolations))
    )
...
def InstantiateTorsionLibraryAlertsClass(Quiet=False):
    """Initialize torsion library alerts class."""

    try:
        TorsionLibraryAlertsHandle = TorsionLibraryAlerts(
            AlertsMode=OptionsInfo["AlertsMode"],
            MinAlertsCount=OptionsInfo["MinAlertsCount"],
            NitrogenLonePairAllowHydrogenNbrs=OptionsInfo["NitrogenLonePairParams"]["AllowHydrogenNbrs"],
            NitrogenLonePairPlanarityTolerance=OptionsInfo["NitrogenLonePairParams"]["PlanarityTolerance"],
        ...
        MiscUtil.PrintError("Failed to instantiate TorsionLibraryAlerts:\n%s\n" % (ErrMsg))

    if not Quiet:
        MiscUtil.PrintInfo(
            "\nRetrieving data from library file %s..." % TorsionLibraryAlertsHandle.GetTorsionLibraryFilePath()
        )
        TorsionLibraryAlertsHandle.ListTorsionLibraryInfo()

    return TorsionLibraryAlertsHandle
...
def ProcessSDFieldLabelsOption():
    ...
    ParamsIDsToLabels = {
        ...
        "TorsionAlertsLabel": "TorsionAlerts (RotBondIndices TorsionAlert TorsionIndices TorsionAngle TorsionAngleViolation HierarchyClass HierarchySubClass TorsionPeaks Tolerances1 Tolerances2 TorsionRule)",
        "TorsionRuleLabel": "TorsionRule (HierarchyClass HierarchySubClass TorsionPeaks Tolerances1 Tolerances2 TorsionRule)",
        "TorsionRuleAlertsCountLabel": "TorsionRuleAlertsCount (Green Orange Red)",
        "TorsionRuleAlertsLabel": "TorsionRuleAlerts (RotBondIndices TorsionAlert TorsionIndices TorsionAngle TorsionAngleViolation)",
    ...
    for Index in range(0, len(ParamsOptionValueWords), 2):
        ...
        if re.match("^PlanarityTolerance$", ParamName, re.I):
            ...
            if not re.match("^(yes|no)$", Value, re.I):
                MiscUtil.PrintError(
                    'The parameter value, %s, specified for parameter name, %s, using "%s" option is not a valid value. Supported values: yes or no'
                    % (Value, Name, ParamsOptionName)
                )
...
def ProcessOptions():
    ...
    OptionsInfo["OutfilesFilteredByRulesMode"] = (
        True if re.match("^yes$", Options["--outfilesFilteredByRules"], re.I) else False
    )
...
def ProcessListTorsionLibraryOption():
    ...
    # Instantiate TorsionLibraryAlerts using defaults...
    TorsionLibraryAlertsHandle = TorsionLibraryAlerts(TorsionLibraryFilePath=OptionsInfo["TorsionLibraryFile"])
    MiscUtil.PrintInfo(
...
Description:
    ...
    files. The molecules must have 3D coordinates in input file.
    
    The default torsion library file, TorsionLibrary.xml, is available under
...
Options:
    ...
    -t, --torsionLibraryFile <FileName or auto>  [default: auto]
        Specify a XML file name containing data for torsion library hierarchy
        or use default file, TorsionLibrary.xml, available in
        MAYACHEMTOOLS/lib/Python/TorsionAlerts directory.
        
        The format of data in local XML file must match format of the data in Torsion
        Library [ Ref 146, 152, 159 ] file available in MAYACHEMTOOLS directory.
    -w, --workingdir <dir>
