...
def VisualizeElectrostaticPotential():
    ...
    for Mol in Mols:
        ...
        GenerateAndWritePMLForGroup(
            OutFH, PyMOLObjectNames["ESPGroup"], PyMOLObjectNames["ESPGroupMembers"], Enable, Action
        )
...
def WriteElectrostaticPotentialView(OutFH, CubeFilesInfo, PyMOLObjectNames):
    ...
    PMLCmds = []
    PMLCmds.append(
        """cmd.ramp_new("%s", "%s", [%s], [%s])"""
        % (
            ESPLegendName,
            ESPCubeName,
            ",".join(CubeFilesInfo["ESPRampValuesList"]),
            MiscUtil.JoinWords(CubeFilesInfo["ESPRampColorsList"], ",", Quote=True),
        )
    )
    ...
    GenerateAndWritePMLForGroup(
        OutFH, PyMOLObjectNames["DensityGroup"], PyMOLObjectNames["DensityGroupMembers"], Enable, "open"
    )
    ...
    GenerateAndWritePMLForGroup(
        OutFH, PyMOLObjectNames["MolSurfaceGroup"], PyMOLObjectNames["MolSurfaceGroupMembers"], Enable, "open"
    )
...
def RetrieveMolCubeFilesInfo(Mol, MolNum):
    ...
    if len(ESPCubeFiles) > 1 or len(DensityCubeFiles) > 1:
        if not OptionsInfo["QuietMode"]:
            MiscUtil.PrintWarning(
                "Multiple ESP and/or density  cube files available for molecule %s; Using first set of cube files, %s and %s, to visualize electrostatic potential..."
                % (RDKitUtil.GetMolName(Mol, MolNum), ESPCubeFileName, DensityCubeFileName)
            )

    CubeFilesInfo["ESPFileName"] = ESPCubeFileName
    CubeFilesInfo["DensityFileName"] = DensityCubeFileName

    DensityIsocontourRangeMin, DensityIsocontourRangeMax, DensityContourLevel = (
        SetupDensityIsocontourRangeAndContourLevel(DensityCubeFileName)
    )
...
def SetupDensityIsocontourRangeAndContourLevel(CubeFileName):
    ...
    if not OptionsInfo["QuietMode"]:
        if IsocontourRangeMin is not None and IsocontourRangeMax is not None:
            MiscUtil.PrintInfo(
                "DensityCubeFileName: %s; Isocontour range for %s percent of the density: %.4f to %.4f; ContourLevel: %s"
                % (
                    CubeFileName,
...
def SetupESPRampValuesAndColors(CubeFileName):
    ...
    if not OptionsInfo["QuietMode"]:
        MiscUtil.PrintInfo(
            "ESPCubeFileName: %s; MinValue: %.4f; MaxValue: %.4f; ESPRampValues: %s; ESPRampColors: %s"
            % (CubeFileName, ESPMinValue, ESPMaxValue, ESPRampValuesList, ESPRampColorsList)
        )
...
def ProcessESPRampOptions():
    ...
    if (ESPRampColorsAuto and not ESPRampValuesAuto) or (ESPRampValuesAuto and not ESPRampColorsAuto):
        MiscUtil.PrintError(
            'The parameter values, "%s" and "%s", specified for paramater names, "espRampValues" and "espRampColors", using "--pymolViewParams" are not valid. "auto" value must be specified for both parameters. '
            % (PyMOLViewParams["ESPRampValues"], PyMOLViewParams["ESPRampColors"])
        )
    ...
    if not ESPRampValuesAuto:
        for Value in PyMOLViewParams["ESPRampValues"].split():
            if not MiscUtil.IsFloat(Value):
                MiscUtil.PrintError(
                    'The value, %s, specified for paramater name, "espRampValues", using "--pymolViewParams" must be a float.'
                    % (Value)
                )
    ...
    if len(ESPRampValuesList) != len(ESPRampColorsList):
        MiscUtil.PrintError(
            'The number of values, "%s" and "%s", specified for paramater names, "espRampValues" and "espRampColors", using "--pymolViewParams" must match.'
            % (len(ESPRampValuesList), len(ESPRampColorsList))
        )
